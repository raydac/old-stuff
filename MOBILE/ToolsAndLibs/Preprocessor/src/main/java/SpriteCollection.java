...
public class SpriteCollection
{
     ...
     */
    public static final int SPRITEBEHAVIOUR_RELEASE = 0xFF00;
     ...
     */
    private int[] ai_spriteAnimationDataArray;
     ...
     */
    private int i_lastActiveSpriteOffset;
     ...
     */
    public SpriteCollection(int _collectionID, int _spriteNumber, int[] _spriteDataArray)
    {
        i_CollectionID = _collectionID;
        ai_spriteAnimationDataArray = _spriteDataArray;
    ...
    public final int getSpriteWidthHeight(int _spriteOffset)
    {
        ...
        final int[] ai_spriteAnimationData = ai_spriteAnimationDataArray;
        int i_dataOffset = ai_spriteData[_spriteOffset + SPRITEDATAOFFSET_STATICDATAOFFSET] + ANIMATIONTABLEEOFFSET_WIDTH;
        int i_w = (ai_spriteAnimationData[i_dataOffset++] + 0x7F) >> 8;
        int i_h = (ai_spriteAnimationData[i_dataOffset] + 0x7F) >> 8;
    ...
    public final long getSpriteHotspotCoordinates(int _spriteOffset)
    {
        ...
        int i_x = (ai_spriteAnimationData[i_dataOffset++] + 0x7F) >> 8;
        int i_y = (ai_spriteAnimationData[i_dataOffset] + 0x7F) >> 8;
        int i_w = (ai_spriteAnimationData[i_dataOffset++] + 0x7F) >> 8;
        int i_h = (ai_spriteAnimationData[i_dataOffset] + 0x7F) >> 8;
    ...
    public final void activateSprite(int _objectType, int _objectState, final int _spriteOffset)
    {
        ...
        final int[] ai_sda = ai_spriteDataArray;
        final int[] ai_ssda = ai_spriteAnimationDataArray;
        ...
        int i_staticDataOffset = ai_ssda[(ai_ssda[_objectType]) + _objectState];
        int i_maxAnimationCounter = ai_ssda[i_staticDataOffset + ANIMATIONTABLEEOFFSET_ANIMATIONDELAY];
    ...
    public final void processAnimationForActiveSprites()
    {
        ...
        while (true)
        {
            ...
            if (i_nextFrameCounter == 0)
            {
                ...
                int i_animationType = ai_ssda[i_staticDataOffset + ANIMATIONTABLEEOFFSET_ANIMATIONTYPE];
                int i_maxFrames = ai_ssda[i_staticDataOffset + ANIMATIONTABLEEOFFSET_FRAMES];
                int i_animationDelay = ai_ssda[i_staticDataOffset + ANIMATIONTABLEEOFFSET_ANIMATIONDELAY];
                ...
                if ((i_nextObjectStateType != i_curObjectStateType) && lg_iterationFinished)
                {
                    ...
                    {
                        switch (i_nextObjectStateType & MASK_STATE)
                        {
                            case SPRITEBEHAVIOUR_RELEASE & MASK_STATE:
                            {
                                // Удаляем объект
                                ...
                                ;
                                break;
                            case SPRITEBEHAVIOUR_NOTIFY & MASK_STATE:
                            {
    ...
    public final int findCollidedSprite(int _spriteOffset, SpriteCollection _spriteCollection)
    {
        ...
        int[] ai_staticData = _spriteCollection.ai_spriteAnimationDataArray;
        ...
        int i_mainHotXoff = i_ScrX + ai_staticData[i_indexOffset++];
        int i_mainHotYoff = i_ScrY + ai_staticData[i_indexOffset++];
        int i_mainHotW = i_mainHotXoff + ai_staticData[i_indexOffset++];
        int i_mainHotH = i_mainHotYoff + ai_staticData[i_indexOffset];

        ai_staticData = ai_spriteAnimationDataArray;
        ...
        while (true)
        {
            ...
            int i_cHotXoff = i_ScrX + ai_staticData[i_dataOffset++];
            int i_cHotYoff = i_ScrY + ai_staticData[i_dataOffset++];
            int i_cHotW = i_cHotXoff + ai_staticData[i_dataOffset++];
            int i_cHotH = i_cHotYoff + ai_staticData[i_dataOffset++];
    ...
    public final void setMainPointXY(int _spriteOffset, int _newMainX, int _newMainY)
    {
        ...
        int i_offsetX = ai_ssda[i_staticDataOffset++];
        int i_offsetY = ai_ssda[i_staticDataOffset];

...
public class SpriteCollection
{
    ...
    private final void _moveInactiveSpriteToActiveList(int _spriteOffset)
    {
        ...
        // подключаем спрайт к списку активных
        ...
        {
            // прописываем у предыдущего спрайта ссылку на текущий
    ...
    public final void processActive()
    {
        ...
        if (i_lastActive<0) return;
        final int[] ai_sda = ai_spriteDataArray;
        final short[] ash_ssda = ash_spriteAnimationDataArray;
        
        while (i_lastActive!=0xFFFF)
        {
            final int i_curOffset = i_lastActive;
            i_lastActive  = (ai_sda[i_lastActive] & MASK_LISTPREVINDEX) >>> SHR_LISTPREVINDEX;
            
            // Обработка анимации
            int i_animationData = ai_sda[i_curOffset + OFFSET_ANIMATIONDATA];

            // Не обрабатываем спрайты на паузе
            if ((i_animationData & MASK_PAUSED) != 0) continue;
     ...
     * @param _sourceSpriteOffset смещение у выравнивающего спрайта
     * @param _destSpriteOffset смещение у выравниваемого спрайта
     */
    public final void alignMainPoint(int _sourceSpriteOffset, int _destSpriteOffset)
    {
        final int[] ai_sda = ai_spriteDataArray;
        int i_off = _sourceSpriteOffset + OFFSET_MAINX;
        int i_mainX = ai_sda[i_off++];
        int i_mainY = ai_sda[i_off];
        setMainPointXY(_destSpriteOffset, i_mainX, i_mainY);
    }

    /**
     * Клонировать спрайт с заданным смещением
     *
     * @param _srcSpriteOffset смещение до области спрайта, подлежащего клонированию
     * @param _dstSpriteOffset смещение до области нового спрайта
    ...
    public final void cloneActive(int _srcSpriteOffset, int _dstSpriteOffset)
    {
        ...
        {
            // Спрайт в который будет произведено копирование данных не активен
            _moveInactiveSpriteToActiveList(_dstSpriteOffset);
        }

        System.arraycopy(ai_spriteDataArray, _srcSpriteOffset+1, ai_spriteDataArray, _dstSpriteOffset+1, SPRITEDATA_LENGTH-1);
    }
    ...
    public final int findCollidedSprite(int _spriteOffset, SpriteCollection _spriteCollection, int _initSpriteOffset)
    {
        ...
        while (true)
        {
            ...
            {
                i_lastActive = (ai_dataArray[i_lastActive] & MASK_LISTPREVINDEX)>>>SHR_LISTPREVINDEX;
                if (i_lastActive==0xFFFF) i_lastActive=-1;
                continue;
            }
     ...
     * @param _i8deltaX смещение по оси X главной точки
     * @param _i8deltaY смещение по оси Y главной точки
     */

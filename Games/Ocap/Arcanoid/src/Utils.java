...
public class Utils
{
    ...
    public static final HSound loadSound(String _resourceName)
    {
        HSound hs = new HSound();
        try
        {
            URL url = p_This.getClass().getResource(RESOURCEPATH+_resourceName);
            hs.load(url);
            System.out.println("Sound "+_resourceName+" loaded");
            return hs;
        }
        catch (Exception e)
        {
            System.out.println("WARNING: " + _resourceName + " can't be loaded");
        }
        return null;
    }

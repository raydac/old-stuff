	...
	
{
	...
	protected RBackDraw previewscreen = null;
	...
	protected Rectangle rectt = new Rectangle(372,5,72,28); 
	...
	public EditorForm(IconMaker appl) throws IOException 
	{
		...
		rrb.setBounds(349,217,101,20);
		add(rrb);

		Color [] phone_color = new Color[]{new Color(158,214,250),Color.black};   

		mscreen = new MobilePhoneScreen(getParentApplet(),scr_grid,72,28,phone_color,6,GB_EDITOR);
		mscreen.setBounds(15,40,432,168);
		mscreen.setMPSListener(this); 
		add(mscreen);
		mscreen.setCursor(Cursor.getPredefinedCursor(Cursor.CROSSHAIR_CURSOR));   

		previewscreen = new RBackDraw(GB_PREVIEW,this);
		previewscreen.setBounds(372,5,72,28);
		add(previewscreen);
		
		mpattern = new PatternLine(getParentApplet(),GB_PATTERNLINE);
		mpattern.setBounds(139,217,190,20);
		add(mpattern);

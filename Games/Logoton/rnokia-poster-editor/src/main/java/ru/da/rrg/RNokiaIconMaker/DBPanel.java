...
public class DBPanel extends RPanel implements ActionListener
{
	...
	public DBPanel(IconMaker appl,EditorForm efr,String sendcmnd,URL okpage) throws IOException 
	{
		...
		lbl_msg = new RLabel("Вы хотите сохранить эту открытку?");
		lbl_msg.setBackground(bckclr);   
		lbl_msg.setFont(new Font("System",Font.PLAIN,14));
		lbl_msg.setBounds((330-lbl_msg.getFontMetrics(lbl_msg.getFont()).stringWidth(lbl_msg.getText()))>>1,87,lbl_msg.getFontMetrics(lbl_msg.getFont()).stringWidth(lbl_msg.getText()),lbl_msg.getFontMetrics(lbl_msg.getFont()).getHeight()+10);    
		
		/* err_msg = new RLabel("I can't save your image!!!"); */
		err_msg = new RLabel("Не могу сохранить открытку!!!");
		err_msg.setForeground(Color.red);  
		err_msg.setBackground(bckclr);   
		err_msg.setFont(new Font("System",Font.PLAIN,20));
		err_msg.setBounds((463-err_msg.getFontMetrics(err_msg.getFont()).stringWidth(err_msg.getText()))>>1,82,err_msg.getFontMetrics(err_msg.getFont()).stringWidth(err_msg.getText()),err_msg.getFontMetrics(err_msg.getFont()).getHeight()+10);

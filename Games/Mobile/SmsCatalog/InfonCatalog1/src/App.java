...
public class App
{
    ...
    private static String s_GameClassName = null;
    ...
    protected static boolean init(Class _canvas, MIDlet _midlet, byte[] _dataBlock)
    {
        ...
        //#if GAMEMODE!=""
        //$s_GameClassName = "/*$GAMEMODE$*/";
        //#else
        s_GameClassName = _midlet.getAppProperty("dgclass");
        //#endif

        if (s_GameClassName!=null)
        {
            s_GameClassName = s_GameClassName.trim();
            if (s_GameClassName.length()==0) s_GameClassName = null;
        }
    ...
    {
        //#if GAMEMODE!=""
        if (s_GameClassName!=null && lg_GameMode)
        {
        ...
        //#endif
        {
            switch (_commandId)
            {
                ...
                case COMMAND_GameCMD:
                {
                    ...
                    if (!Game.initNewGame(Main.p_MainCanvas, Main.p_CurrentDisplay.numColors(), ab_LocalBytes,s_GameClassName))
                    {
                        lg_GameMode = false;
                        Main.changeVideoMode(Main.VIDEOMODE_MENU);
        ...
        switch (_screenId)
        {
            case SCR_MainPageCR:
            {
                switch (_commandId)
                {
                    ...
                    case COMMAND_GameCMD:
                        //#if GAMEMODE!=""
                        return s_GameClassName!=null && Game.hasGame();
                        //#else
                        //$return false;
                        //#endif

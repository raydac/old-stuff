...
p_imageForOutput = getImage(2);
    ...
    private void gamePlayDrawing(Graphics _graphics)
    {
        ...
        int li,i_sx,i_sy,i_w,i_h,i_x, i_y , i_monkeyX,i_monkeyY;
        Sprite p_spr;
        Image img;
    ...
    _graphics.translate(0,16);
        ...
        li = (p_spr.i_ObjectState & 1); 


        switch(p_spr.i_ObjectType)
        {
            default:
            case MobileGameletImpl.SPRITE_APE:           li = (p_spr.i_mainY >= MobileGameletImpl.TREE_Y) ? IMG_MONKEY_CLIMB : li+IMG_MONKEY; break;
            case MobileGameletImpl.SPRITE_APE_JUMPLEFT:  li += IMG_MONKEY_JUMP_LEFT; break;
            case MobileGameletImpl.SPRITE_APE_JUMPRIGHT: li += IMG_MONKEY_JUMP_RIGHT; break;
            case MobileGameletImpl.SPRITE_APE_DOWN:      li = IMG_MONKEY_CLIMB; break;
            case MobileGameletImpl.SPRITE_APE_UP:        li = IMG_MONKEY_CLIMB; break;
            case MobileGameletImpl.SPRITE_APE_DEAD:
                 {
                    if((i_Timer & 2) == 0)
                    {
                       li = IMG_NET;
                       img = getImage(IMG_MONKEY);
                 ...
                 break;
            case MobileGameletImpl.SPRITE_APE_PICK:      li = IMG_MONKEY_PLUCK; break;
        }

        img = getImage(li);
        i_monkeyX = i_x - (img.getWidth()>>1);
        i_monkeyY = i_y - img.getHeight();
        _graphics.drawImage(img, i_monkeyX, i_monkeyY,0);
        ...
        for(li=p_Gamelet.ap_Sprites.length-1 ; li>=0 ; li--)
        {
            ...
            if (p_spr.lg_SpriteActive)
            {
                ...
                switch(p_spr.i_ObjectType)
                {
                    case MobileGameletImpl.SPRITE_TREE:
                    case MobileGameletImpl.SPRITE_TREE_TOUCHED:
                        img = getImage(IMG_PALM);
                        i_sx = i_x - (img.getWidth()>>1);
                        if(p_spr.i_mainX == p_Gamelet.i_CoconutX)
                        {
                           _graphics.drawImage(img,i_sx, i_sy, 0);

                           img = getImage(IMG_COCO);
                           i_sx = i_x + MobileGameletImpl.COCONUT_OFFSET_X - 4;
                           i_sy = i_y + MobileGameletImpl.COCONUT_OFFSET_Y - 4;
                        }
                        break;
                    case MobileGameletImpl.SPRITE_CLIMBER:
                        img = getImage(IMG_MAN_GO_TOP_1 + (p_spr.i_Frame&1));
                        break;
                    case MobileGameletImpl.SPRITE_CLIMBER_STAND:
                        img = getImage(IMG_MAN_GO_TOP_1);
                        break;
                    case MobileGameletImpl.SPRITE_CLIMBER_FALL:
                        img = getImage(IMG_MAN_FALL_DOWN);
                        break;
                }
                _graphics.drawImage(img, i_sx, i_sy,0);
        ...
        if(p_Gamelet.pPlayerSprite.i_ObjectType == MobileGameletImpl.SPRITE_APE_PICK)
        {
            _graphics.drawImage(getImage(IMG_MONKEY_PLUCK01), i_monkeyX, i_monkeyY+4,0);
        }


          p_spr = p_Gamelet.pCoconut;
          if(p_spr.lg_SpriteActive)
          {
              i_x = p_spr.i_ScreenX>>8;
              i_y = p_spr.i_ScreenY>>8;

              _graphics.drawImage(getImage(IMG_COCO),i_x,i_y,0);
          }


        p_spr = p_Gamelet.pHelicopter;
        if(p_spr.lg_SpriteActive)
        {
            i_x = (p_spr.i_mainX>>8) - (copter.fontWidth>>1);
            i_y = (p_spr.i_mainY>>8) - (copter.fontHeight>>1);
            li = p_spr.i_Frame + ((p_spr.i_ObjectState > 0) ? 2 : 0);
            copter.drawDigits(_graphics,i_x,i_y,li);
        }
...
if (lg_FinishImagePictured)
    ...
    public int processGameAction(int _arg)
    {
          ...
          switch(_arg)
          {
               ...
               case MobileGameletImpl.GAMEACTION_JUMPED:
                          {
                            i_action_sound = SND_OBEZYANA_PRYGAET_NA_PALMU_MMF;
                          }
                          break;
               case MobileGameletImpl.GAMEACTION_PICK:
                          {
                            i_action_sound = SND_SORVAN_KOKOS_MMF;
                          }
                          break;
               case MobileGameletImpl.GAMEACTION_FIRE:
                          {
                            i_action_sound = SND_BROSOK_MMF;
                          }
                          break;
               case MobileGameletImpl.GAMEACTION_FIRED:
                          {
                          }
                          break;
               case MobileGameletImpl.GAMEACTION_DEAD:
                          {
                            i_action_sound = SND_OBEZYANA_V_SETKE_MMF;
                          }
                          break;
               case MobileGameletImpl.GAMEACTION_TREE_TOUCH:
                          {
                          }
                          break;
               case MobileGameletImpl.GAMEACTION_CLIMBER_STAND:
                          {
                            i_action_sound = SND_CHELOVECHEK_NA_PALME_MMF;
                          }
                          break;
               case MobileGameletImpl.GAMEACTION_CLIMBER_HIT:
                          {
                            i_action_sound = SND_POPAL_V_CHELOVEKA_MMF;
                          }
                          break;
               case MobileGameletImpl.GAMEACTION_CLIMBER_GENERATED:
                          {
                          }
                          break;
               case MobileGameletImpl.GAMEACTION_HELICOPTER_GENERATED:
                          {
                             i_action_sound = SND_VERTOLET_MMF;
                          }
                          break;
          }
    ...
    public Object processGameAction(int _arg1, int _arg2, Object _obj1)
    {
     ...
     private void prepareGraphics()
     {
        ...
        copter    = new drawDigits(getImage(IMG_HELICOPTER),4, false);
    ...
    {
                             int [] sort_idx = {
SND_OBEZYANA_PRYGAET_NA_PALMU_MMF ,185,
SND_SORVAN_KOKOS_MMF              ,86,
SND_BROSOK_MMF                    ,502,
SND_POPAL_V_CHELOVEKA_MMF         ,860,
SND_VERTOLET_MMF                  ,1432,
SND_CHELOVECHEK_NA_PALME_MMF      ,1052,
SND_OBEZYANA_V_SETKE_MMF          ,504,
SND_FINAL_MMF                     ,0,
SND_START_MMF                     ,0
                             };
...
private static final int IMG_COCO = 6;
private static final int IMG_HELICOPTER = 7;
private static final int IMG_ICON_LIFE = 8;
private static final int IMG_MAN_FALL_DOWN = 9;
private static final int IMG_MAN_GO_TOP_1 = 10;
private static final int IMG_MAN_GO_TOP_2 = 11;
private static final int IMG_MONKEY = 12;
private static final int IMG_MONKEY_COCO = 13;
private static final int IMG_MONKEY_CLIMB = 14;
private static final int IMG_MONKEY_JUMP_LEFT = 15;
private static final int IMG_MONKEY_JUMP_COCO_LEFT = 16;
private static final int IMG_MONKEY_JUMP_RIGHT = 17;
private static final int IMG_MONKEY_JUMP_COCO_RIGHT = 18;
private static final int IMG_MONKEY_PLUCK = 19;
private static final int IMG_MONKEY_PLUCK01 = 20;
private static final int IMG_NET = 21;
private static final int IMG_NUMBERS = 22;
private static final int IMG_PALM = 23;
private static final int TOTAL_IMAGES_NUMBER = 24;

private static final int SND_BROSOK_MMF = 0;
private static final int SND_CHELOVECHEK_NA_PALME_MMF = 1;
private static final int SND_FINAL_MMF = 2;
private static final int SND_OBEZYANA_PRYGAET_NA_PALMU_MMF = 3;
private static final int SND_OBEZYANA_V_SETKE_MMF = 4;
private static final int SND_POPAL_V_CHELOVEKA_MMF = 5;
private static final int SND_SORVAN_KOKOS_MMF = 6;
private static final int SND_START_MMF = 7;
private static final int SND_VERTOLET_MMF = 8;

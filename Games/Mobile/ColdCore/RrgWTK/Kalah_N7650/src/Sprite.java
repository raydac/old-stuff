...
public class Sprite
{
 ...
 if (i_sX+i_sw < i_dX || i_sY+i_sh < i_dY || i_dX+i_dw < i_sX || i_dY+i_dh < i_sY)
    ...
    public void readSpriteFromStream(DataInputStream _dis) throws IOException
    {
        ...
        int i_mnX = _dis.readShort();
        int i_mnY = _dis.readShort();

        setMainPointXY(i_mnX,i_mnY);

        i_DestHole = _dis.readByte();
        i_DestX = _dis.readShort();
        i_DestY = _dis.readShort();
        i_Color = _dis.readByte();
    ...
    public void writeSpriteToStream(DataOutputStream _dis) throws IOException
    {
        ...
        _dis.writeShort(i_mainX);
        _dis.writeShort(i_mainY);

        _dis.writeByte(i_DestHole);
        _dis.writeShort(i_DestX);
        _dis.writeShort(i_DestY);
        _dis.writeByte(i_Color);

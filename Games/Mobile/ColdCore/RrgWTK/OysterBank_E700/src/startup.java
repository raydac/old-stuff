                ...
                case STATE_GAMEPLAY:
                    {
                        ...
                        else
                        {
                            ...
                            if(p_Gamelet.getWinningList()==null && ++i_showdeath_delay < 0)
                            {
                                   paintOnBuffer();
                            }
                            else
...
{
final int c_white     = 0xffffff;
final int c_gray      = 0xd0d0d0;
final int c_lightgray = 0xcccccc;
final int c_darkGray  = 0x808080;
final int c_darkgray  = 0x808080;
final int c_black     = 0x000000;
final int c_red       = 0xff0000;
final int c_green     = 0x00ff00;
final int c_blue      = 0x0000ff;
final int c_yellow    = 0xffff00;
final int c_orange    = 0xffcc00;
final int c_cyan      = 0x00ffff;
final int c_magenta   = 0xff00ff;
final int c_pink      = 0xffcccc;
final int c_BLACK     = 0x000000;
final int c_RED       = 0xff0000;
final int c_GREEN     = 0x00ff00;
final int c_BLUE      = 0x0000ff;
...
final int c_PINK      = 0xffcccc;
        ...
        Sprite p_spr;
        int i_sx, i_sy, i_w, i_h;
        int i_asx, i_asy, i_aw, i_ah;
        int dead_line = 128 - 32;

        Image BackImage = getImage(IMG_BACK);

 final int [] ai_SharkAnimation = new int[]
        {
            0,1,0,2
        };
 ...
 g.translate(0,32-5);
        ...
        bad_g.drawImage(BackImage, 0, 0 - (BackImage.getHeight() - 32 - 5), 0);


        Sprite [] ap_sprites2 = p_Gamelet.ap_Sprites;

        for(int li=0;li<ap_sprites2.length;li++)
        {
            p_spr = ap_sprites2[li];
            if((p_spr.i_mainY + p_spr.i_height / 2) < (dead_line)) continue;
            if (p_spr.lg_SpriteActive)
            {
                i_sx = p_spr.i_ScreenX>>8;
                i_sy = (p_spr.i_ScreenY>>8) - dead_line;
                i_w = p_spr.i_width>>8;
                i_h = p_spr.i_height>>8;

                i_asx = i_sx + (p_spr.i_col_offsetX>>8);
                i_asy = i_sy + (p_spr.i_col_offsetY>>8);
                i_aw = p_spr.i_col_width>>8;
                i_ah = p_spr.i_col_height>>8;

                switch(p_spr.i_ObjectType)
                {
                    case MobileGameletImpl.SPRITE_SHELL_OPEN:
                        shell.drawDigits(bad_g, i_sx, i_sy, p_spr.i_Frame);
                        break;
                    case MobileGameletImpl.SPRITE_SHELL_CLOSE:
                        shell.drawDigits(bad_g, i_sx, i_sy, p_spr.i_Frame);
                        break;
                    case MobileGameletImpl.SPRITE_SHARK_LEFT:
                        shark_left.drawDigits(bad_g, i_sx, i_sy, ai_SharkAnimation[p_spr.i_Frame]);
                        break;
                    case MobileGameletImpl.SPRITE_SHARK_RIGHT:
                        shark_right.drawDigits(bad_g, i_sx, i_sy, ai_SharkAnimation[p_spr.i_Frame]);
                        break;
                    case MobileGameletImpl.SPRITE_OXYGEN_BUBBLE:
                        bad_g.drawImage(getImage(IMG_BLEB), i_sx, i_sy, 0);
                        break;
                }
        ...
        if((p_spr.i_mainY + p_spr.i_height / 2) > (dead_line))
        {
            i_sy = (p_spr.i_ScreenY>>8) - dead_line;
            i_asx = i_sx + (p_spr.i_col_offsetX>>8);
            i_asy = i_sy + (p_spr.i_col_offsetY>>8);
            ...
            switch(p_spr.i_ObjectType)
            {
                default:
                    break;
                case MobileGameletImpl.SPRITE_DIVER_DEAD:
                    diver_dead.drawDigits(bad_g, i_sx, i_sy, p_spr.i_Frame);
                    break;
                case MobileGameletImpl.SPRITE_DIVER_TAKE_PEARL_LEFT:
                    diver_left.drawDigits(bad_g, i_sx, i_sy, p_spr.i_Frame);
                    break;
                case MobileGameletImpl.SPRITE_DIVER_TAKE_PEARL_RIGHT:
                    diver_right.drawDigits(bad_g, i_sx, i_sy, p_spr.i_Frame);
                    break;
                case MobileGameletImpl.SPRITE_DIVER_TURN_UP:
                case MobileGameletImpl.SPRITE_DIVER_TURN_LEFT:
                case MobileGameletImpl.SPRITE_DIVER_TURN_RIGHT:
                    g.drawImage(getImage(IMG_DIVER_GROUP), i_sx, i_sy, 0);
                    break;
                case MobileGameletImpl.SPRITE_DIVER_UP:
                    diver_up.drawDigits(bad_g, i_sx, i_sy, p_spr.i_Frame);
                    break;
                case MobileGameletImpl.SPRITE_DIVER_LEFT:
                    diver_left.drawDigits(bad_g, i_sx, i_sy, p_spr.i_Frame);
                    break;
                case MobileGameletImpl.SPRITE_DIVER_RIGHT:
                    diver_right.drawDigits(bad_g, i_sx, i_sy, p_spr.i_Frame);
                    break;
            }
        }
 ...
 g.translate(0,-(32-5));
        ...
        g.drawImage(getImage(IMG_BACK),0,0,0);

 g.translate(0,32-5);

        g.drawImage(BadImage,0, 128-32, 0);
        ...
        switch(p_spr.i_ObjectType)
        {
            default:
                break;
            case MobileGameletImpl.SPRITE_DIVER_DEAD:
                diver_dead.drawDigits(g, i_sx, i_sy, p_spr.i_Frame);
                break;
            case MobileGameletImpl.SPRITE_DIVER_TAKE_PEARL_LEFT:
                diver_left.drawDigits(g, i_sx, i_sy, p_spr.i_Frame);
                break;
            case MobileGameletImpl.SPRITE_DIVER_TAKE_PEARL_RIGHT:
                diver_right.drawDigits(g, i_sx, i_sy, p_spr.i_Frame);
                break;
            case MobileGameletImpl.SPRITE_DIVER_TURN_UP:
            case MobileGameletImpl.SPRITE_DIVER_TURN_LEFT:
            case MobileGameletImpl.SPRITE_DIVER_TURN_RIGHT:
                ...
                break;
            case MobileGameletImpl.SPRITE_DIVER_UP:
                diver_up.drawDigits(g, i_sx, i_sy, p_spr.i_Frame);
                break;
            case MobileGameletImpl.SPRITE_DIVER_LEFT:
                diver_left.drawDigits(g, i_sx, i_sy, p_spr.i_Frame);
                break;
        ...
        for(int li=0;li<ap_sprites.length;li++)
        {
            ...
            if (p_spr.lg_SpriteActive)
            {
                ...
                switch(p_spr.i_ObjectType)
                {
                    ...
                    case MobileGameletImpl.SPRITE_SHELL_CLOSE:
                        break;
                    case MobileGameletImpl.SPRITE_PEARL_BOAT_WEST:
                        cater.drawDigits(g, i_sx, i_sy, 2);
                        if(p_Gamelet.pPlayerSprite.i_ObjectType == MobileGameletImpl.SPRITE_DIVER)
                        {
                            g.drawImage(getImage(IMG_DIVER_CUTTER), i_sx+38, i_sy-1, 0);
                        }
                        else
                        {
                        }
                        break;
                    case MobileGameletImpl.SPRITE_PEARL_BOAT_EAST:
                        cater.drawDigits(g, i_sx, i_sy, 0);
                        if(p_Gamelet.pPlayerSprite.i_ObjectType == MobileGameletImpl.SPRITE_DIVER)
                        {
                            g.drawImage(getImage(IMG_DIVER_CUTTER), i_sx+15, i_sy-1, 0);
                        }
                        else
                        {
                        }
                        break;
                    case MobileGameletImpl.SPRITE_PEARL_BOAT_SOUTH:
                        cater.drawDigits(g, i_sx, i_sy, 1);
                        break;
                    case MobileGameletImpl.SPRITE_SHARK_LEFT:
                        shark_left.drawDigits(g, i_sx, i_sy, ai_SharkAnimation[p_spr.i_Frame]);
                        break;
                    case MobileGameletImpl.SPRITE_SHARK_RIGHT:
                        shark_right.drawDigits(g, i_sx, i_sy, ai_SharkAnimation[p_spr.i_Frame]);
                        break;
                    case MobileGameletImpl.SPRITE_SHARK_ATTACK_LEFT:
                        g.drawImage(getImage(IMG_SHARK_BITE_LEFTGIF), i_sx, i_sy, 0);
                        break;
                    case MobileGameletImpl.SPRITE_SHARK_ATTACK_RIGHT:
                        g.drawImage(getImage(IMG_SHARK_BITE_RIGHT), i_sx, i_sy, 0);
                        break;
                    case MobileGameletImpl.SPRITE_OXYGEN_BUBBLE:
                        g.drawImage(getImage(IMG_BLEB), i_sx, i_sy, 0);
                        break;
                }
 ...
 g.translate(0,-(32-5));
        ...
        Image img = getImage(IMG_ICON_LIFE);

        int i_x = 2; int i_y = 2;

        int li = p_Gamelet.i_playerAttemptions-1;

        boolean flash_lives = true;

        if(p_Gamelet.pPlayerSprite.i_ObjectType == MobileGameletImpl.SPRITE_DIVER_DEAD)
        {
            if((i_Timer & 4)==0) flash_lives = false;
        }
 ...
 if(flash_lives)
        ...
        i_x = SCREENWIDTH - (score_font.fontWidth<<2) - 2;
        score_font.drawDigits(_graphics,i_x,i_y,4,p_Player.m_iPlayerGameScores,10);


        i_x = 49;
 img = getImage(IMG_BALLOON);
        li = p_Gamelet.pPlayerSprite.i_ObjectState / 10;
        if( li > 7 || li <= 0 || (i_Timer & 2) == 0)
        {
            _graphics.drawImage(img, i_x, i_y,0);
            i_x += (img.getWidth()+1);
            font.drawDigits(_graphics,i_x,i_y,2,li,10);
        }
...
if (lg_FinishImagePictured)
    ...
    public int processGameAction(int _arg)
    {
          ...
          switch(_arg)
          {
               case MobileGameletImpl.GAMEACTION_DIVER_GET_PEARL:
                          {
                             i_action_sound = SND_VZYATA_JEMCHUJINA_MMF;
                          }
                          break;
               case MobileGameletImpl.GAMEACTION_DIVER_DEAD:
                          {
                             i_action_sound = SND_PLOVETS_UMER_MMF;
                          }
                          break;
               case MobileGameletImpl.GAMEACTION_SHELL_OPEN:
                          {
                             i_action_sound = SND_RAKOVINA_OTKRYVAETSA_MMF;
                          }
                          break;
               case MobileGameletImpl.GAMEACTION_SHELL_CLOSE:
                          {
                          }
                          break;
               case MobileGameletImpl.GAMEACTION_SHARK_ATTACK:
                          {
                          }
                          break;
               case MobileGameletImpl.GAMEACTION_SHARK_GENERATED:
                          {
                          }
                          break;
               case MobileGameletImpl.GAMEACTION_OXYGEN:
                          {
                          }
                          break;
               case MobileGameletImpl.GAMEACTION_BUBBLE_GENERATED:
                          {
                          }
                          break;
               case MobileGameletImpl.GAMEACTION_SPLASH:
                          {
                            i_action_sound = SND_VSPLESK_MMF;
                          }
                          break;
    ...
    drawDigits font;
    drawDigits score_font;
    drawDigits cater;
    drawDigits diver_up;
    drawDigits diver_left;
    drawDigits diver_right;
    drawDigits diver_dead;
    drawDigits shell;
    drawDigits shark_left;
    drawDigits shark_right;
    ...
    Graphics bad_g;
     ...
     {
         font = new drawDigits(getImage(IMG_NUMBERS),11);
         score_font = new drawDigits(getImage(IMG_SCORE),10);
         cater = new drawDigits(getImage(IMG_CUTTER),3, false);
         diver_up = new drawDigits(getImage(IMG_DIVER_GO_TOP),2,false);
         diver_left = new drawDigits(getImage(IMG_DIVER_GO_LEFT),2,false);
         diver_right = new drawDigits(getImage(IMG_DIVER_GO_RIGHT),2,false);

         diver_dead = new drawDigits(getImage(IMG_DIVER_DEAD),2,false);
         shell = new drawDigits(getImage(IMG_PEARL),4,false);

         shark_left = new drawDigits(getImage(IMG_SHARK_LEFT),3,false);
         shark_right = new drawDigits(getImage(IMG_SHARK_RIGHT),3,false);

         BadImage = Image.createImage(GAMESCREEN_WIDTH, 32);
         bad_g = BadImage.getGraphics();
    ...
    {
                             int [] sort_idx = {
SND_RAKOVINA_OTKRYVAETSA_MMF ,200,
SND_VZYATA_JEMCHUJINA_MMF    ,300,
SND_VSPLESK_MMF              ,419,
SND_PLOVETS_UMER_MMF         ,1067,
SND_FINAL_MMF                ,0,
SND_START_MMF                ,0
                             };
...
private static final int IMG_BACKGROUND128X160 = 5;
private static final int IMG_BALLOON = 6;
private static final int IMG_BLEB = 7;
private static final int IMG_CUTTER = 8;
private static final int IMG_DIVER_DEAD = 9;
private static final int IMG_DIVER_CUTTER = 10;
private static final int IMG_DIVER_GO_LEFT = 11;
private static final int IMG_DIVER_GO_RIGHT = 12;
private static final int IMG_DIVER_GO_TOP = 13;
private static final int IMG_DIVER_GROUP = 14;
private static final int IMG_ICON_LIFE = 15;
private static final int IMG_NUMBERS = 16;
private static final int IMG_PEARL = 17;
private static final int IMG_SCORE = 18;
private static final int IMG_SHARK_BITE_LEFTGIF = 19;
private static final int IMG_SHARK_BITE_RIGHT = 20;
private static final int IMG_SHARK_LEFT = 21;
private static final int IMG_SHARK_RIGHT = 22;
private static final int TOTAL_IMAGES_NUMBER = 23;

private static final int SND_FINAL_MMF = 0;
private static final int SND_PLOVETS_UMER_MMF = 1;
private static final int SND_RAKOVINA_OTKRYVAETSA_MMF = 2;
private static final int SND_START_MMF = 3;
private static final int SND_VSPLESK_MMF = 4;
private static final int SND_VZYATA_JEMCHUJINA_MMF = 5;

...
public class TileBackground
{
    ...
    public final void initTileBackground(int _width, int _height, int _cellWidth, int _cellHeight, boolean _makeDoubleBuffer)
    {
        ...
        i8_viewCoordX = -10000000;
        i8_viewCoordY = -10000000;
    }
    ...
    public final void setXY(int _x8, int _y8)
    {
        ...
        if (Math.abs(i_oldX - i_x) <= i_blockWidth && Math.abs(i_oldY - i_y) <= i_blockHeight)
        {
            scrollHorizontal((i_x-i_oldX) << 8);
            scrollVertical((i_y-i_oldY) << 8);
        }
        else
        {
            i8_viewCoordX = _x8;

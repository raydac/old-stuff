       ...
       logo = null;
        ...
        p_Player.i_buttonState = i_state;
            ...
            catch (InterruptedException e)
            {
               ...
               if (lg_BackLightEnabled && (i_Timer & 0x2F) == 0 ) setDisplayLight(lg_BackLightEnabled);
...
p_imageForOutput = getImage(2);
    ...
    private void gamePlayDrawing(Graphics _graphics)
    {
        ...
        if (p_Gamelet.getGameWorldState() != MobileGamelet.GAMEWORLDSTATE_PLAYED)
        {
          ...
          Graphics infopanel = gInfoPanel;

          infopanel.drawImage(getImage(IMG_INFO_PANEL),0,0,0);

          i_x = ((GAMESCREEN_WIDTH - li*(img.getWidth()+3)) >> 1) +1;
          i_y = ((imgInfoPanel.getHeight() - img.getHeight())>>1) +1;


        for(; li > 0 ;li--)
        {
            infopanel.drawImage(img, i_x, i_y,0);
            i_x += img.getWidth()+3;
        }

        if(p_Player.m_iPlayerGameScores > 9999)
        {
           font.drawDigits(infopanel,score_x - 3,ball_score_y,5,p_Player.m_iPlayerGameScores,10);
        }
          else
               {
                  font.drawDigits(infopanel,score_x,ball_score_y,4,p_Player.m_iPlayerGameScores,10);
               }
        font.drawDigits(infopanel,balls_counter_x,ball_score_y,3,p_Gamelet.i_BallCounter,10);


          _graphics.drawImage(imgInfoPanel,0,GAMESCREEN_HEIGHT-imgInfoPanel.getHeight(),0);


_graphics.translate(0,intermodel_y);


        p_spr = p_Gamelet.pBall;
        if(p_spr.lg_SpriteActive)
        {
            ...
            i_x = p_spr.i_mainX >> 8;
            i_y = p_spr.i_ScreenY + p_spr.i_height;

            if(MobileGameletImpl.LAND_LINE > i_y)
            {
                i_y = MobileGameletImpl.LAND_LINE;
            }
            i_y = (i_y>>8);

            _graphics.drawImage(img, i_x - (img.getWidth()>>1), i_y -1, 0);
        }

        p_spr = p_Gamelet.pPlayerFootballer;
        if(p_spr.lg_SpriteActive)
        {
            ...
            {
                case MobileGameletImpl.SPRITE_FOOTBALLER:        
                     {
                        img = getImage(IMG_PLAYER_01);


                       _graphics.drawImage(img, i_x, i_y, 0);
                     }
                     break;
                case MobileGameletImpl.SPRITE_FOOTBALLER_LEFT:  
                     {
                         player_run_left.drawDigits(_graphics,i_x,i_y,p_spr.i_Frame);
                     }
                     break;
                case MobileGameletImpl.SPRITE_FOOTBALLER_RIGHT: 
                     {
                         player_run_right.drawDigits(_graphics,i_x,i_y,p_spr.i_Frame);
                     }
                     break;
                case MobileGameletImpl.SPRITE_FOOTBALLER_HEAD_KICK: 
                     {
                        img = getImage(IMG_PLAYER_02);


                       _graphics.drawImage(img, i_x, i_y, 0);
                     }
                     break;
                case MobileGameletImpl.SPRITE_FOOTBALLER_OVERHEAD_KICK: 
                     {
                        img = getImage(IMG_PLAYER_05);


                       _graphics.drawImage(img, i_x, i_y, 0);
                     }
                     break;
                case MobileGameletImpl.SPRITE_FOOTBALLER_RIGHT_KICK: 
                     {
                     }
                case MobileGameletImpl.SPRITE_FOOTBALLER_GATE_KICK: 
                     {
                        img = getImage(IMG_PLAYER_06);


                       _graphics.drawImage(img, i_x, i_y, 0);
                     }
                     break;
                case MobileGameletImpl.SPRITE_FOOTBALLER_HAPPY:     
                     {
                        img = getImage(IMG_PLAYER_07_SHADOW);       

                        _graphics.drawImage(img
                             ,(p_spr.i_mainX >> 8) - (img.getWidth() >> 1)
                             ,i_y + (p_spr.i_height >> 8) - (img.getHeight() >> 1)
                             ,0);

                        i_y -= 1+ happy_jumps[p_spr.i_Frame % happy_jumps.length];

                        img = getImage(IMG_PLAYER_07);              
                     ...
                     break;
                case MobileGameletImpl.SPRITE_FOOTBALLER_SAD:       
                     {
                        if(p_spr.i_Frame == 0)
                           {
                              img = getImage(IMG_PLAYER_09);
                              i_y -=2;
                              i_x +=1;
                           }
                           else
                                img = getImage(IMG_PLAYER_08);
        ...
        if(p_spr.lg_SpriteActive)
        {
            ...
            {
                default:
                case MobileGameletImpl.SPRITE_COACH:            li = 0; break;
                case MobileGameletImpl.SPRITE_COACH_KICK:       li = 1; break;
                case MobileGameletImpl.SPRITE_COACH_AFTER_KICK: li = 2; break;
            }

            trainer.drawDigits(_graphics,i_x,i_y,li);
        }
        ...
        _graphics.drawImage(getImage(IMG_GATE),118,77,0);
...
if (lg_FinishImagePictured)
    ...
    public int processGameAction(int _arg)
    {
          ...
          switch(_arg)
          {
               ...
               case MobileGameletImpl.GAMEACTION_FOOTBALLER_KICK:
                          {
                            i_action_sound = SND_UDAR_O_GOLOVU_MMF;
                          }
                          break;
               case MobileGameletImpl.GAMEACTION_FOOTBALLER_KICKED:
                          {
                          }
                          break;
               case MobileGameletImpl.GAMEACTION_FOOTBALLER_HAPPY:
                          {
                            i_action_sound = SND_GOAL_MMF;
                          }
                          break;
               case MobileGameletImpl.GAMEACTION_FOOTBALLER_SAD:
                          {
                            i_action_sound = SND_BALL_LOST_MMF;
                          }
                          break;
               case MobileGameletImpl.GAMEACTION_FOOTBALLER_BORED:
                          {
                          }
                          break;
               case MobileGameletImpl.GAMEACTION_COACH_KICK:
                          {
                          }
                          break;
               case MobileGameletImpl.GAMEACTION_COACH_KICKED:
                          {
                            i_action_sound = SND_UDAR_O_GOLOVU_MMF;
                          }
                          break;
               case MobileGameletImpl.GAMEACTION_BALL_STOPED:
                          {
                            if(i_action_sound >= 0) return 0;
                            i_action_sound = SND_UDAR_O_ZEMLU_MMF;
                          }
                          break;
               case MobileGameletImpl.GAMEACTION_ADD_BONUS_LIVE:
                          {
                          }
                          break;
          }
    ...
    public Object processGameAction(int _arg1, int _arg2, Object _obj1)
    {
     ...
     drawDigits font;
     drawDigits trainer;
     drawDigits player_run_left, player_run_right;

      final static int [] happy_jumps = new int[] {3,6,7,6,3};
      final static int [] runningman_ofsX = new int[] {
                          MobileGameletImpl.FOOTBALLER_MOVE_SPEED*0 >>8
                         ,MobileGameletImpl.FOOTBALLER_MOVE_SPEED*1 >>8
                         ,MobileGameletImpl.FOOTBALLER_MOVE_SPEED*0 >>8
                         ,MobileGameletImpl.FOOTBALLER_MOVE_SPEED*0 >>8
                         };
      final static int [] runningman_frames = new int[] {0,1,2,3};

      final static int balls_counter_x = 16;
      final static int score_x = 99;


      Image imgInfoPanel;
      Graphics gInfoPanel;
      final static int ball_score_y = 3+2;
      final static int intermodel_y = 14;


     private void prepareGraphics()
     {
        font              = new drawDigits(getImage(IMG_FONT_SCORE),10);
        trainer           = new drawDigits(getImage(IMG_TRAINER),3,false);
        player_run_left   = new drawDigits(getImage(IMG_PLAYER_03),4,false);
        player_run_right  = new drawDigits(getImage(IMG_PLAYER_04),4,false);


         Image img = getImage(IMG_INFO_PANEL);
         imgInfoPanel = Image.createImage(img.getWidth(),img.getHeight());
    ...
    {
                             int [] sort_idx = {
SND_UDAR_O_GOLOVU_MMF  ,142,
SND_UDAR_O_ZEMLU_MMF   ,245,
SND_BALL_LOST_MMF      ,539,
SND_GOAL_MMF           ,856,
SND_FINAL_MMF          ,0,
SND_START_MMF          ,0
                             };
...
private static final int IMG_INFO_PANEL = 6;
private static final int IMG_BALL = 7;
private static final int IMG_FONT_SCORE = 8;
private static final int IMG_ICON_LIFE = 9;
private static final int IMG_ICON_LIFE160 = 10;
private static final int IMG_PLAYER_01 = 11;
private static final int IMG_PLAYER_02 = 12;
private static final int IMG_PLAYER_03 = 13;
private static final int IMG_PLAYER_04 = 14;
private static final int IMG_PLAYER_05 = 15;
private static final int IMG_PLAYER_06 = 16;
private static final int IMG_PLAYER_07 = 17;
private static final int IMG_PLAYER_07_SHADOW = 18;
private static final int IMG_PLAYER_08 = 19;
private static final int IMG_PLAYER_09 = 20;
private static final int IMG_TRAINER = 21;
private static final int IMG_SHADOW = 22;
private static final int IMG_GATE = 23;
private static final int TOTAL_IMAGES_NUMBER = 24;

private static final int SND_BALL_LOST_MMF = 0;
private static final int SND_FINAL_MMF = 1;
private static final int SND_GOAL_MMF = 2;
private static final int SND_START_MMF = 3;
private static final int SND_UDAR_O_GOLOVU_MMF = 4;
private static final int SND_UDAR_O_ZEMLU_MMF = 5;

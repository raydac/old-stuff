                             ...
                             if(p_Canvas.isShown() && i_MIDletState == STATE_GAMEPLAY)
                             {
                               ...
                               else
                               {
                                 playMelody(ap_melody[i_action_sound],false);
                                    ...
                                    if(i_action_sound == SND_GOAL_MMF) System.out.println("\n\n\n!!! GOAL !!!\n\n\n");
                                 i_action_sound = -1;
                               }
                             }
                        }
...
p_imageForOutput = getImage(2);
    ...
    private void gamePlayDrawing(Graphics _graphics)
    {
     ...
     showBall = (i_Timer&2)==0 || p_Gamelet.getGameWorldState() == MobileGamelet.GAMEWORLDSTATE_PLAYED;


     p_spr = p_Gamelet.pPlayerSprite;
     if(p_spr.lg_SpriteActive && showBall)
     {
       i_x = (p_spr.i_ScreenX >> 8);

         i_y = (p_spr.i_ScreenY >> 8) + 16;

       _graphics.drawImage(getImage(IMG_BALL),i_x,i_y,0);
     }

     if(last_score != p_Player.m_iPlayerGameScores)
     {
        last_score = p_Player.m_iPlayerGameScores;
        gScore.drawImage(img,-score_x,-score_y,0);
        font.drawDigits(gScore,score_dx,score_dy,4,last_score,10);
     }
     _graphics.drawImage(imgScore,score_x,score_y,0);


     if(last_balls1 != p_Gamelet.i_Balls1 || last_balls2 != p_Gamelet.i_Balls2)
     {
        last_balls1 = p_Gamelet.i_Balls1;
        last_balls2 = p_Gamelet.i_Balls2;
        gGoals.drawImage(img,-goals_x,-goals_y,0);
        font.drawDigits(gGoals,goals_dx,goals_dy,2,p_Gamelet.i_Balls2,10);
        font.drawDigits(gGoals,(font.fontWidth<<1)+goals_dx,goals_dy,10);
        font.drawDigits(gGoals,(font.fontWidth<<1)+font.fontWidth+goals_dx,goals_dy,2,p_Gamelet.i_Balls1,10);
     }

     if(showBall)
     {
        _graphics.drawImage(imgGoals,goals_x,goals_y,0);
     }

     img = getImage(IMG_FLAG);
     i_x = lives_x;  i_y = lives_y;
     ...
     _graphics.drawImage(img, bar_x, bar1_y, 0);
     _graphics.drawImage(img, bar_x, bar2_y, 0);
         ...
         if (p_spr.lg_SpriteActive)
         {
             ...
             switch(p_spr.i_ObjectType)
             {
                 case MobileGameletImpl.SPRITE_FOOTBALLER:
                         ...
                         break;
                 case MobileGameletImpl.SPRITE_FOOTBALLER_STRIKE:
                         {
                            i_y+=2;
                            man = blue_team;
                            i_idx = p_spr.i_Frame<ani_sequence.length ? ani_sequence[p_spr.i_Frame] : 2;
                         }
                         break;

                 case MobileGameletImpl.SPRITE_FOOTBALLER_AFTER_STRIKE:
                         {
                            i_y+=2;
                            man = blue_team;
                            i_idx = 2;
                 ...
                 case MobileGameletImpl.SPRITE_FOOTBALLER_ENEMY:
                         {
                            man = red_team;
                            i_idx = 0;
                         }
                         break;
                 case MobileGameletImpl.SPRITE_FOOTBALLER_ENEMY_STRIKE:
                         {
                            man = red_team;
                            i_idx = p_spr.i_Frame<ani_sequence.length ? ani_sequence[p_spr.i_Frame] : 2;
                         }
                 ...
                 case MobileGameletImpl.SPRITE_GATE:
                 case MobileGameletImpl.SPRITE_GATE_ENEMY:
                 default:
                      continue;
             }
             man.drawDigits(_graphics,i_x - (man.fontWidth>>1),i_y - (man.fontHeight>>1),i_idx);
         }
     }
...
if (lg_FinishImagePictured)
    ...
    public int processGameAction(int _arg)
    {
          ...
          switch(_arg)
          {
               case MobileGameletImpl.GAMEACTION_PLAYER_STRIKE:
                          {
                          }
                          break;
               case MobileGameletImpl.GAMEACTION_ENEMY_STRIKE:
                          {
                          }
                          break;
               case MobileGameletImpl.GAMEACTION_PLAYER_STRIKE_BALL:
                          {
                            i_action_sound = SND_UDAR_FUTBOLISTOM_MMF;
                          }
                          break;
               case MobileGameletImpl.GAMEACTION_ENEMY_STRIKE_BALL:
                          {
                             i_action_sound = SND_UDAR_FUTBOLISTOM_MMF;
                          }
                          break;
               case MobileGameletImpl.GAMEACTION_TOUCH_WALL:
                          {
                             i_action_sound = SND_UDAR_O_STENKU_MMF;
                          }
                          break;
               case MobileGameletImpl.GAMEACTION_TOUCH_FOOTBALLER:
                          {
                             i_action_sound = SND_UDAR_O_FUTBOLISTA_MMF;
                          }
                          break;
               case MobileGameletImpl.GAMEACTION_PLAYER_GOAL:
                          {
                            i_action_sound = SND_GOAL_MMF;
                          }
                          break;
               case MobileGameletImpl.GAMEACTION_ENEMY_GOAL:
                          {
                            i_action_sound = SND_GOAL_MMF;
                          }
                          break;
    ...
    public Object processGameAction(int _arg1, int _arg2, Object _obj1)
    {
     ...
     final static int [] ani_sequence = {1,1,0,2};



       final static int score_dx = 0;
       final static int score_dy = 0;
       final static int score_x = 94  -score_dx;
       final static int score_y = 148 -score_dy;

       final static int goals_dx = 10;
       final static int goals_dy = 3;
       final static int goals_x = 51 -goals_dx;
       final static int goals_y = 3  -goals_dy;

       final static int lives_x = 6;
       final static int lives_y = 148;

       final static int bar_x = 4;
       final static int bar1_y = 39 +16;
       final static int bar2_y = 85 +16;
     ...
     private void prepareGraphics()
     {
        ...
        red_team          = new drawDigits(getImage(IMG_OPPONENT),3,false);
        blue_team         = new drawDigits(getImage(IMG_PLAYER),3,false);

        imgScore = Image.createImage(font.fontWidth*4 +score_dx*2,font.fontHeight +score_dy*2);
        gScore = imgScore.getGraphics();

        imgGoals = Image.createImage(font.fontWidth*5+goals_dx*2,font.fontHeight+goals_dy*2);
        gGoals = imgGoals.getGraphics();
     }
    ...
    {
                             int [] sort_idx = {
SND_UDAR_FUTBOLISTOM_MMF  ,0,
SND_UDAR_O_FUTBOLISTA_MMF ,0,
SND_UDAR_O_STENKU_MMF     ,0,
SND_GOAL_MMF              ,0,
SND_START_MMF             ,0,
SND_FINAL_MMF             ,0
...
private static final int IMG_FLAG = 7;
private static final int IMG_ICON_FLAG128X160 = 8;
private static final int IMG_NUMBERS = 9;
private static final int IMG_OPPONENT = 10;
private static final int IMG_PLAYER = 11;
private static final int IMG_BAR = 12;
private static final int TOTAL_IMAGES_NUMBER = 13;

private static final int SND_FINAL_MMF = 0;
private static final int SND_GOAL_MMF = 1;
private static final int SND_START_MMF = 2;
private static final int SND_UDAR_FUTBOLISTOM_MMF = 3;
private static final int SND_UDAR_O_FUTBOLISTA_MMF = 4;
private static final int SND_UDAR_O_STENKU_MMF = 5;

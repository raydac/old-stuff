...
public final class PathController
{
    ...
    public boolean lg_Paused; // Флаг, показывающий, что путь на паузе
    ...
    public final void writePathToStream(DataOutputStream _stream) throws IOException
    {
        ...
        int i_pack = (lg_Completed ? 0x80 :0) | (lg_back ? 0x40:0) | (lg_Paused ? 0x20:0) | i_modify;
    ...
    {
        if (lg_Completed || lg_Paused) return false;
        ...
        if (i8_curX == i8_targetX && i8_curY == i8_targetY)
        {
            ...
            if (lg_Completed && ((i_type & NOTIFY_ENDPOINT)!=0))
                    Gamelet.notifyPathCompleted(this);
    ...
    /**
     * Производит смещение координат пути
     * @param _dx8 дельта изменения координаты X (8 бит, фиксированная точка)
     * @param _dy8 дельта изменения координаты Y (8 бит, фиксированная точка) 
     */
    public void moveMainPoint(int _dx8,int _dy8)
    {
        i8_centerX += _dx8;
        i8_centerY += _dy8;

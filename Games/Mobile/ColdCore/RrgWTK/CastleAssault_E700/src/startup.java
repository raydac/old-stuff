...
p_imageForOutput = getImage(2);
    ...
    private void gamePlayDrawing(Graphics _graphics)
    {
        ...
        int li,i_w,i_h,i_x, i_y;
        Sprite p_spr;
        Image img;
        boolean draw_enemy_stone = false;
        ...
        gBuffer.drawImage(getImage(IMG_BACK),0,-buffer_y,0);

gBuffer.translate(0,intermodel_y - buffer_y);

        p_spr = p_Gamelet.pStoneSupplier;
        if(p_spr.lg_SpriteActive && p_spr.i_ObjectType == MobileGameletImpl.SPRITE_STONESUPPLIER_DEAD)
        {
            i_x = p_spr.i_ScreenX >> 8;
            i_y = p_spr.i_ScreenY >> 8;

            gBuffer.drawImage(getImage(IMG_SOLDIER_KILLED),i_x +2 ,i_y +5 ,0);
        }


        p_spr = p_Gamelet.pStone;
        i_y = p_spr.i_ScreenY >> 8;
        if(p_spr.lg_SpriteActive && i_y >= buffer_y - intermodel_y && p_spr.i_Frame > 1)
        {
            i_x = p_spr.i_ScreenX >> 8;
            if(p_spr.i_ObjectState > 0 )
            {
                li = 2;
                  ...
                  if(p_spr.i_ObjectState > -MobileGameletImpl.G_SPEED * p_spr.i_Frame)
                  {
                      li = 1;
                  }
                    else
                           li = 0;
            stone.drawDigits(gBuffer,i_x,i_y,li);
        }


        p_spr = p_Gamelet.pPlayerCatapult;
        i_x = p_spr.i_mainX >> 8;
        i_y = (p_spr.i_ScreenY + p_spr.i_height)>> 8;

        switch(p_spr.i_ObjectType)
        {
            case MobileGameletImpl.SPRITE_CATAPULT          :
                      {
                        i_x -= catapult_fire.fontWidth>>1;
                        i_y -= catapult_fire.fontHeight;
                        catapult_fire.drawDigits(gBuffer,i_x,i_y,0);
                      }
                      break;
            case MobileGameletImpl.SPRITE_CATAPULT_LEFT     :
                      {
                        i_x -= catapult_left.fontWidth>>1;
                        i_y -= catapult_left.fontHeight;
                        catapult_left.drawDigits(gBuffer,i_x,i_y,p_spr.i_Frame);
                      }
                      break;
            case MobileGameletImpl.SPRITE_CATAPULT_RIGHT    :
                      {
                        i_x -= catapult_right.fontWidth>>1;
                        i_y -= catapult_right.fontHeight;
                        catapult_right.drawDigits(gBuffer,i_x,i_y,p_spr.i_Frame);
                      }
                      break;
            case MobileGameletImpl.SPRITE_CATAPULT_READY    :
                      {
                        i_x -= catapult_fire.fontWidth>>1;
                        i_y -= catapult_fire.fontHeight;
                        catapult_fire.drawDigits(gBuffer,i_x,i_y,1);

                        {
                          ...
                          arr_y = (
                                    ((MobileGameletImpl.WALL_Y - MobileGameletImpl.CELL_HEIGHT / 2 + MobileGameletImpl.CELL_HEIGHT * 4) >> 8) + 10
                                   )
                                   - (MobileGameletImpl.LAUNCH_INITIAL_SPEED - p_Gamelet.i_Speed)
                                     /
                                      (
                                         (MobileGameletImpl.LAUNCH_INITIAL_SPEED - MobileGameletImpl.LAUNCH_MAX_SPEED)
                                       / (((MobileGameletImpl.CELL_HEIGHT >> 8) * 4) + 20)
                                      )
                                   ;

                         if(arr_y > buffer_y - intermodel_y)
                         {
                           img = getImage(IMG_ARROW);
                           gBuffer.drawImage(img,5,arr_y - (img.getHeight() >> 1),0);
                           arr_y = 0;
                         }
                      ...
                      break;
            case MobileGameletImpl.SPRITE_CATAPULT_FIRE     :
                      {
                        i_x -= catapult_fire.fontWidth>>1;
                        i_y -= catapult_fire.fontHeight;
                        catapult_fire.drawDigits(gBuffer,i_x,i_y,2);
                      }
                      break;
            case MobileGameletImpl.SPRITE_CATAPULT_AFTER_FIRE     :
                      {
                        i_x -= catapult_fire.fontWidth>>1;
                        i_y -= catapult_fire.fontHeight;
                        catapult_fire.drawDigits(gBuffer,i_x,i_y,2);
                      ...
                      break;
            case MobileGameletImpl.SPRITE_CATAPULT_DESTROYED:
                      {
                        img = getImage(IMG_CATAPULT_DESTRUCT);
                        i_x -= img.getWidth() >> 1;
                        i_y -= img.getHeight() -2;
        ...
        if(p_spr.lg_SpriteActive && p_spr.i_ObjectType != MobileGameletImpl.SPRITE_STONESUPPLIER_DEAD)
        {
            ...
            if(p_spr.i_ObjectState > 0)
            {
                soldier_right.drawDigits(gBuffer,i_x,i_y,p_spr.i_Frame);
            }
               else
                     {
                        soldier_left.drawDigits(gBuffer,i_x,i_y,p_spr.i_Frame);
                     }
        }


        p_spr = p_Gamelet.pEnemyStone;
        i_y = p_spr.i_ScreenY >> 8;

        if(p_spr.lg_SpriteActive && i_y >= buffer_y - intermodel_y)
        {
            i_x = p_spr.i_ScreenX >> 8;
            if(p_spr.i_ObjectState > MobileGameletImpl.G_SPEED * 27/2)
            {
                li = 0;
            }
              else
        ...
        font.drawDigits(gBuffer,score_buffer_x,score_buffer_y,4,p_Player.m_iPlayerGameScores,10);

        img = getImage(IMG_LIFE);

        li = p_Gamelet.m_iPlayerAttemptions-1;
        if (p_Gamelet.getGameWorldState() != MobileGamelet.GAMEWORLDSTATE_PLAYED && p_Gamelet.getWinningList() == null)
        {
        ...
        for(; li > 0 ;li--)
        {
            gBuffer.drawImage(img, i_x, life_y,0);
            i_x += img.getWidth()+3;
        }

gBuffer.translate(0, buffer_y - intermodel_y);


        img = getImage(IMG_BACK);

        _graphics.drawImage(imgBuffer,0,buffer_y,0);
        _graphics.setClip(0,0,GAMESCREEN_WIDTH,buffer_y);
        ...
        {
           if(lg_sing_a_whistle && p_spr.i_ObjectState >= i_WHISTLE_CAN_BE_HEARD)
           {
               processGameAction(WHISTLE_OF_ROCK);
               lg_sing_a_whistle = false;
           }

           if(p_spr.i_ObjectState < 0)
           {
                li = 3;
                switch(p_spr.i_mainX)
                {
                    case MobileGameletImpl.CELL3_X: li = 2; break;
                    case MobileGameletImpl.CELL2_X: li = 1; break;
                    case MobileGameletImpl.CELL1_X: li = 0; break;
                }
                 if(wall_storedstate[li][0] != MobileGameletImpl.EMPTY_WALL)
                {
                  i_x = p_spr.i_ScreenX >> 8;
                  i_y = (p_spr.i_ScreenY >> 8) + intermodel_y;
                  stone.drawDigits(_graphics,i_x, i_y, 2);
                }
                  else
                       draw_enemy_stone = true;
           }
            else
                draw_enemy_stone = true;
        }
        ...
        for(int wx=0, start_x = 0; wx < 4;  start_x += (MobileGameletImpl.CELL_WIDTH >> 8), wx++)
        {
            for(int wy=0, start_y = 0; wy < 4;  start_y += (MobileGameletImpl.CELL_HEIGHT >> 8), wy++)
            {
                li = p_Gamelet.getCell(wx, wy);
                if(li != wall_storedstate[wx][wy])
                {
                    wall_storedstate[wx][wy] = li;

                    gWall.setClip(start_x,start_y,(MobileGameletImpl.CELL_WIDTH >> 8),(MobileGameletImpl.CELL_HEIGHT >> 8));
                    gWall.drawImage(img,-wall_x,-wall_y,0);

                    switch(li)
                    {
                        case MobileGameletImpl.EMPTY_WALL    : continue;
                        case MobileGameletImpl.NORMAL_WALL   : li = IMG_WALL_MEDIUM; break;
                        case MobileGameletImpl.HEAD_WALL     : li = IMG_WALL_TOP; break;
                        case MobileGameletImpl.DESTROYED_WALL: li = IMG_WALL_DESTRUCT; break;
                        default:
                          continue;
                    }
                    gWall.drawImage(getImage(li),start_x,start_y,0);
                }
            }
        }

        _graphics.drawImage(imgWall,wall_x,wall_y,0);


_graphics.translate(0,intermodel_y);

        flag1.drawDigits(_graphics,10,1,frames_4in3[i_Timer &3 ]);
        flag1.drawDigits(_graphics,115,1,frames_4in3[(i_Timer+1) &3 ]);

        p_spr = p_Gamelet.pEnemyStone;
        i_y = p_spr.i_ScreenY >> 8;

        if(draw_enemy_stone && i_y < buffer_y - intermodel_y)
        {
            i_x = p_spr.i_ScreenX >> 8;
            if(p_spr.i_ObjectState > MobileGameletImpl.G_SPEED * 27/2)
            {
            ...
            stone.drawDigits(_graphics,i_x,i_y,li);
        }
        ...
        {
             img = getImage(IMG_ARROW);
             _graphics.drawImage(img,5,arr_y - (img.getHeight() >> 1),0);
        }
...
if (lg_FinishImagePictured)
    ...
    public int processGameAction(int _arg)
    {
          ...
          switch(_arg)
          {
               case MobileGameletImpl.GAMEACTION_ON_TARGET:
                          {
                            i_action_sound = SND_STENA_RZBITA_MMF;
                          }
                          break;
               case MobileGameletImpl.GAMEACTION_OVER_TARGET:
                          {
                          }
                          break;
               case MobileGameletImpl.GAMEACTION_HIT_THE_WALL:
                          {
                            i_action_sound = SND_POPADANIE_V_STENU_BEZ_RAZRUSHENIA_MMF;
                          }
                          break;
               case MobileGameletImpl.GAMEACTION_CATAPULT_READY_TO_FIRE:
                          {
                            i_action_sound = SND_ZAGRUZKA_KATAPULTY_MMF;
                          }
                          break;
               case MobileGameletImpl.GAMEACTION_CATAPULT_INCREASE_SPEED:
                          {
                          }
                          break;
               case MobileGameletImpl.GAMEACTION_CATAPULT_FIRE:
                          {
                            i_action_sound = SND_PUSK_KAMNYA_MMF;
                          }
                          break;
               case MobileGameletImpl.GAMEACTION_CATAPULT_DEAD:
                          {
                            i_action_sound = SND_VZRYV_KATAPULTY_MMF;
                          }
                          break;
               case MobileGameletImpl.GAMEACTION_SUPPLIER_GENERATED:
                          {
                          }
                          break;
               case MobileGameletImpl.GAMEACTION_SUPPLIER_DEAD:
                          {
                            i_action_sound = SND_UBIT_MUJIK_S_KAMNEM_MMF;
                          }
                          break;
               case MobileGameletImpl.GAMEACTION_ENEMY_FIRE:
                          {
                             lg_sing_a_whistle = true;
                          }
                          break;
               case MobileGameletImpl.GAMEACTION_ENEMY_ON_THE_LAND:
                          {
                          }
                          break;
               case WHISTLE_OF_ROCK:
                          {
                            i_action_sound = SND_SVIST_PADENIYA_MMF;
                          }
                          break;
          }
    ...
    public Object processGameAction(int _arg1, int _arg2, Object _obj1)
    {
     ...
     final static int WHISTLE_OF_ROCK = 11;
     int i_WHISTLE_CAN_BE_HEARD = MobileGameletImpl.G_SPEED * 13;

     boolean lg_sing_a_whistle = false;

     drawDigits font;
     drawDigits catapult_fire,catapult_left,catapult_right;
     drawDigits soldier_left,soldier_right;
     drawDigits stone;
     drawDigits flag1;
     ...
     Image imgWall;
     Graphics gWall;
     int [][] wall_storedstate = new int[4][4];
     ...
     int [] frames_4in3 = new int[]{0,1,2,1};

      final static int intermodel_y = 15;

     final static int buffer_y = 84 + intermodel_y;
     ...
     private void prepareGraphics()
     {
        ...
        catapult_fire     = new drawDigits(getImage(IMG_CATAPULT),3,false);
        catapult_left     = new drawDigits(getImage(IMG_CATAPULT_LEFT),3,false);
        catapult_right    = new drawDigits(getImage(IMG_CATAPULT_RIGHT),3,false);
        soldier_left      = new drawDigits(getImage(IMG_SOLDIER_STONE_LEFT),3,false);
        soldier_right     = new drawDigits(getImage(IMG_SOLDIER_STONE_RIGHT),3,false);
        stone             = new drawDigits(getImage(IMG_STONES),3,false);
        flag1             = new drawDigits(getImage(IMG_FLAG),3,false);

        Image _heart      = getImage(IMG_LIFE);
        int _middle_line  = GAMESCREEN_HEIGHT - 4 - Math.max(font.fontHeight,_heart.getHeight())/2 - intermodel_y;
        score_buffer_x    = GAMESCREEN_WIDTH  - 5 - font.fontWidth*4;
        score_buffer_y    = _middle_line - (font.fontHeight>>1);
        life_y            = _middle_line - (_heart.getHeight()>>1);

        wall_x = ((MobileGameletImpl.WALL_X - MobileGameletImpl.CELL_WIDTH / 2) >> 8) +1;
        wall_y = (((MobileGameletImpl.WALL_Y - MobileGameletImpl.CELL_HEIGHT / 2) >> 8)) + intermodel_y;

        imgBuffer = Image.createImage(GAMESCREEN_WIDTH,GAMESCREEN_HEIGHT - buffer_y);
        gBuffer = imgBuffer.getGraphics();

        imgWall = Image.createImage((MobileGameletImpl.CELL_WIDTH >> 8) *4 , (MobileGameletImpl.CELL_HEIGHT >> 8) *4);
        gWall = imgWall.getGraphics();
        gWall.drawImage(getImage(IMG_BACK),-wall_x,-wall_y,0);
...
SND_PUSK_KAMNYA_MMF        ,196,
SND_ZAGRUZKA_KATAPULTY_MMF ,699,
SND_POPADANIE_V_STENU_BEZ_RAZRUSHENIA_MMF,196,
SND_SVIST_PADENIYA_MMF     ,1131,
SND_UBIT_MUJIK_S_KAMNEM_MMF,708,
SND_STENA_RZBITA_MMF       ,1668,
SND_VZRYV_KATAPULTY_MMF    ,1671,
SND_LOST_MMF               ,0,
SND_WON_MMF                ,0,
SND_THEME_MMF              ,0
                             };
...
private static final int IMG_LOST128X160 = 1;
private static final int IMG_WIN128X160 = 2;
private static final int IMG_DEMO = 3;
private static final int IMG_BACK = 4;
private static final int IMG_BACKGR128X160 = 5;
private static final int IMG_CATAPULT = 6;
private static final int IMG_CATAPULT_DESTRUCT = 7;
private static final int IMG_CATAPULT_LEFT = 8;
private static final int IMG_CATAPULT_RIGHT = 9;
private static final int IMG_FLAG = 10;
private static final int IMG_LIFE = 11;
private static final int IMG_SOLDIER_KILLED = 12;
private static final int IMG_SOLDIER_STONE_LEFT = 13;
private static final int IMG_SOLDIER_STONE_RIGHT = 14;
private static final int IMG_STONES = 15;
private static final int IMG_WALL_DESTRUCT = 16;
private static final int IMG_WALL_MEDIUM = 17;
private static final int IMG_WALL_TOP = 18;
private static final int IMG_SCORE = 19;
private static final int IMG_ARROW = 20;
private static final int TOTAL_IMAGES_NUMBER = 21;

private static final int SND_LOST_MMF = 0;
private static final int SND_POPADANIE_V_STENU_BEZ_RAZRUSHENIA_MMF = 1;
private static final int SND_PUSK_KAMNYA_MMF = 2;
private static final int SND_STENA_RZBITA_MMF = 3;
private static final int SND_SVIST_PADENIYA_MMF = 4;
private static final int SND_THEME_MMF = 5;
private static final int SND_UBIT_MUJIK_S_KAMNEM_MMF = 6;
private static final int SND_VZRYV_KATAPULTY_MMF = 7;
private static final int SND_WON_MMF = 8;
private static final int SND_ZAGRUZKA_KATAPULTY_MMF = 9;

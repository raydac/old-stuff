...
public class App
{
    ...
    {
        boolean lg_AsTranslit = Main.p_languageBlock.LB_i_CurrentLanguageIndex == 0;
        ...
        //#endif
        {
            ...
            if (p_catalog.l_ValidityCatalogDate != 0)
            {
                if (p_catalog.l_ValidityCatalogDate < l_CurrentDate)
                {
                    ...
                    if (!SMSSender.isMIDP20() && p_catalog.getURL() != null)
                    {
                        p_strbuff.append('\n');
    ...
    private static void sendOrderSMS(boolean _flag,String _resourceID)
    {
        ...
        if (SMSSender.sendTxtSMS(s_ResourceID, s_SMSNumber))
        {
            // Удачно
    ...
    private static void sendOrderSMSForFriend(String _friendNumber, boolean _flag)
    {
        ...
        if (SMSSender.sendTxtSMS(s_ResourceID + ' ' + _friendNumber, s_Send2Friend))
        {
            // Удачно
            MenuBlock.MB_viewAlert(Main.getStringForIndex(SMSSentTitleTXT), Main.getStringForIndex(SMSSentTextTXT), null,AlertType.INFO, 3000, true);
        }
        ...
        //#endif
        {
            switch (_commandId)
            {
                ...
                case COMMAND_GameCMD:
                {
                    ...
                    byte [] ab_gameData = new byte[Game.GAMEDATAMAXLEN];
                    System.arraycopy(ab_AppData, ab_AppData.length - Game.GAMEDATAMAXLEN, ab_gameData, 0, Game.GAMEDATAMAXLEN);

                    //#-
                    try{Thread.sleep(2000);}catch(Throwable _ex){}
                    //#+

                    if (!Game.initNewGame(Main.p_MainCanvas, Main.p_CurrentDisplay.numColors(), ab_gameData))
                    {
                        lg_GameMode = false;
                        Main.changeVideoMode(Main.VIDEOMODE_MENU);
                ...
                case COMMAND_OrderCMD:
                {
                    ...
                    {
                        Form p_frm = (Form) _screen;
                        TextField p_phone = (TextField) p_frm.get(1);
                        TextField p_msg = (TextField) p_frm.get(1);
                        ...
                        if (s_phone.length() != 11)
                        {
                            ...
                            if (Main.p_languageBlock.LB_i_CurrentLanguageIndex == 0)
                ...
                case COMMAND_UpgradeCMD:
                {
                    ...
                    if (s_url != null && SMSSender.isMIDP20())
                    {
                        if (SMSSender.plReq(p_ParentMidlet, s_url))
                        {
                            lg_isAlive = false;
                        }
                        else
    ...
    protected synchronized static Displayable customScreen(int _screenId)
    {
        ...
        SMSCatalog p_cat = p_catalog;
        switch (_screenId)
        {
            //#if SEND2FRIEND
            case SCR_Send2FriendSCR:
            {
                String s_contentID = p_cat.getItemNameForIndex(i_SelectedContentOffset, lg_AsTranslit);

                Form p_form = new Form(s_contentID);
                String s_str = Main.getStringForIndex(Send2FriendMsgTXT);
                StringItem p_strItm = new StringItem(null, s_str);
                SMSSender.algnItem(p_strItm, SMSSender.LAYOUT_NEWLINE_AFTER | SMSSender.LAYOUT_NEWLINE_BEFORE);
                TextField p_phone = new TextField(Main.getStringForIndex(PhoneNumberTXT), "+7", 16, TextField.PHONENUMBER);
                SMSSender.algnItem(p_phone, SMSSender.LAYOUT_NEWLINE_AFTER | SMSSender.LAYOUT_NEWLINE_BEFORE);
                TextField p_message = new TextField(Main.getStringForIndex(Send2FriendMessageFieldCaptionTXT), "", 32, TextField.ANY);
                SMSSender.algnItem(p_message, SMSSender.LAYOUT_NEWLINE_AFTER | SMSSender.LAYOUT_NEWLINE_BEFORE);
                p_form.append(p_strItm);
                p_form.append(p_phone);
                p_form.append(p_message);
                p_result = p_form;
            }
            ;
            break;
            ...
            case SCR_SupportSCR:
            {
                Form p_form = new Form(Main.getStringForIndex(SupportTitleTXT));
                StringItem p_itm = new StringItem(null, Main.getStringForIndex(SupportTextTXT));
                SMSSender.algnItem(p_itm, SMSSender.LAYOUT_LEFT | SMSSender.LAYOUT_NEWLINE_AFTER);
                p_form.append(p_itm);
                if (p_cat.i_SupportPhones != 0xFFFF)
                {
                    String s_phones = p_cat.getSupportPhones();
                    p_itm = new StringItem(lg_AsTranslit ? SMSCatalog.russian2translit("Телефон") : "Телефон", s_phones);
                    SMSSender.algnItem(p_itm, SMSSender.LAYOUT_LEFT | SMSSender.LAYOUT_NEWLINE_AFTER | SMSSender.LAYOUT_NEWLINE_BEFORE);
                    p_form.append(p_itm);
                }
                if (p_cat.i_SupportEmail != 0xFFFF)
                {
                    String s_email = p_cat.getSupportEmail();
            ...
            case SCR_MainPageCR:
            {
                p_result = p_cat.makeScreenForOffset(SMSCatalog.OFFSET_MAINSCREEN, lg_AsTranslit);
                ash_ScreensStack[0] = SMSCatalog.OFFSET_MAINSCREEN;
                i_ScreenStackPointer = 0;

                ((List) p_result).setSelectedIndex(i_CurrentSelectedItem, true);
            }
            ;
            break;
            case SCR_OtherPageSCR:
            {
                p_result = p_cat.makeScreenForOffset(p_catalog.getCurrentScreenOffset(), lg_AsTranslit);
                ((List) p_result).setSelectedIndex(i_CurrentSelectedItem, true);
            }
            ;
            break;
            ...
            case SCR_AgreementSCR:
            {
                ...
                Form p_form = new Form(s_str);
                s_str = p_cat.getStringForIndex(p_cat.i_AgreementText);
                if(lg_AsTranslit) s_str = SMSCatalog.russian2translit(s_str);
                StringItem p_text = new StringItem(null,s_str);
                p_form.append(p_text);
                p_result = p_form;
            }
            ;
            break;
            ...
            {
                Form p_form = new Form(Main.getStringForIndex(ConfirmationTitleTXT));
                Ticker p_ticker = new Ticker(Main.getStringForIndex(SMSWillBeSendTXT));
                p_form.setTicker(p_ticker);
                StringItem p_text = new StringItem(null, Main.getStringForIndex(OrderConfirmTXT) + " \'" + p_cat.getItemNameForIndex(i_SelectedContentOffset, lg_AsTranslit) + '\'');
                p_form.append(p_text);
                p_result = p_form;
            }
            ;
            ...
            case SCR_ResourceInfoSCR:
            {
                ...
                int i_type = p_cat.getItemTypeForIndex(i_CurrentSelectedItem);

                String s_contentID = p_cat.getItemNameForIndex(i_SelectedContentOffset, lg_AsTranslit);
                //System.out.println("s_contentID "+s_contentID);

                String s_resID = p_cat.getItemResourceIDForIndex(i_SelectedContentOffset);
                //System.out.println("s_resID "+s_resID );

                String s_phoneSMS = p_cat.getItemSMSnumForIndex(i_SelectedContentOffset);
                //System.out.println("s_phoneSMS "+s_phoneSMS );

                String s_contentReference = p_cat.getItemReferenceForIndex(i_SelectedContentOffset, lg_AsTranslit);
                //System.out.println("s_contentReference "+s_contentReference );

                String s_contentCost = p_cat.getItemCostForIndex(i_SelectedContentOffset, lg_AsTranslit);
                //System.out.println("s_contentCost "+s_contentCost );

                String s_compatibleSet = null;
                if (i_type!=SMSCatalog.CONTENTTYPE_INFO ) s_compatibleSet = p_cat.getItemCompatibleModels(i_SelectedContentOffset);

                int i_previewURL = p_cat.getItemPrevURLForIndex(i_SelectedContentOffset);
                ...
                if (i_previewURL != 0xFFFF)
                {
                    ...
                    try
                    {
                        ...
                        {
                            byte [] ab_arr = p_cat.getResourceForIndex(i_previewURL & 0x7FFF);
                            p_image = Image.createImage(ab_arr, 0, ab_arr.length);
                            ab_arr = null;
                        }
                        else
                        //#endif
                        {
                            ...
                            //#if RSRC_HTTP
                            String s_previewURL = p_cat.getStringForIndex(i_previewURL - 1);
                            if (s_previewURL.startsWith("http://"))
                            {
                                // Загрузка через вап
                                byte [] ab_data = HTTPInterface.processGETRequestToServer(s_previewURL);
                                p_image = Image.createImage(ab_data, 0, ab_data.length);
                                ab_data = null;
                            }
                            //#endif
                ...
                boolean lg_midp20 = SMSSender.isMIDP20();
                ...
                if (i_type!=SMSCatalog.CONTENTTYPE_INFO  && s_compatibleSet != null)
                {
                    ...
                    if (lg_midp20)
                    {
                        SMSSender.algnItem(p_compatible, SMSSender.LAYOUT_LEFT | SMSSender.LAYOUT_NEWLINE_AFTER);
                    }
                }
                ...
                if (lg_midp20)
                {
                    if (p_text!=null) SMSSender.algnItem(p_text, SMSSender.LAYOUT_LEFT | SMSSender.LAYOUT_NEWLINE_AFTER);
                    if (p_cost!=null) SMSSender.algnItem(p_cost, SMSSender.LAYOUT_LEFT | SMSSender.LAYOUT_NEWLINE_AFTER);
                }

                if (lg_midp20 && p_operation != null)
                {
                    SMSSender.algnItem(p_operation, SMSSender.LAYOUT_LEFT | SMSSender.LAYOUT_NEWLINE_AFTER);
                }
                ...
                if (p_previewImageItem != null)
                {
                    if (lg_midp20)
                    {
                        SMSSender.algnItem(p_previewImageItem, SMSSender.LAYOUT_CENTER | SMSSender.LAYOUT_NEWLINE_AFTER);
                    }
                    p_form.append(p_previewImageItem);
                }

                if (p_text!=null) p_form.append(p_text);
                if (i_type!=SMSCatalog.CONTENTTYPE_INFO && p_cost != null) p_form.append(p_cost);

                //#if SMSENGINE
                if (i_type!=SMSCatalog.CONTENTTYPE_INFO && i_type!=SMSCatalog.CONTENTTYPE_SMSSERVICE && p_operation != null) p_form.append(p_operation);
                //#endif

                if (p_compatible != null) p_form.append(p_compatible);
                ...
                {
                    TextField [] ap_fields = null;
                    ap_fields = makeTextFieldsArrayForSMSService(s_resID);
                    if (ap_fields!=null)
                    {
                        for(int li=0;li<ap_fields.length;li++)
                        {
                            TextField p_field = ap_fields[li];
                            SMSSender.algnItem(p_field, SMSSender.LAYOUT_CENTER | SMSSender.LAYOUT_NEWLINE_AFTER);
                            p_form.append(p_field);
                        }
                        ap_SMSServiceTextFields = ap_fields;
                    }
                }
                //#endif
                Main.changeVideoMode(Main.VIDEOMODE_MENU);
    ...
    private static String makeSMSServiceQueryFromTextFields(String _smsServiceID,TextField [] _fieldsArray)
    {
        ...
        for(int li=0;li<_smsServiceID.length();li++)
        {
            ...
            switch(ch_char)
            {
                ...
                case ' ' :
                {
                    if (lg_inTag) lg_inTag = false;
                    p_outStrBuff.append(ch_char);
                };break;
                default:
                {
        ...
        switch (_screenId)
        {
            case SCR_MainPageCR:
            {
                switch (_commandId)
                {
                    ...
                    case COMMAND_UpgradeCMD:
                    {
                        return (p_catalog.getURL() != null && SMSSender.isMIDP20());
                    }
                    //#endif
                }
            }
            ...
            {
                //#if SMSENGINE
                boolean lg_supportSMS = SMSSender.doesSupportSMS();
                switch (_commandId)
                {
                    case COMMAND_OrderCMD:
                    {
                    ...
                    case COMMAND_Send2FriendCMD :
                    {
                        ...
                        return SMSSender.doesSupportSMS();
                        //#else
                        //$return false;
                        //#endif
                    }

...
public class SMSCatalog
{
    ...
    public final String getStringForIndex(int _index)
    {
        ...
        try
        {
            ...
            final byte [] ab_strings = ab_StringArray;
            int i_len = ab_strings.length - i_offset;

            //#if STR_UTF8
            if ((i_Flags & FLAG_STRINGS_ASUTF8) != 0)
            {
                ByteArrayInputStream p_bas = new ByteArrayInputStream(ab_strings, i_offset, i_len);
            ...
            //#endif
            {
                ...
                while (true)
                {
                    int i_char = ab_strings[i_offset++] & 0xFF;
                    if (i_char == 0) break;
    ...
    public final String getItemCompatibleModels(int _index)
    {
        ...
        short [] ash_comp = ash_CompatibleBlock;
        ...
        while(i_number!=0)
        {
            ...
            int i_vendor = ash_comp[i_index++]&0xFFFF;

            //System.out.println("vendor = "+i_vendor);

            int i_modelsNumber = ash_comp[i_index++]&0xFFFF;
            ...
            for(int li=0;li<i_modelsNumber;li++)
            {
                ...
                int i_model = ash_comp[i_index++]&0xFFFF;
    ...
    public SMSCatalog(String _resource) throws Exception
    {
        ...
        try
        {
            ...
            if (lg_StringsAsUTF)
            {
                p_strb = new ByteArrayInputStream(ab_strarr);
            ...
            for (int li = 0; li < i_StringsNumber; li++)
            {
                ...
                if (lg_StringsAsUTF)
                {
                    ash_links[li] = (short) (i_stringsBlockSize - p_strb.available());
                    p_strin.readUTF();
                }
                else
                //#endif
                {
                    //#if !STR_UTF8
                    ash_links[li] = (short) i_offst;
                    while (true)
                    {
                        int i_val = ab_strarr[i_offst++];
                        if (i_val == 0) break;
                    }
                    //#endif
                }
            ...
            ab_DataBlock = ab_formsData;
            ai_ResourcesOffset = ai_resourcesTable;
            ash_CompatibleBlock = ash_compatibleTable;

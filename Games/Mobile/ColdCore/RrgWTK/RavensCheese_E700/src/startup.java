                ...
                case STATE_FRONTPAGE:
                    {
                         ...
                         {
                            Thread.yield();
                            Melody theme = ap_melody[SND_MAIN_MMF];
                            if(theme != null)
                            {
...
p_imageForOutput = getImage(2);
    ...
    private void gamePlayDrawing(Graphics _graphics)
    {
        ...
        int li,i_sx,i_sy,i_w,i_h,i;
        Sprite p_spr;
        Image img;

        int SCREEN_Y_OFFSET = 0;

        if(GAMESCREEN_HEIGHT == 160) SCREEN_Y_OFFSET = 24;
        ...
        img = getImage(IMG_CHEESE02);

        i = GAMESCREEN_HEIGHT - font_background.getHeight();
        i_sy = font_background.getHeight() - ((img.getHeight() + font.fontHeight) >> 1) - 5;
        i_sx = 5+ img.getWidth() +2;

        Graphics fbg = font_background.getGraphics();
        fbg.drawImage(getImage(IMG_BACK), - i_sx, -i, 0);
        font.drawDigits(fbg, 0, i_sy, 1, 10, 11);                           
        font.drawDigits(fbg, 2+font.fontWidth, i_sy, 3, p_Player.m_iPlayerGameScores, 10); 
        _graphics.drawImage(font_background, i_sx, i, 0);



        Sprite [] ap_sprites = p_Gamelet.ap_Sprites;
        int top_level_objects = ap_sprites.length;   
        _i_amount_of_falling_cheeses = 0;            
        ...
        for(li=0; li < ap_sprites.length; li++)
        {
            ...
            if (p_spr.lg_SpriteActive)
            {
                ...
                {
                    case MobileGameletImpl.SPRITE_RAVEN_LEFT_CROAKING :
                        {
                            img = getImage(IMG_CROW04A);
                        }
                        break;
                    case MobileGameletImpl.SPRITE_RAVEN_RIGHT_CROAKING :
                        {
                            img = getImage(IMG_CROW04);
                            i_sx -= img.getWidth() - i_w;
                        }
                        break;
                    case MobileGameletImpl.SPRITE_RAVEN_EMPTY_LEFT :
                        {
                            img = getImage(IMG_CROW01A);
                        }
                        break;
                    case MobileGameletImpl.SPRITE_RAVEN_EMPTY_RIGHT :
                        {
                            img = getImage(IMG_CROW01);
                            i_sx -= img.getWidth() - i_w;
                        }
                        break;

                    case MobileGameletImpl.SPRITE_RAVEN_WITH_CHEESE_LEFT :
                        {
                            img = getImage(IMG_CROW03A);
                        }
                        break;
                    case MobileGameletImpl.SPRITE_RAVEN_WITH_CHEESE_RIGHT:
                        {
                            img = getImage(IMG_CROW03);
                            i_sx -= img.getWidth() - i_w;
                        }
                        break;

                    case MobileGameletImpl.SPRITE_MOUSE_EMPTY_LEFT :
                    case MobileGameletImpl.SPRITE_MOUSE_EMPTY_RIGHT:
                    case MobileGameletImpl.SPRITE_MOUSE_WITH_CHEESE_LEFT :
                    case MobileGameletImpl.SPRITE_MOUSE_WITH_CHEESE_RIGHT:
                        {
                            _ap_falling_cheeses[--top_level_objects] = p_spr;
                        }
                        continue;

                    case MobileGameletImpl.SPRITE_DROPPED_CHEESE :
                        {
                          if(p_spr.i_ObjectState != MobileGameletImpl.STATE_CHEESE_CATCHED)
                          {
                            _ap_falling_cheeses[_i_amount_of_falling_cheeses++] = p_spr;
                            continue;
                          }
                          img = getImage(IMG_CHEESE01);
                        }
                        break;
                    default: continue;
                }
                ...
                i_sy += SCREEN_Y_OFFSET + i_h - img.getHeight();
        ...
        if(p_spr.lg_SpriteActive)
        {
           ...
           img = getImage(IMG_FOX01);

           switch(p_spr.i_ObjectType)
           {
                case MobileGameletImpl.SPRITE_FOX_DOWN_LEFT :
                        {
                            img = getImage(IMG_FOX01);
                            i_sx += (i_w - img.getWidth())>>1;
                        }
                        break;
                case MobileGameletImpl.SPRITE_FOX_DOWN_RIGHT :
                        {
                            img = getImage(IMG_FOX01A);
                            i_sx += (i_w - img.getWidth())>>1;
                        }
                        break;
                case MobileGameletImpl.SPRITE_FOX_DOWN_OPEN_LEFT :
                        {
                            img = getImage(IMG_FOX02);
                            i_sx += (i_w - img.getWidth())>>1;
                        }
                        break;
                case MobileGameletImpl.SPRITE_FOX_DOWN_OPEN_RIGHT :
                        {
                            img = getImage(IMG_FOX02A);
                            i_sx += (i_w - img.getWidth())>>1;
                        }
                        break;
                case MobileGameletImpl.SPRITE_FOX_TOP_LEFT :
                        {
                            img = getImage(IMG_FOX03);
                            i_sx += i_w - img.getWidth();
                        }
                        break;
                case MobileGameletImpl.SPRITE_FOX_TOP_RIGHT :
                        {
                            img = getImage(IMG_FOX03A);
                        }
                        break;
                case MobileGameletImpl.SPRITE_FOX_TOP_OPEN_LEFT :
                        {
                            img = getImage(IMG_FOX04);
                            i_sx += (i_w - img.getWidth());
                        }
                        break;
                case MobileGameletImpl.SPRITE_FOX_TOP_OPEN_RIGHT :
                        {
                            img = getImage(IMG_FOX04A);
                        }
                        break;
           }
               i_sy += SCREEN_Y_OFFSET + i_h - img.getHeight();
        ...
        for(li=top_level_objects;li<ap_sprites.length;li++)
        {
            p_spr = _ap_falling_cheeses[li];
            _ap_falling_cheeses[li] = null;

            {
                ...
                switch(p_spr.i_ObjectType)
                {
                    case MobileGameletImpl.SPRITE_MOUSE_EMPTY_LEFT :
                        {
                            img = getImage(IMG_MOUSE01A);
                        }
                        break;
                    case MobileGameletImpl.SPRITE_MOUSE_EMPTY_RIGHT:
                        {
                            img = getImage(IMG_MOUSE01);
                            i_sx -= img.getWidth() - i_w;
                        }
                        break;
                    ...
                    case MobileGameletImpl.SPRITE_MOUSE_WITH_CHEESE_LEFT :
                        {
                            img = getImage(IMG_MOUSE02A);
                        }
                        break;
                    case MobileGameletImpl.SPRITE_MOUSE_WITH_CHEESE_RIGHT:
                        {
                            img = getImage(IMG_MOUSE02);
                            i_sx -= img.getWidth() - i_w;
                        }
                        break;
                    default: continue;
        ...
        _graphics.drawImage(getImage(IMG_TREE_LEFT),0,0,0);
        img = getImage(IMG_TREE_RIGHT);
        _graphics.drawImage(img,SCREENWIDTH - img.getWidth(),0,0);




        img = getImage(IMG_CHEESE01);
        for(li=0;li<_i_amount_of_falling_cheeses;li++)
        {
                p_spr = _ap_falling_cheeses[li];
                _ap_falling_cheeses[li] = null;
                ...
                i_sx = p_spr.i_ScreenX>>8;
                i_sy = (p_spr.i_ScreenY)>>8;
                _graphics.drawImage(img,i_sx,i_sy+SCREEN_Y_OFFSET,0);
        }
        ...
        img = getImage(IMG_CHEESE02);
        i_w = img.getWidth();
        i_h = Math.max(img.getHeight(),font.fontHeight);


        i_sy = GAMESCREEN_HEIGHT - i_h - 5;
        i_sx = GAMESCREEN_WIDTH - 5;

        for(li = MobileGameletImpl.MAX_ATTEMPTIONS; li > 0; li--)
        {
            i_sx -= i_w;
            if(li <= p_Gamelet.i_PlayerAttemptions)
            {
               _graphics.drawImage(img,i_sx,i_sy,0);
            }
              else
                   {
                       _graphics.drawImage(getImage(IMG_MOUSE_CHEESE),i_sx,i_sy-3,0);
                   }
        }


        i_sx = 5;                                   
        _graphics.drawImage(img,i_sx,i_sy + ((i_h - img.getHeight())>>1),0);    
...
if (lg_FinishImagePictured)
    ...
    public int processGameAction(int _arg)
    {
          ...
          switch(_arg)
          {
               case MobileGameletImpl.GAMEACTION_RAVEN_CROAKING:
                          {
                            i_action_sound = SND_CROAKING_MMF;
                          }
                          break;
               case MobileGameletImpl.GAMEACTION_MOUSE_GENERATED:
                          {
                          }
                          break;
               case MobileGameletImpl.GAMEACTION_MOUSE_HIDDEN:
                          {
                          }
                          break;
               case MobileGameletImpl.GAMEACTION_CHEESE_TAKEN_BY_MOUSE:
                          {
                               i_action_sound = SND_LAUGH_MOUSE_MMF; 
                          }
                          break;
               case MobileGameletImpl.GAMEACTION_CHEESE_TAKEN_BY_FOX:
                          {
                               i_action_sound = SND_CHEESE_CATCHED_MMF; 
                          }
                          break;
               case MobileGameletImpl.GAMEACTION_CHEESE_FALLEN:
                          {
                               i_action_sound = SND_DROPEN_CHEESE_MMF; 
                          }
                          break;
               case MobileGameletImpl.GAMEACTION_RAVEN_GENERATED:
                          {
                          }
                          break;
               case MobileGameletImpl.GAMEACTION_RAVEN_HIDDEN:
                          {
                          }
                          break;
               case MobileGameletImpl.GAMEACTION_ATTEMPTION_INCREASED:
                          {
                               i_action_sound = SND_ATT_REST_MMF; 
                          }
                          break;
          }
        return 0;
    ...
    public Object processGameAction(int _arg1, int _arg2, Object _obj1)
    {
     ...
     drawDigits font;
     Image font_background;
     Sprite _ap_falling_cheeses[] = new Sprite[MobileGameletImpl.MAX_SPRITES];
     int _i_amount_of_falling_cheeses;

     private void prepareGraphics()
     {
        font = new drawDigits(getImage(IMG_NUMBERS),11);
        font_background = Image.createImage(font.fontWidth*6,20);
     }
...
private static final int IMG_BACK = 3;
private static final int IMG_BACK128X160 = 4;
private static final int IMG_TREE_LEFT = 5;
private static final int IMG_TREE_RIGHT = 6;
private static final int IMG_DEMO = 7;
private static final int IMG_CHEESE01 = 8;
private static final int IMG_CHEESE02 = 9;
private static final int IMG_CROW01 = 10;
private static final int IMG_CROW01A = 11;
private static final int IMG_CROW02 = 12;
private static final int IMG_CROW02A = 13;
private static final int IMG_CROW03 = 14;
private static final int IMG_CROW03A = 15;
private static final int IMG_CROW04 = 16;
private static final int IMG_CROW04A = 17;
private static final int IMG_FLASH = 18;
private static final int IMG_FOX01 = 19;
private static final int IMG_FOX01A = 20;
private static final int IMG_FOX02 = 21;
private static final int IMG_FOX02A = 22;
private static final int IMG_FOX03 = 23;
private static final int IMG_FOX03A = 24;
private static final int IMG_FOX04 = 25;
private static final int IMG_FOX04A = 26;
private static final int IMG_MOUSE01 = 27;
private static final int IMG_MOUSE01A = 28;
private static final int IMG_MOUSE02 = 29;
private static final int IMG_MOUSE02A = 30;
private static final int IMG_MOUSE_CHEESE = 31;
private static final int IMG_NUMBERS = 32;
private static final int TOTAL_IMAGES_NUMBER = 33;

private static final int SND_ATT_REST_MMF = 0;
private static final int SND_CHEESE_CATCHED_MMF = 1;
private static final int SND_CROAKING_MMF = 2;
private static final int SND_DROPEN_CHEESE_MMF = 3;
private static final int SND_LAUGH_MOUSE_MMF = 4;
private static final int SND_LOST_MMF = 5;
private static final int SND_MAIN_MMF = 6;
private static final int SND_WIN_MMF = 7;

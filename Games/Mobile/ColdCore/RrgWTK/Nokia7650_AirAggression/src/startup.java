...
private GameletImpl p_Gamelet;
    ...
    private static final int GAMESCREEN_WIDTH = SCREENWIDTH - 24*2;
    private static final int GAMESCREEN_HEIGHT = SCREENHEIGHT - 40*2;
...
InitGameDataStorage(p_Gamelet.getGameID(), p_Gamelet.getMaxSizeOfSavedGameBlock(), 8, 6, 10);
       ...
       logo = null;
...

                            {
                             ...
                             SND_KRIK_STAI_PTITS_OTT,
                             SND_MALENKIY_VZRIV_OTT,
                             SND_POPADANIE_V_IGROKA_OTT,
                             SND_SMERT_PILOTA_OTT,
                             SND_VERTOLET_PODDERJKI_OTT,
                             SND_VYLET_IGROKA_NA_EKRAN_OTT,
                             SND_VZYATIE_BONUSA_OTT,
                             SND_BOLSHOI_VZRIV_OTT,
                             SND_POYAVLENIE_BOSSA_OTT,
                             SND_SMERT_IGROKA_OTT,
                             SND_ZASTAVKA_OTT,
                             SND_DEATH_THEME_OTT,
                             SND_WON_THEME_OTT
                             };
...
//--------------------------------
        ...
        switch(_key)
        {
            case Canvas.KEY_NUM1 :
                                       {
                                          ...
                                          p_Gamelet.i_PlayerKey = (p_Gamelet.i_PlayerKey & ~GameletImpl.PLAYER_BUTTON_DOWN)
                                             /* up */             | GameletImpl.PLAYER_BUTTON_TOP;
                                       }
                                        break;
            ...
            case Canvas.KEY_NUM5 :
                                       {
                                          p_Gamelet.i_PlayerKey |= GameletImpl.PLAYER_BUTTON_BULLETFIRE;
                                       }
                                        break;
            ...
            case Canvas.KEY_NUM7 :
                                       {
                                          p_Gamelet.i_PlayerKey = (p_Gamelet.i_PlayerKey & ~GameletImpl.PLAYER_BUTTON_RIGHT)
                                             /* left */           | GameletImpl.PLAYER_BUTTON_LEFT;
                                          p_Gamelet.i_PlayerKey = (p_Gamelet.i_PlayerKey & ~GameletImpl.PLAYER_BUTTON_TOP)
...
/*
            ...
            case Canvas.KEY_NUM0 :
                                       {
                                         p_Gamelet.i_PlayerKey |= GameletImpl.PLAYER_BUTTON_MISSLEFIRE;
                                       }
                                        break;
*/
            default :
...
//--------------------------------
        ...
        //---------------Insert the game drawing block here
         ...
         int i,j,k,item_index;
         Image img;
         Sprite sp;

         // draw sea
         img = getImage(IMG_TILE_64); //64x64x64 !!!
         i = img.getWidth();
         k = ((p_Gamelet.horizont * p_Gamelet.LANDSCAPE_SPEED) >>8 );
         k = (k & 63) - GAMESCREEN_HEIGHT; // height=64 (!)
         while(k < GAMESCREEN_HEIGHT)
         {
           j = 0;
           while(j < GAMESCREEN_WIDTH)
           {
             _graphics.drawImage(img,j,k,0);
             j+=i;
           }
           k+=64;
         }
        ...
        for (i = 0; i < p_Gamelet.MAX_LANDSCAPE; i++)
        {
            sp = p_Gamelet.ap_Landscape[i];
            if (sp.lg_SpriteActive)
            {
                drawItem(_graphics,sp,objects[sp.i_ObjectType]+sp.i_Frame,true);
            }
        }
        ...
        int i_sec = 0;
        Sprite [] sec = new Sprite[GameletImpl.SPRITES_NUMBER];
        int i_thrd = 0;
        Sprite [] thrd = new Sprite[GameletImpl.SPRITES_NUMBER];

        // first pass
        int bottom = p_Gamelet.SPRITES_NUMBER;
        for (i = 0; i < bottom; i++)
        {
            sp = p_Gamelet.ap_Sprites[i];
            if (sp.lg_SpriteActive)
            {
                switch(sp.i_ObjectType)
                {
                   case GameletImpl.OBJECT_BONUS1:
                   case GameletImpl.OBJECT_BONUS2:
                   case GameletImpl.OBJECT_DEAD_PLANE:
                   case GameletImpl.OBJECT_DEAD_BOMBER:
                   case GameletImpl.OBJECT_DEAD_FIGHTER:
                   case GameletImpl.OBJECT_DEAD_SPY:
                   case GameletImpl.OBJECT_DEAD_PLAYER:
                   case GameletImpl.OBJECT_PARACHUTER:
                   case GameletImpl.OBJECT_DEADBODY:
                           {
                               drawItem(_graphics,sp,objects[sp.i_ObjectType]+sp.i_Frame,true);
                           }
                           break;
                   case GameletImpl.OBJECT_AIR_FIGHTER:
                   case GameletImpl.OBJECT_AIR_BOMBER:
                   case GameletImpl.OBJECT_AIR_PLANE:
                   case GameletImpl.OBJECT_EXPLOSION:
                   case GameletImpl.OBJECT_BONUS3:
                   case GameletImpl.OBJECT_BONUS4:
                   case GameletImpl.OBJECT_BONUS5:
                   case GameletImpl.OBJECT_BONUS6:
                   case GameletImpl.OBJECT_BONUS7:
                   case GameletImpl.OBJECT_BONUS8:
                   case GameletImpl.OBJECT_BONUS9:
                   case GameletImpl.OBJECT_BONUS10:
                   case GameletImpl.OBJECT_BONUS11:
                   case GameletImpl.OBJECT_HELICOPTER:
                           {
                              sec[i_sec++] = sp;
                           }
                           break;
                    case GameletImpl.OBJECT_CLOUD:
                    case GameletImpl.OBJECT_AIR_SPY:
                           {
                              thrd[i_thrd++] = sp;
                           }
                           break;
                     default:
                           p_Gamelet.ap_Sprites[i] = p_Gamelet.ap_Sprites[--bottom];
                           p_Gamelet.ap_Sprites[bottom] = sp;
                           i--;
                }
            }
        }
        ...
        for (j = 0; j < i_sec; j++)
        {
            ...
            switch(sp.i_ObjectType)
            {
                   ...
                   case GameletImpl.OBJECT_BONUS11:
                           {
                               drawItem(_graphics,sp,IMG_HELICOPTER+sp.i_Frame,false);
                               sp.i_mainX+=sp.i_col_offsetX;sp.i_mainY+=sp.i_col_offsetY;
                               drawItem(_graphics,sp,objects[sp.i_ObjectType],false);
                               sp.i_mainX-=sp.i_col_offsetX;sp.i_mainY-=sp.i_col_offsetY;
                           }
                           break;
                   case GameletImpl.OBJECT_HELICOPTER:
        ...
        // four layer
        bottom = p_Gamelet.SPRITES_NUMBER;
        for (; i < bottom; i++)
        {
            sp = p_Gamelet.ap_Sprites[i];
            switch(sp.i_ObjectType)
                {
                    case GameletImpl.OBJECT_BOSS:
                           {
                               j = bosses[p_Gamelet.i_bossStyle];
                               drawItem(_graphics,sp,j,false);
                               if(sp.i_Frame>0)
                               drawItem(_graphics,sp,j+sp.i_Frame,false);
                           }
                           break;
                     default:
                         drawItem(_graphics,sp,objects[sp.i_ObjectType]+sp.i_Frame,true);
...
/*
        // Drawing Enemy objects
        for (i = 0; i < p_Gamelet.SPRITES_NUMBER; i++)
        {
            sp = p_Gamelet.ap_Sprites[i];
            if (sp.lg_SpriteActive)
            {
...
*/
        ...
        sp = p_Gamelet.p_PlayerSprite;
        if(sp.lg_SpriteActive)
        {
          if (sp.i_ObjectType == GameletImpl.OBJECT_PLAYER)
          {
            int frame = p_Gamelet.i8_player_HorzSpeed;

            item_index = IMG_PLAYER_KREN_VLEVO02
                + (p_Gamelet.i8_player_HorzSpeed + p_Gamelet.I8_PLAYER_MAX_HORZSPEED) * 2
                         / p_Gamelet.I8_PLAYER_MAX_HORZSPEED;
          }
          else
              {
                item_index =  objects[sp.i_ObjectType] + sp.i_Frame;
                if((i_Timer&2)==0) attemptions++;
                   else attemptions = 0;
              }
          if(p_Gamelet.lg_justStarted || !p_Gamelet.lg_immortality || (i_Timer & 1) == 0)
                    drawItem(_graphics,sp,item_index,false);
        }




        _graphics.translate((GAMESCREEN_WIDTH - SCREENWIDTH) >> 1,(GAMESCREEN_HEIGHT - SCREENHEIGHT) >> 1);
        _graphics.setClip(0,0,SCREENWIDTH,SCREENHEIGHT);

//        if(lg_firstPaint)
        {
          // draw left, right
          img = getImage(IMG_RAMKA_LEVO);
          _graphics.drawImage(img,0,0,0);                             //left
          img = getImage(IMG_RAMKA_PRAVO);
          _graphics.drawImage(img,SCREENWIDTH - img.getWidth(),0,0); //right
        }


        img = getImage(IMG_RAMKA_VERH);
        _graphics.drawImage(img,0,0,0);                               //top
        img = getImage(IMG_RAMKA_NIZ);
        _graphics.drawImage(img,0,SCREENHEIGHT - img.getHeight(),0);                                //bottom

        int x = 25, y = 32;

        img = getImage(IMG_PLANE);
        j = 15;                                      // x
        k = SCREENHEIGHT - 21 -(img.getHeight()>>1); // y


        drawBar(_graphics, x,y,p_Gamelet.i_playerHealth,p_Gamelet.i_playerHealthInit,0xff0000 - (Math.abs(16-(i_Timer&31))<<19),0xcccccc);

        if(p_Gamelet.i_bossmode)
        {
          int c = 0x10-Math.abs(16-(i_Timer&31));
          c = (c<<19)|(c<<11)|(c<<3);

          drawBar(_graphics, SCREENWIDTH-x-BAR_WIDTH,y,p_Gamelet.i_bossHealth,p_Gamelet.i_bossInitHealth,0x280078+c,0xcccccc);
        }

        for(item_index = 0; item_index<attemptions; item_index++)
        {
          _graphics.drawImage(img,j,k,0);
          j+=img.getWidth();
        }
        ...
        k += (img.getHeight() - font.fontHeight)>>1;
        j = SCREENWIDTH - 15;

        //                                          /* 5 digits */
        font.drawDigits(_graphics, j - (font.fontWidth<<2) - font.fontWidth,k, 5 , p_Gamelet.getPlayerScore());


        // Drawing status
//        _graphics.setColor(/*Color.white*/0xffffff);
//        _graphics.drawString("X: " + (p_Gamelet.horizont / 10) + " S: " + p_Gamelet.getPlayerScore() + " H: " + p_Gamelet.i_playerHealth , 4, p_Gamelet.i_ScreenHeight - 10);
//        _graphics.drawString("B: " + p_Gamelet.i_bossStyle + " L: " + p_Gamelet.i_Attemptions  + " ", 4, 12);
...
//--------------------------------
        ...
        {
            case Canvas.KEY_NUM1 :
                                   p_Gamelet.i_PlayerKey &= ~(GameletImpl.PLAYER_BUTTON_LEFT | GameletImpl.PLAYER_BUTTON_TOP);
                                   break;
            case Canvas.KEY_NUM2 :
                                   p_Gamelet.i_PlayerKey &= ~GameletImpl.PLAYER_BUTTON_TOP;
                                   break;
            case Canvas.KEY_NUM3 :
                                   p_Gamelet.i_PlayerKey &= ~(GameletImpl.PLAYER_BUTTON_RIGHT | GameletImpl.PLAYER_BUTTON_TOP);
                                   break;
            case Canvas.KEY_NUM4 :
...
//--------------------------------
    ...
    protected void gameAction(int actionID)
    {
          ...
          switch(actionID)
          {
            ...
            case GameletImpl.GAMEACTION_SND_PLAYERAPPEARED : playMelody(ap_melody[SND_VYLET_IGROKA_NA_EKRAN_OTT],false);break;
            case GameletImpl.GAMEACTION_SND_PLAYERSHOT     : playMelody(ap_melody[SND_VYSTREL_IGROKA_OTT],false);break;
            case GameletImpl.GAMEACTION_SND_PLAYERDEAD     : playMelody(ap_melody[SND_SMERT_IGROKA_OTT],false);break;
            case GameletImpl.GAMEACTION_SND_SMALLEXPLOSION :
                      {
                         if ( ((Sound)ap_melody[SND_BOLSHOI_VZRIV_OTT].p_NativeFormat).getState()
                              != Sound.SOUND_PLAYING)
                         {
                                 playMelody(ap_melody[SND_MALENKIY_VZRIV_OTT],false);
                         }
                      }
                      break;
            case GameletImpl.GAMEACTION_SND_HUGEEXPLOSION  : playMelody(ap_melody[SND_BOLSHOI_VZRIV_OTT],false);break;
            case GameletImpl.GAMEACTION_SND_BIRDSCRY       : playMelody(ap_melody[SND_KRIK_STAI_PTITS_OTT],false);break;
            case GameletImpl.GAMEACTION_SND_BOSSAPPEARS    : playMelody(ap_melody[SND_POYAVLENIE_BOSSA_OTT],false);break;
            case GameletImpl.GAMEACTION_SND_PARADEAD       : playMelody(ap_melody[SND_SMERT_PILOTA_OTT],false);break;
            case GameletImpl.GAMEACTION_SND_HELYAPPEARS    : playMelody(ap_melody[SND_VERTOLET_PODDERJKI_OTT],false);break;
            case GameletImpl.GAMEACTION_SND_BONUSTAKEN     : playMelody(ap_melody[SND_VZYATIE_BONUSA_OTT],false);break;
            case GameletImpl.GAMEACTION_SND_PLAYERWOUNDED  :
                      {
...
//--------------------------------
    ...
    //=================================================================================
        ...
        drawDigits font;
        int []objects = new int[] {
          IMG_PLAYER_DVIZSHENIE,    // 1, -2..+2
          IMG_RAKETA_IGROKA,        // 1
          IMG_SNARYAD_IGROKA,       // 1
          IMG_AIR_FIGHTER,          // 1, +7
          IMG_AIR_BOMBER,           // 1, +8

          IMG_AIR_PLANE,            // 1, +8
          IMG_AIR_SPY,              // 1, +6
          IMG_SNARYAD_VRAGA01,      // 1
          IMG_SNARYAD_VRAGA01,      // 1
          IMG_RAKETA_VRAGA,         // 1

          IMG_VZRIV01,              // 7
          IMG_BIRDS_VLEVO01,        // 2
          IMG_BIRDS_VPRAVO01,       // 2
          IMG_BONUS03,              // 1
          IMG_BONUS04,              // 1
          ...
          IMG_PARASHUT_RASKRITIE01, // 6,
          IMG_PARASHUT_PADENIE01,   // 6,
          IMG_CLOUD01,              // 1+1
    ...
    // Boss explosion
          ...
          IMG_AIR_PLANE_PADENIE01,  // 8
          IMG_AIR_BOMBER_PADENIE01, // 8
          IMG_AIR_FIGHTER_PADENIE01,// 7
          IMG_AIR_SPY_PADENIE01,    // 6
          IMG_PLAYER_PADENIE01,
          IMG_VZRIV01,              // 7
          IMG_HELICOPTER            // 2
        };
...
private static final int IMG_AIR_AGRESSION = 0;
private static final int IMG_PROIGRISH = 1;
private static final int IMG_VIIGRISH = 2;
private static final int IMG_DEMO = 3;
private static final int IMG_AIR_BOMBER = 4;
private static final int IMG_AIR_BOMBER_PADENIE01 = 5;
private static final int IMG_AIR_BOMBER_PADENIE02 = 6;
private static final int IMG_AIR_BOMBER_PADENIE03 = 7;
private static final int IMG_AIR_BOMBER_PADENIE04 = 8;
private static final int IMG_AIR_BOMBER_PADENIE05 = 9;
private static final int IMG_AIR_BOMBER_PADENIE06 = 10;
private static final int IMG_AIR_BOMBER_PADENIE07 = 11;
private static final int IMG_AIR_BOMBER_PADENIE08 = 12;
private static final int IMG_AIR_FIGHTER = 13;
private static final int IMG_AIR_FIGHTER_PADENIE01 = 14;
private static final int IMG_AIR_FIGHTER_PADENIE02 = 15;
private static final int IMG_AIR_FIGHTER_PADENIE03 = 16;
private static final int IMG_AIR_FIGHTER_PADENIE04 = 17;
private static final int IMG_AIR_FIGHTER_PADENIE05 = 18;
private static final int IMG_AIR_FIGHTER_PADENIE06 = 19;
private static final int IMG_AIR_FIGHTER_PADENIE07 = 20;
private static final int IMG_AIR_PLANE = 21;
private static final int IMG_AIR_PLANE_PADENIE01 = 22;
private static final int IMG_AIR_PLANE_PADENIE02 = 23;
private static final int IMG_AIR_PLANE_PADENIE03 = 24;
private static final int IMG_AIR_PLANE_PADENIE04 = 25;
private static final int IMG_AIR_PLANE_PADENIE05 = 26;
private static final int IMG_AIR_PLANE_PADENIE06 = 27;
private static final int IMG_AIR_PLANE_PADENIE07 = 28;
private static final int IMG_AIR_PLANE_PADENIE08 = 29;
private static final int IMG_AIR_SPY = 30;
private static final int IMG_AIR_SPY_PADENIE01 = 31;
private static final int IMG_AIR_SPY_PADENIE02 = 32;
private static final int IMG_AIR_SPY_PADENIE03 = 33;
private static final int IMG_AIR_SPY_PADENIE04 = 34;
private static final int IMG_AIR_SPY_PADENIE05 = 35;
private static final int IMG_AIR_SPY_PADENIE06 = 36;
private static final int IMG_TILE_64 = 37;
private static final int IMG_BIRDS_VLEVO01 = 38;
private static final int IMG_BIRDS_VLEVO02 = 39;
private static final int IMG_BIRDS_VPRAVO01 = 40;
private static final int IMG_BIRDS_VPRAVO02 = 41;
private static final int IMG_BONUS03 = 42;
private static final int IMG_BONUS04 = 43;
private static final int IMG_BOSS01 = 44;
private static final int IMG_BOSS01_01 = 45;
private static final int IMG_BOSS01_02 = 46;
private static final int IMG_BOSS01_03 = 47;
private static final int IMG_BOSS02 = 48;
private static final int IMG_BOSS02_01 = 49;
private static final int IMG_BOSS02_02 = 50;
private static final int IMG_BOSS02_03 = 51;
private static final int IMG_BOSS03 = 52;
private static final int IMG_BOSS03_01 = 53;
private static final int IMG_BOSS03_02 = 54;
private static final int IMG_BOSS03_03 = 55;
private static final int IMG_BOSS04 = 56;
private static final int IMG_BOSS04_01 = 57;
private static final int IMG_BOSS04_02 = 58;
private static final int IMG_BOSS04_03 = 59;
private static final int IMG_BOSS05 = 60;
private static final int IMG_BOSS05_01 = 61;
private static final int IMG_BOSS05_02 = 62;
private static final int IMG_BOSS05_03 = 63;
private static final int IMG_BOSS06 = 64;
private static final int IMG_BOSS06_01 = 65;
private static final int IMG_BOSS06_02 = 66;
private static final int IMG_BOSS06_03 = 67;
private static final int IMG_BOSS07 = 68;
private static final int IMG_BOSS07_01 = 69;
private static final int IMG_BOSS07_02 = 70;
private static final int IMG_BOSS07_03 = 71;
private static final int IMG_BOSS08 = 72;
private static final int IMG_BOSS08_01 = 73;
private static final int IMG_BOSS08_02 = 74;
private static final int IMG_BOSS08_03 = 75;
private static final int IMG_BOSS_VZRIV01 = 76;
private static final int IMG_BOSS_VZRIV02 = 77;
private static final int IMG_BOSS_VZRIV03 = 78;
private static final int IMG_BOSS_VZRIV04 = 79;
private static final int IMG_BOSS_VZRIV05 = 80;
private static final int IMG_BOSS_VZRIV06 = 81;
private static final int IMG_BOSS_VZRIV07 = 82;
private static final int IMG_BOSS_VZRIV08 = 83;
private static final int IMG_BOSS_VZRIV09 = 84;
private static final int IMG_CLOUD01 = 85;
private static final int IMG_HELICOPTER = 86;
private static final int IMG_HELICOPTER_VRAZSHENIE = 87;
private static final int IMG_ISLAND01 = 88;
private static final int IMG_ISLAND02 = 89;
private static final int IMG_ISLAND03 = 90;
private static final int IMG_PARASHUT_PADENIE01 = 91;
private static final int IMG_PARASHUT_PADENIE02 = 92;
private static final int IMG_PARASHUT_PADENIE03 = 93;
private static final int IMG_PARASHUT_PADENIE04 = 94;
private static final int IMG_PARASHUT_PADENIE05 = 95;
private static final int IMG_PARASHUT_PADENIE06 = 96;
private static final int IMG_PARASHUT_PADENIE07 = 97;
private static final int IMG_PARASHUT_PADENIE08 = 98;
private static final int IMG_PARASHUT_RASKRITIE01 = 99;
private static final int IMG_PARASHUT_RASKRITIE02 = 100;
private static final int IMG_PARASHUT_RASKRITIE03 = 101;
private static final int IMG_PARASHUT_RASKRITIE04 = 102;
private static final int IMG_PARASHUT_RASKRITIE05 = 103;
private static final int IMG_PARASHUT_RASKRITIE06 = 104;
private static final int IMG_PIK_01 = 105;
private static final int IMG_PIK_02 = 106;
private static final int IMG_PIK_03 = 107;
private static final int IMG_PIK_04 = 108;
private static final int IMG_PIK_05 = 109;
private static final int IMG_PIK_06 = 110;
private static final int IMG_PLAYER_KREN_VLEVO02 = 111;
private static final int IMG_PLAYER_KREN_VLEVO01 = 112;
private static final int IMG_PLAYER_DVIZSHENIE = 113;
private static final int IMG_PLAYER_KREN_VPRAVO01 = 114;
private static final int IMG_PLAYER_KREN_VPRAVO02 = 115;
private static final int IMG_PLAYER_PADENIE01 = 116;
private static final int IMG_PLAYER_PADENIE02 = 117;
private static final int IMG_PLAYER_PADENIE03 = 118;
private static final int IMG_PLAYER_PADENIE04 = 119;
private static final int IMG_PLAYER_PADENIE05 = 120;
private static final int IMG_PLAYER_PADENIE06 = 121;
private static final int IMG_PLAYER_PADENIE07 = 122;
private static final int IMG_PLAYER_PADENIE08 = 123;
private static final int IMG_RAKETA_IGROKA = 124;
private static final int IMG_RAKETA_VRAGA = 125;
private static final int IMG_SNARYAD_IGROKA = 126;
private static final int IMG_SNARYAD_VRAGA01 = 127;
private static final int IMG_STONE01 = 128;
private static final int IMG_STONE02 = 129;
private static final int IMG_STONE03 = 130;
private static final int IMG_VZRIV01 = 131;
private static final int IMG_VZRIV02 = 132;
private static final int IMG_VZRIV03 = 133;
private static final int IMG_VZRIV04 = 134;
private static final int IMG_VZRIV05 = 135;
private static final int IMG_VZRIV06 = 136;
private static final int IMG_VZRIV07 = 137;
private static final int IMG_POINT = 138;
private static final int IMG_PLANE = 139;
private static final int IMG_LIFE = 140;
private static final int IMG_NUMBERS = 141;
private static final int IMG_RAMKA_LEVO = 142;
private static final int IMG_RAMKA_NIZ = 143;
private static final int IMG_RAMKA_PRAVO = 144;
private static final int IMG_RAMKA_VERH = 145;
private static final int TOTAL_IMAGES_NUMBER = 146;
//---The end of the included file images.java---

// The start of including of sounds.java
//--------------------------------
private static final int SND_BOLSHOI_VZRIV_OTT = 0;
private static final int SND_DEATH_THEME_OTT = 1;
private static final int SND_KRIK_STAI_PTITS_OTT = 2;
private static final int SND_MALENKIY_VZRIV_OTT = 3;
private static final int SND_POPADANIE_V_IGROKA_OTT = 4;
private static final int SND_POYAVLENIE_BOSSA_OTT = 5;
private static final int SND_SMERT_IGROKA_OTT = 6;
private static final int SND_SMERT_PILOTA_OTT = 7;
private static final int SND_VERTOLET_PODDERJKI_OTT = 8;
private static final int SND_VYLET_IGROKA_NA_EKRAN_OTT = 9;
private static final int SND_VYSTREL_IGROKA_OTT = 10;
private static final int SND_VZYATIE_BONUSA_OTT = 11;

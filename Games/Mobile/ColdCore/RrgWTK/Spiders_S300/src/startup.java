                        ...
                        if (p_Gamelet.getGameWorldState() == MobileGamelet.GAMEWORLDSTATE_PLAYED)
                        {
                            ...
                            if (i_timeDelay <= 0) i_timeDelay = 1;
                             ...
                             if(p_Canvas.isShown() && i_MIDletState == STATE_GAMEPLAY)
                             {
                               if(i_action_sound < 0)
                               {
                                       if(p_Gamelet.pPlayerAnt.i_ObjectType == MobileGameletImpl.SPRITE_ANT_LEFT
                                       || p_Gamelet.pPlayerAnt.i_ObjectType == MobileGameletImpl.SPRITE_ANT_RIGHT)
                                       {
                                           if(_playing_melody != SND_SHAGI_MURAVIA_MMF)
                                           {
                                              i_action_sound = SND_SHAGI_MURAVIA_MMF;
...
p_imageForOutput = getImage(2);
    ...
    private void gamePlayDrawing(Graphics _graphics)
    {
        ...
        _graphics.drawImage(getImage(IMG_BACK),0,0,0);
          ...
          i_x = 2;  i_y = GAMESCREEN_HEIGHT - img.getHeight()-1;
          ...
          if(p_spr.lg_SpriteActive)
          {
              ...
              switch(p_spr.i_ObjectType)
              {
                  case MobileGameletImpl.SPRITE_ANT:
                      {
                         img = getImage(p_Gamelet.b_AntTwig ? IMG_ANT_LOAD : IMG_ANT);
                         _graphics.drawImage(img, i_x - (img.getWidth()>>1),i_y - img.getHeight(),0);
                      }
                      break;
                  case MobileGameletImpl.SPRITE_ANT_LEFT:
                      {
                          antMove = p_Gamelet.b_AntTwig ? antLoadGoLeft : antGoLeft;

                          i_x -= (antMove.fontWidth>>1);
                          i_y -= antMove.fontHeight;

                           i_sy = i_y+antMove.fontHeight-imgBuffer.getHeight();
                           gBuffer.drawImage(getImage(IMG_BACK),-i_x, -i_sy,0);
                           antMove.drawDigits(gBuffer,0,-antMove.fontHeight+imgBuffer.getHeight(),p_spr.i_Frame);
                           _graphics.drawImage(imgBuffer,i_x, i_sy,0);

                          antMove.drawDigits(_graphics,i_x, i_y ,p_spr.i_Frame);
                      }
                      break;
                  case MobileGameletImpl.SPRITE_ANT_RIGHT:
                      {
                          antMove = p_Gamelet.b_AntTwig ? antLoadGoRight : antGoRight;
                      ...
                      break;
                  case MobileGameletImpl.SPRITE_ANT_UNLOAD:
                      {
                         img = getImage(IMG_ANT_THROW_LOAD);
                         _graphics.drawImage(img, i_x + i_w - img.getWidth(),i_y - img.getHeight(),0);
                      }
                      break;
                  case MobileGameletImpl.SPRITE_ANT_LOAD:
                      {
                         img = getImage(IMG_ANT_TAKE_LOAD);
                         _graphics.drawImage(img, i_x - i_w,i_y - img.getHeight()+2,0);
                      }
                      break;
                  case MobileGameletImpl.SPRITE_ANT_DEAD:
                      {
                         img = getImage(IMG_ANT_CONSTRAINED);
                         _graphics.drawImage(img, i_x - (img.getWidth()>>1)-1 ,i_y - img.getHeight(),0);
                      }
                      break;
              }
          }
          ...
          for (li = 0; li < p_Gamelet.ap_Sprites.length; li++)
          {
              ...
              _graphics.setColor(0xcccccc);
              _graphics.drawLine(i_x, 0, i_x, i_y);

              switch (p_spr.i_ObjectType)
              {
                  case MobileGameletImpl.SPRITE_SPIDER:       spider = spiderHang; i_y+=2; break;
                  case MobileGameletImpl.SPRITE_SPIDER_UP:    spider = spiderUp;    break;
                  case MobileGameletImpl.SPRITE_SPIDER_DOWN:  spider = spiderDown;  break;
                  case MobileGameletImpl.SPRITE_SPIDER_CATCH: spider = spiderCatch; break;
                  default: continue;
              }
              spider.drawDigits(_graphics,i_x - (spider.fontWidth>>1),i_y - (spider.fontHeight>>1),p_spr.i_Frame);
          }
...
if (lg_FinishImagePictured)
    ...
    public int processGameAction(int _arg)
    {
          ...
          switch(_arg)
          {
               case MobileGameletImpl.GAMEACTION_ANT_LOAD:
                          {
                             i_action_sound = SND_VZYAL_BREVNO_MMF;
                          }
                          break;
               case MobileGameletImpl.GAMEACTION_ANT_UNLOAD:
                          {
                            i_action_sound = SND_POLOJIL_BREVNO_MMF;
                          }
                          break;
               case MobileGameletImpl.GAMEACTION_ANT_UNLOADED:
                          {
                          }
                          break;
               case MobileGameletImpl.GAMEACTION_SPIDER_CATCH:
                          {
                            i_action_sound = SND_MURAVEI_UMER_MMF;
                          }
                          break;
          }
        checkPlayabilityMusic();
    ...
    public Object processGameAction(int _arg1, int _arg2, Object _obj1)
    {
     ...
     drawDigits antGoLeft,antGoRight,antLoadGoLeft,antLoadGoRight;
     drawDigits spiderHang,spiderUp,spiderDown,spiderCatch;
     drawDigits font;
     ...
     private void prepareGraphics()
     {
        ...
        antGoLeft         = new drawDigits(getImage(IMG_ANT_GO_LEFT),3,false);
        antGoRight        = new drawDigits(getImage(IMG_ANT_GO_RIGHT),3,false);
        antLoadGoLeft     = new drawDigits(getImage(IMG_ANT_LOAD_GO_LEFT),2,false);
        antLoadGoRight    = new drawDigits(getImage(IMG_ANT_LOAD_GO_RIGHT),2,false);

        spiderHang        = new drawDigits(getImage(IMG_SPIDER),2,false);
        spiderUp          = new drawDigits(getImage(IMG_SPIDER_GO_TOP),2,false);
        spiderDown        = new drawDigits(getImage(IMG_SPIDER_GO_DOWN),2,false);
        spiderCatch       = new drawDigits(getImage(IMG_SPIDER_CATCH),2,false);

        imgBuffer = Image.createImage(antLoadGoLeft.fontWidth,2);
    ...
    {
                             int [] sort_idx = {
SND_SHAGI_MURAVIA_MMF  ,477,
SND_POLOJIL_BREVNO_MMF ,878,
SND_VZYAL_BREVNO_MMF   ,884,
SND_MURAVEI_UMER_MMF   ,1493,
SND_FINAL_MMF          ,0,
SND_START_MMF          ,0
...
private static final int IMG_ANT = 6;
private static final int IMG_ANT_CONSTRAINED = 7;
private static final int IMG_ANT_GO_LEFT = 8;
private static final int IMG_ANT_GO_RIGHT = 9;
private static final int IMG_ANT_LOAD = 10;
private static final int IMG_ANT_LOAD_GO_LEFT = 11;
private static final int IMG_ANT_LOAD_GO_RIGHT = 12;
private static final int IMG_ANT_TAKE_LOAD = 13;
private static final int IMG_ANT_THROW_LOAD = 14;
private static final int IMG_ICON_LIFE = 15;
private static final int IMG_NUMBERS = 16;
private static final int IMG_SPIDER = 17;
private static final int IMG_SPIDER_CATCH = 18;
private static final int IMG_SPIDER_GO_DOWN = 19;
private static final int IMG_SPIDER_GO_TOP = 20;
private static final int TOTAL_IMAGES_NUMBER = 21;

private static final int SND_FINAL_MMF = 0;
private static final int SND_MURAVEI_UMER_MMF = 1;
private static final int SND_POLOJIL_BREVNO_MMF = 2;
private static final int SND_SHAGI_MURAVIA_MMF = 3;
private static final int SND_START_MMF = 4;
private static final int SND_VZYAL_BREVNO_MMF = 5;

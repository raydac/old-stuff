...
public class Sprite
{
    ...
    //
    public int i_Control;
    public int i_Speed;
    public int i_HSpeed;
    public int i_CrashTime;
    public int i_RoadPosition;
    public int i_RoadX;
    //
 ...
 if (i_sX+i_sw < i_dX || i_sY+i_sh < i_dY || i_dX+i_dw < i_sX || i_dY+i_dh < i_sY)
    ...
    public void readSpriteFromStream(DataInputStream _dis) throws IOException
    {
        ...
        i_JumpAngle = _dis.readInt();

        i_Control = _dis.readByte();
 i_Speed   = _dis.readShort();
 i_HSpeed  = _dis.readShort();
 i_CrashTime = _dis.readByte();
 i_RoadPosition = _dis.readInt();
 i_RoadX   = _dis.readInt();
    }
    ...
    public void writeSpriteToStream(DataOutputStream _dis) throws IOException
    {
        ...
        _dis.writeInt(i_JumpAngle);

        _dis.writeByte(i_Control);
 _dis.writeShort(i_Speed);
 _dis.writeShort(i_HSpeed);
 _dis.writeByte(i_CrashTime);
 _dis.writeInt(i_RoadPosition);
 _dis.writeInt(i_RoadX);
                                     // 30 bytes
    }

...
*/
     ...
     paintOnBuffer(_graphics);
        ...
        protected void showNotify()
        {
           lg_firstPaint = true;
           if(p_Gamelet != null)
             p_Gamelet.i_PlayerKey = GameletImpl.PLAYER_BUTTON_NONE;
        }
/*
...
private GameletImpl p_Gamelet;
    ...
    private Image rocket_icon_image;
    private Image cancel_button_image;

    private static final int GAMESCREEN_WIDTH = SCREENWIDTH;
    private static final int GAMESCREEN_HEIGHT = SCREENHEIGHT-26;
...
//        if (i_MIDletState == STATE_MENU) return;
        ...
        try
        {
         rocket_icon_image = Image.createImage("/res/button_rocket.png");
        }catch(Exception e){rocket_icon_image = null;}
...
p_Gamelet = new GameletImpl(GAMESCREEN_WIDTH,GAMESCREEN_HEIGHT,this,null);
    ...
    private void initState(int _state)
    {
        ...
        if (_state != STATE_LOADING && _state != STATE_GAMEPLAY) stopAllMelodies();
...
InitGameDataStorage(p_Gamelet.getGameID(), p_Gamelet.getMaxSizeOfSavedGameBlock(), 8, 6, 10);
                            ...
                            TB = new TileBackground(GAMESCREEN_WIDTH,GAMESCREEN_HEIGHT,16,16);
       logo = null;
                            cancel_button_image = null;

                            {
                             int [] sort_idx = {
SND_VYSTREL_IZ_PUSHKI_IGROKA_OTT,
SND_POPADANIE_V_IGROKA_OTT,
SND_START_RAKETY_IGROKA_OTT,
SND_VZRIV_NADVODNIY_OTT,
SND_VZRIV_NAZEMNIY_OTT,
SND_UNICHTOJENIE_SAMOLETA_OTT,
SND_VYLET_EKRANOPLANA_NA_EKRAN_OTT,
SND_UNICHTOJENIE_MOSTA_OTT,
SND_UTOPLENIE_KORABLA_OTT,
SND_OKONCHANIE_UROVNYA_OTT,
SND_VZRYV_EKRANOPLANA_OTT,
SND_DEATH_THEME_OTT,
SND_WON_THEME_OTT,
SND_ZASTAVKA_OTT
                             };
...
//--------------------------------
        ...
        switch(_key)
        {
            case Canvas.KEY_NUM1 :
                                       {
                                          p_Gamelet.i_PlayerKey = (p_Gamelet.i_PlayerKey & ~GameletImpl.PLAYER_BUTTON_RIGHT)
                                             /* left */           | GameletImpl.PLAYER_BUTTON_LEFT;
                                          p_Gamelet.i_PlayerKey = (p_Gamelet.i_PlayerKey & ~GameletImpl.PLAYER_BUTTON_DOWN)
                                             /* up */             | GameletImpl.PLAYER_BUTTON_UP;
                                       }
                                        break;
            ...
            case Canvas.KEY_NUM3 :
                                       {
                                          p_Gamelet.i_PlayerKey = (p_Gamelet.i_PlayerKey & ~GameletImpl.PLAYER_BUTTON_LEFT)
                                             /* right */          | GameletImpl.PLAYER_BUTTON_RIGHT;
                                          p_Gamelet.i_PlayerKey = (p_Gamelet.i_PlayerKey & ~GameletImpl.PLAYER_BUTTON_DOWN)
                                             /* up */             | GameletImpl.PLAYER_BUTTON_UP;
                                       }
                                        break;
            ...
            case Canvas.KEY_NUM5 :
                                       {
                                          p_Gamelet.i_PlayerKey |= GameletImpl.PLAYER_BUTTON_FIREGUN;
                                       }
                                        break;
            ...
            case Canvas.KEY_NUM7 :
                                       {
                                          p_Gamelet.i_PlayerKey = (p_Gamelet.i_PlayerKey & ~GameletImpl.PLAYER_BUTTON_RIGHT)
                                             /* left */           | GameletImpl.PLAYER_BUTTON_LEFT;
                                          p_Gamelet.i_PlayerKey = (p_Gamelet.i_PlayerKey & ~GameletImpl.PLAYER_BUTTON_UP)
                                             /* down */           | GameletImpl.PLAYER_BUTTON_DOWN;
                                       }
                                        break;
            ...
            case Canvas.KEY_NUM0 :
                                       {
                                         p_Gamelet.i_PlayerKey |= GameletImpl.PLAYER_BUTTON_FIREROCKET;
                                       }
                                        break;
                ...
                case STATE_GAMEPLAY:
                    {
                        ...
                        {
                            i_SystemTimeMark = System.currentTimeMillis();
                            ...
                            i_timeDelay = Math.min(i_timeDelay,i_timeDelay - (int) (System.currentTimeMillis() - i_SystemTimeMark));
                            if (i_timeDelay < 1)  i_timeDelay = 1;
                        }
                        else
                        {
                            ...
                            if (p_Gamelet.i_GameState == Gamelet.GAMESTATE_OVER)
                            {
                                ...
                                {
                                    i_StageCounter++;
                                    playMelody(ap_melody[SND_OKONCHANIE_UROVNYA_OTT],true);
...
//--------------------------------
        ...
        Image img;
          ...
          // drawing panel
          img = getImage(IMG_RAMKA);
          _graphics.drawImage(img,0,0,0);
          int iScreenY = img.getHeight();
          _graphics.translate(0,iScreenY);
...
////////////////////////////////////
        ...
        int i_start_viewY = TB.i_viewCoordY;
        int i_end_viewY = TB.i_viewCoordY + GAMESCREEN_HEIGHT;

        int i_start_viewX = TB.i_viewCoordX;
        int i_end_viewX = TB.i_viewCoordX + GAMESCREEN_WIDTH;


        // Drawing the crashed way
        int i_start_index = 0;
        while ((p_sprite = p_Gamelet.ap_ActiveObjects[i_start_index]).lg_SpriteActive
               && p_sprite.i_ObjectType == GameletImpl.INVISIBLECORRIDOR_NORMAL)
        {
               drawItem(_graphics,p_sprite,IMG_MOST_RAZRUSHENIE01,false);
               i_start_index++;
        }

        // Drawing the player's splashes
        for (li = 0; li < p_Gamelet.ap_Splashes.length; li++)
        {
            p_sprite = p_Gamelet.ap_Splashes[li];
            if (p_sprite.lg_SpriteActive)
            {
               drawItem(_graphics,p_sprite,IMG_BRIZGI01,false);
            }
        }
        ...
        p_sprite = null;
        j = 0;
        k = p_Gamelet.ap_ActiveObjects.length;
        byte []index = new byte[k];

        for (li = i_start_index; li < p_Gamelet.ap_ActiveObjects.length; li++)
        {
            p_sprite = p_Gamelet.ap_ActiveObjects[li];
            if (!p_sprite.lg_SpriteActive) continue;

            switch (p_sprite.i_ObjectType)
            {
                case GameletImpl.ARTILLERY1_NORMAL:
                    {
                       i = IMG_PUSHKA02;
                    }
                    ;
                    break;
                case GameletImpl.ARTILLERY1_DESTROYED:
                    {
                       i = IMG_PUSHKA02_OBLOMKI;
                    }
                    ;
                    break;
                case GameletImpl.ARTILLERY2_NORMAL:
                    {
                       i = IMG_PUSHKA01;
                    }
                    ;
                    break;
                case GameletImpl.ARTILLERY2_DESTROYED:
                    {
                       i = IMG_PUSHKA01_OBLOMKI;
                    }
                    ;
                    break;

                case GameletImpl.TANKLEFT_NORMAL:
                    {
                       i = IMG_KORPUS_VLEVO;
                    }
                    ;
                    break;
                case GameletImpl.TANKLEFT_DESTROYED:
                    {
                       i = IMG_TANK_OBLOMKI;
                    }
                    ;
                ...
                case GameletImpl.TANKRIGHT_NORMAL:
                    {
                       i = IMG_KORPUS_VPRAVO;
                    }
                    ;
                    break;
                case GameletImpl.TANKRIGHT_DESTROYED:
                    {
                       i = IMG_TANK_OBLOMKI;
                    }
                    ;
                ...
                case GameletImpl.ARTILLERY1GUN_NORMAL:
                    {

                       p_sprite.i_keySet = IMG_BASHNYA_POV03 + p_sprite.i_ObjectState;
                       index[j++]=(byte)li;
                    }
                    ;
                    continue;
                case GameletImpl.ARTILLERY2GUN_NORMAL:
                    {
                       p_sprite.i_keySet = IMG_BASHNYA03 + p_sprite.i_ObjectState;
                       index[j++]=(byte)li;
                    }
                    ;
                    continue;
                case GameletImpl.TANKGUN_NORMAL:
                    {
                       p_sprite.i_keySet = IMG_BASHNYA03 + p_sprite.i_ObjectState;
                       index[j++]=(byte)li;
                    }
                ...
                case GameletImpl.WATERMINE_NORMAL:
                    {
                       i = IMG_MINA_POGRUZSHENIE01;
                    }
                    ;
                    break;

                case GameletImpl.RADAR_NORMAL:
                    {
                       i = IMG_RADAR_POVOROT01;
                    }
                    ;
                    break;
                case GameletImpl.RADAR_DESTROYED:
                    {
                       i = IMG_RADAR_OBLOMKI;
                    }
                    ;
                    break;
                case GameletImpl.LINEARSHIP_NORMAL:
                    {
                       i = IMG_NEW_KORABL;
                    }
                    ;
                    break;
                case GameletImpl.LINEARSHIP_DESTROYED:
                    {
                       i = IMG_KORABL_TONET01;
                    }
                    ;
                    break;
                case GameletImpl.WATEREXPLOSION_NORMAL:
                    {
                       p_sprite.i_keySet = IMG_VZRIV_HADVODNII01;
                       index[--k]=(byte)li;
                    }
                    ;
                    continue;
                case GameletImpl.EARTHEXPLOSION_NORMAL:
                    {
                       p_sprite.i_keySet = IMG_VZRIV_NAZEMN01;
                       index[--k]=(byte)li;
                    }
                    ;
                    continue;
                case GameletImpl.SMALLEXPLOSION_NORMAL:
                    {
                       p_sprite.i_keySet = IMG_VZRIVCHIK01;
                       index[--k]=(byte)li;
                    }
                    ;
                    continue;
           ...
           drawItem(_graphics,p_sprite, i ,false);
        }

        // Drawing the player's shells
        for(li=0;li<p_Gamelet.ap_UserGunShells.length;li++)
        {
            p_sprite = p_Gamelet.ap_UserGunShells[li];
            if (p_sprite.lg_SpriteActive)
            {
              drawItem(_graphics,p_sprite,IMG_SNARYAD_IGROKA_STATIKA, false);
            }
        }

        // Drawing the enemy shells
        for(li=0;li<p_Gamelet.ap_EnemyShells.length;li++)
        {
            p_sprite = p_Gamelet.ap_EnemyShells[li];
            if (p_sprite.lg_SpriteActive)
            {
              drawItem(_graphics,p_sprite,IMG_SNARYAD_VRAGA_STATIKA, false);
            }
        }
        ...
        for(li=0;li<j;li++)
        {
            p_sprite = p_Gamelet.ap_ActiveObjects[index[li]];
            drawItem(_graphics,p_sprite,p_sprite.i_keySet, false);
        }

        // Drawing explosions
        for(li=k;li<index.length;li++)
        {
            p_sprite = p_Gamelet.ap_ActiveObjects[index[li]];
            drawItem(_graphics,p_sprite,p_sprite.i_keySet, false);
        }
        ...
        // Drawing the player's rocket
        p_sprite = p_Gamelet.p_UserRocket;
        if (p_sprite.lg_SpriteActive)
        {
            if (p_sprite.i_ObjectType == GameletImpl.SMARTROCKET_NORMAL)
            {
                drawItem(_graphics,p_sprite,IMG_RAKETA_SAMONAV_POVOROT03 + p_sprite.i_ObjectState, false);

                p_sprite = p_Gamelet.p_WarheadTargetSprite;
                if (p_sprite.lg_SpriteActive && (i_Timer&1) == 0)
                {
                    x = (p_sprite.i_ScreenX >> 8) - i_start_viewX;
                    y = (p_sprite.i_ScreenY >> 8) - i_start_viewY;

                    _graphics.setColor(0x0000ff);
                    _graphics.drawArc(x,y, p_sprite.i_width >> 8, p_sprite.i_height >> 8,0,360);
                }

            }
             else
               drawItem(_graphics,p_sprite,IMG_RAKETA_NEUPR_POVOROT01 + p_sprite.i_ObjectState, false);
        }


        // Drawing the Player object
        p_sprite = p_Gamelet.p_PlayerSprite;
        if(p_sprite.i_ObjectType == GameletImpl.PLAYER_NORMAL)
        {
           i = IMG_MONSTER_POVOROT01+p_sprite.i_ObjectState;

           if (p_Gamelet.i_nonKilledDelay <= 0 || (i_Timer & 0x1) != 0)
           {
              drawItem(_graphics,p_sprite,i, false);
           }
        }
         else
              drawItem(_graphics,p_sprite,IMG_MONSTER_GIBEL01+p_sprite.i_ObjectState, false);


        // Drawing the bombers
        for(li=0;li<p_Gamelet.ap_Bombers.length;li++)
        {
            p_sprite = p_Gamelet.ap_Bombers[li];
            if (p_sprite.lg_SpriteActive)
            {
                if (p_sprite.i_ObjectType == GameletImpl.BOMBER_NORMAL)
                {
                    drawItem(_graphics,p_sprite,IMG_DVIZSHENIE, false);
                }
                else
                {
                    drawItem(_graphics,p_sprite,IMG_ISTREBITEL_PADENIE01, false);
                }
            }
        }
        ...
        img = getImage(IMG_LIFE);
         ...
         _graphics.translate(0,-iScreenY);
         _graphics.setClip(0,0,SCREENWIDTH,SCREENHEIGHT);
         j = 9; x = 9; y = (iScreenY-img.getHeight())>>1;
        drawBar(_graphics ,SCREENWIDTH-BAR_WIDTH >> 1 ,iScreenY-BAR_HEIGHT >> 1 ,p_Gamelet.i_playerPower,p_Gamelet.INITPLAYERFORCE,0xff0000 - (Math.abs(16-(i_Timer&31))<<19),0xcccccc);

        int attemptions = p_Gamelet.i_Attemptions-1;
        for(i = 0; i<attemptions; i++)
        {
          _graphics.drawImage(img,x,y,0);
          x+=img.getWidth();
        }


        y += (img.getHeight() - font.fontHeight)>>1;

        //                                          /* 4 digits */
        font.drawDigits(_graphics, GAMESCREEN_WIDTH - (font.fontWidth<<2) -j,y, 4 , p_Gamelet.getPlayerScore());
       ...
       {
         if(rocket_icon_image != null)
         {
            _graphics.drawImage(rocket_icon_image,
              SCREENWIDTH - rocket_icon_image.getWidth()
              +-5

            ,SCREENHEIGHT - rocket_icon_image.getHeight()
            ...
            case STATE_GAMEPLAY:
                {
                    synchronized (p_Gamelet)
                    {
                        if (_key == KEY_MENU) initState(STATE_MENU);
// The start of including of _KeyPressed.java
//--------------------------------
        //---------Insert the keyPressed processing block here
...
//--------------------------------
        ...
        {
            case Canvas.KEY_NUM1 :
                                   p_Gamelet.i_PlayerKey &= ~(GameletImpl.PLAYER_BUTTON_LEFT | GameletImpl.PLAYER_BUTTON_UP);
                                   break;
            case Canvas.KEY_NUM2 :
                                   p_Gamelet.i_PlayerKey &= ~GameletImpl.PLAYER_BUTTON_UP;
                                   break;
            case Canvas.KEY_NUM3 :
                                   p_Gamelet.i_PlayerKey &= ~(GameletImpl.PLAYER_BUTTON_RIGHT | GameletImpl.PLAYER_BUTTON_UP);
                                   break;
            case Canvas.KEY_NUM4 :
                                   p_Gamelet.i_PlayerKey &= ~GameletImpl.PLAYER_BUTTON_LEFT;
                                   break;
            case Canvas.KEY_NUM5 :
                                   p_Gamelet.i_PlayerKey &= ~GameletImpl.PLAYER_BUTTON_FIREGUN;
                                   break;
            case Canvas.KEY_NUM6 :
                                   p_Gamelet.i_PlayerKey &= ~GameletImpl.PLAYER_BUTTON_RIGHT;
                                   break;
            case Canvas.KEY_NUM7 :
                                   p_Gamelet.i_PlayerKey &= ~(GameletImpl.PLAYER_BUTTON_LEFT | GameletImpl.PLAYER_BUTTON_DOWN);
                                   break;
            case Canvas.KEY_NUM8 :
                                   p_Gamelet.i_PlayerKey &= ~GameletImpl.PLAYER_BUTTON_DOWN;
                                   break;
            case Canvas.KEY_NUM9 :
                                   p_Gamelet.i_PlayerKey &= ~(GameletImpl.PLAYER_BUTTON_RIGHT | GameletImpl.PLAYER_BUTTON_DOWN);
                                   break;
            case KEY_CANCEL :
            case Canvas.KEY_NUM0 :
                                   p_Gamelet.i_PlayerKey &= ~GameletImpl.PLAYER_BUTTON_FIREROCKET;
...
//--------------------------------
    ...
    protected void gameAction(int actionID)
    {
      ...
      if(actionID != GameletImpl.GAMEACTION_SND_PLAYERDEATH &&
         (
            (
              ap_melody[SND_UNICHTOJENIE_MOSTA_OTT].p_NativeFormat != null &&
              ((Sound)ap_melody[SND_UNICHTOJENIE_MOSTA_OTT].p_NativeFormat).getState() == Sound.SOUND_PLAYING
            )
              ||
            (
              ap_melody[SND_UTOPLENIE_KORABLA_OTT].p_NativeFormat != null &&
              ((Sound)ap_melody[SND_UTOPLENIE_KORABLA_OTT].p_NativeFormat).getState() == Sound.SOUND_PLAYING
            )
         )
      ) return;
          ...
          switch(actionID)
          {
            case GameletImpl.GAMEACTION_SND_EARTHEXPLOSION    : playMelody(ap_melody[SND_VZRIV_NAZEMNIY_OTT],false);break;
            case GameletImpl.GAMEACTION_SND_WATEREXPLOSION    : playMelody(ap_melody[SND_VZRIV_NADVODNIY_OTT],false);break;
            case GameletImpl.GAMEACTION_SND_PLAYERNURSLAUNCH  : playMelody(ap_melody[SND_START_RAKETY_IGROKA_OTT],false);break;
            case GameletImpl.GAMEACTION_SND_PLAYERSMARTLAUNCH : playMelody(ap_melody[SND_START_RAKETY_IGROKA_OTT],false);break;
            case GameletImpl.GAMEACTION_SND_PLAYERFIRE        : playMelody(ap_melody[SND_VYSTREL_IZ_PUSHKI_IGROKA_OTT],false);break;
            case GameletImpl.GAMEACTION_SND_PLAYERREADY       : playMelody(ap_melody[SND_VYLET_EKRANOPLANA_NA_EKRAN_OTT],false);break;
            case GameletImpl.GAMEACTION_SND_AIRBOMBERREADY    : break;
            case GameletImpl.GAMEACTION_SND_AIRBOMBERDROP     : playMelody(ap_melody[SND_UNICHTOJENIE_SAMOLETA_OTT],false);break;
            case GameletImpl.GAMEACTION_SND_BRIDGEDESTROYED   : playMelody(ap_melody[SND_UNICHTOJENIE_MOSTA_OTT],false);break;
            case GameletImpl.GAMEACTION_SND_SHIPDESTROYED     : playMelody(ap_melody[SND_UTOPLENIE_KORABLA_OTT],false);break;
            case GameletImpl.GAMEACTION_SND_PLAYERHIT         : break;
            case GameletImpl.GAMEACTION_SND_ENEMYHIT          :
                          {
                                  playMelody(ap_melody[SND_POPADANIE_V_IGROKA_OTT],false);
                                  break;
                          }
            case GameletImpl.GAMEACTION_SND_PLAYERDEATH       :
                          {
                                  stopAllMelodies();
                                  playMelody(ap_melody[SND_VZRYV_EKRANOPLANA_OTT],false);
                                  break;
                          }
          }
    }
...
private short [] ai_stage_tiles = {13,13,13,13};
   ...
   private final static int MAP_ITEM_WIDTH = GameletImpl.VIRTUALCELLWIDTH * Stages.PARTCELLWIDTH;
   private final static int MAP_ITEM_HEIGHT = GameletImpl.VIRTUALCELLHEIGHT * Stages.PARTCELLHEIGHT;
   ...
   private void drawBar(Graphics _graphics, int x, int y, int cur, int max, int col1, int col2)
   {
    ...
    public int getCenterX()
    {
        ...
        int visX = (sp.i_mainX)>>8;
        visX = Math.max(0,
               Math.min(
                    MAP_ITEM_WIDTH-GAMESCREEN_WIDTH,
                    (visX-(GAMESCREEN_WIDTH>>1)-16)));
        return visX;
    }
    public int getCenterY()
    {
        return p_Gamelet.i8_backgroundOffsetStart>>8;
    }
...
*/
   Image p_tiledecorset;
   private void loadStageDecoration(){
         try
         {
             if (p_tiledecorset == null) p_tiledecorset = Image.createImage("/res/sprites.png");
             TB.setBlockImage(p_tiledecorset);
             TB.setGameRoomArray(MAP_ITEM_WIDTH>>4,Stages.getDecorationForStage(i_StageCounter));
         }
           catch ( Exception e )
           {
   ...
   private void drawItem(Graphics _graphics, Sprite p_spr, int i_indx, boolean align)
   {
        ...
        _x = (p_spr.i_ScreenX >> 8) - TB.i_viewCoordX;;
        _y = (p_spr.i_ScreenY >> 8) - TB.i_viewCoordY;;
// #-
        if(align)
        {
           _x += (_dw >> 1 );
           _y += ((p_spr.i_height>>8) - img.getHeight())>>1;
        }
        else
// #+
        {
        ...
        if (h_flip)
        {
           DirectUtils.getDirectGraphics(_graphics).drawImage(img,_x,_y,0,0x2000 /*FLIP_HORIZONTAL*/);
        }
        else
        {
           _graphics.drawImage(img,_x,_y,0);
        }
...
private static final int IMG_LIFE = 4;
private static final int IMG_NUMBERS = 5;
private static final int IMG_RAMKA = 6;
private static final int IMG_DVIZSHENIE = 7;
private static final int IMG_ISTREBITEL_PADENIE01 = 8;
private static final int IMG_ISTREBITEL_PADENIE02 = 9;
private static final int IMG_ISTREBITEL_PADENIE03 = 10;
private static final int IMG_ISTREBITEL_PADENIE04 = 11;
private static final int IMG_ISTREBITEL_PADENIE05 = 12;
private static final int IMG_NEW_KORABL = 13;
private static final int IMG_KORABL_TONET01 = 14;
private static final int IMG_KORABL_TONET02 = 15;
private static final int IMG_KORABL_TONET03 = 16;
private static final int IMG_KORABL_TONET04 = 17;
private static final int IMG_MINA_POGRUZSHENIE01 = 18;
private static final int IMG_MINA_POGRUZSHENIE02 = 19;
private static final int IMG_MINA_POGRUZSHENIE03 = 20;
private static final int IMG_MINA_POGRUZSHENIE04 = 21;
private static final int IMG_MINA_POGRUZSHENIE05 = 22;
private static final int IMG_BRIZGI01 = 23;
private static final int IMG_BRIZGI02 = 24;
private static final int IMG_BRIZGI03 = 25;
private static final int IMG_BRIZGI04 = 26;
private static final int IMG_MONSTER_GIBEL01 = 27;
private static final int IMG_MONSTER_GIBEL02 = 28;
private static final int IMG_MONSTER_GIBEL03 = 29;
private static final int IMG_MONSTER_GIBEL04 = 30;
private static final int IMG_MONSTER_POVOROT01 = 31;
private static final int IMG_MONSTER_POVOROT02 = 32;
private static final int IMG_MONSTER_POVOROT03 = 33;
private static final int IMG_MONSTER_POVOROT04 = 34;
private static final int IMG_MONSTER_POVOROT05 = 35;
private static final int IMG_MONSTER_POVOROT06 = 36;
private static final int IMG_MONSTER_POVOROT07 = 37;
private static final int IMG_MOST_RAZRUSHENIE01 = 38;
private static final int IMG_MOST_RAZRUSHENIE02 = 39;
private static final int IMG_MOST_RAZRUSHENIE03 = 40;
private static final int IMG_MOST_RAZRUSHENIE04 = 41;
private static final int IMG_PUSHKA01 = 42;
private static final int IMG_PUSHKA01_OBLOMKI = 43;
private static final int IMG_PUSHKA02 = 44;
private static final int IMG_PUSHKA02_OBLOMKI = 45;
private static final int IMG_BASHNYA_POV03 = 46;
private static final int IMG_BASHNYA_POV04 = 47;
private static final int IMG_BASHNYA_POV05 = 48;
private static final int IMG_BASHNYA_POV06 = 49;
private static final int IMG_BASHNYA_POV07 = 50;
private static final int IMG_BASHNYA_POV08 = 51;
private static final int IMG_BASHNYA_POV01 = 52;
private static final int IMG_BASHNYA_POV02 = 53;
private static final int IMG_RADAR_OBLOMKI = 54;
private static final int IMG_RADAR_POVOROT01 = 55;
private static final int IMG_RADAR_POVOROT02 = 56;
private static final int IMG_RADAR_POVOROT03 = 57;
private static final int IMG_RADAR_POVOROT04 = 58;
private static final int IMG_RADAR_POVOROT05 = 59;
private static final int IMG_RADAR_POVOROT06 = 60;
private static final int IMG_RADAR_POVOROT07 = 61;
private static final int IMG_RADAR_POVOROT08 = 62;
private static final int IMG_RAKETA_NEUPR_POVOROT01 = 63;
private static final int IMG_RAKETA_NEUPR_POVOROT02 = 64;
private static final int IMG_RAKETA_NEUPR_POVOROT03 = 65;
private static final int IMG_RAKETA_NEUPR_POVOROT04 = 66;
private static final int IMG_RAKETA_NEUPR_POVOROT05 = 67;
private static final int IMG_RAKETA_NEUPR_POVOROT06 = 68;
private static final int IMG_RAKETA_NEUPR_POVOROT07 = 69;
private static final int IMG_RAKETA_SAMONAV_POVOROT03 = 70;
private static final int IMG_RAKETA_SAMONAV_POVOROT04 = 71;
private static final int IMG_RAKETA_SAMONAV_POVOROT05 = 72;
private static final int IMG_RAKETA_SAMONAV_POVOROT06 = 73;
private static final int IMG_RAKETA_SAMONAV_POVOROT07 = 74;
private static final int IMG_RAKETA_SAMONAV_POVOROT08 = 75;
private static final int IMG_RAKETA_SAMONAV_POVOROT01 = 76;
private static final int IMG_RAKETA_SAMONAV_POVOROT02 = 77;
private static final int IMG_SNARYAD_IGROKA_STATIKA = 78;
private static final int IMG_SNARYAD_VRAGA_STATIKA = 79;
private static final int IMG_KORPUS_VLEVO = 80;
private static final int IMG_KORPUS_VPRAVO = 81;
private static final int IMG_TANK_OBLOMKI = 82;
private static final int IMG_BASHNYA03 = 83;
private static final int IMG_BASHNYA04 = 84;
private static final int IMG_BASHNYA05 = 85;
private static final int IMG_BASHNYA06 = 86;
private static final int IMG_BASHNYA07 = 87;
private static final int IMG_BASHNYA08 = 88;
private static final int IMG_BASHNYA01 = 89;
private static final int IMG_BASHNYA02 = 90;
private static final int IMG_VZRIV_HADVODNII01 = 91;
private static final int IMG_VZRIV_HADVODNII02 = 92;
private static final int IMG_VZRIV_HADVODNII03 = 93;
private static final int IMG_VZRIV_HADVODNII_01 = 94;
private static final int IMG_VZRIV_HADVODNII_02 = 95;
private static final int IMG_VZRIV_HADVODNII04 = 96;
private static final int IMG_VZRIV_HADVODNII05 = 97;
private static final int IMG_VZRIV_HADVODNII06 = 98;
private static final int IMG_VZRIV_HADVODNII07 = 99;
private static final int IMG_VZRIV_HADVODNII08 = 100;
private static final int IMG_VZRIV_HADVODNII09 = 101;
private static final int IMG_VZRIV_NAZEMN01 = 102;
private static final int IMG_VZRIV_NAZEMN02 = 103;
private static final int IMG_VZRIV_NAZEMN03 = 104;
private static final int IMG_VZRIV_NAZEMN04 = 105;
private static final int IMG_VZRIV_NAZEMN05 = 106;
private static final int IMG_VZRIV_NAZEMN06 = 107;
private static final int IMG_VZRIV_NAZEMN07 = 108;
private static final int IMG_VZRIVCHIK01 = 109;
private static final int IMG_VZRIVCHIK02 = 110;
private static final int IMG_VZRIVCHIK03 = 111;
private static final int IMG_VZRIVCHIK04 = 112;
private static final int TOTAL_IMAGES_NUMBER = 113;
...
private static final int SND_OKONCHANIE_UROVNYA_OTT = 1;
private static final int SND_POPADANIE_V_IGROKA_OTT = 2;
private static final int SND_START_RAKETY_IGROKA_OTT = 3;
private static final int SND_UNICHTOJENIE_MOSTA_OTT = 4;
private static final int SND_UNICHTOJENIE_SAMOLETA_OTT = 5;
private static final int SND_UTOPLENIE_KORABLA_OTT = 6;
private static final int SND_VYLET_EKRANOPLANA_NA_EKRAN_OTT = 7;
private static final int SND_VYSTREL_IZ_PUSHKI_IGROKA_OTT = 8;
private static final int SND_VZRIV_NADVODNIY_OTT = 9;
private static final int SND_VZRIV_NAZEMNIY_OTT = 10;
private static final int SND_VZRYV_EKRANOPLANA_OTT = 11;
private static final int SND_WON_THEME_OTT = 12;
private static final int SND_ZASTAVKA_OTT = 13;

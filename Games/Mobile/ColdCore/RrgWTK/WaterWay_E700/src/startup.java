...
p_imageForOutput = getImage(2);
    ...
    private void gamePlayDrawing(Graphics _graphics)
    {
        ...
        int li,i_sx,i_sy,i_w,i_h,i_x, i_y, pdMark;
        Sprite p_spr;
        Image img;
     ...
     p_spr = p_Gamelet.pPlayerAnt;

     i_x = p_spr.i_mainX >> 8;
     i_y = (p_spr.i_ScreenY + p_spr.i_height>> 8) -1;

     img = getImage(IMG_SHADOW);
     ...
     for (li = 0; li < p_Gamelet.ap_Sprites.length; li++)
     {
         ...
         {
             case MobileGameletImpl.SPRITE_DROP_PUDDLET_DRYING:
                 i_x = p_spr.i_mainX >> 8;
                 i_y = p_spr.i_mainY >> 8;

                 puddle.drawDigits(_graphics,
                      i_x - (puddle.fontWidth>>1),
                      i_y - (puddle.fontHeight>>1),
                      p_spr.i_Frame >3 ? 0 : 1);
                 break;
             case MobileGameletImpl.SPRITE_DROP_PUDDLET:
                 i_x = p_spr.i_mainX >> 8;
                 i_y = p_spr.i_mainY >> 8;
                 ...
                 puddle.drawDigits(_graphics,
                      ...
                      i_y - (puddle.fontHeight>>1),
                      1);
                 sort_buffer[pdMark++] = p_spr;
                 break;
             case MobileGameletImpl.SPRITE_DROP:
                 sort_buffer[pdMark++] = p_spr;
                 break;
         }
     }
     ...
     li = p_spr.i_Frame;
     i_sy = p_spr.i_Frame == 1 ? -2 : 0;

     if(p_spr.i_ObjectType == MobileGameletImpl.SPRITE_ANT_DEAD)
     {
           AntWet.drawDigits(_graphics,i_x - (AntWet.fontWidth>>1),i_y, li);
     }
     else
     {
         ...
         {
             default:
             case MobileGameletImpl.SPRITE_ANT:
                 {
                    img = getImage(IMG_ANT_FRONT);
                    _graphics.drawImage(img, i_x - (img.getWidth()>>1),i_y+4,0);
                    i_x--;
                    img = getImage(IMG_LEAF);
                    _graphics.drawImage(img, i_x +2 - (img.getWidth()>>1),i_y +3 -(img.getHeight()>>1),0);
                 }
                 break;
             case MobileGameletImpl.SPRITE_ANT_LEFT:
                 {
                    AntLeft.drawDigits(_graphics,i_x - (AntLeft.fontWidth>>1),i_y,li);
                    img = getImage(IMG_LEAF);
                    _graphics.drawImage(img, i_x +2 - (img.getWidth()>>1),i_y + i_sy,0);
                    i_sy++;
                 }
                 break;
             case MobileGameletImpl.SPRITE_ANT_RIGHT:
                 {
                    AntRight.drawDigits(_graphics,i_x - (AntRight.fontWidth>>1),i_y,li);
                    i_x--;
                    img = getImage(IMG_LEAF);
                    _graphics.drawImage(img, i_x +2 - (img.getWidth()>>1),i_y + i_sy,0);
                    i_sy++;
                 ...
                 break;
             case MobileGameletImpl.SPRITE_ANT_COLLECTED_DROP:
             case MobileGameletImpl.SPRITE_ANT_COLLECTED_DROP_LEFT:
             case MobileGameletImpl.SPRITE_ANT_COLLECTED_DROP_RIGHT:
                 {
                    img = getImage(IMG_ANT_FRONT);
                    _graphics.drawImage(img, i_x - (img.getWidth()>>1),i_y+4,0);
                    i_x--;
                 ...
                 break;
             case MobileGameletImpl.SPRITE_ANT_POUR_OUT_LEFT:
                 {
                    switch(li)
                    {
                       case 0:
                       case 1:
                              {
                                img = getImage(IMG_ANT_WATER_LEFT);
                                _graphics.drawImage(img, i_x - (img.getWidth()>>1),i_y + (p_spr.i_height>>8) - img.getHeight(),0);
                              }
                              break;
                       case 2:
                       case 3:
                              {
                                img = getImage(IMG_DROP);
                                _graphics.drawImage(img, i_x - 8 -(img.getWidth()>>1),i_y + (p_spr.i_height>>9) - (img.getHeight()>>1) -4 + (li>>2),0);
                              }
                       default:
                              {
                                AntLeft.drawDigits(_graphics,i_x - (AntLeft.fontWidth>>1),i_y,1);
                                img = getImage(IMG_LEAF);
                                _graphics.drawImage(img, i_x +2 - (img.getWidth()>>1),i_y,0);
                              }
                              break;
                    }
                 }
                 break;
             case MobileGameletImpl.SPRITE_ANT_POUR_OUT_RIGHT:
                 {
                    switch(li)
                    {
                       case 0:
                       case 1:
                              {
                                img = getImage(IMG_ANT_WATER_RIGHT);
                                _graphics.drawImage(img, i_x - (img.getWidth()>>1),i_y + (p_spr.i_height>>8) - img.getHeight(),0);
                              }
                              break;
                       case 2:
                       ...
                       default:
                              {
                                AntRight.drawDigits(_graphics,i_x - (AntRight.fontWidth>>1),i_y,1);
                                img = getImage(IMG_LEAF);
                                _graphics.drawImage(img, i_x +2 - (img.getWidth()>>1),i_y,0);
                              }
                              break;
         ...
         if(p_Gamelet.b_AntDrop)
         {
                img = getImage(IMG_DROP_LIE);
                _graphics.drawImage(img, i_x +1 - (img.getWidth()>>1),i_y +1 + i_sy -(img.getHeight()>>1),0);
         }
     }

     p_spr = p_Gamelet.pAnt;
     if(p_spr.lg_SpriteActive)
     {
              ...
              if(i_x > (GAMESCREEN_WIDTH>>1))
              {
                  img = getImage(IMG_ANT_RIGHT_FLOWER);
              }
                else
                     {
                        img = getImage(IMG_ANT_LEFT_FLOWER);
                     }

              _graphics.setClip(3,0,GAMESCREEN_WIDTH-6,GAMESCREEN_HEIGHT);
              _graphics.drawImage(img, i_x,i_y - img.getHeight(),0);
              _graphics.setClip(0,0,GAMESCREEN_WIDTH,GAMESCREEN_HEIGHT);
     }


     for (li = 0; li < pdMark; li++)
     {
         ...
         {
             case MobileGameletImpl.SPRITE_DROP_PUDDLET:
                 if(i_sx < splashes.i_origRadix)
                    splashes.drawDigits(_graphics,
                        i_x - (splashes.fontWidth>>1),
                        i_y - (splashes.fontHeight>>1),
                        i_sx);
                 break;
             case MobileGameletImpl.SPRITE_DROP:
                img = getImage(IMG_DROP);
                _graphics.drawImage(img, i_x - (img.getWidth()>>1),i_y + i_h - img.getHeight(),0);
                 break;
         }
     }
...
if (lg_FinishImagePictured)
    ...
    public int processGameAction(int _arg)
    {
          ...
          switch(_arg)
          {
               case MobileGameletImpl.GAMEACTION_ANT_CATCH_DROP:
                          {
                            i_action_sound = SND_KAPLYA_GLUHO_MMF;
                          }
                          break;
               case MobileGameletImpl.GAMEACTION_ANT_POUR_OUT:
                          {
                            i_action_sound = SND_VYLIVAET_VODU_MMF;
                          }
                          break;
               case MobileGameletImpl.GAMEACTION_ANT_POURED_OUT:
                          {
                          }
                          break;
               case MobileGameletImpl.GAMEACTION_ANT_DEAD:
                          {
                            i_action_sound = SND_OBLIVANIE_VODOI_MMF;
                          }
                          break;
               case MobileGameletImpl.GAMEACTION_ANT_FLOWER:
                          {
                            i_action_sound = SND_POYAVLENIE_MMF;
                          }
                          break;
               case MobileGameletImpl.GAMEACTION_DROP_GENERATED:
                          {
                          }
                          break;
               case MobileGameletImpl.GAMEACTION_DROP_ON_THE_LAND:
                          {
                            i_action_sound = SND_KAPLYA_GLUHO_MMF;
                          }
                          break;
               case MobileGameletImpl.GAMEACTION_DROP_INTO_THE_PUDDLE:
                          {
                            i_action_sound = SND_KAPLYA_ZVONKO_MMF;
                          }
                          break;
               case MobileGameletImpl.GAMEACTION_DROP_DRYING:
                          {
                          }
                          break;
          }
    ...
    public Object processGameAction(int _arg1, int _arg2, Object _obj1)
    {
     ...
     drawDigits font;
     drawDigits AntLeft,AntRight,AntWet,puddle,splashes;
     ...
     private void prepareGraphics()
     {
        ...
        AntLeft           = new drawDigits(getImage(IMG_ANT_GO_LEFT),2,false);
        AntRight          = new drawDigits(getImage(IMG_ANT_GO_RIGHT),2,false);
        AntWet            = new drawDigits(getImage(IMG_ANT_WET),3,false);
        puddle            = new drawDigits(getImage(IMG_PUDDLE),2,false);
        splashes            = new drawDigits(getImage(IMG_SPLASHES),3,false);
    ...
    {
                             int [] sort_idx = {
SND_KAPLYA_GLUHO_MMF    ,341,
SND_KAPLYA_ZVONKO_MMF   ,341,
SND_POYAVLENIE_MMF      ,1544,
SND_OBLIVANIE_VODOI_MMF ,898,
SND_VYLIVAET_VODU_MMF   ,771,
SND_FINAL_MMF           ,0,
SND_START_MMF           ,0
                             };
...
private static final int IMG_ANT_FRONT = 6;
private static final int IMG_ANT_GO_LEFT = 7;
private static final int IMG_ANT_GO_RIGHT = 8;
private static final int IMG_ANT_LEFT_FLOWER = 9;
private static final int IMG_ANT_RIGHT_FLOWER = 10;
private static final int IMG_ANT_WATER_LEFT = 11;
private static final int IMG_ANT_WATER_RIGHT = 12;
private static final int IMG_ANT_WET = 13;
private static final int IMG_DROP = 14;
private static final int IMG_DROP_LIE = 15;
private static final int IMG_ICON_LIFE = 16;
private static final int IMG_LEAF = 17;
private static final int IMG_PUDDLE = 18;
private static final int IMG_SPLASHES = 19;
private static final int IMG_NUMBERS = 20;
private static final int IMG_SHADOW = 21;
private static final int TOTAL_IMAGES_NUMBER = 22;

private static final int SND_FINAL_MMF = 0;
private static final int SND_KAPLYA_GLUHO_MMF = 1;
private static final int SND_KAPLYA_ZVONKO_MMF = 2;
private static final int SND_OBLIVANIE_VODOI_MMF = 3;
private static final int SND_POYAVLENIE_MMF = 4;
private static final int SND_START_MMF = 5;
private static final int SND_VYLIVAET_VODU_MMF = 6;

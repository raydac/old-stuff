                        ...
                        if (p_Gamelet.getGameWorldState() == MobileGamelet.GAMEWORLDSTATE_PLAYED)
                        {
                            ...
                            if (i_timeDelay <= 0) i_timeDelay = 1;
                             ...
                             if(p_Canvas.isShown() && i_MIDletState == STATE_GAMEPLAY)
                             {
                               ...
                               {
                                 playMelody(ap_melody[i_action_sound],(i_action_sound == SND_VYHOD_MMF));
                                 i_action_sound = -1;
                               }
                             }
...
p_imageForOutput = getImage(2);
    ...
    private void gamePlayDrawing(Graphics _graphics)
    {
        ...
        int li,i_sx,i_sy,i_w,i_h,i_x, i_y, i_ndx;
        Sprite p_spr;
        Image img;


          updateBackScreen(getCenterX(),getCenterY());
          drawImageToGraphics(_graphics,0,0);
        ...
        int stX = i_x - (GAMESCREEN_WIDTH >> 1);
        int stY = i_y - (GAMESCREEN_HEIGHT>> 1);
        int enX = stX - (GAMESCREEN_WIDTH >> 1);
        int enY = stY - (GAMESCREEN_HEIGHT>> 1);

        _graphics.translate(-stX,-stY);
...
i_y+=1;
...

        {
             ...
             {
                 default:
                 case MobileGameletImpl.SPRITE_TARAKAN_LEFT:      cockroach = (p_Gamelet.b_Skin) ? cockroach_green_left : cockroach_left; break;
                 case MobileGameletImpl.SPRITE_TARAKAN_RIGHT:     cockroach = (p_Gamelet.b_Skin) ? cockroach_green_right:cockroach_right; break;
                 case MobileGameletImpl.SPRITE_TARAKAN_UP:        cockroach = (p_Gamelet.b_Skin) ? cockroach_green_up   :   cockroach_up; break;
                 case MobileGameletImpl.SPRITE_TARAKAN_DOWN:      cockroach = (p_Gamelet.b_Skin) ? cockroach_green_down : cockroach_down; break;
                 case MobileGameletImpl.SPRITE_TARAKAN_TURN_LEFT:
                 case MobileGameletImpl.SPRITE_TARAKAN_TURN_UP :
                 case MobileGameletImpl.SPRITE_TARAKAN_TURN_RIGHT:
                 case MobileGameletImpl.SPRITE_TARAKAN_TURN_DOWN:
                 case MobileGameletImpl.SPRITE_TARAKAN_DISABLED :
                      {
                               cockroach = (p_Gamelet.b_Skin) ? cockroach_green_rotate : cockroach_rotate;
                               boolean clockwise = true;
                               switch (p_spr.i_ObjectState)
                               {
                                     default:
                                     case MobileGameletImpl.SPRITE_TARAKAN_LEFT:
                                          {
                                              if(p_spr.i_ObjectType == MobileGameletImpl.SPRITE_TARAKAN_TURN_DOWN)
                                                 i_sx = 3;
                                                else
                                                   i_sx = 0;
                                          }
                                          break;
                                     case MobileGameletImpl.SPRITE_TARAKAN_RIGHT:
                                          {
                                              if(p_spr.i_ObjectType == MobileGameletImpl.SPRITE_TARAKAN_TURN_UP)
                                                 i_sx = 1;
                                                else
                                                   i_sx = 2;
                                          }
                                          break;
                                     case MobileGameletImpl.SPRITE_TARAKAN_UP:
                                          {
                                              if(p_spr.i_ObjectType == MobileGameletImpl.SPRITE_TARAKAN_TURN_LEFT)
                                                 i_sx = 0;
                                                else
                                                   i_sx = 1;
                                          }
                                          break;
                                     case MobileGameletImpl.SPRITE_TARAKAN_DOWN:
                                          {
                                              if(p_spr.i_ObjectType == MobileGameletImpl.SPRITE_TARAKAN_TURN_RIGHT)
                                                 i_sx = 2;
                                                else
                                                   i_sx = 3;
                                          }
        ...
        cockroach.drawDigits(_graphics,i_x - (cockroach.fontWidth>>1),i_y - (cockroach.fontHeight>>1),li);
        ...
        for(li=0;li<ap_sprites.length;li++)
        {
           ...
           if (p_spr.lg_SpriteActive)
           {
              switch(p_spr.i_ObjectType)
              {
                 ...
                 case MobileGameletImpl.SPRITE_SCORE       :
                     {
                        i_sx = (p_spr.i_mainX>>8) - score_blue.fontWidth - (score_blue.fontWidth >> 1);
                        i_sy = (p_spr.i_ScreenY>>8) - (p_spr.i_Frame * 3);

                        if(p_spr.i_ObjectState < 0)
                        {
                             i_sx -= score_blue.fontWidth >> 1;
                             score_blue.drawDigits(_graphics, i_sx, i_sy, 10);
                             i_sx += score_blue.fontWidth;
                             score_blue.drawDigits(_graphics, i_sx, i_sy, 3, -p_spr.i_ObjectState, 10);
                        }
                         else
                              score_blue.drawDigits(_graphics, i_sx, i_sy, 3, p_spr.i_ObjectState, 10);
                     }
        ...
        score_red.drawDigits(_graphics,GAMESCREEN_WIDTH - (score_red.fontWidth<<2)-3,2,4,p_Player.m_iPlayerGameScores,10);
...
return true;
    ...
    private void startGame()
    {
      ...
      for(int x=0; x < 16; x++)
      {
        for(int y=0; y < 16; y++)
        {
          ab_tile_set[(y<<4)+x]=getDecorationFromWallMap(x,y);
        }
      }

      setGameRoomArray(16,ab_tile_set);
      setXY(getCenterX(),getCenterY());
...
if (lg_FinishImagePictured)
    ...
    public int processGameAction(int _arg)
    {
          ...
          switch(_arg)
          {
               ...
               case MobileGameletImpl.GAMEACTION_TARAKAN_WIN:
                          {
                            i_action_sound = SND_VYHOD_MMF;
                          }
                          break;
               case MobileGameletImpl.GAMEACTION_TARAKAN_PARALIZED:
                          {
                            i_action_sound = SND_SMERT_TARAKANA_MMF;
                          }
                          break;
               case MobileGameletImpl.GAMEACTION_TARAKAN_RELIEF:
                          {
                          }
                          return 0; 
               case MobileGameletImpl.GAMEACTION_TARAKAN_RESTORE_SPEED:
                          {
                            i_action_sound = SND_VOSST_SKOROSTI_MMF;
                          }
                          break;
               case MobileGameletImpl.GAMEACTION_TARAKAN_CHANGE_SKIN_BACK:
                          {
                          }
                          return 0;
               case MobileGameletImpl.GAMEACTION_TARAKAN_EAT_SAUSAGE:
                          {
                            i_action_sound = SND_EST_KOLBASU_MMF;
                          }
                          break;
               case MobileGameletImpl.GAMEACTION_TARAKAN_EAT_FISH_SKELETON:
                          {
                            i_action_sound = SND_EST_KOSTI_MMF;
                          }
                          break;
               case MobileGameletImpl.GAMEACTION_TARAKAN_EAT_MUSHROOM:
                          {
                            i_action_sound = SND_EST_MUHOMOR_MMF;
                          }
                          break;
               case MobileGameletImpl.GAMEACTION_TARAKAN_EAT_BREAD_CRUST:
                          {
                            i_action_sound = SND_EST_HLEB_MMF;
                          }
                          break;
               case MobileGameletImpl.GAMEACTION_TARAKAN_EAT_APPLE_BIT:
                          {
                            i_action_sound = SND_EST_OGRYZOK_MMF;
                          }
                          break;
               case MobileGameletImpl.GAMEACTION_TARAKAN_EAT_JELLY:
                          {
                            i_action_sound = SND_EST_KISEL_MMF;
                          }
                          break;
          }
    ...
    public byte getDecorationFromWallMap(int x, int y)
    {
        int val,tmp=0;
        x&=0xf; y&=0xf;


        val = p_Gamelet.as_GameArray[y][x];
        tmp |=  ((val&1) << 3) | ((val&2) << 1);               

        val = p_Gamelet.as_GameArray[(y-1)&0xf][x];
        tmp |=  ((val&1) << 1) | ((val&8) >> 1);               

        val = p_Gamelet.as_GameArray[y][(x-1)&0xf];
        tmp |=  ((val&2) >> 1) | ((val&4) << 1);               

        val = p_Gamelet.as_GameArray[(y-1)&0xf][(x-1)&0xf];
        tmp |=  ((val&4) >> 1) | ((val&8) >> 3);               

        val = x+y;
        tmp |= ((val + (val<<1)) & 2) <<3;

        return ab_translation_table[tmp];
    ...
    public int processGameAction(int _arg1, int _arg2)
    {
        ...
        for(int i = 0; i < 4; i++)
        {
          ...
          TB_changeArrayCell(dx, dy, getDecorationFromWallMap(dx,dy));
        }
        return 0;
    }
    ...
    public Object processGameAction(int _arg1, int _arg2, Object _obj1)
    {
     ...
     Sprite sort_buffer[] = new Sprite[ 15 ];
     int sort_mark;

     byte []ab_tile_set;              
     byte []ab_translation_table      = {
                                            0,19,12,15,16,      18,13,14,4, 7,
                                            8,11, 5, 6, 9,      10, 0,19,12,15,
                                           16,17,13,14, 4,       7, 3,11, 5, 6,
                                            9,10
                                        };

     drawDigits cockroach_left,cockroach_right,cockroach_up,cockroach_down,cockroach_rotate;
     drawDigits cockroach_green_left,cockroach_green_right,cockroach_green_up,cockroach_green_down,cockroach_green_rotate;
     drawDigits score_red,score_blue;


     private void prepareGraphics() throws IOException
     {
        byte[] ab_original_decoration = new byte[256];
        p_Gamelet.ab_initGameArray = new byte[16][16];
        ab_tile_set = new byte[256];


        try
        {
              Runtime.getRuntime().gc();
              InputStream dis = getClass().getResourceAsStream("/res/level.bin");
              dis.read(ab_original_decoration);

              for(int y=0; y < 16; y++)
              {
                 for(int x=0; x < 16; x++)
                 {
                     p_Gamelet.ab_initGameArray[y][x] = ab_original_decoration[x+(y<<4)];
                 }
              }
              dis.close();
        }
        ...
        {
             throw new IOException("Can't load a game data");
        }

        TileBackground(GAMESCREEN_WIDTH,GAMESCREEN_HEIGHT,32,32);
        setBlockImage(getImage(IMG_TILES));
        setGroundImage(getImage(IMG_BG_TILES));


        cockroach_left          = new drawDigits(getImage(IMG_COCKROACH_GO_LEFT),2,false);
        cockroach_right         = new drawDigits(getImage(IMG_COCKROACH_GO_RIGHT),2,false);
        cockroach_up            = new drawDigits(getImage(IMG_COCKROACH_GO_TOP),2,false);
        cockroach_down          = new drawDigits(getImage(IMG_COCKROACH_GO_DOWN),2,false);
        cockroach_rotate        = new drawDigits(getImage(IMG_COCKROACH_ROTATE),4,false);

        cockroach_green_left    = new drawDigits(getImage(IMG_COCKROACH_GR_GO_LEFT),2,false);
        cockroach_green_right   = new drawDigits(getImage(IMG_COCKROACH_GR_GO_RIGHT),2,false);
        cockroach_green_up      = new drawDigits(getImage(IMG_COCKROACH_GR_GO_TOP),2,false);
        cockroach_green_down    = new drawDigits(getImage(IMG_COCKROACH_GR_GO_DOWN),2,false);
        cockroach_green_rotate  = new drawDigits(getImage(IMG_COCKROACH_GREEN_ROTATE),4,false);

        score_red               = new drawDigits(getImage(IMG_SCORE_RED),10);
        score_blue              = new drawDigits(getImage(IMG_SCORE_BLUE),11);
     }

    public int getCenterX()
    {
        return (p_Gamelet.pPlayerSprite.i_mainX>>8) - (GAMESCREEN_WIDTH >> 1) +16;
    }
    public int getCenterY()
    {
        return (p_Gamelet.pPlayerSprite.i_mainY>>8) - (GAMESCREEN_HEIGHT >> 1) +16;
    }
...
SND_EST_HLEB_MMF       ,0,
SND_EST_KISEL_MMF      ,0,
SND_EST_KOLBASU_MMF    ,0,
SND_EST_KOSTI_MMF      ,0,
SND_EST_MUHOMOR_MMF    ,0,
SND_EST_OGRYZOK_MMF    ,0,
SND_SMERT_TARAKANA_MMF ,0,
SND_VOSST_SKOROSTI_MMF ,0,
SND_VYHOD_MMF          ,1000,
SND_START_MMF          ,0,
SND_WON_MMF            ,0,
                             };
  ...
  private int bsWIDTH, bsHEIGHT, bs_X_FRAMES, bs_X_LAST_FRAME, bs_X_POS, LOC_X
                 , bs_Y_FRAMES, bs_Y_LAST_FRAME, bs_Y_POS, LOC_Y
                 ;
  private int center_x,center_y;

  private Image BackScreen;
  private Image p_BlockImage;
  private Image p_GroundImage;
  ...
  public int i_viewCoordX,i_viewCoordY;
    ...
    public void TileBackground(int _width, int _height, int _cellWidth, int _cellHeight)
    {

        bs_X_FRAMES = (_width + _cellWidth -1) / _cellWidth + 1;       
        bs_Y_FRAMES = (_height + _cellHeight - 1) / _cellHeight + 1;   
 ...
 int i = LOC_X-(i_viewCoordX&0x1f)+_x, j = LOC_Y-(i_viewCoordY&0x1f)+_y;
   ...
   _g.drawImage(BackScreen,i-bsWIDTH,j-bsHEIGHT,0);
    ...
    public void setGroundImage(Image _image)
    {
        synchronized (BackScreen)
        {
            p_GroundImage = _image;
        }
    }
    ...
    public void setXY(int _xx, int _yy)
    {
      ...
      bs_X_POS = 0;
      bs_Y_POS = 0;
      bs_X_LAST_FRAME = _xx/CELL_SIZE +1;
      bs_Y_LAST_FRAME = _yy/CELL_SIZE +1;
      LOC_X = 0;
      LOC_Y = 0;

      int from_x = bs_X_LAST_FRAME -1, from_y = bs_Y_LAST_FRAME -1;
 ...
 if(by!=bs_Y_LAST_FRAME){
   ...
   if(dy>0){
    if(dy>bs_Y_FRAMES){bs_Y_LAST_FRAME+=dy-bs_Y_FRAMES; dy=bs_Y_FRAMES;}
    for (int i = 0 ; i <dy ; i++) {
 ...
 if(bx!=bs_X_LAST_FRAME){
   ...
   if(dx>0){
    if(dx>bs_X_FRAMES){bs_X_LAST_FRAME+=dx-bs_X_FRAMES; dx=bs_X_FRAMES;}
    for (int i = 0 ; i <dx ; i++) {
    ...
    for (int i = 0 ; i >dx ; i--) {
      ...
      bs_X_LAST_FRAME--;
      repaintVerticalLine(bs_X_LAST_FRAME -1 ,bs_X_POS);
    }
   }
   LOC_X -= dx<<5;
   ...
   if(_x>=bsWIDTH)LOC_X-=bsWIDTH;
    ...
    {
         ab_roomArray[((_celly & 0xf) << 4)+(_cellx & 0xf)] = _newvalue;
         ...
         int dx = _cellx - bs_X_LAST_FRAME+bs_X_POS+1-bs_X_FRAMES,
             ...
             if (dx<0)dx+=bsWIDTH;
                ...
                if (dx>=bsWIDTH)dx-=bsWIDTH;

             if (dy<0)dy+=bsHEIGHT;
               else
                if (dy>=bsHEIGHT)dy-=bsHEIGHT;

             drawCell(_cellx,_celly,dx,dy);
    }
    ...
    private void drawCell(int x, int y ,int _x,int _y){
          ...
          qd = ab_roomArray[(y<<4)+x];


          gBackScreen.setClip(_x,_y,CELL_SIZE,CELL_SIZE);

          gBackScreen.drawImage(p_GroundImage, _x,_y -ofs,0);
       ...
       if(ofs >= 0)
              {
                 img = getImage(ofs);
                 gBackScreen.drawImage(img, _x+32-dx - (img.getWidth()>>1),_y+32-dy - (img.getHeight()>>1),0);
              }
   }

   gBackScreen.drawImage(p_BlockImage,_x - ((qd&3)<<5), _y - ((qd&0xfc)<<3),0);
    }

    private int getItem(int cellx, int celly)
    {
        switch(p_Gamelet.getTile(cellx & 0xf,celly & 0xf))
        {
          case MobileGameletImpl.OBJECT_SAUSAGE       : return IMG_SAUSAGE;
          case MobileGameletImpl.OBJECT_MUSHROOM      : return IMG_MUSHROOM;
          case MobileGameletImpl.OBJECT_APPLE_BIT     : return IMG_APPLE;
          case MobileGameletImpl.OBJECT_JELLY         : return IMG_KISSEL;
          case MobileGameletImpl.OBJECT_BREAD_CRUST   : return IMG_BREAD;
          case MobileGameletImpl.OBJECT_POISON        : return IMG_POISON;
          case MobileGameletImpl.OBJECT_FISH_SKELETON : return IMG_FISH;
          case MobileGameletImpl.OBJECT_FINISH        : return IMG_GRATE;
          default: return -1;
        }
...
private static final int IMG_FINAL160 = 1;
private static final int IMG_WIN = 2;
private static final int IMG_DEMO = 3;
private static final int IMG_BG_TILES = 4;
private static final int IMG_TILES = 5;
private static final int IMG_COCKROACH_GO_DOWN = 6;
private static final int IMG_COCKROACH_GO_LEFT = 7;
private static final int IMG_COCKROACH_GO_RIGHT = 8;
private static final int IMG_COCKROACH_GO_TOP = 9;
private static final int IMG_COCKROACH_GREEN_ROTATE = 10;
private static final int IMG_COCKROACH_GR_GO_DOWN = 11;
private static final int IMG_COCKROACH_GR_GO_LEFT = 12;
private static final int IMG_COCKROACH_GR_GO_RIGHT = 13;
private static final int IMG_COCKROACH_GR_GO_TOP = 14;
private static final int IMG_COCKROACH_ROTATE = 15;
private static final int IMG_APPLE = 16;
private static final int IMG_BREAD = 17;
private static final int IMG_FISH = 18;
private static final int IMG_GRATE = 19;
private static final int IMG_KISSEL = 20;
private static final int IMG_MUSHROOM = 21;
private static final int IMG_POISON = 22;
private static final int IMG_SAUSAGE = 23;
private static final int IMG_SCORE_RED = 24;
private static final int IMG_SCORE_BLUE = 25;
private static final int TOTAL_IMAGES_NUMBER = 26;

private static final int SND_EST_HLEB_MMF = 0;
private static final int SND_EST_KISEL_MMF = 1;
private static final int SND_EST_KOLBASU_MMF = 2;
private static final int SND_EST_KOSTI_MMF = 3;
private static final int SND_EST_MUHOMOR_MMF = 4;
private static final int SND_EST_OGRYZOK_MMF = 5;
private static final int SND_SMERT_TARAKANA_MMF = 6;
private static final int SND_START_MMF = 7;
private static final int SND_VOSST_SKOROSTI_MMF = 8;
private static final int SND_VYHOD_MMF = 9;
private static final int SND_WON_MMF = 10;

...
InitGameDataStorage(p_Gamelet.getGameTextID(), p_Gamelet.getMaximumDataSize(), 8, 6, 10);
       ...
       logo = null;
                             ...
                             int [] sort_idx = {
SND_ENEMY_FIRE_MMF,  80  *200,
SND_PLAYER_FIRE_MMF, 160 *200,
SND_EXPLOSION_MMF,   1099,
SND_END_MMF,         0,
SND_MAIN_TH_MMF,     0
                             };
             ...
             final int keyset[] = {Canvas.LEFT,Canvas.RIGHT,Canvas.UP,Canvas.DOWN};
                ...
                case STATE_GAMEPLAY:
                    {
                        ...
                        if (p_Gamelet.getGameWorldState() == MobileGamelet.GAMEWORLDSTATE_PLAYED)
                        {
                            ...
                            if (i_timeDelay <= 0) i_timeDelay = 1;
                               ...
                               if(i_action_sound < 0)
                               {
                                       if(p_Gamelet.pPlayerSight.i_ObjectType == MobileGameletImpl.SPRITE_HELICOPTER_FIRE && _playing_melody != SND_PLAYER_FIRE_MMF)
                                       {
                                           i_action_sound = SND_PLAYER_FIRE_MMF;
                                           checkPlayabilityMusic();
                                       }
                                         else
                                         if(p_Gamelet.pPlayerSight.i_ObjectType != MobileGameletImpl.SPRITE_HELICOPTER_FIRE && _playing_melody == SND_PLAYER_FIRE_MMF)
                                         {
                                             stopMelody(ap_melody[SND_PLAYER_FIRE_MMF]);
                                             _melody_priority = -1;
                                             _playing_melody = -1;
                                         }
                                         else
                                           if(p_Gamelet.m_nFireCounter > 0 && _playing_melody != SND_ENEMY_FIRE_MMF)
                                           {
                                               i_action_sound = SND_ENEMY_FIRE_MMF;
                                               checkPlayabilityMusic();
                                           }
                                           else
                                              if(p_Gamelet.m_nFireCounter <= 0 && _playing_melody == SND_ENEMY_FIRE_MMF)
                                              {
                                                 stopMelody(ap_melody[SND_ENEMY_FIRE_MMF]);
                                                 _melody_priority = -1;
                                                 _playing_melody = -1;
                                              }
                                       if(i_action_sound >= 0)
                                       {
                                           playMelody(ap_melody[i_action_sound],200,false);
                                           i_action_sound = -1;
                                       }
                               }
                               else
                ...
                case STATE_FRONTPAGE:
                    {
                         ...
                         {
                            Thread.yield();
                            Melody theme = ap_melody[SND_MAIN_TH_MMF];
                            if(theme != null)
                            {
...
p_imageForOutput = getImage(2);
    ...
    private void gamePlayDrawing(Graphics _graphics)
    {
        ...
        i_h = 0;
        for (li = 0; li < precalculated_grad_colors.length; li++)
        {
   _graphics.setColor(precalculated_grad_colors[li]);
   _graphics.fillRect(0,i_h,SCREENWIDTH,grad_step);
   i_h+=grad_step;
        }



        img = getImage(IMG_HORIZONT);
        i_h = sky_height + img.getHeight();
        _graphics.drawImage(img,0,sky_height,0);

        _graphics.drawImage(getImage(IMG_LAND_0000 + i_Timer % 6),0,i_h,0);


       low_sort_border = 0;
       high_sort_border = MobileGameletImpl.MAX_SPRITES;
       current_mark = MobileGameletImpl.ENEMY_MAX_DISTANCE_FRAMES -1;
       Sprite[] spr = p_Gamelet.ap_Sprites;

       while(current_mark >= 0 && high_sort_border > 0)
       {
         for (li = 0; li < high_sort_border; li++)
         {
            ...
            {
              if(p_spr.i_ObjectState < current_mark)
              {
                sort_buffer[low_sort_border++] = p_spr;
                continue;
              }
              ...
              i_w = p_Gamelet.i_GameTimer&1;

              switch(p_spr.i_ObjectType)
              {
                  case MobileGameletImpl.SPRITE_ENEMY_HELICOPTER_FIRE:
                      {
                         img = getImage(IMG_ENEMY01+p_spr.i_ObjectState);
                         i_y -= (img.getHeight()>>1);

                         if(i_w == 0)
                         {
                             i_w = blade_len[(p_spr.i_ObjectState<<1) + i_w];
                             i_sx = i_x - (i_w>>1);
                             i_sy = i_y + blade_y[p_spr.i_ObjectState];

                             _graphics.setColor(73,131,165);
                             _graphics.drawLine(i_sx,i_sy,i_sx+i_w,i_sy);
                         }
                        ...
                        if((i_Timer & 1) == 0)
                        {
                          img = getImage(p_spr.i_ObjectState > 2 ? IMG_FLASH_ENEMY02 : IMG_FLASH_ENEMY01);
                          _graphics.drawImage(img
                                   ,i_x - (img.getWidth()>>1)
                                   ,i_y + _enemy_shots_offset[p_spr.i_ObjectState]
                                   ,0 );
                        }
                      }
                      continue;
                  case MobileGameletImpl.SPRITE_ENEMY_HELICOPTER_EXPLOSION:
                      {
                          explosion_enemy.drawDigits(_graphics
                                ,i_x-(explosion_enemy.fontWidth>>1)
                                ,i_y - (explosion_enemy.fontHeight>>1)
                                ,1,p_spr.i_Frame,8);
                      }
                      continue;
                  case MobileGameletImpl.SPRITE_ENEMY_HELICOPTER_SHOOTED:
                  case MobileGameletImpl.SPRITE_ENEMY_HELICOPTER:
                  default:
                      {
                        img = getImage(IMG_ENEMY01+p_spr.i_ObjectState);
                      }
         ...
         current_mark--;
         high_sort_border = low_sort_border;
         low_sort_border = 0;
         spr = sort_buffer;
       }
        ...
        i_x = p_spr.i_mainX >> 8;
        i_y = (p_spr.i_ScreenY + p_spr.i_col_offsetY + (p_spr.i_col_width>>1)) >> 8;


        if(p_spr.i_ObjectType != MobileGameletImpl.SPRITE_HELICOPTER_DESTROYED)
        {
        ...
        switch(p_spr.i_ObjectType)
        {
             case MobileGameletImpl.SPRITE_HELICOPTER_LEFT:
                  {

                      img = getImage(IMG_PLAYER_0001);
                      _graphics.drawImage(img,i_x-(img.getWidth()>>1)+2,i_y,0);
                      propeller_left.drawDigits(_graphics,
                              i_x-(propeller_left.fontWidth>>1),
                              i_y-(propeller_left.fontHeight>>1),
                              1,p_spr.i_Frame,3);
                  }
                  break;
             case MobileGameletImpl.SPRITE_HELICOPTER_RIGHT:
                  {
                      img = getImage(IMG_PLAYER_0002);
                      _graphics.drawImage(img,i_x-(img.getWidth()>>1)-2,i_y,0);
                      propeller_right.drawDigits(_graphics,
                              i_x-(propeller_right.fontWidth>>1),
                              i_y-(propeller_right.fontHeight>>1),
                              1,p_spr.i_Frame,3);
                  }
                  break;
             case MobileGameletImpl.SPRITE_HELICOPTER_FIRE:
                  {
                      if((p_Gamelet.i_GameTimer & 1) == 0)
                      {
                        img = getImage(IMG_FLASH_PLAYER);
                        _graphics.drawImage(img, i_x-(img.getWidth()>>1), i_y+7,0);
                      }

                      img = getImage(IMG_PLAYER_0000);
                      _graphics.drawImage(img,i_x-(img.getWidth()>>1),i_y,0);
                      propeller_center.drawDigits(_graphics,
                              i_x-(propeller_center.fontWidth>>1),
                              i_y-(propeller_center.fontHeight>>1),
                              1,p_spr.i_Frame,3);
                  }
                  break;
             case MobileGameletImpl.SPRITE_HELICOPTER_DESTROYED:
                  {
                      if(p_spr.i_Frame<8)
                      {
                        i_y -= (explosion_player.fontHeight)>>1;
                        explosion_player.drawDigits(_graphics,i_x-(explosion_player.fontWidth>>1),i_y,1,p_spr.i_Frame,8);
                      }
                  }
                  break;
             case MobileGameletImpl.SPRITE_HELICOPTER_NORMAL:
             case MobileGameletImpl.SPRITE_HELICOPTER_UP:
             case MobileGameletImpl.SPRITE_HELICOPTER_DOWN:
             case MobileGameletImpl.SPRITE_HELICOPTER_SHOOTED:
             default:
                  {
                      img = getImage(IMG_PLAYER_0000);
                      _graphics.drawImage(img,i_x-(img.getWidth()>>1),i_y,0);
        ...
        i_x = 4;
        i_y = 3;
        for(li = p_Gamelet.m_iPlayerAttemptions-1; li > 0 ;li--)
        {
          _graphics.drawImage(img, i_x, i_y,0);
          i_x += img.getWidth()+3;
        }

        i_x = SCREENWIDTH - (font.fontWidth<<2) - 4;
...
if (lg_FinishImagePictured)
    ...
    public int processGameAction(int _arg)
    {
          ...
          switch(_arg)
          {
               case MobileGameletImpl.GAMEACTION_PLAYER_FIRE:
                          {
                          }
                          break;
               case MobileGameletImpl.GAMEACTION_PLAYER_SHOOTED:
                          {
                          }
                          break;
               case MobileGameletImpl.GAMEACTION_PLAYER_DESTROYED:
                          {
                            i_action_sound = SND_EXPLOSION_MMF;
                          }
                          break;
               case MobileGameletImpl.GAMEACTION_ENEMY_FIRE:
                          ...
                          break;
               case MobileGameletImpl.GAMEACTION_ENEMY_SHOOTED:
                          {
                          }
                          break;
               case MobileGameletImpl.GAMEACTION_ENEMY_EXPLODED:
                          {
                            i_action_sound = SND_EXPLOSION_MMF;
                          }
                          break;
    ...
    public Object processGameAction(int _arg1, int _arg2, Object _obj1)
    {
     ...
     drawDigits font;
     drawDigits propeller_left,propeller_center,propeller_right;
     drawDigits explosion_enemy, explosion_player;
     Sprite[] sort_buffer = new Sprite[MobileGameletImpl.MAX_SPRITES];
     int[] _enemy_shots_offset = {8,7,6,5,3};

     short[] blade_len = {32,24, 30,22, 24,18, 17,14, 13,11};
     short[] blade_y   = {4,4,3,2,1};


     Image background;


     final static int ground_width = 128;
     final static int ground_height = 57;
     static int sky_height;

     int low_sort_border;
     int high_sort_border;
     int current_mark;

     int grad_step = 5;
     int[] precalculated_grad_colors;

     private void prepareGraphics()
     {

        font              = new drawDigits(getImage(IMG_NUMBERS),10);
        propeller_left    = new drawDigits(getImage(IMG_PROPELLER_LEFT),3,false);
        propeller_center  = new drawDigits(getImage(IMG_PROPELLER),3,false);
        propeller_right   = new drawDigits(getImage(IMG_PROPELLER_RIGHT),3,false);
        explosion_enemy   = new drawDigits(getImage(IMG_EXPLOS_ENEMY),8,false);
        explosion_player  = new drawDigits(getImage(IMG_EXPLOS_PLAYER),8,false);

        sky_height = SCREENHEIGHT - ground_height - getImage(IMG_HORIZONT).getHeight();


        int height = sky_height;
        int step = grad_step;
        int startc = 0x449bf1;
        int endc = 0xaae4ff;

        precalculated_grad_colors = new int[(height + step-1) / step];
        ...
        int i;
        height = precalculated_grad_colors.length;

        for (i = 0; i < height; i++){

          precalculated_grad_colors[i] = 0xff000000 |
                                         (((rs+rd*i/height) & 0xff) << 16) |
                                  (((gs+gd*i/height) & 0xff) << 8)  |
                            ((bs+bd*i/height) & 0xff);
        }
     }
...
private static final int IMG_LAND_0000 = 4;
private static final int IMG_LAND_0001 = 5;
private static final int IMG_LAND_0002 = 6;
private static final int IMG_LAND_0003 = 7;
private static final int IMG_LAND_0004 = 8;
private static final int IMG_LAND_0005 = 9;
private static final int IMG_HORIZONT = 10;
private static final int IMG_ENEMY01 = 11;
private static final int IMG_ENEMY02 = 12;
private static final int IMG_ENEMY03 = 13;
private static final int IMG_ENEMY04 = 14;
private static final int IMG_ENEMY05 = 15;
private static final int IMG_EXPLOS_ENEMY = 16;
private static final int IMG_EXPLOS_PLAYER = 17;
private static final int IMG_FLASH_ENEMY01 = 18;
private static final int IMG_FLASH_ENEMY02 = 19;
private static final int IMG_FLASH_PLAYER = 20;
private static final int IMG_PLAYER_0000 = 21;
private static final int IMG_PLAYER_0001 = 22;
private static final int IMG_PLAYER_0002 = 23;
private static final int IMG_PROPELLER = 24;
private static final int IMG_PROPELLER_LEFT = 25;
private static final int IMG_PROPELLER_RIGHT = 26;
private static final int IMG_LIFE = 27;
private static final int IMG_NUMBERS = 28;
private static final int IMG_SIGHT = 29;
private static final int TOTAL_IMAGES_NUMBER = 30;

private static final int SND_END_MMF = 0;
private static final int SND_ENEMY_FIRE_MMF = 1;
private static final int SND_EXPLOSION_MMF = 2;
private static final int SND_MAIN_TH_MMF = 3;
private static final int SND_PLAYER_FIRE_MMF = 4;

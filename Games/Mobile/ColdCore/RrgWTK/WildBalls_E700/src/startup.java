...
p_imageForOutput = getImage(2);
    ...
    private void gamePlayDrawing(Graphics _graphics)
    {
        ...
        gInfoPanel.fillRect(0,0,SCREENWIDTH,panel_height);

        i_x = SCREENWIDTH - (score_font.fontWidth<<2) - 4;
        i_y = (panel_height - score_font.fontHeight)>>1;
        score_font.drawDigits(gInfoPanel,i_x,i_y,4,p_Player.m_iPlayerGameScores,10);

        i_x = 4;
        img = getImage(IMG_ICON_SKITTLE);
        gInfoPanel.drawImage(img,i_x,(panel_height - img.getHeight())>>1,0);

        i_x += img.getWidth()+3;
        i_y = (panel_height - skittles_font.fontHeight)>>1;
        skittles_font.drawDigits(gInfoPanel,i_x,i_y,1,10,11);

        i_x += skittles_font.fontWidth;
        skittles_font.drawDigits(gInfoPanel,i_x,i_y,3,p_Gamelet.m_nSkittles,10);

        img = getImage(IMG_ICON_BALL);
        ...
        i_x = ((GAMESCREEN_WIDTH - li*(img.getWidth()+3)) >> 1) +5;
        ...
        boolean draw_player = true;
        i_y = p_Gamelet.pPlayerBall.i_mainY >> 8;

        if(i_y > (GAMESCREEN_HEIGHT >> 1))
        {
               p_spr = p_Gamelet.pPlayerBall;

               i_x = p_spr.i_mainX >> 8;

               if(p_spr.i_ObjectType == MobileGameletImpl.SPRITE_BALL_APPEARING)
               {
                   i_y += (p_spr.i_maxFrames - p_spr.i_Frame) *3;
               }
        ...
        _graphics.drawImage(imgInfoPanel,0,GAMESCREEN_HEIGHT - panel_height,0);
        ...
        for (li = 0; li < p_Gamelet.ap_Sprites.length; li++)
        {
            ...
            i_x = (p_spr.i_mainX >> 8) - (skittle.fontWidth>>1);
            i_y = (p_spr.i_mainY >> 8) - (skittle.fontHeight>>1);
            switch (p_spr.i_ObjectType)
            {

                case MobileGameletImpl.SPRITE_SKITTLE_APPEARING:
                    i_y -= (p_spr.i_maxFrames - p_spr.i_Frame) << 1;
                    skittle.drawDigits(_graphics,i_x -4,i_y -2,1,0,2);
                    break;
                case MobileGameletImpl.SPRITE_SKITTLE:
                    skittle.drawDigits(_graphics,i_x -4,i_y -2,1,0,2);
                    break;
                case MobileGameletImpl.SPRITE_SKITTLE_FALL:
                    skittle.drawDigits(_graphics,i_x+(p_spr.i_Frame>>1) -4,i_y-(p_spr.i_Frame<<1),1,1,2);
                    break;
                case MobileGameletImpl.SPRITE_BAR:
            System.out.println(li);
                    i_sx = ((p_spr.i_Frame + 1) << 1);
                    i_x = p_spr.i_ScreenX >> 8;
                    i_y = (p_spr.i_ScreenY >> 8) - i_sx;
                    _graphics.setClip(i_x, i_y, GAMESCREEN_WIDTH, 10 + i_sx);
                    _graphics.drawImage(getImage(IMG_BARRIER),i_x,i_y,0);
        ...
        {
           i_x = (p_spr.i_mainX >> 8) - (balls.fontWidth>>1);
           i_y = (p_spr.i_mainY >> 8) - (balls.fontHeight>>1);
           balls.drawDigits(_graphics,i_x,i_y,1,p_spr.i_spriteID,4);
        }


        p_spr = p_Gamelet.pPlayerBall;

        if(p_spr.lg_SpriteActive && draw_player)
        {
          i_x = p_spr.i_mainX >> 8;
          i_y = p_spr.i_mainY >> 8;
          ...
          switch(p_spr.i_ObjectType)
          {
             case MobileGameletImpl.SPRITE_BALL_DEAD:
                   {
                        crash.drawDigits(_graphics,i_x - (crash.fontWidth>>1),i_y - (crash.fontHeight>>1),1,p_spr.i_Frame,crash.i_origRadix);
                   }
                   break;
             case MobileGameletImpl.SPRITE_BALL: break;
             default:
                   {
                        img = getImage(IMG_PLAYER);
                        _graphics.drawImage(img,i_x - (img.getWidth()>>1),i_y - (img.getHeight()>>1),0);
...
if (lg_FinishImagePictured)
    ...
    public int processGameAction(int _arg)
    {
          ...
          switch(_arg)
          {
                   case MobileGameletImpl.GAMEACTION_SKITTLE_HIT:
                          {
                            i_action_sound = SND_SBIL_MMF;
                          }
                          break;
                   case MobileGameletImpl.GAMEACTION_SKITTLE_FALLEN:
                          {
                          }
                          break;
                   case MobileGameletImpl.GAMEACTION_NEW_BALL_GENERATED:
                          {
                          }
                          break;
                   case MobileGameletImpl.GAMEACTION_BALLS_HIT:
                          {
                            i_action_sound = SND_BALLS_COLLIDING_MMF;
                          }
                          break;
                   case MobileGameletImpl.GAMEACTION_RAISE_BAR:
                          {
                          }
                          break;
                   case MobileGameletImpl.GAMEACTION_RUN_BALL:
                          {
                          }
                          break;
                   case MobileGameletImpl.GAMEACTION_ALL_SKITTLE:
                          {
                             i_action_sound = SND_VYSTAVLAEMKEGLI_MMF;
                          }
                          break;
                   case MobileGameletImpl.GAMEACTION_BALL_CRASH:
                          {
                            i_action_sound = SND_BALL_GLASS_MMF;
                          }
                          break;
          }
    ...
    public Object processGameAction(int _arg1, int _arg2, Object _obj1)
    {
     ...
     drawDigits score_font, skittles_font;
     drawDigits balls,skittle;
     drawDigits crash;

     Image imgInfoPanel;
     Graphics gInfoPanel;
     final static int panel_height = 13;

     private void prepareGraphics()
     {
        score_font   = new drawDigits(getImage(IMG_SCORE_NUMBERS),10);
        skittles_font   = new drawDigits(getImage(IMG_SCORE_SKITTLES),11);
        balls   = new drawDigits(getImage(IMG_BALLS),4,false);
        crash   = new drawDigits(getImage(IMG_BALL_PLAYER),4,false);
        skittle = new drawDigits(getImage(IMG_SKITTLE),2,false);

        imgInfoPanel = Image.createImage(SCREENWIDTH,panel_height);
        gInfoPanel = imgInfoPanel.getGraphics();
        gInfoPanel.setColor(0x0);
     }
    ...
    {
                             int [] sort_idx = {
SND_BALLS_COLLIDING_MMF , 921,
SND_SBIL_MMF            , 501,
SND_PODNALI_PREGRADU_MMF, 805,
SND_VYSTAVLAEMKEGLI_MMF , 604,
SND_BALL_GLASS_MMF      , 437,
SND_FINAL_MMF           , 0,
SND_START_MMF           , 0
                             };
...
private static final int IMG_GAME_OVER128X160 = 1;
...
private static final int IMG_BALLS = 6;
private static final int IMG_BALL_PLAYER = 7;
private static final int IMG_BARRIER = 8;
private static final int IMG_ICON_BALL = 9;
private static final int IMG_ICON_SKITTLE = 10;
private static final int IMG_PLAYER = 11;
private static final int IMG_SCORE_NUMBERS = 12;
private static final int IMG_SCORE_SKITTLES = 13;
private static final int IMG_SKITTLE = 14;
private static final int TOTAL_IMAGES_NUMBER = 15;

private static final int SND_BALL_GLASS_MMF = 0;
private static final int SND_BALLS_COLLIDING_MMF = 1;
private static final int SND_FINAL_MMF = 2;
private static final int SND_PODNALI_PREGRADU_MMF = 3;
private static final int SND_SBIL_MMF = 4;
private static final int SND_START_MMF = 5;
private static final int SND_VYSTAVLAEMKEGLI_MMF = 6;

...
public class ImageManager
{
    ...
    protected static final void drawImage(int _imageOffset, int _x, int _y, boolean _flipVert, boolean _flipHorz)
    {
        ...
        if (i_type == FLAG_LINK)
        {
            int i_imageOffset = ash_map[_imageOffset++];
            i_linkXOffset = ash_map[_imageOffset++];
            i_linkYOffset = ash_map[_imageOffset];
        ...
        if (_flipVert)
        {
            ...
            //#else
            //#if MIDP=="2.0"
            i_flag |= javax.microedition.lcdui.game.Sprite.TRANS_MIRROR | javax.microedition.lcdui.game.Sprite.TRANS_MIRROR_ROT180;
            //#endif
            //#endif
        ...
        //#endif
                ...
                //#endif
                {
                    ...
                    //#if MIDP=="1.0" && VENDOR=="NOKIA"
                    //$p_dirGraphics.drawImage(p_img, _x,_y, 0, i_flag);
                    //#else
        ...
        ;
                ...
                //#endif
                {
                    ...
                    //#if MIDP=="1.0" && VENDOR=="NOKIA"
                    //$p_dirGraphics.drawImage(p_fullImage, _x-i_xMap, _y-i_yMap,0,  i_flag);
                    //#else
    ...
    {
        //#if SHOWSYS
        System.out.println("Loading all external images");
        //#endif
        ...
        //#+
            ...
            int i_extFileNumber = p_extBlockStream.read() & 0xFF;
            i_extFileNumber = (i_extFileNumber << 8) | (p_extBlockStream.read() & 0xFF);
            //$int i_extOffset = 0;
        //#-
            i_extOffset = 0;
        }
        ...
        for (int li = 0; li < i_imageNum; li += IMAGEINFO_LENGTH)
        {
            ...
            if (i_type == FLAG_EXTERNAL)
            {
                ...
                if (lg_externalInOneFile)
                {
                    int i_offst = ((ash_imageInfo[li + 1] & 0xFFFF)<<16) | (ash_imageInfo[li + 2] & 0xFFFF);
                    if (i_extOffset != i_offst) throw new Exception();

                    int i_b0 = p_extBlockStream.read()&0xFF;
                    int i_b1 = p_extBlockStream.read()&0xFF;
                    int i_len = (i_b0<<8)|i_b1;
                    byte [] ab_byteImage = new byte[i_len];
                    p_extBlockStream.read(ab_byteImage);
                    p_img = Image.createImage(ab_byteImage,0,i_len);
                    ab_byteImage = null;
                    i_extOffset += i_len + 2;
                }
                //#else

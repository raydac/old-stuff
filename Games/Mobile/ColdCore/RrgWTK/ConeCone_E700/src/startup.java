                ...
                case STATE_GAMEPLAY:
                    {
                        ...
                        else
                        {
                            ...
                            if(p_Gamelet.getWinningList()==null && ++i_showdeath_delay < 20)
                            {
                                   paintOnBuffer();
                            }
                            else
...
p_imageForOutput = getImage(2);
    ...
    private void gamePlayDrawing(Graphics _graphics)
    {
        ...
        int li,i_sx,i_sy,i_x, i_y, loMark,hiMark,coMark, pl_x,pl_y,i_ndx;
        Sprite p_spr;
        Image img;
        Image squirrel;
        ...
        Image back = getImage(IMG_BACK);
        _graphics.drawImage(back,0,0,0);

          if(last_score != p_Player.m_iPlayerGameScores)
          {
             last_score = p_Player.m_iPlayerGameScores;
             img = getImage(IMG_BACK);
             gScore.drawImage(img,-score_x,-img.getHeight() + (GAMESCREEN_HEIGHT - score_y),0);
             score.drawDigits(gScore,0,0,4,last_score,10);
          }
        ...
        coMark = 0;
        loMark = 0;
        hiMark = sort_buffer.length;
        i_ndx = 3;

        for(li=0;li<p_Gamelet.ap_Sprites.length;li++)
        {
            ...
            if (p_spr.lg_SpriteActive)
            {
                ...
                {
                    case MobileGameletImpl.SPRITE_CONE:
                               {
                                   i_y = (p_spr.i_mainY>>8)-2;
                                   i_x = (p_spr.i_mainX>>8);
                                   i_ndx = IMG_CONE_ROTATE_0000;
                                   cones[coMark++] = p_spr;
                               }
                               break;
                    case MobileGameletImpl.SPRITE_CONE_APPEAR:
                               {
                                   i_y = p_spr.i_mainY>>8;
                                   i_x = (p_spr.i_mainX>>8);
                                   i_ndx = IMG_CONE_GROW_0000 + (p_spr.i_Frame&3);
                                   cones[coMark++] = p_spr;
                               }
                               break;
                    case MobileGameletImpl.SPRITE_CONE_FIRE:
                    case MobileGameletImpl.SPRITE_CROW_LEFT_FLY_IN:
                    case MobileGameletImpl.SPRITE_CROW_LEFT_FLY:
                    case MobileGameletImpl.SPRITE_CROW_RIGHT_FLY_IN:
                    case MobileGameletImpl.SPRITE_CROW_RIGHT_FLY:
                               {
                                   sort_buffer[--hiMark] = p_spr;
                               }
                               continue;

                    case MobileGameletImpl.SPRITE_HOLLOW:  continue;
                    default:
                                   sort_buffer[loMark++] = p_spr;
                               continue;
        ...
        pl_x = p_spr.i_mainX>>8;
        i_y = (p_spr.i_mainY>>8);
        pl_y = (p_spr.i_ScreenY + p_spr.i_height>>8) ;


        switch(p_spr.i_ObjectType)
        {
            default:
            case MobileGameletImpl.SPRITE_SQUIRREL_LEFT:
                     squirrel = getImage(IMG_SQUIREL_SIT_LEFT);
                     pl_x -= (p_spr.i_width>>9);
                     pl_y+=2;
                     break;
            case MobileGameletImpl.SPRITE_SQUIRREL_RIGHT:
                     squirrel = getImage(IMG_SQUIREL_SIT_RIGHT);
                     pl_x += (p_spr.i_width>>9) - squirrel.getWidth();
                     pl_y+=2;
                     break;
            case MobileGameletImpl.SPRITE_SQUIRREL_JUMP_LEFT:
                     pl_x -= (p_spr.i_ObjectState * MobileGameletImpl.SQUIRREL_JUMP_SPEED)>>8;
                     if(p_spr.i_ObjectState >= MobileGameletImpl.SQUIRREL_JUMP_STEPS-2)
                     {
                        squirrel = getImage(IMG_SQUIREL_JUMP_LEFT_0001);
                        pl_x -= 8;
                     }
                       else
                            {
                               squirrel = getImage(IMG_SQUIREL_JUMP_LEFT_0000);
                               pl_x -= (squirrel.getWidth()>>1) -8;
                            }
                     break;
            case MobileGameletImpl.SPRITE_SQUIRREL_JUMP_RIGHT:
                     pl_x += (p_spr.i_ObjectState * MobileGameletImpl.SQUIRREL_JUMP_SPEED)>>8;
                     if(p_spr.i_ObjectState >= MobileGameletImpl.SQUIRREL_JUMP_STEPS-2)
                     {
                        squirrel = getImage(IMG_SQUIREL_JUMP_RIGHT_0001);
                        pl_x -= squirrel.getWidth() - 8;
                     }
                       else
                            {
                               squirrel = getImage(IMG_SQUIREL_JUMP_RIGHT_0000);
                               pl_x -= (squirrel.getWidth()>>1) + 8;
                            }
                     break;
            case MobileGameletImpl.SPRITE_SQUIRREL_TAKE_LEFT:
            case MobileGameletImpl.SPRITE_SQUIRREL_TAKE_RIGHT:
                    squirrel = getImage(IMG_SQUIREL_PICK_CONE_0000 + (p_spr.i_Frame&1));
                    for(li=0;li<coMark;li++)
                    {
                        if(cones[li].isCollided(p_spr))
                        {
                             pl_x = cones[li].i_mainX>>8;
                        }
                    }
                    pl_x -= (squirrel.getWidth()>>1);
                    pl_y+=14;
                    break;
            case MobileGameletImpl.SPRITE_SQUIRREL_FIRE_LEFT:
                    squirrel = getImage(IMG_SQUIREL_THROW_CONE_LEFT_0000 + (p_spr.i_Frame & 1));
                    pl_x = (p_spr.i_ScreenX>>8) + qSitLeft_fontWidth - squirrel.getWidth();
                    pl_y+=16;
                    break;
            case MobileGameletImpl.SPRITE_SQUIRREL_FIRE_RIGHT:
                    squirrel = getImage(IMG_SQUIREL_THROW_CONE_RIGHT_0000 + (p_spr.i_Frame & 1));
                    pl_x = (p_spr.i_ScreenX + p_spr.i_height >>8) - qSitRight_fontWidth;
                    pl_y+=16;
                    break;
            case MobileGameletImpl.SPRITE_SQUIRREL_DEAD:
                    squirrel = getImage(IMG_SQUIREL_DEATH_0000 + (p_spr.i_Frame & 1));
                    pl_x -= (squirrel.getWidth()>>1);
                    pl_y+=3;
                    break;
            case MobileGameletImpl.SPRITE_SQUIRREL_FALL:
                    squirrel = getImage(IMG_SQUIREL_DEATH_0001);
                    pl_x -= (squirrel.getWidth()>>1);
                    pl_y+=3;
                    break;
        }

        pl_y -= squirrel.getHeight() -1;


        if(p_spr.i_ObjectType != MobileGameletImpl.SPRITE_SQUIRREL_FALL || p_spr.i_ObjectState < 0)
        {
            _graphics.drawImage(squirrel,pl_x,pl_y,0);

            if (p_spr.i_mainY == MobileGameletImpl.HOLLOW1_Y || p_spr.i_mainY == MobileGameletImpl.HOLLOW2_Y)
            {
               if(pl_x + squirrel.getWidth() >= (MobileGameletImpl.HOLLOW1_X>>8)+3)
               {
                  gCover.drawImage(back,-(MobileGameletImpl.HOLLOW1_X>>8)-3,-pl_y-interphone_gap,0);
                  _graphics.drawImage(imgCover,(MobileGameletImpl.HOLLOW1_X>>8)+3,pl_y,0);
               }
            }
              else
                   {
                        if(pl_x <= (MobileGameletImpl.HOLLOW3_X>>8)-1)
                        {
                           gCover.drawImage(back,-(MobileGameletImpl.HOLLOW3_X>>8)+1 +COVER_WIDTH,-pl_y-interphone_gap,0);
                           _graphics.drawImage(imgCover,(MobileGameletImpl.HOLLOW3_X>>8)-1 -COVER_WIDTH,pl_y,0);
                        }
                   }
            squirrel = null;
        }
        ...
        for(li=0;li<loMark;li++)
        {
            p_spr = sort_buffer[li];
            {
                sort_buffer[li] = null;

                i_sy = (p_spr.i_ScreenY + p_spr.i_height>>8) + CROW_OFFSET_Y;
                i_x = p_spr.i_mainX>>8;

                switch(p_spr.i_ObjectType)
                {
                    case MobileGameletImpl.SPRITE_CROW_LEFT:
                    case MobileGameletImpl.SPRITE_CROW_LEFT_MOVE:
                               {
                                   i_ndx = IMG_CROW_PECK_RIGHT_0000;
                                   i_x -= CROW_OFFSET_X;
                               }
                               break;
                    case MobileGameletImpl.SPRITE_CROW_LEFT_ATTACK:
                               {
                                   i_ndx = IMG_CROW_PECK_RIGHT_0000 + (p_spr.i_Frame & 1);
                                   i_x -= CROW_OFFSET_X;
                               }
                               break;
                    case MobileGameletImpl.SPRITE_CROW_LEFT_PAIN:
                               {
                                   i_ndx = IMG_CROW_BRISTLE_RIGHT;
                                   i_x -= CROW_OFFSET_X - crowCryOffset;
                               }
                               break;

                    case MobileGameletImpl.SPRITE_CROW_RIGHT:
                    case MobileGameletImpl.SPRITE_CROW_RIGHT_MOVE:
                               {
                                   i_ndx = IMG_CROW_PECK_LEFT_0000;
                                   i_x += CROW_OFFSET_X;
                               }
                               break;
                    case MobileGameletImpl.SPRITE_CROW_RIGHT_ATTACK:
                               {
                                   i_ndx = IMG_CROW_PECK_LEFT_0000 + (p_spr.i_Frame & 1);
                                   i_x += CROW_OFFSET_X;
                               }
                               break;
                    case MobileGameletImpl.SPRITE_CROW_RIGHT_PAIN:
                               {
                                   i_ndx = IMG_CROW_BRISTLE_LEFT;
                                   i_x += CROW_OFFSET_X - crowCryOffset;

                               }
                               break;

                    case MobileGameletImpl.SPRITE_CROW_WIN:
                               {
                                 i_y = p_spr.i_mainY>>8;
                                 if (i_y  == (MobileGameletImpl.HOLLOW1_Y>>8) || i_y  == (MobileGameletImpl.HOLLOW2_Y>>8))
                                 {
                                    i_x -= 8;
                                    if(p_spr.i_Frame == MobileGameletImpl.CROW_WIN_CROAC_FRAME)
                                    {
                                      i_ndx = IMG_CROW_BRISTLE_LEFT;
                                    }
                                     else
                                      ...
                                      i_x =  (MobileGameletImpl.HOLLOW1_X>>8)+3;
                                 }
                                   else
                                        {
                                           i_x += 8;
                                           if(p_spr.i_Frame == MobileGameletImpl.CROW_WIN_CROAC_FRAME)
                                           {
                                            ...
                                            i_x = (MobileGameletImpl.HOLLOW3_X>>8)-2 - COVER_WIDTH;
                                        }
                                 gCover.drawImage(back,-i_x,-i_sy-interphone_gap,0);
                                 _graphics.drawImage(imgCover,i_x,i_sy,0);
                ...
                img = getImage(i_ndx);
                _graphics.drawImage(img, i_x - (img.getWidth()>>1), i_sy - img.getHeight(), 0);
            }
        }


        for(li=hiMark;li<sort_buffer.length;li++)
        {
            p_spr = sort_buffer[li];
            {
                sort_buffer[li] = null;

                i_sy = p_spr.i_ScreenY + p_spr.i_height>>8;
                i_x = p_spr.i_mainX>>8;
                switch(p_spr.i_ObjectType)
                {
                    case MobileGameletImpl.SPRITE_CONE_FIRE:
                               {
                                   i_y = p_spr.i_mainY>>8;
                                   img = getImage(IMG_CONE_ROTATE_0000 + (p_spr.i_Frame&7));
                ...
                img = getImage(i_ndx);
                _graphics.drawImage(img, i_x - (img.getWidth()>>1), i_sy - img.getHeight() + CROW_OFFSET_Y, 0);
            }
        }
   ...
   _graphics.translate(0,-16);
          ...
          _graphics.drawImage(getImage(IMG_CONE_ROTATE_0000),i_x=cones_x,cones_y-2,0);
          _graphics.drawImage(getImage(IMG_NUMBERS_0010),i_x+=coneRotate_fontWidth,cones_y,0);
          _graphics.drawImage(getImage(IMG_NUMBERS_0000),i_x+=font_fontWidth,cones_y,0);
          _graphics.drawImage(getImage(IMG_NUMBERS_0000+p_Gamelet.i_ConeCounter),i_x+=font_fontWidth,cones_y,0);
          _graphics.drawImage(imgScore,score_x,score_y,0);
...
if (lg_FinishImagePictured)
    ...
    public int processGameAction(int _arg)
    {
          ...
          switch(_arg)
          {
               case MobileGameletImpl.GAMEACTION_SQUIRREL_JUMP:
                          {
                          }
                          break;
               case MobileGameletImpl.GAMEACTION_SQUIRREL_TELEPORTED:
                          {
                          }
                          break;
               case MobileGameletImpl.GAMEACTION_SQUIRREL_FIRE:
                          {
                          }
                          break;
               case MobileGameletImpl.GAMEACTION_SQUIRREL_DEAD:
                          {
                            i_action_sound = SND_GIBEL_BELKI_MMF;
                          }
                          break;

               case MobileGameletImpl.GAMEACTION_CROW_GENERATED:
                          {
                          }
                          break;
               case MobileGameletImpl.GAMEACTION_CROW_FLY_AWAY:
                          {
                          }
                          break;
               case MobileGameletImpl.GAMEACTION_CROW_ATTACK:
                          {
                          }
                          break;
               case MobileGameletImpl.GAMEACTION_CROW_PAIN:
                          {
                            i_action_sound = SND_POPALI_V_VORONU_MMF;
                          }
                          break;
               case MobileGameletImpl.GAMEACTION_CROW_WIN:
                          {
                          }
                          break;
               case MobileGameletImpl.GAMEACTION_CROW_WIN_CROAK:
                          {
                            i_action_sound = SND_POPALI_V_VORONU_MMF;
                          }
                          break;

               case MobileGameletImpl.GAMEACTION_CONE_GENERATED:
                          {
                            i_action_sound = SND_POYAVLENIE_SHISHKI_MMF;
                          }
                          break;
               case MobileGameletImpl.GAMEACTION_CONE_READY:
                          {
                          }
                          break;
               case MobileGameletImpl.GAMEACTION_CONE_TAKEN:
                          {
                            i_action_sound = SND_BELKA_BERET_SHISHKU_MMF;
                          }
                          break;
               case MobileGameletImpl.GAMEACTION_CONE_FIRE_GENERATED:
                          {
                            i_action_sound = SND_BROSOK_SHISHKI_MMF;
                          }
                          break;
          }
    ...
    public Object processGameAction(int _arg1, int _arg2, Object _obj1)
    {
     ...
     Image imgCover;
     Graphics gCover;
     final static int COVER_WIDTH = 29;
     final static int COVER_HEIGHT = 29;

     int coneRotate_fontWidth, font_fontWidth;
     int crowPeckHeight,crowPeckWidth,crowCryWidth, crowCryOffset;
     int qSitLeft_fontWidth,qSitRight_fontWidth;

     Sprite [] sort_buffer = new Sprite[MobileGameletImpl.MAX_SPRITES_NUMBER];
     Sprite [] cones = new Sprite[MobileGameletImpl.MAX_SPRITES_NUMBER];
       final static int interphone_gap = 16;

     final static int CROW_OFFSET_Y = +4;
     final static int CROW_OFFSET_X = +4;

     private void prepareGraphics()
     {
        score             = new drawDigits(getImage(IMG_SCORE),10);

        imgScore = Image.createImage(score.fontWidth*4,score.fontHeight);
        gScore = imgScore.getGraphics();

        score_x = GAMESCREEN_WIDTH - imgScore.getWidth() - 2;
        score_y = 1;

        imgCover = Image.createImage(COVER_WIDTH,COVER_HEIGHT);
        gCover = imgCover.getGraphics();

        Image font = getImage(IMG_NUMBERS_0000);
        Image cone = getImage(IMG_CONE_ROTATE_0000);
        Image peck = getImage(IMG_CROW_PECK_RIGHT_0000);
        Image cry = getImage(IMG_CROW_BRISTLE_RIGHT);

        coneRotate_fontWidth = cone.getWidth();
        font_fontWidth = font.getWidth();
        crowPeckHeight = peck.getHeight();
        crowPeckWidth = peck.getWidth();
        crowCryWidth = cry.getWidth();

        crowCryOffset =  crowCryWidth - crowPeckWidth +1 >> 1;

        qSitLeft_fontWidth = getImage(IMG_SQUIREL_SIT_LEFT).getWidth();
        qSitRight_fontWidth = getImage(IMG_SQUIREL_SIT_RIGHT).getWidth();

        cones_x = GAMESCREEN_WIDTH - font.getWidth()*3 - cone.getWidth() - 2;
          cones_y = GAMESCREEN_HEIGHT - Math.max(font.getHeight(),cone.getHeight()) -1;
     }
...
SND_POYAVLENIE_SHISHKI_MMF        ,521,
SND_BELKA_BERET_SHISHKU_MMF       ,92,
SND_BROSOK_SHISHKI_MMF            ,425,
SND_POPALI_V_VORONU_MMF           ,227,
SND_GIBEL_BELKI_MMF               ,1686,
SND_FINAL_MMF                     ,0,
SND_START_MMF                     ,0
                             };
...
private static final int IMG_TITLE160 = 0;
...
private static final int IMG_BACKGR128X160 = 5;
private static final int IMG_CONE_GROW_0000 = 6;
private static final int IMG_CONE_GROW_0001 = 7;
private static final int IMG_CONE_GROW_0002 = 8;
private static final int IMG_CONE_GROW_0003 = 9;
private static final int IMG_CONE_ROTATE_0000 = 10;
private static final int IMG_CONE_ROTATE_0001 = 11;
private static final int IMG_CONE_ROTATE_0002 = 12;
private static final int IMG_CONE_ROTATE_0003 = 13;
private static final int IMG_CONE_ROTATE_0004 = 14;
private static final int IMG_CONE_ROTATE_0005 = 15;
private static final int IMG_CONE_ROTATE_0006 = 16;
private static final int IMG_CONE_ROTATE_0007 = 17;
private static final int IMG_CROW_FLY_LEFT = 18;
private static final int IMG_CROW_PECK_LEFT_0000 = 19;
private static final int IMG_CROW_PECK_LEFT_0001 = 20;
private static final int IMG_CROW_BRISTLE_LEFT = 21;
private static final int IMG_CROW_FLY_RIGHT = 22;
private static final int IMG_CROW_PECK_RIGHT_0000 = 23;
private static final int IMG_CROW_PECK_RIGHT_0001 = 24;
private static final int IMG_CROW_BRISTLE_RIGHT = 25;
private static final int IMG_SQUIREL_DEATH_0000 = 26;
private static final int IMG_SQUIREL_DEATH_0001 = 27;
private static final int IMG_SQUIREL_PICK_CONE_0000 = 28;
private static final int IMG_SQUIREL_PICK_CONE_0001 = 29;
private static final int IMG_SQUIREL_SIT_LEFT = 30;
private static final int IMG_SQUIREL_JUMP_LEFT_0000 = 31;
private static final int IMG_SQUIREL_JUMP_LEFT_0001 = 32;
private static final int IMG_SQUIREL_THROW_CONE_LEFT_0000 = 33;
private static final int IMG_SQUIREL_THROW_CONE_LEFT_0001 = 34;
private static final int IMG_SQUIREL_SIT_RIGHT = 35;
private static final int IMG_SQUIREL_JUMP_RIGHT_0000 = 36;
private static final int IMG_SQUIREL_JUMP_RIGHT_0001 = 37;
private static final int IMG_SQUIREL_THROW_CONE_RIGHT_0000 = 38;
private static final int IMG_SQUIREL_THROW_CONE_RIGHT_0001 = 39;
private static final int IMG_ICON_LIFE = 40;
private static final int IMG_SCORE = 41;
private static final int IMG_NUMBERS_0010 = 42;
private static final int IMG_NUMBERS_0000 = 43;
private static final int IMG_NUMBERS_0001 = 44;
private static final int IMG_NUMBERS_0002 = 45;
private static final int IMG_NUMBERS_0003 = 46;
private static final int IMG_NUMBERS_0004 = 47;
private static final int TOTAL_IMAGES_NUMBER = 48;

private static final int SND_BELKA_BERET_SHISHKU_MMF = 0;
private static final int SND_BROSOK_SHISHKI_MMF = 1;
private static final int SND_FINAL_MMF = 2;
private static final int SND_GIBEL_BELKI_MMF = 3;
private static final int SND_POPALI_V_VORONU_MMF = 4;
private static final int SND_POYAVLENIE_SHISHKI_MMF = 5;
private static final int SND_START_MMF = 6;

...
public class Sprite
{
    ...
    public int i_ActionCount;
    public int i_ActionDelay;
    public boolean b_Staying;
 ...
 if (i_sX+i_sw < i_dX || i_sY+i_sh < i_dY || i_dX+i_dw < i_sX || i_dY+i_dh < i_sY)
    ...
    public void readSpriteFromStream(DataInputStream _dis) throws IOException
    {
        ...
        i_TTL = _dis.readUnsignedByte();

        lg_backMove = _dis.readBoolean();
        b_Staying = _dis.readBoolean();

        i_ObjectState = _dis.readInt();
        i_ActionDelay = _dis.readInt();
        i_ActionCount = _dis.readInt();
    }

    public static final int DATASIZE_BYTES = 27;
    ...
    public void writeSpriteToStream(DataOutputStream _dis) throws IOException
    {
        ...
        _dis.writeByte(i_TTL);

        _dis.writeBoolean(lg_backMove);
        _dis.writeBoolean(b_Staying);

        _dis.writeInt(i_ObjectState);
        _dis.writeInt(i_ActionDelay);
        _dis.writeInt(i_ActionCount);

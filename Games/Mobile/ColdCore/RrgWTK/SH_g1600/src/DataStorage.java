...
public class DataStorage
{
    ...
    public static final String getNameInPosition(byte[] _scoreTable, int _position)
    {
        ...
        if (LangBlock.p_FontImage!=null)
            return LangBlock.decodeString(LangBlock.encodeStringToTiles(_scoreTable, i_scoffset),0);
        else
    ...
    public static final int init(String _name) throws Exception
    {
        ...
        catch (Exception _ex)
        {
            ...
            if (_ex instanceof javax.microedition.rms.RecordStoreFullException)
            {
                ...
                try
                {
                    s_Status = Integer.toString(p_recStore.getSizeAvailable());
                }
                catch (Exception e)
                {
                    s_Status = "-1";

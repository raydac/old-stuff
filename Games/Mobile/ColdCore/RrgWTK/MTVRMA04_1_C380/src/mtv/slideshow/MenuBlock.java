...
public class MenuBlock implements CommandListener
{
    ...
    //protected Display MB_p_Display;
    ...
    public synchronized final void commandAction(Command _command, Displayable _displayable)
    {
        ...
        switch (i_type)
        {
            case Command.BACK:
                {
                    // Processing of a MB_back command
    ...
    public void MB_initScreen(int _screenPointer,boolean _displayImmediately)
    {
        ...
        if ((i_ScreenFlags & MB_SCREEN_FLAG_CUSTOMSCREEN) != 0)
        {
            ...
            if (MB_p_Form instanceof Form) lg_isForm = true;
        }
        else
        {
    ...
    {
        if (MB_currentOkScreen < 0) return;
        MB_initScreen(MB_currentOkScreen,true);
    }
    ...
    {
        if (MB_currentErrorScreen < 0) return;
        MB_initScreen(MB_currentErrorScreen,true);
    }
    ...
    public void MB_viewAlert(String _caption,String _text,AlertType _type,int _delay)
    {
        Display p_display = MB_p_menuActionListener.getDisplay();

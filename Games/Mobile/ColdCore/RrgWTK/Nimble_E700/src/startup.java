                        ...
                        if (p_Gamelet.getGameWorldState() == MobileGamelet.GAMEWORLDSTATE_PLAYED)
                        {
                            ...
                            if (i_timeDelay <= 0) i_timeDelay = 1;
                             ...
                             if(p_Canvas.isShown() && i_MIDletState == STATE_GAMEPLAY)
                             {
                               ...
                               {
                                       {
                                           if(_playing_melody != SND_WORKING_TERMINAL_MMF)
                                           {
                                              i_action_sound = SND_WORKING_TERMINAL_MMF;
                                              checkPlayabilityMusic();
                                              if(i_action_sound >= 0)
                                              {
                                                 playMelody(ap_melody[i_action_sound],200,false);
                                                 _last_actioncall = i_timeStamp + ap_melody[i_action_sound].i_MelodyLength * 200;
                                                 i_action_sound = -1;
                                              }
                                           }
                                       }
...
p_imageForOutput = getImage(2);
    ...
    private void gamePlayDrawing(Graphics _graphics)
    {
        ...
        int li,i_idx,i_sx,i_sy,i_w,i_h,i_x, i_y;
        Sprite p_spr;
        Image img;
     ...
     for( li=0; li<p_Gamelet.ap_Sprites.length; li++)
     {
         ...
         if (p_spr.lg_SpriteActive)
         {
             ...
             {
                 case MobileGameletImpl.SPRITE_SUITCASE1:      i_idx = IMG_BAG_GREEN; break;
                 case MobileGameletImpl.SPRITE_SUITCASE2:      i_idx = IMG_BAG_BLUE; break;
                 case MobileGameletImpl.SPRITE_SUITCASE3:      i_idx = IMG_BAG_RED; break;
                 case MobileGameletImpl.SPRITE_SUITCASE4:      i_idx = IMG_BAG_YELLOW; break;
                 case MobileGameletImpl.SPRITE_SUITCASE_EMPTY: i_idx = IMG_SPOT; break;
                   default: continue;
             }
             _graphics.drawImage(getImage(i_idx),i_x+1,i_y,0);
         }
     }
     ...
     {
           default:
          case MobileGameletImpl.SPRITE_CARRIER:       li = IMG_MAN;       i_sy=1;  break;
          case MobileGameletImpl.SPRITE_CARRIER_LEFT:  li = IMG_MAN_LEFT;  i_sx=-1; break;
          case MobileGameletImpl.SPRITE_CARRIER_UP:    li = IMG_MAN_TOP;   i_sy=-1; break;
          case MobileGameletImpl.SPRITE_CARRIER_RIGHT: li = IMG_MAN_RIGHT; i_sx=1;  break;
          case MobileGameletImpl.SPRITE_CARRIER_DOWN:  li = IMG_MAN_DOWN;  i_sy=1; i_y -= 3; break;
     ...
     _graphics.drawImage(img,i_x-i_w,i_y-i_h,0);

     if(i_idx > 0)
     {
         img = getImage(i_idx);
         i_x += i_w*i_sx - (img.getWidth()>>1);
         i_y += i_h*i_sy - (img.getHeight()>>1);

         if(i_sx!=0) i_y-=1;
         if(i_sy==1) i_y-=img.getHeight()>>1;
     ...
     _graphics.translate(0,-16);
     _graphics.drawImage(getImage(IMG_BACKGR_TOP128X160),0,0,0);
     _graphics.drawImage(getImage(IMG_BACKGR_DOWN128X160),0,160-16,0);

     i_y = 3;

       i_x = GAMESCREEN_WIDTH-10-(font.fontWidth<<2);
     if(p_Player.m_iPlayerGameScores < 10000)
     {
        font.drawDigits(_graphics,i_x,i_y-1,4,p_Player.m_iPlayerGameScores,10);
     }
       else
             {
                font.drawDigits(_graphics,i_x-font.fontWidth,i_y-1,5,p_Player.m_iPlayerGameScores,10);
             }
...
if (lg_FinishImagePictured)
    ...
    public int processGameAction(int _arg)
    {
          ...
          switch(_arg)
          {
               case MobileGameletImpl.GAMEACTION_SUITCASE_GET:
                          {
                            i_action_sound = SND_VZYAL_CHEMODAN_MMF;
                          }
                          break;
               case MobileGameletImpl.GAMEACTION_SUITCASE_RIGHT:
                          {
                            i_action_sound = SND_PRAVILN_CHEMODAN_MMF;
                          }
                          break;
               case MobileGameletImpl.GAMEACTION_SUITCASE_WRONG:
                          {
                            i_action_sound = SND_NEPRAVILN_CHEMODAN_MMF;
                          }
                          break;
               case MobileGameletImpl.GAMEACTION_SUITCASE_GENERATED:
                          {
                          }
                          break;
               case MobileGameletImpl.GAMEACTION_SUITCASE_DISAPPEAR:
                          {
                          }
                          break;
               case MobileGameletImpl.GAMEACTION_PLAYER_LOST:
                          {
                             i_action_sound = SND_GRUZCHIK_USTAL_MMF;
                          }
                          break;
          }
        checkPlayabilityMusic();
    ...
    {
                             int [] sort_idx = {
SND_WORKING_TERMINAL_MMF    ,599,
SND_NEPRAVILN_CHEMODAN_MMF  ,931,
SND_PRAVILN_CHEMODAN_MMF    ,498,
SND_VZYAL_CHEMODAN_MMF      ,127,
SND_GRUZCHIK_USTAL_MMF      ,1225,
SND_FINAL_MMF               ,0,
SND_START_MMF               ,0
                             };
...
private static final int IMG_BACKGR_DOWN128X160 = 6;
private static final int IMG_BACKGR_TOP128X160 = 7;
private static final int IMG_BAG_BLUE = 8;
private static final int IMG_BAG_GREEN = 9;
private static final int IMG_BAG_RED = 10;
private static final int IMG_BAG_YELLOW = 11;
private static final int IMG_ICON_LIFE = 12;
private static final int IMG_MAN = 13;
private static final int IMG_MAN_DOWN = 14;
private static final int IMG_MAN_LEFT = 15;
private static final int IMG_MAN_RIGHT = 16;
private static final int IMG_MAN_TOP = 17;
private static final int IMG_MEN_SIT_DOWN = 18;
private static final int IMG_NUMBERS = 19;
private static final int IMG_SPOT = 20;
private static final int TOTAL_IMAGES_NUMBER = 21;

private static final int SND_FINAL_MMF = 0;
private static final int SND_GRUZCHIK_USTAL_MMF = 1;
private static final int SND_NEPRAVILN_CHEMODAN_MMF = 2;
private static final int SND_PRAVILN_CHEMODAN_MMF = 3;
private static final int SND_START_MMF = 4;
private static final int SND_VZYAL_CHEMODAN_MMF = 5;
private static final int SND_WORKING_TERMINAL_MMF = 6;

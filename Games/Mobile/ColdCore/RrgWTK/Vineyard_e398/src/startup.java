...
public class startup extends MIDlet implements Runnable, CommandListener
{
            ...
            //#endif
    {
        ...
        //#endif
        {
            ...
            switch (i_MidletMode)
            {
                ...
                {
                    //#if DBUFFER
                    //$onPaintGameStage(p_doubleGraphics);
                    //#else
    ...
    private static final void setMode(int _newMode)
    {
        ...
        //#-
        System.out.println("Gamemode to "+_newMode);
        //#+
    ...
    private static final void loadGame()
    {
        try
        {
            ...
            Gamelet.loadGameStateFromByteArray(p_This,ab_array);
    ...
    //#-
    private static final int STAGENUMBER_FIRST = 1;
    //#+
    //$private static final int STAGENUMBER_FIRST = /*$STARTLEVEL$*/;
    private static final int STAGENUMBER_LAST = 3;
    ...
    public static final int processGameAction(int _arg)
    {
        ...
        switch(_arg)
        {
            case Gamelet.GAMEACTION_BASKETNUMBERCHANGED:
            {
                makeCachedLeftPanelImage();
            };break;
            case Gamelet.GAMEACTION_ATTEMPTIONSCHANGED:
            {
                makeCachedRightPanelImage();
            };break;
        }
        ...
        if (lg_Option_Sound)
        {
            switch (_arg)
            {
                case Gamelet.GAMEACTION_PLAYERKILLED :
                {
                    SoundManager.playSound(SoundManager.SOUND_PLAYERKILLED,1);
                };break;
                case Gamelet.GAMEACTION_PLAYERTAKEBUNCH :
                {
                    SoundManager.playSound(SoundManager.SOUND_BUNCH,1);
                };break;
                case Gamelet.GAMEACTION_PLAYERWIN :
                {
                    SoundManager.playSound(SoundManager.SOUND_PLAYERWIN,1);
                };break;
                case Gamelet.GAMEACTION_BASKETNUMBERCHANGED :
                {
    ...
    {
        //TileManager.setXY((Gamelet.i8_TerrainPos+0x7F)>>8,0);
    }
    ...
    protected static final void onLoadingGameResources(Class _this) throws Exception //ONEVENT onLoadingGameResources
    {
        ...
        // Грузим тайловый набор
        int i_tileW = ((16*SCALE_WIDTH)+0x7F)>>8;
        int i_tileH = ((16*SCALE_HEIGHT)+0x7F)>>8;

        //#if SHOWSYS
        System.out.println("Init tileset "+i_tileW+'x'+i_tileH);
        try
        {
        //#endif
        TileManager.initTileBackground(GAMESCREEN_WIDTH,GAMESCREEN_HEIGHT,i_tileW,i_tileH,GAMESCREEN_WIDTH,GAMESCREEN_HEIGHT);


        ap_tiles = null;
        p_tileImg = null;

        //#if VENDOR=="SAMSUNG" || VENDOR=="LG"
            //$ap_tiles = TileManager.loadSeparatedImagesContainer(_this,"/tileset.tls");
        //#else
            p_tileImg = Image.createImage("/tile.png");
        //#endif

        //#if SHOWSYS
        System.out.println("Tileset inited");
        }
        catch(Throwable _ex)
        {
            _ex.printStackTrace();
    ...
    public static final boolean onInitNewGameStage(int _gameStage) //ONEVENT onInitNewGameStage
    {
        ...
        // Загружаем массив тайлов для уровня
        String s_name = "/tile"+_gameStage+".bin";

        TileManager.setGameRoomArray(0,null);
        Runtime.getRuntime().gc();

            byte [] ab_array = (byte []) loadArray(p_This,s_name);
            TileManager.setGameRoomArray(Gamelet.TERRAIN_CELLS_WIDTH,ab_array);
            TileManager.setBlockImage(p_tileImg, ap_tiles);

            TileManager.setXY(0,0);
        ...
        // Берем координаты игрока и выставляем их в камеру
        TileManager.setXY(0xFFFF00,0xFFFF00);
        setCameraCoords(true);
    ...
    private static final void onGameStep() //ONEVENT onGameStep
    {
        // Смещаем бэкграунд
        //TileManager.setXY((Gamelet.i8_TerrainPos+0x7F)>>8,0);
        // сбрасываем флаг выстрела и кнопки вверх
        i_KeyFlags &= (~(Gamelet.BUTTON_FIRE | Gamelet.BUTTON_UP));
    }
    ...
    private static final void setCameraCoords(boolean _toPlayer)
    {
        // Получаем координаты игрока
        SpriteCollection p_col = Gamelet.ap_SpriteCollections[Gamelet.COLLECTION_PLAYER];
        int [] ai_sprarray = p_col.ai_spriteDataArray;

        int i_plX = ai_sprarray[SpriteCollection.SPRITEDATAOFFSET_MAINX];
        int i_plY = ai_sprarray[SpriteCollection.SPRITEDATAOFFSET_MAINY];

        int i_cameraX = ((i_plX+0x7F)>>8)-(GAMESCREEN_WIDTH>>1);
        int i_cameraY = ((i_plY+0x7F)>>8)-(GAMESCREEN_HEIGHT>>1);

        int i_lastCameraX = i_GameOptionalValue1;
        int i_lastCameraY = i_GameOptionalValue2;

        int i_dX = i_cameraX-i_lastCameraX;
        int i_dY = i_cameraY-i_lastCameraY;

        final int MAXCAMERASTEPX = ((16 * SCALE_WIDTH)+0x7F)>>8;
        final int MAXCAMERASTEPY = ((16 * SCALE_HEIGHT)+0x7F)>>8;

        if (!_toPlayer)
        {
            if (Math.abs(i_dX)>MAXCAMERASTEPX)
            {
                if (i_dX<0) i_dX = -MAXCAMERASTEPX;
                else
                    i_dX = MAXCAMERASTEPX;
            }
        if (Math.abs(i_dY)>MAXCAMERASTEPY)
        {
            if (i_dY<0) i_dY = -MAXCAMERASTEPY;
            else
                i_dY = MAXCAMERASTEPY;
        }
        }
        ...
        i_cameraX = i_lastCameraX+i_dX;
        i_cameraY = i_lastCameraY+i_dY;

        if (i_cameraX<0)
            i_cameraX = 0;
        else
        if (i_cameraX+GAMESCREEN_WIDTH>=Gamelet.TERRAIN_PIXELS_WIDTH)
            i_cameraX = Gamelet.TERRAIN_PIXELS_WIDTH-GAMESCREEN_WIDTH;

        if (i_cameraY<0)
             i_cameraY = 0;
        else
            if (i_cameraY+GAMESCREEN_HEIGHT>=Gamelet.TERRAIN_PIXELS_HEIGHT)
                i_cameraY = Gamelet.TERRAIN_PIXELS_HEIGHT-GAMESCREEN_HEIGHT;


        i_GameOptionalValue1 = i_cameraX;
        i_GameOptionalValue2 = i_cameraY;
        TileManager.setXY(i_cameraX,i_cameraY);
    }
    ...
    public static final void onAreaComponentPaint(Graphics _g, int _formIndex, int _x, int _y, int _width, int _height, int _channel) //ONEVENT onAreaComponentPaint
    {
        ...
        try
        {
            ...
            switch (_formIndex)
            {
                case MCFFormRender.MCF_FORM_GAMEFORM:
                {
                    switch (_channel)
                    {
                        case 1:
                        {
                            _g.setClip(_x,_y,_width,_height);
                            _g.drawImage(p_CachedLeftPanelImage,_x,_y,0);
                        };break;
                        case 2:
                        {
                            _g.setClip(_x,_y,_width,_height);
                            _g.drawImage(p_CachedRightPanelImage,_x,_y,0);
                        };break;
                        case 0:
                        {
                            _g.translate(_x, _y);
                            ...
                            int i_cameraX = i_GameOptionalValue1 ;
                            int i_cameraY = i_GameOptionalValue2 ;

                            //#if (VENDOR=="NOKIA" && MODEL=="6100")
                            //$TileManager.directPaint(_g,0,0);
                            //#else
                            TileManager.drawBufferToGraphics(_g,0,0);
                            //#endif

                            SpriteCollection.setVisibleScreenAreaCoords(i_cameraX,i_cameraY,GAMESCREEN_WIDTH,GAMESCREEN_HEIGHT);


                            // Отрисовываем Виноград
                            SpriteCollection p_col = Gamelet.ap_SpriteCollections[Gamelet.COLLECTION_BUNCHES];
                            int [] ai_sprarray = p_col.ai_spriteDataArray;
                            p_col.initIterator();
                            while(true)
                            {
                                ...
                                if (p_col.isSpriteNoVisibled(i_sprOffset))continue;

                                int i_xy = p_col.getSpriteScreenXY(i_sprOffset);
                                int i_x = (i_xy >> 16)-i_cameraX;
                                int i_y = ((short)i_xy)-i_cameraY;

                                int i_TypeState = ai_sprarray[i_sprOffset+SpriteCollection.SPRITEDATAOFFSET_OBJECTTYPESTATE] & 0xFFFF;
                                int i_Type = i_TypeState >>> 8;
                                int i_State = i_TypeState & 0xFF;
                                int i_frame = ai_sprarray[i_sprOffset+SpriteCollection.SPRITEDATAOFFSET_ANIMATIONDATA] & SpriteCollection.MASK_FRAMENUMBER;


                                int i_map = 0;
                                switch(i_Type)
                                {
                                    case Gamelet.SPRITE_OBJ_BUNCH:
                                    {
                                        i_map = MAP_GRAPE01;
                                    };break;
                                }

                                i_map += (i_frame*ImageManager.IMAGEINFO_LENGTH);
                                ImageManager.drawImage(i_map,i_x,i_y);

                                //#if SHOWSYS

                                int i_hzxy = p_col.getSpriteHotzoneXY(i_sprOffset);
                                int i_hzwh = p_col.getSpriteHotzoneWidthHeight(i_sprOffset);

                                _g.setColor(0);
                                _g.drawRect(i_x+(i_hzxy>>16),i_y+((short)i_hzxy),i_hzwh>>>16,i_hzwh&0xFFFF);

                                _g.drawString(Integer.toString(i_frame),i_x,i_y,0);
                                //#endif
                            }

                            // Отрисовываем Улиток
                            p_col = Gamelet.ap_SpriteCollections[Gamelet.COLLECTION_SNAILS];
                            ai_sprarray = p_col.ai_spriteDataArray;
                            p_col.initIterator();
                            while(true)
                            {
                                int i_sprOffset = p_col.nextActiveSpriteOffset();
                                ...
                                switch(i_Type)
                                {
                                    case Gamelet.SPRITE_OBJ_SNAIL_LEFT :
                                    {
                                        switch(i_State)
                                        {
                                            case Gamelet.SPRITE_STATE_SNAIL_LEFT_BITE:
                                            {
                                                i_map = MAP_SLUGSTUNGL01;
                                            };break;
                                            case Gamelet.SPRITE_STATE_SNAIL_LEFT_MOVING:
                                            {
                                                i_map = MAP_SLUG_GOL_01;
                                            };break;
                                            case Gamelet.SPRITE_STATE_SNAIL_LEFT_TURN:
                                            {
                                                i_map = MAP_SLUG_TURN_LR01;
                                            };break;
                                        }
                                    };break;
                                    case Gamelet.SPRITE_OBJ_SNAIL_RIGHT :
                                    {
                                        switch(i_State)
                                        {
                                            case Gamelet.SPRITE_STATE_SNAIL_LEFT_BITE:
                                            {
                                                i_map = MAP_SLUGSTUNGR01;
                                            };break;
                                            case Gamelet.SPRITE_STATE_SNAIL_LEFT_MOVING:
                                            {
                                                i_map = MAP_SLUG_GOR_01;
                                            };break;
                                            case Gamelet.SPRITE_STATE_SNAIL_LEFT_TURN:
                                            {
                                                i_map = MAP_SLUG_TURN_RL01;
                                            };break;
                                        }
                                    };break;
                                }
                            ...
                            // Отрисовываем игрока
                            p_col = Gamelet.ap_SpriteCollections[Gamelet.COLLECTION_PLAYER];
                            ai_sprarray = p_col.ai_spriteDataArray;
                            p_col.initIterator();
                            while(true)
                            {
                                ...
                                switch(i_Type)
                                {
                                    case Gamelet.SPRITE_OBJ_PLAYER_EMPTY :
                                    {
                                        switch(i_State)
                                        {
                                            case Gamelet.SPRITE_STATE_PLAYER_EMPTY_JUMPLEFT:
                                            {
                                                i_map = MAP_GRAPERJUMPL;
                                            };break;
                                            case Gamelet.SPRITE_STATE_PLAYER_EMPTY_JUMPRIGHT:
                                            {
                                                i_map = MAP_GRAPERJUMPR;
                                            };break;
                                            case Gamelet.SPRITE_STATE_PLAYER_EMPTY_MOVINGLEFT:
                                            {
                                                i_map = MAP_GRAPER_GOL01;
                                            };break;
                                            case Gamelet.SPRITE_STATE_PLAYER_EMPTY_MOVINGRIGHT:
                                            {
                                                i_map = MAP_GRAPER_GOR01;
                                            };break;
                                            case Gamelet.SPRITE_STATE_PLAYER_EMPTY_STANDLEFT:
                                            {
                                                i_map = MAP_GRAPERSTANDL;
                                            };break;
                                            case Gamelet.SPRITE_STATE_PLAYER_EMPTY_STANDRIGHT:
                                            {
                                                i_map = MAP_GRAPERSTANDR;
                                            };break;
                                        }
                                    };break;
                                    case Gamelet.SPRITE_OBJ_PLAYER_FULL :
                                    {
                                        switch(i_State)
                                        {
                                            case Gamelet.SPRITE_STATE_PLAYER_EMPTY_JUMPLEFT:
                                            {
                                                i_map = MAP_GRAPERJUMPLWG;
                                            };break;
                                            case Gamelet.SPRITE_STATE_PLAYER_EMPTY_JUMPRIGHT:
                                            {
                                                i_map = MAP_GRAPERJUMPRWG;
                                            };break;
                                            case Gamelet.SPRITE_STATE_PLAYER_EMPTY_MOVINGLEFT:
                                            {
                                                i_map = MAP_GRAPER_GOLWG01;
                                            };break;
                                            case Gamelet.SPRITE_STATE_PLAYER_EMPTY_MOVINGRIGHT:
                                            {
                                                i_map = MAP_GRAPER_GORWG01;
                                            };break;
                                            case Gamelet.SPRITE_STATE_PLAYER_EMPTY_STANDLEFT:
                                            {
                                                i_map = MAP_GRAPERSTANDLWG;
                                            };break;
                                            case Gamelet.SPRITE_STATE_PLAYER_EMPTY_STANDRIGHT:
                                            {
                                                i_map = MAP_GRAPERSTANDRWG;
                                            };break;
                                        }
                                    };break;
                                    ...
                                    case Gamelet.SPRITE_OBJ_PLAYER_KILLED :
                                    {
                                        i_map = MAP_GRAPER_DEAD01;
                                    };break;
                                    case Gamelet.SPRITE_OBJ_PLAYER_WIN :
                                    {
                                        i_map = MAP_GRAP_WIN_01;
                                    };break;
                                }
                            ...
                            // Отрисовываем Корзины
                            p_col = Gamelet.ap_SpriteCollections[Gamelet.COLLECTION_BASKETS];
                            ai_sprarray = p_col.ai_spriteDataArray;
                            p_col.initIterator();
                            while(true)
                            {
                                ...
                                switch(i_Type)
                                {
                                    case Gamelet.SPRITE_OBJ_BASKET:
                                    {
                                        switch(i_State)
                                        {
                                            case Gamelet.SPRITE_STATE_BASKET_EMPTY:
                                            {
                                                i_map = MAP_BASKET0;
                                            };break;
                                            case Gamelet.SPRITE_STATE_BASKET_FULL:
                                            {
                                                i_map = MAP_BASKET1;
                                            };break;
                                        }
    ...
    {
        p_GameForm.selectForm(MCFFormRender.MCF_FORM_GAMEFORM);

        //#if (VENDOR=="NOKIA" && MODEL=="7650") || (VENDOR=="MOTOROLA" && MODEL=="E398") || VENDOR=="SIEMENS"
            p_GameForm.paint(_g, GAMESCREEN_OFFSETX, GAMESCREEN_OFFSETY, false);
        //#else
    ...
    private static final void onPaintGameStage(Graphics _g) //ONEVENT onPaintGameStage
    {
        _g.setClip(GAMESCREEN_OFFSETX,GAMESCREEN_OFFSETY,GAMESCREEN_WIDTH,GAMESCREEN_HEIGHT);

        switch(i_selectedGameStage)
        {
            case 1:
            {
                p_GameForm.selectForm(MCFFormRender.MCF_FORM_LEVEL1);
            };break;
            case 2:
            {
                p_GameForm.selectForm(MCFFormRender.MCF_FORM_LEVEL2);
            };break;
            case 3:
            {
                p_GameForm.selectForm(MCFFormRender.MCF_FORM_LEVEL3);
            };break;
        }
    ...
    private static Image p_tileImg;
    private static Image [] ap_tiles;
    private static Image p_CachedLeftPanelImage;
    private static Image p_CachedRightPanelImage;

    private static void makeCachedLeftPanelImage()
    {
        Image p_img = p_GameForm.ap_Images[MCFFormRender.MCF_IMG_LEFT_PANEL];
        int i_w = p_img.getWidth();
        int i_h = p_img.getHeight();

        if (p_CachedLeftPanelImage==null)
        {
            p_CachedLeftPanelImage = Image.createImage(i_w,i_h);
        }

        Graphics p_gr = p_CachedLeftPanelImage.getGraphics();
        p_gr.drawImage(p_img,0,0,0);

        int i_basketNumber = Gamelet.i_EmptyBasketsCounter;
        int i_b1 = i_basketNumber / 10;
        int i_b2 = i_basketNumber % 10;

        p_img = p_GameForm.ap_Images[MCFFormRender.MCF_IMG_dig0];
        final int NUMBER_CHAR_WIDTH = p_img.getWidth();
        ...
        int i_x = i_w - (NUMBER_CHAR_WIDTH<<1)-i_offset;

        p_gr.drawImage(p_GameForm.ap_Images[MCFFormRender.MCF_IMG_dig0+i_b1],i_x,0,0);
        i_x+=NUMBER_CHAR_WIDTH;
        p_gr.drawImage(p_GameForm.ap_Images[MCFFormRender.MCF_IMG_dig0+i_b2],i_x,0,0);
    }

    private static void makeCachedRightPanelImage()
    {
        Image p_img = p_GameForm.ap_Images[MCFFormRender.MCF_IMG_RIGHT_PANEL];
        int i_w = p_img.getWidth();
        int i_h = p_img.getHeight();

        if (p_CachedRightPanelImage==null)
        {
            p_CachedRightPanelImage = Image.createImage(i_w,i_h);
        }
        Graphics p_gr = p_CachedRightPanelImage.getGraphics();
        p_gr.drawImage(p_img,0,0,0);

        p_img = p_GameForm.ap_Images[MCFFormRender.MCF_IMG_dig0+(Gamelet.i_PlayerAttemptions-1)];
        final int NUMBER_CHAR_WIDTH = p_img.getWidth();
        ...
        p_gr.drawImage(p_img,i_w-NUMBER_CHAR_WIDTH-i_offset,0,0);
    }

//TODO ДАННЫЕ IMAGE MANAGERa

    //#-
    private static final int MAP_BASKET0 = 0;
    private static final int MAP_BASKET1 = 7;
    private static final int MAP_GRAPE01 = 14;
    private static final int MAP_GRAPE02 = 21;
    private static final int MAP_GRAPE03 = 28;
    private static final int MAP_GRAPE04 = 35;
    private static final int MAP_GRAPERJUMPL = 42;
    private static final int MAP_GRAPERJUMPLWG = 49;
    private static final int MAP_GRAPERJUMPR = 56;
    private static final int MAP_GRAPERJUMPRWG = 63;
    private static final int MAP_GRAPERSTANDLWG = 70;
    private static final int MAP_GRAPERSTANDL = 77;
    private static final int MAP_GRAPERSTANDRWG = 84;
    private static final int MAP_GRAPERSTANDR = 91;
    private static final int MAP_GRAPER_DEAD01 = 98;
    private static final int MAP_GRAPER_DEAD02 = 105;
    private static final int MAP_GRAPER_DEAD03 = 112;
    private static final int MAP_GRAPER_DEAD04 = 119;
    private static final int MAP_GRAPER_GOL01 = 126;
    private static final int MAP_GRAPER_GOL02 = 133;
    private static final int MAP_GRAPER_GOL03 = 140;
    private static final int MAP_GRAPER_GOL04 = 147;
    private static final int MAP_GRAPER_GOL05 = 154;
    private static final int MAP_GRAPER_GOL06 = 161;
    private static final int MAP_GRAPER_GOL07 = 168;
    private static final int MAP_GRAPER_GOL08 = 175;
    private static final int MAP_GRAPER_GOL09 = 182;
    private static final int MAP_GRAPER_GOL10 = 189;
    private static final int MAP_GRAPER_GOLWG01 = 196;
    private static final int MAP_GRAPER_GOLWG02 = 203;
    private static final int MAP_GRAPER_GOLWG03 = 210;
    private static final int MAP_GRAPER_GOLWG04 = 217;
    private static final int MAP_GRAPER_GOLWG05 = 224;
    private static final int MAP_GRAPER_GOLWG06 = 231;
    private static final int MAP_GRAPER_GOLWG07 = 238;
    private static final int MAP_GRAPER_GOLWG08 = 245;
    private static final int MAP_GRAPER_GOLWG09 = 252;
    private static final int MAP_GRAPER_GOLWG10 = 259;
    private static final int MAP_GRAPER_GOR01 = 266;
    private static final int MAP_GRAPER_GOR02 = 273;
    private static final int MAP_GRAPER_GOR03 = 280;
    private static final int MAP_GRAPER_GOR04 = 287;
    private static final int MAP_GRAPER_GOR05 = 294;
    private static final int MAP_GRAPER_GOR06 = 301;
    private static final int MAP_GRAPER_GOR07 = 308;
    private static final int MAP_GRAPER_GOR08 = 315;
    private static final int MAP_GRAPER_GOR09 = 322;
    private static final int MAP_GRAPER_GOR10 = 329;
    private static final int MAP_GRAPER_GORWG01 = 336;
    private static final int MAP_GRAPER_GORWG02 = 343;
    private static final int MAP_GRAPER_GORWG03 = 350;
    private static final int MAP_GRAPER_GORWG04 = 357;
    private static final int MAP_GRAPER_GORWG05 = 364;
    private static final int MAP_GRAPER_GORWG06 = 371;
    private static final int MAP_GRAPER_GORWG07 = 378;
    private static final int MAP_GRAPER_GORWG08 = 385;
    private static final int MAP_GRAPER_GORWG09 = 392;
    private static final int MAP_GRAPER_GORWG10 = 399;
    private static final int MAP_GRAP_WIN_01 = 406;
    private static final int MAP_GRAP_WIN_02 = 413;
    private static final int MAP_GRAP_WIN_03 = 420;
    private static final int MAP_GRAP_WIN_04 = 427;
    private static final int MAP_GRAP_WIN_05 = 434;
    private static final int MAP_GRAP_WIN_06 = 441;
    private static final int MAP_GRAP_WIN_07 = 448;
    private static final int MAP_GRAP_WIN_08 = 455;
    private static final int MAP_SCORE100 = 462;
    private static final int MAP_SLUGRAZV01 = 469;
    private static final int MAP_SLUGRAZV02 = 476;
    private static final int MAP_SLUGRAZV03 = 483;
    private static final int MAP_SLUGSTUNGL01 = 490;
    private static final int MAP_SLUGSTUNGL02 = 497;
    private static final int MAP_SLUGSTUNGL03 = 504;
    private static final int MAP_SLUGSTUNGL04 = 511;
    private static final int MAP_SLUGSTUNGL05 = 518;
    private static final int MAP_SLUGSTUNGR01 = 525;
    private static final int MAP_SLUGSTUNGR02 = 532;
    private static final int MAP_SLUGSTUNGR03 = 539;
    private static final int MAP_SLUGSTUNGR04 = 546;
    private static final int MAP_SLUGSTUNGR05 = 553;
    private static final int MAP_SLUG_GOL_01 = 560;
    private static final int MAP_SLUG_GOL_02 = 567;
    private static final int MAP_SLUG_GOL_03 = 574;
    private static final int MAP_SLUG_GOL_04 = 581;
    private static final int MAP_SLUG_GOR_01 = 588;
    private static final int MAP_SLUG_GOR_02 = 595;
    private static final int MAP_SLUG_GOR_03 = 602;
    private static final int MAP_SLUG_GOR_04 = 609;
    private static final int MAP_SLUG_TURN_LR01 = 616;
    private static final int MAP_SLUG_TURN_LR02 = 623;
    private static final int MAP_SLUG_TURN_LR03 = 630;
    private static final int MAP_SLUG_TURN_RL01 = 637;
    private static final int MAP_SLUG_TURN_RL02 = 644;
    private static final int MAP_SLUG_TURN_RL03 = 651;
    //#+

    //#include "./imageconstants.txt"

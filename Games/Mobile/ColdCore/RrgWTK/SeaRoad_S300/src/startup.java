...
p_imageForOutput = getImage(2);
    ...
    private void gamePlayDrawing(Graphics _graphics)
    {
        ...
        int li,i_sx,i_sy,i_w,i_h,i_x, i_y, i_ndx, loMark;
        Sprite p_spr;
        Image img;
        ...
        i_y = ((p_Gamelet.i_tripDistance>>8) & 63) - 63;
        while(i_y < GAMESCREEN_HEIGHT)
        {
            _graphics.drawImage(water,0,i_y,0);
            i_y += 64;
        }
        ...
        for(li=0;li < MobileGameletImpl.MAX_SPLASHES + MobileGameletImpl.MAX_LOCATIONS;li++)
        {
            p_spr = ap_sprites[li];
            if (p_spr.lg_SpriteActive)
            {
                ...
                {
                    case MobileGameletImpl.SPRITE_SOUTH_BOAT:
                    case MobileGameletImpl.SPRITE_SOUTH_BARGE:
                    case MobileGameletImpl.SPRITE_SOUTH_TUGBOAT:
                    case MobileGameletImpl.SPRITE_SOUTH_SPEEDBOAT:
                         {
                                img = getImage(IMG_SPLASHES_0000 + p_spr.i_Frame);
                                i_x = (p_spr.i_mainX>>8)-(img.getWidth()>>1);
                                i_y = (p_spr.i_ScreenY>>8) - img.getHeight() +1;
                                _graphics.drawImage(img,i_x,i_y,0);

                                sort_buffer[loMark++] = p_spr;
                         }
                         break;
                    case MobileGameletImpl.SPRITE_NORTH_BARGE:
                         {

                                img = getImage(IMG_SPLASHES_0002 + p_spr.i_Frame);
                                i_x = (p_spr.i_mainX>>8)-(img.getWidth()>>1)+1;
                                i_y = (p_spr.i_ScreenY + p_spr.i_height>>8) + 7;
                                _graphics.drawImage(img,i_x,i_y,0);

                                sort_buffer[loMark++] = p_spr;
                         }
                         break;
                    case MobileGameletImpl.SPRITE_SPLASH:
                         {
                                img = getImage(IMG_SPLASHES_CUTTER01 + p_spr.i_Frame);
                                i_x = (p_spr.i_mainX>>8)-(img.getWidth()>>1) +1;
        ...
        for(li = MobileGameletImpl.MAX_SPLASHES + MobileGameletImpl.MAX_LOCATIONS; li < MobileGameletImpl.MAX_SPRITES_NUMBER;li++)
        {
            p_spr = ap_sprites[li];
            if (p_spr.lg_SpriteActive)
            {
                switch(p_spr.i_ObjectType)
                {
                    case MobileGameletImpl.SPRITE_BUOY_LEFT:
                         {
                                i_ndx = IMG_BUOY_GREEN_0000;
                         }
                         break;
                    case MobileGameletImpl.SPRITE_BUOY_RIGHT:
                         {
                                i_ndx = IMG_BUOY_RED_0000;
                         }
                         break;
                    default: continue;
                }

                i_ndx += buoy_animation_sequence[p_spr.i_Frame];
                _graphics.drawImage(getImage(i_ndx),(p_spr.i_ScreenX>>8) -2,(p_spr.i_ScreenY>>8) -2, 0);
            }
        }


        sort_buffer[loMark++] = p_Gamelet.pPlayerSprite;
        ...
        for(li = 0;li < loMark;li++)
        {
            ...
            switch(p_spr.i_ObjectType)
            {
                ...
                case MobileGameletImpl.SPRITE_BOAT_APPEAR:
                     {
                         img = getImage(IMG_CUTTER_HOR_0000 + 6 - ((p_spr.i_ObjectState - MobileGameletImpl.MIN_ANGLE) >> 2));
                         i_x -= 1;
                         i_y += (p_spr.i_height>>8) - img.getHeight() + 4;
                         _graphics.drawImage(img,i_x,i_y,0);
                     }
                     continue;
                case MobileGameletImpl.SPRITE_BOAT_DEAD:
                     if(p_spr.i_Frame < (IMG_EXPLOZ_0005 - IMG_EXPLOZ_0000 +1))
                     {
                         img = getImage(IMG_EXPLOZ_0000 + p_spr.i_Frame);
                         _graphics.drawImage(img,i_x,i_y,0);
                     }
                     continue;
                case MobileGameletImpl.SPRITE_SOUTH_BOAT:      i_ndx = IMG_PLEASURE_BOAT ;break;
                case MobileGameletImpl.SPRITE_SOUTH_BARGE:     i_ndx = IMG_BARGE_DOWN ;break;
                case MobileGameletImpl.SPRITE_NORTH_BARGE:     i_ndx = IMG_BARGE_TOP ;break;
                case MobileGameletImpl.SPRITE_SOUTH_TUGBOAT:   i_ndx = IMG_TOWBOAT ;break;
                case MobileGameletImpl.SPRITE_SOUTH_SPEEDBOAT: i_ndx = IMG_METEOR ;break;
                  default: continue;

            }
            img = getImage(i_ndx);
            _graphics.drawImage(img,(p_spr.i_mainX>>8)-(img.getWidth()>>1),i_y,0);
        }


          i_x = 2;  i_y = GAMESCREEN_HEIGHT - 20; img = getImage(IMG_CLOCK);
          _graphics.drawImage(img, i_x, i_y, 0);

          i_ndx = p_Gamelet.i_PlayerTimeLimit;
          i_x += img.getWidth() + (font_width<<1);
          i_y += img.getHeight() - font_height>>1;

          for(li = 0; li < 3; li++)
          {
                _graphics.drawImage(getImage(IMG_NUMBERS_0000 + i_ndx % 10), i_x, i_y, 0);
                i_ndx /= 10;
                i_x -= font_width;
          }
          ...
          score.drawDigits(_graphics,score_x,score_y,4,p_Player.m_iPlayerGameScores,10);
...
if (lg_FinishImagePictured)
    ...
    public int processGameAction(int _arg)
    {
          ...
          switch(_arg)
          {
               case MobileGameletImpl.GAMEACTION_BOAT_APPEARING:
                          {
                            i_action_sound = SND_POYAVLENIE_LODKI_MMF;
                          }
                          break;
               case MobileGameletImpl.GAMEACTION_BOAT_READY:
                          {
                          }
                          break;
               case MobileGameletImpl.GAMEACTION_BOAT_DEAD:
                          {
                            i_action_sound = SND_VZRYV_LODKI_MMF;
                          }
                          break;
               case MobileGameletImpl.GAMEACTION_BUOY_HIT:
                          {
                          }
                          break;
               case MobileGameletImpl.GAMEACTION_GATE_OK:
                          {
                            i_action_sound = SND_PROHOJDENIE_MOSTA_MMF;
                          }
                          break;
               case MobileGameletImpl.GAMEACTION_GATE_GENERATED:
                          {
                          }
                          break;
               case MobileGameletImpl.GAMEACTION_BOAT_GENERATED:
                          {
                          }
                          break;
               case MobileGameletImpl.GAMEACTION_SPLASH_GENERATED:
                          {
                          }
                          break;
          }
    ...
    public Object processGameAction(int _arg1, int _arg2, Object _obj1)
    {
     ...
     Sprite [] sort_buffer = new Sprite[MobileGameletImpl.MAX_LOCATIONS +1];
     final static int [] buoy_animation_sequence = new int[] {0,1,0,2,0,3,0,4};
     ...
     private void prepareGraphics()
     {
        ...
        water     = Image.createImage(GAMESCREEN_WIDTH,img.getHeight());
        Graphics g = water.getGraphics();
        for(int x = 0; x < GAMESCREEN_WIDTH; x+=img.getWidth())
        {
            g.drawImage(img,x,0,0);
        }

        score_x = GAMESCREEN_WIDTH - score.fontWidth*4 - 2;
        score_y = 1;

        img = getImage(IMG_NUMBERS_0000);
        font_width = img.getWidth();
        font_height = img.getWidth();
     }
    ...
    {
                             int [] sort_idx = {
SND_POYAVLENIE_LODKI_MMF   ,2001,
SND_PROHOJDENIE_MOSTA_MMF  ,836,
SND_VZRYV_LODKI_MMF        ,918,
SND_FINAL_MMF              ,0,
SND_START_MMF              ,0
                             };
...
private static final int IMG_BARGE_DOWN = 4;
private static final int IMG_BARGE_TOP = 5;
private static final int IMG_TOWBOAT = 6;
private static final int IMG_METEOR = 7;
private static final int IMG_PLEASURE_BOAT = 8;
private static final int IMG_BUOY_GREEN_0000 = 9;
private static final int IMG_BUOY_GREEN_0001 = 10;
private static final int IMG_BUOY_GREEN_0002 = 11;
private static final int IMG_BUOY_GREEN_0003 = 12;
private static final int IMG_BUOY_GREEN_0004 = 13;
private static final int IMG_BUOY_RED_0000 = 14;
private static final int IMG_BUOY_RED_0001 = 15;
private static final int IMG_BUOY_RED_0002 = 16;
private static final int IMG_BUOY_RED_0003 = 17;
private static final int IMG_BUOY_RED_0004 = 18;
private static final int IMG_CUTTER_HOR_0000 = 19;
private static final int IMG_CUTTER_HOR_0001 = 20;
private static final int IMG_CUTTER_HOR_0002 = 21;
private static final int IMG_CUTTER_HOR_0003 = 22;
private static final int IMG_CUTTER_HOR_0004 = 23;
private static final int IMG_CUTTER_HOR_0005 = 24;
private static final int IMG_CUTTER_HOR_0006 = 25;
private static final int IMG_EXPLOZ_0000 = 26;
private static final int IMG_EXPLOZ_0001 = 27;
private static final int IMG_EXPLOZ_0002 = 28;
private static final int IMG_EXPLOZ_0003 = 29;
private static final int IMG_EXPLOZ_0004 = 30;
private static final int IMG_EXPLOZ_0005 = 31;
private static final int IMG_SPLASHES_0000 = 32;
private static final int IMG_SPLASHES_0001 = 33;
private static final int IMG_SPLASHES_0002 = 34;
private static final int IMG_SPLASHES_0003 = 35;
private static final int IMG_NUMBERS_0000 = 36;
private static final int IMG_NUMBERS_0001 = 37;
private static final int IMG_NUMBERS_0002 = 38;
private static final int IMG_NUMBERS_0003 = 39;
private static final int IMG_NUMBERS_0004 = 40;
private static final int IMG_NUMBERS_0005 = 41;
private static final int IMG_NUMBERS_0006 = 42;
private static final int IMG_NUMBERS_0007 = 43;
private static final int IMG_NUMBERS_0008 = 44;
private static final int IMG_NUMBERS_0009 = 45;
private static final int IMG_SPLASHES_CUTTER01 = 46;
private static final int IMG_SPLASHES_CUTTER02 = 47;
private static final int IMG_SPLASHES_CUTTER03 = 48;
private static final int IMG_SPLASHES_CUTTER04 = 49;
private static final int IMG_WATER = 50;
private static final int IMG_SCORE = 51;
private static final int IMG_CLOCK = 52;
private static final int IMG_ICON_LIFE = 53;
private static final int TOTAL_IMAGES_NUMBER = 54;

private static final int SND_FINAL_MMF = 0;
private static final int SND_POYAVLENIE_LODKI_MMF = 1;
private static final int SND_PROHOJDENIE_MOSTA_MMF = 2;
private static final int SND_START_MMF = 3;
private static final int SND_VZRYV_LODKI_MMF = 4;

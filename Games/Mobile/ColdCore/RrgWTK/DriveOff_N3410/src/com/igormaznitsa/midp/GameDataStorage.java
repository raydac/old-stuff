...
/**
 * This class is implementing of Game Data Storage and contains
...
public class GameDataStorage
{
    ...
    public GameDataStorage(String _gameID,int _maxsaveddata,int _maxcharinname,int _maxsavedgames,int _maxrecoedsintoplist) throws IOException
    {
        ...
        as_topnames = new String[MAX_TOPRECORDS+1];
        ai_topscores = new int[MAX_TOPRECORDS+1];
        lg_changed = false;
        ...
        try
        {
            ...
            if (p_optionstore.getNumRecords()!=0)
            {
                ...
                for(int li=0;li<MAX_TOPRECORDS;li++)
                {
                    ...
                    if (s_name.length() != 0)
		       addRecordToTopList(s_name,i_score);
                }
            ...
            else
            {
                ...
                int i_maxrecords = (p_optionstore.getSizeAvailable()-i_optionlen-16)/(_maxsaveddata+5);
		...
		} catch (Exception e){MAX_SAVEDRECORDS = 0;}
                ab_bytearr = null;
                Runtime.getRuntime().gc();
                ab_bytearr = new byte[i_maxgamerecordsize];
                for(int li=0;li<MAX_SAVEDRECORDS;li++)
                {
                    p_optionstore.addRecord(ab_bytearr,0,ab_bytearr.length);
                    Runtime.getRuntime().gc();
    ...
    {
//	System.out.println("WR:"+_dataarray.length);
	...
	int i_inactive = convertSavedNameToIndex(_name);
        if (i_inactive<0)
	{
     ...
     */
    public void addRecordToTopList(String _name,int _score)
    {
      int i=MAX_TOPRECORDS-1;
      for (;i>=0;i--)
       if (/*(ACSENDING_HIGHSCORE && */_score<ai_topscores[i]/*) || (!ACSENDING_HIGHSCORE && _score>ai_topscores[i])*/) break;
         else {
           ai_topscores[i+1]=ai_topscores[i];
           as_topnames[i+1]=as_topnames[i];
         }
       ai_topscores[i+1]=_score;
       as_topnames[i+1]=_name;
       if(i<MAX_TOPRECORDS)lg_changed = true;
//        sortTopList();
    }
    ...
    {
      return ai_topscores[MAX_TOPRECORDS-1];
/*
                                    return
	   (MAX_TOPRECORDS>0 && as_topnames[MAX_TOPRECORDS-1]!= null)
	                              ?
 	               ai_topscores[MAX_TOPRECORDS-1] : -1;
*/
    }
    ...
    {
      return ai_topscores[0];
/*
         return (as_topnames[0]!=null) ? ai_topscores[0] : -1;
*/
    }
    ...
    public void setIntOption(int _fieldId,int _value)
    {
        ...
        switch(_fieldId)
        {
            case FIELD_LANGUAGE : i_language = _value;
        }
    }
    ...
    public int getIntOption(int _fieldId)
    {
        switch(_fieldId)
        {
            case FIELD_LANGUAGE : return i_language;
        }
        return -1;
    }

  ...
  public int i_realScreenWidth,i_realScreenHeight;
    ...
    protected class insideCanvas extends Canvas
    {
        ...
        protected void paint(Graphics _graphics)
        {
            ...
            _graphics.drawImage(p_InsideDoubleBuffer, i_xoffst, i_yoffst, 0);
            lg_painted = false;
...
private MobileGameletImpl p_Gamelet;
    ...
    protected void startApp() throws MIDletStateChangeException
    {
        if (i_MIDletState >= 0) return;
          ...
          p_InsideDoubleBuffer = Image.createImage(SCREENWIDTH, SCREENHEIGHT);
          p_InsideDoubleBufferGraphics = p_InsideDoubleBuffer.getGraphics();
                        ...
                        if (p_Gamelet.getGameWorldState() == MobileGamelet.GAMEWORLDSTATE_PLAYED)
                        {
                            ...
                            if (i_timeDelay <= 0) i_timeDelay = 1;
                             ...
                             if(p_Canvas.isShown() && i_MIDletState == STATE_GAMEPLAY)
                             {
                               if(i_action_sound < 0)
                               {
                                       if(p_Gamelet.pPlayerFiremen.i_ObjectType == MobileGameletImpl.SPRITE_FIREMEN_PUT_OUT)
                                       {
                                           if(_playing_melody != SND_KACHAYUT_VODU_MMF)
                                           {
                                              i_action_sound = SND_KACHAYUT_VODU_MMF;
...
p_imageForOutput = getImage(2);
    ...
    private void gamePlayDrawing(Graphics _graphics)
    {
        ...
        int intermodel_y_gap = 0;

        int house_x = ((MobileGameletImpl.WINDOWS_X - 0x0B00) >> 8);
        int house_y = ((MobileGameletImpl.WINDOWS_Y - 0x0E00) >> 8)  + intermodel_y_gap;
        int house_w = 84;
        int house_h = 98 + 2;

        int fire_y = house_y;
        int fire_x = house_x;
        int fire_h = p_Gamelet.m_nFireLevel / 10;
        int fire_w = house_w;

        fire_y += (house_h - p_Gamelet.m_nFireLevel / 10);

        int floor_x = house_x +1;
        int floor_w = house_w;
        int floor_h = (house_h) / 5;

        int floor_y = house_y + house_h - floor_h;
           floor_y -=2;

        int fire_timing = i_Timer % fire.i_origRadix;
        int firemen_putout_x = floor_x + (floor_w>>2);

        for(li=4 ; li >= 0 ; li--)
        {
            switch(p_Gamelet.m_nFloor_States[li])
            {
                case MobileGameletImpl.STATE_FLOOR_NORMAL:    break;
                case MobileGameletImpl.STATE_FLOOR_FIRING:    fire.drawDigits(_graphics, floor_x, floor_y,1,fire_timing,fire.i_origRadix); break;
                case MobileGameletImpl.STATE_FLOOR_BURNT:     _graphics.drawImage(getImage(IMG_WIN_EXTINGUISHED),floor_x, floor_y,0); break;
            }
            floor_y -= floor_h -fire_offset[li];
        }
        ...
        for (li = 0; li < p_Gamelet.ap_Sprites.length; li++)
        {
            ...
            i_y = (p_spr.i_ScreenY >> 8)+intermodel_y_gap;
            i_w = p_spr.i_width >> 8;
            i_h = p_spr.i_height >> 8;

            switch (p_spr.i_ObjectType)
            {
                case MobileGameletImpl.SPRITE_VICTIM_HELP:
                    {
                       man_cry.drawDigits(_graphics,i_x, i_y,1,p_Gamelet.i_GameTimer & 1,2);
                    }
                    break;
                case MobileGameletImpl.SPRITE_VICTIM_RUN:
                    {
                       if(i_x + i_w > firemen_putout_x && p_Gamelet.pPlayerFiremen.i_ObjectType == MobileGameletImpl.SPRITE_FIREMEN_PUT_OUT)
                       {
                        sort_buffer[sort_mark++] = p_spr;
                        continue;
                       }
                       i_y += i_h - man_going.fontHeight;
                       man_going.drawDigits(_graphics,i_x, i_y,1,p_Gamelet.i_GameTimer % 3,3);
                    }
                    break;
                case MobileGameletImpl.SPRITE_VICTIM_JUMP_OUT:
                case MobileGameletImpl.SPRITE_VICTIM_LANDING:
                case MobileGameletImpl.SPRITE_VICTIM_DEAD:
                    {
                        sort_buffer[sort_mark++] = p_spr;
                    }
                    continue;
        ...
        img = getImage(IMG_AMBULANCE);
        _graphics.drawImage(img,0,(MobileGameletImpl.DEAD_LINE >>8) + 0x05 - img.getHeight() + intermodel_y_gap,0);


        p_spr = p_Gamelet.pPlayerFiremen;
        ...
        if (p_spr.lg_SpriteActive)
        {
            ...
            switch (p_spr.i_ObjectType)
            {
                case MobileGameletImpl.SPRITE_FIREMEN_LEFT   :
                case MobileGameletImpl.SPRITE_FIREMEN_RIGHT  :
                    {
                       i_x += ((i_w - catching.fontWidth) >>1);
                       i_y += i_h - catching.fontHeight;
                       catching.drawDigits(_graphics,i_x, i_y,1,p_Gamelet.i_GameTimer % 3,4);
                    }
                    break;
                case MobileGameletImpl.SPRITE_FIREMEN_PUT_OUT:
                    {
                       i_y = house_y + house_h - floor_h;
                         i_y -=1;
                       flooding.drawDigits(_graphics,firemen_putout_x,i_y,1,fire_timing,flooding.i_origRadix);
                    }
                    break;
                case MobileGameletImpl.SPRITE_FIREMEN        :
                default:
                    {
                       i_x += ((i_w - catching.fontWidth) >>1);
                       i_y += i_h - catching.fontHeight;
                       catching.drawDigits(_graphics,i_x, i_y,1,3,4);
                    }
                    break;
            }
        }
        ...
        for (li = 0; li < sort_mark; li++)
        {
            ...
            switch (p_spr.i_ObjectType)
            {
                ...
                case MobileGameletImpl.SPRITE_VICTIM_LANDING:
                    {
                       if(p_spr.i_Frame >= 0 && p_spr.i_Frame < reflect_offset.length)
                       {
                           i_y += reflect_offset[p_spr.i_Frame];
                       }
                    }
                case MobileGameletImpl.SPRITE_VICTIM_JUMP_OUT:
                    {
                       img = getImage(IMG_MAN_FALLING);
                       i_x += ((i_w - img.getWidth())>>1);
                       i_y += i_h - img.getHeight();
                       _graphics.drawImage(img,i_x,i_y,0);
                    }
                    break;
                case MobileGameletImpl.SPRITE_VICTIM_DEAD:
                    {
                       img = getImage(IMG_MAN_LYING);
                       i_x += ((i_w - img.getWidth())>>1);
                       i_y += i_h - img.getHeight() -2;
                       _graphics.drawImage(img,i_x,i_y,0);
                    }
                    break;
            }
...
if (lg_FinishImagePictured)
    ...
    public int processGameAction(int _arg)
    {
          ...
          switch(_arg)
          {
               case MobileGameletImpl.GAMEACTION_FLOOR_BURNING:
                          {
                            i_action_sound = SND_FIRE_MMF;
                          }
                          break;
               case MobileGameletImpl.GAMEACTION_FIREMEN_PUT_OUT:
                          {
                          }
                          break;
               case MobileGameletImpl.GAMEACTION_VICTIM_JUMP:
                          {
                            i_action_sound = SND_PRYJOK_MMF;
                          }
                          break;
               case MobileGameletImpl.SPRITE_VICTIM_LANDING:
                          {
                            i_action_sound = SND_SAVED_MMF;
                          }
                          break;
               case MobileGameletImpl.GAMEACTION_VICTIM_DEAD:
                          {
                            i_action_sound = SND_SMERT_MMF;
                          }
                          break;
          }
        checkPlayabilityMusic();
    ...
    public Object processGameAction(int _arg1, int _arg2, Object _obj1)
    {
     ...
     int fire_offset[]={0,1,2,2,2};
     int reflect_offset[]={2,-2,-5,-1,-2,1,1,1};
     ...
     drawDigits font;
     drawDigits fire;
     drawDigits catching,flooding;
     drawDigits man_cry,man_going;


     private void prepareGraphics()
     {
        font              = new drawDigits(getImage(IMG_NUMBERS),10);
        fire              = new drawDigits(getImage(IMG_FIRE),5,false);
        catching          = new drawDigits(getImage(IMG_FIREMEN),4,false);
        flooding          = new drawDigits(getImage(IMG_POMPA),5,false);
        man_cry           = new drawDigits(getImage(IMG_MAN_CRY),2,false);
        man_going         = new drawDigits(getImage(IMG_MAN_GOING),3,false);
...
private static final int IMG_AMBULANCE = 4;
private static final int IMG_BACK = 5;
private static final int IMG_BACKGROUND128X128 = 6;
private static final int IMG_FIRE = 7;
private static final int IMG_FIREMEN = 8;
private static final int IMG_LIFE = 9;
private static final int IMG_MAN_CRY = 10;
private static final int IMG_MAN_FALLING = 11;
private static final int IMG_MAN_GOING = 12;
private static final int IMG_MAN_LYING = 13;
private static final int IMG_NUMBERS = 14;
private static final int IMG_POMPA = 15;
private static final int IMG_WIN_EXTINGUISHED = 16;
private static final int TOTAL_IMAGES_NUMBER = 17;

private static final int SND_END_MMF = 0;
private static final int SND_FIRE_MMF = 1;
private static final int SND_KACHAYUT_VODU_MMF = 2;
private static final int SND_PRYJOK_MMF = 3;
private static final int SND_SAVED_MMF = 4;
private static final int SND_SMERT_MMF = 5;

...
p_imageForOutput = getImage(2);
    ...
    private void gamePlayDrawing(Graphics _graphics)
    {
        ...
        int li,i_w,i_h,i_x,i_y, i_ndx=0, i_cx=0,i_cy=0,i_cndx;
        boolean lg_crockyKeepAnimal = false;
        Sprite p_spr;
        Image img;
          ...
          gCroCache.drawImage(getImage(IMG_BACK),0,-unsafe,0);

          p_spr = p_Gamelet.pPlayerSprite;
          drawDigits croc = null;

          if(p_spr.lg_SpriteActive)
          {
            i_cx = p_spr.i_mainX>>8;
            i_cy = p_spr.i_mainY>>8;
            ...
            {
                case MobileGameletImpl.SPRITE_CROCODILE_LEFT:  croc = crocLeft; break;
                case MobileGameletImpl.SPRITE_CROCODILE_RIGHT: croc = crocRight; break;
                case MobileGameletImpl.SPRITE_CROCODILE_BITE:  croc = crocBite; i_ndx = 1; break;
                case MobileGameletImpl.SPRITE_CROCODILE_KEEP_ANIMAL:  croc = crocBite; i_ndx = 0; lg_crockyKeepAnimal = true; break;
                case MobileGameletImpl.SPRITE_CROCODILE_SURFACE: croc = (i_ndx==0) ? crocBite : crocky; break;
                case MobileGameletImpl.SPRITE_CROCODILE_DIVE:  croc = crocDive; i_cy += p_spr.i_Frame + 1; break;
                case MobileGameletImpl.SPRITE_CROCODILE_DEAD:
                                                           if(i_ndx == 0) croc = crocBite;
                                                              else {
                                                                     croc = crocOut;
                                                                     i_ndx--;
                                                                   }
            ...
            i_cx -=croc.fontWidth>>1;
            i_cy -=croc.fontHeight>>1;

            if(i_cy+croc.fontHeight >= unsafe)
            {
              croc.drawDigits(gCroCache,i_cx,i_cy - unsafe,i_ndx);
            }
          }


          font.drawDigits(gCroCache,score_x,score_y - unsafe,4,p_Player.m_iPlayerGameScores,10);

          img = getImage(IMG_ICON_LIFE);
          i_x = 2;  i_y = GAMESCREEN_HEIGHT - img.getHeight()-1 -unsafe;
          ...
          if (p_Gamelet.getGameWorldState() == MobileGamelet.GAMEWORLDSTATE_PLAYERLOST)
          {
              ...
              for(; li > 0 ;li--)
              {
                  gCroCache.drawImage(img, i_x, i_y,0);
                  i_x += img.getWidth();
              }
          }
            else
                 for(; li > 0 ;li--)
                 {
                    ...
                    {
                        i_h = img.getHeight();
                        i_w = i_y + i_h - i_h * (p_Gamelet.i_MissCounter + 1) / MobileGameletImpl.MAXIMUM_ALLOWED_MISSES;

                        gCroCache.setClip(0,i_w,GAMESCREEN_WIDTH,i_h);
                        gCroCache.drawImage(img, i_x, i_y,0);
                        gCroCache.setClip(0,0,GAMESCREEN_WIDTH,20);
                    }
                      else
                           gCroCache.drawImage(img, i_x, i_y,0);
          ...
          _graphics.drawImage(imgCroCache,0,unsafe,0);


          if(croc != null)
          {
            if(lg_crockyKeepAnimal && (i_Timer & 1)==0)
            {
               for(li=0;li<p_Gamelet.ap_Sprites.length;li++)
               {
                   p_spr = p_Gamelet.ap_Sprites[li];
                   if (p_spr.lg_SpriteActive && p_spr.i_ObjectState == MobileGameletImpl.STATE_ANIMAL_BITTEN)
                   {
                         ...
                         switch(p_spr.i_ObjectType)
                         {
                             case MobileGameletImpl.SPRITE_ANIMAL_01:  i_h = IMG_HIPPO + 1;    break;
                             case MobileGameletImpl.SPRITE_ANIMAL_02:  i_h = IMG_TURTLE + 1;   break;
                             case MobileGameletImpl.SPRITE_ANIMAL_03:  i_h = IMG_FOX + 1;      break;
                             case MobileGameletImpl.SPRITE_ANIMAL_04:  i_h = IMG_LION + 1;     break;
                             case MobileGameletImpl.SPRITE_ANIMAL_05:  i_h = IMG_SKUNK + 1;    break;
                             case MobileGameletImpl.SPRITE_ANIMAL_06:  i_h = IMG_GOAT + 1;     break;
                             case MobileGameletImpl.SPRITE_ANIMAL_07:  i_h = IMG_HEDGEHOG + 1; break;
                             case MobileGameletImpl.SPRITE_ANIMAL_08:  i_h = IMG_BEAR + 1;     break;
                             default: continue;
                         }
                         img = getImage(i_h);
               ...
               croc.drawDigits(_graphics,i_cx,i_cy,i_ndx);
          }
          ...
          {
              p_spr = p_Gamelet.ap_Sprites[li];
              if (p_spr.lg_SpriteActive && !(lg_crockyKeepAnimal && p_spr.i_ObjectState == MobileGameletImpl.STATE_ANIMAL_BITTEN))
              {
                  i_x = p_spr.i_mainX>>8;
                  i_y = p_spr.i_ScreenY+p_spr.i_height>>8;

                  i_h = (p_spr.i_ObjectState==MobileGameletImpl.STATE_ANIMAL_DRINK || p_spr.i_ObjectState==MobileGameletImpl.STATE_ANIMAL_BITTEN) ? 1 : 0;


                  switch(p_spr.i_ObjectType)
                  {
                      case MobileGameletImpl.SPRITE_HUNTER:
                      case MobileGameletImpl.SPRITE_HUNTER_STAY:
                      case MobileGameletImpl.SPRITE_HUNTER_BACK:
                           hunter.drawDigits(_graphics,i_x - (hunter.fontWidth>>1),i_y - hunter.fontHeight,p_spr.i_Frame);
                           continue;
                      case MobileGameletImpl.SPRITE_HUNTER_FIRE:
                           hunter.drawDigits(_graphics,i_x - (hunter.fontWidth>>1),i_y - hunter.fontHeight,p_spr.i_Frame);
                           i_ndx = IMG_EXPLOZ;
                           i_y += 4;
                           break;
                      case MobileGameletImpl.SPRITE_BULLET:
                          i_ndx = IMG_BULLET;
                          break;

                      case MobileGameletImpl.SPRITE_ANIMAL_01:  i_ndx = IMG_HIPPO + i_h;    break;
                      case MobileGameletImpl.SPRITE_ANIMAL_02:  i_ndx = IMG_TURTLE + i_h;   break;
                      case MobileGameletImpl.SPRITE_ANIMAL_03:  i_ndx = IMG_FOX + i_h;      break;
                      case MobileGameletImpl.SPRITE_ANIMAL_04:  i_ndx = IMG_LION + i_h;     break;
                      case MobileGameletImpl.SPRITE_ANIMAL_05:  i_ndx = IMG_SKUNK + i_h;    break;
                      case MobileGameletImpl.SPRITE_ANIMAL_06:  i_ndx = IMG_GOAT + i_h;     break;
                      case MobileGameletImpl.SPRITE_ANIMAL_07:  i_ndx = IMG_HEDGEHOG + i_h; break;
                      case MobileGameletImpl.SPRITE_ANIMAL_08:  i_ndx = IMG_BEAR + i_h;     break;
                      default: continue;
                  }
                  img = getImage(i_ndx);
                  _graphics.drawImage(img,i_x - (img.getWidth()>>1), i_y - img.getHeight(),0);
        ...
        _graphics.drawImage(getImage(IMG_GRASS),0,0,0);
...
if (lg_FinishImagePictured)
    ...
    public int processGameAction(int _arg)
    {
          ...
          switch(_arg)
          {
               case MobileGameletImpl.GAMEACTION_CROCODILE_DIVE:
                          {
                            i_action_sound = SND_PROMAH_MMF;
                          }
                          break;
               case MobileGameletImpl.GAMEACTION_CROCODILE_ATTACK:
                          {
                          }
                          break;
               case MobileGameletImpl.GAMEACTION_CROCODILE_BITE:
                          {
                            i_action_sound = SND_POIMAL_JIVOTNOE_MMF;
                          }
                          break;
               case MobileGameletImpl.GAMEACTION_CROCODILE_BACK:
                          {
                          }
                          break;
               case MobileGameletImpl.GAMEACTION_CROCODILE_DEAD:
                          {
                            i_action_sound = SND_GIBEL_KROKODILA_MMF;
                          }
                          break;
               case MobileGameletImpl.GAMEACTION_HUNTER:
                          {
                          }
                          break;
               case MobileGameletImpl.GAMEACTION_HUNTER_STAY:
                          {
                          }
                          break;
               case MobileGameletImpl.GAMEACTION_HUNTER_FIRE:
                          {
                            i_action_sound = SND_VYSTREL_OHOTNIKA_MMF;
                          }
                          break;
               case MobileGameletImpl.GAMEACTION_ANIMAL_GENERATED:
                          {
                          }
                          break;
               case MobileGameletImpl.GAMEACTION_ANIMAL_GULP:
                          {
                            i_action_sound = SND_JIVOTNOE_PIET_MMF;
                          }
                          break;
               case MobileGameletImpl.GAMEACTION_ANIMAL_DEAD:
                          {
                          }
                          break;
          }
    ...
    public Object processGameAction(int _arg1, int _arg2, Object _obj1)
    {
     ...
     drawDigits font;
     drawDigits crocky,crocBite,crocDive,crocLeft,crocRight,crocOut;
     drawDigits hunter;
     ...
     final static int unsafe = GAMESCREEN_HEIGHT + (-1 +(-2 + (-3+ (-4-4) -3) -2) -1);

     private void prepareGraphics()
     {
        font      = new drawDigits(getImage(IMG_NUMBERS),10);
        crocky    = new drawDigits(getImage(IMG_CROCODILE),3,false);
        crocBite  = new drawDigits(getImage(IMG_CROCODILE_BITE),2,false);
        crocDive  = new drawDigits(getImage(IMG_CROCODILE_GO_DOWN),3,false);
        crocLeft  = new drawDigits(getImage(IMG_CROCODILE_LEFT),3,false);
        crocRight = new drawDigits(getImage(IMG_CROCODILE_RIGHT),3,false);
        crocOut   = new drawDigits(getImage(IMG_CROCODILE_UPTURNED),2,false);

        hunter    = new drawDigits(getImage(IMG_HUNTER),2,false);

        imgScore = Image.createImage(font.fontWidth*4,font.fontHeight);
        gScore = imgScore.getGraphics();

        score_x = GAMESCREEN_WIDTH - imgScore.getWidth() - 2;
        score_y = GAMESCREEN_HEIGHT - imgScore.getHeight() - 1;

        imgCroCache = Image.createImage(GAMESCREEN_WIDTH,20);
        gCroCache = imgCroCache.getGraphics();
     }
...
SND_JIVOTNOE_PIET_MMF    ,0,
SND_PROMAH_MMF           ,1270,
SND_POIMAL_JIVOTNOE_MMF  ,370,
SND_VYSTREL_OHOTNIKA_MMF ,688,
SND_GIBEL_KROKODILA_MMF  ,995,
SND_FINAL_MMF            ,0,
SND_START_MMF            ,0
                             };
...
private static final int IMG_GRASS = 6;
private static final int IMG_GRASS128X160 = 7;
private static final int IMG_BULLET = 8;
private static final int IMG_CROCODILE = 9;
private static final int IMG_CROCODILE_BITE = 10;
private static final int IMG_CROCODILE_GO_DOWN = 11;
private static final int IMG_CROCODILE_LEFT = 12;
private static final int IMG_CROCODILE_RIGHT = 13;
private static final int IMG_CROCODILE_UPTURNED = 14;
private static final int IMG_EXPLOZ = 15;
private static final int IMG_HIPPO = 16;
private static final int IMG_HIPPO_DRINK = 17;
private static final int IMG_TURTLE = 18;
private static final int IMG_TURTLE_DRINK = 19;
private static final int IMG_FOX = 20;
private static final int IMG_FOX_DRINK = 21;
private static final int IMG_LION = 22;
private static final int IMG_LION_DRINK = 23;
private static final int IMG_SKUNK = 24;
private static final int IMG_SKUNK_DRINK = 25;
private static final int IMG_GOAT = 26;
private static final int IMG_GOAT_DRINK = 27;
private static final int IMG_HEDGEHOG = 28;
private static final int IMG_HEDGEHOG_DRINK = 29;
private static final int IMG_BEAR = 30;
private static final int IMG_BEAR_DRINK = 31;
private static final int IMG_HUNTER = 32;
private static final int IMG_NUMBERS = 33;
private static final int IMG_ICON_LIFE = 34;
private static final int TOTAL_IMAGES_NUMBER = 35;

private static final int SND_FINAL_MMF = 0;
private static final int SND_GIBEL_KROKODILA_MMF = 1;
private static final int SND_JIVOTNOE_PIET_MMF = 2;
private static final int SND_POIMAL_JIVOTNOE_MMF = 3;
private static final int SND_PROMAH_MMF = 4;
private static final int SND_START_MMF = 5;
private static final int SND_VYSTREL_OHOTNIKA_MMF = 6;

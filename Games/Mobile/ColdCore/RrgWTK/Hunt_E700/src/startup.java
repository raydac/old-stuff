                ...
                case STATE_FRONTPAGE:
                    {
                         ...
                         {
                            Thread.yield();
                            Melody theme = ap_melody[SND_MAINTHEME_MMF];
                            if(theme != null)
                            {
                ...
                case STATE_FINISHIMAGE:
                    {
                         if(lg_MainThemeSound && ap_melody!=null)
                         {
                           ...
                           {
                              theme = ap_melody[SND_LOSTTHEME_MMF];
                           }
                           else
                           {
                              theme = ap_melody[SND_WINTHEME_MMF];
                           }
                           if(theme != null)
                           {
                               i_Timer -= theme.i_MelodyLength/200;
...
p_imageForOutput = getImage(2);
    ...
    private void gamePlayDrawing(Graphics _graphics)
    {
        ...
        img = getImage(IMG_BACK07C);
        int DecorStartX=0;
        int DecorStartY=img.getHeight()-160;
        ...
        for (int li = p_Gamelet.ap_Sprites.length-1; li >= 0 ; li--)
        {
             ap_Sprites[li] = p_Gamelet.ap_Sprites[li];
        }

        if(p_Gamelet.p_currentUnderSightAnimal != null)
        {
          int li = p_Gamelet.p_currentUnderSightAnimal.i_spriteID;
          Sprite swap_sprite = ap_Sprites[0];           
          ap_Sprites[0] = ap_Sprites[li];               
          ap_Sprites[li] = swap_sprite;                 
        }




        for (int li = ap_Sprites.length-1; li >= 0 ; li--)
        {
            p_sprite = ap_Sprites[li];
            if (!p_sprite.lg_SpriteActive) continue;

            i_x = p_sprite.i_mainX >> 8;
            i_y = p_sprite.i_mainY >> 8;

            switch (p_sprite.i_ObjectType)
            {
                case MobileGameletImpl.SPRITE_PIG:         i = IMG_BOAR01+p_sprite.i_Frame; break;
                case MobileGameletImpl.SPRITE_PIGAFRAID:   i = IMG_BOAR_FEAR01+p_sprite.i_Frame; break;
                case MobileGameletImpl.SPRITE_PIGKILLED:   i = IMG_FLASH01+p_sprite.i_Frame;  break;
                    default: continue;
            }
        ...
        _graphics.drawImage(getImage(IMG_BUSH),61,108+DecorStartY,0);
        ...
        for (int li = ap_Sprites.length-1; li >= 0 ; li--)
        {
            ...
            switch (p_sprite.i_ObjectType)
            {
                case MobileGameletImpl.SPRITE_WOLF:        i = IMG_WOLF01+p_sprite.i_Frame; break;
                case MobileGameletImpl.SPRITE_WOLFAFRAID:  i = IMG_WOLF_FEAR01+p_sprite.i_Frame; break;
                case MobileGameletImpl.SPRITE_WOLFKILLED:  i = IMG_FLASH01+p_sprite.i_Frame; break;
                   default: continue;
            }
        ...
        _graphics.drawImage(getImage(IMG_BACK04B),110,58+DecorStartY,0);
        _graphics.drawImage(getImage(IMG_BACK05B),55,136+DecorStartY,0);
        _graphics.drawImage(getImage(IMG_BACK06B),0,64+DecorStartY,0);
        ...
        for (int li = ap_Sprites.length-1; li >= 0 ; li--)
        {
            ...
            switch (p_sprite.i_ObjectType)
            {
                case MobileGameletImpl.SPRITE_FOX:          i = IMG_FOX01 +p_sprite.i_Frame; break;
                case MobileGameletImpl.SPRITE_FOXAFRAID:    i = IMG_FOX_FEAR01 +p_sprite.i_Frame; break;
                case MobileGameletImpl.SPRITE_FOXKILLED:    i = IMG_FLASH01 +p_sprite.i_Frame; break;

                case MobileGameletImpl.SPRITE_RABBIT:       i = IMG_HARE01; break;
                case MobileGameletImpl.SPRITE_RABBITAFRAID: i = IMG_HARE_FEAR01 +p_sprite.i_Frame; break;
                case MobileGameletImpl.SPRITE_RABBITKILLED: i = IMG_FLASH01 +p_sprite.i_Frame; break;

                case MobileGameletImpl.SPRITE_RAVEN:        i = IMG_CROW01+p_sprite.i_Frame; break;
                case MobileGameletImpl.SPRITE_RAVENAFRAID:  i = IMG_CROW_FEAR01+p_sprite.i_Frame; break;
                case MobileGameletImpl.SPRITE_RAVENKILLED:  i = IMG_FLASH01+p_sprite.i_Frame; break;
                   default: continue;
            }
        ...
        _graphics.drawImage(getImage(IMG_BACK01A),20,132+DecorStartY,0);
        _graphics.drawImage(getImage(IMG_BACK02A),98,138+DecorStartY,0);




        p_sprite = p_Gamelet.p_gunSight;
        i_x = p_sprite.i_mainX >> 8;
        i_y = p_sprite.i_mainY >> 8;

        if (p_sprite.i_ObjectType == MobileGameletImpl.SPRITE_GUNSIGHT)
           img = getImage(IMG_SIGHT01+p_sprite.i_Frame);
        else
           img = getImage(IMG_SIGHT02+p_sprite.i_Frame);

        _graphics.drawImage(img,i_x-(img.getWidth()>>1),i_y-(img.getHeight()>>1),0);



        Sprite gun = p_Gamelet.p_gunBarrel;
        ...
        if( i_y > GAMESCREEN_HEIGHT/40 * (i_x - 17))
        {
           i = IMG_GUN_LEFT;
        }
          else
               {
                  if( i_y > GAMESCREEN_HEIGHT/(111-78) * (111 - i_x))
                  {
                        i = IMG_GUN_RIGHT;
                  }
               }
        ...
        i_x = ((GAMESCREEN_WIDTH<<1)+GAMESCREEN_WIDTH >> 3) + (p_Gamelet.p_gunSight.i_mainX >> 8+2) - (img.getWidth()>>1);
        j = (p_Gamelet.p_gunSight.i_height>>9);
        i_y =  GAMESCREEN_HEIGHT - img.getHeight()
               + (i_y - j)* (img.getHeight()-15) / (GAMESCREEN_HEIGHT - j -j);

        _graphics.drawImage(img, i_x, i_y,0);


        img = getImage(IMG_ICON);
        i = Math.max(img.getHeight(),font.fontHeight)+2;
        _graphics.drawImage(img,2,(i-img.getHeight())>>1,0);
        font.drawDigits(_graphics,3+img.getWidth(),(i-font.fontHeight)>>1,2,p_Gamelet.i_bulletNumber,10);

        font.drawDigits(_graphics,GAMESCREEN_WIDTH - (font.fontWidth<<2) - 5,2,4,p_Player.m_iPlayerGameScores,10);
...
if (lg_FinishImagePictured)
    ...
    public int processGameAction(int _arg)
    {
          ...
          switch(_arg)
          {
            case MobileGameletImpl.GAMEACTION_FOX_KILLED     :
            case MobileGameletImpl.GAMEACTION_PIG_KILLED     :
            case MobileGameletImpl.GAMEACTION_RABBIT_KILLED  :
            case MobileGameletImpl.GAMEACTION_WOLF_KILLED    :
            case MobileGameletImpl.GAMEACTION_RAVEN_KILLED   : playMelody(ap_melody[SND_HIT_MMF],false);break;

            case MobileGameletImpl.GAMEACTION_PLAYER_FIRING  : playMelody(ap_melody[SND_SHOOT_MMF],false);break;
          }
        return 0;
    }
    ...
    public Object processGameAction(int _arg1, int _arg2, Object _obj1)
    {
     ...
     drawDigits font;
     Sprite [] ap_Sprites = new Sprite[MobileGameletImpl.MAX_SPRITES];
...
private static final int IMG_ICON = 4;
private static final int IMG_NUMBERS = 5;
private static final int IMG_BUSH = 6;
private static final int IMG_BACK01A = 7;
private static final int IMG_BACK02A = 8;
private static final int IMG_BACK04B = 9;
private static final int IMG_BACK05B = 10;
private static final int IMG_BACK06B = 11;
private static final int IMG_BACK07C = 12;
private static final int IMG_BACK07C128X160 = 13;
private static final int IMG_FLASH01 = 14;
private static final int IMG_FLASH02 = 15;
private static final int IMG_FLASH03 = 16;
private static final int IMG_GUN_LEFT = 17;
private static final int IMG_GUN_CENTER = 18;
private static final int IMG_GUN_RIGHT = 19;
private static final int IMG_SIGHT01 = 20;
private static final int IMG_SIGHT02 = 21;
private static final int IMG_SIGHT03 = 22;
private static final int IMG_SIGHT04 = 23;
private static final int IMG_BOAR01 = 24;
private static final int IMG_BOAR02 = 25;
private static final int IMG_BOAR03 = 26;
private static final int IMG_BOAR_FEAR01 = 27;
private static final int IMG_BOAR_FEAR02 = 28;
private static final int IMG_CROW01 = 29;
private static final int IMG_CROW02 = 30;
private static final int IMG_CROW03 = 31;
private static final int IMG_CROW_FEAR01 = 32;
private static final int IMG_CROW_FEAR02 = 33;
private static final int IMG_FOX01 = 34;
private static final int IMG_FOX02 = 35;
private static final int IMG_FOX03 = 36;
private static final int IMG_FOX_FEAR01 = 37;
private static final int IMG_FOX_FEAR02 = 38;
private static final int IMG_HARE01 = 39;
private static final int IMG_HARE_FEAR01 = 40;
private static final int IMG_HARE_FEAR02 = 41;
private static final int IMG_WOLF01 = 42;
private static final int IMG_WOLF02 = 43;
private static final int IMG_WOLF03 = 44;
private static final int IMG_WOLF_FEAR01 = 45;
private static final int IMG_WOLF_FEAR02 = 46;
private static final int TOTAL_IMAGES_NUMBER = 47;

private static final int SND_HIT_MMF = 0;
private static final int SND_LOSTTHEME_MMF = 1;
private static final int SND_MAINTHEME_MMF = 2;
private static final int SND_SHOOT_MMF = 3;
private static final int SND_WINTHEME_MMF = 4;

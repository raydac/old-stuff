...
p_imageForOutput = getImage(2);
    ...
    private void gamePlayDrawing(Graphics _graphics)
    {
        ...
        int li,i_sx,i_sy,i_w,i_h,i_x, i_y, i_idx;
        Sprite p_spr;
        Image img;
     ...
     _graphics.drawImage(sky,0,0,0);
     _graphics.drawImage(getImage(IMG_WATER),0,sky_height,0);
     ...
     for(li=0;li<p_Gamelet.ap_Sprites.length;li++)
     {
         ...
         {
             i_x = p_spr.i_mainX>>8;
             i_y = (p_spr.i_mainY>>8) + (GAMESCREEN_HEIGHT-128);

             switch(p_spr.i_ObjectType)
             {
                 case MobileGameletImpl.SPRITE_TURTLE:         i_idx = IMG_TURTLE; break;
                 case MobileGameletImpl.SPRITE_TURTLE_TOUCHED: i_idx = IMG_TURTLE_LIFT_HEAD; break;
                 case MobileGameletImpl.SPRITE_TURTLE_DIVE:
                      {
                         if(p_spr.i_mainY < MobileGameletImpl.TURTLE_Y +0x200)
                         {
                              i_idx = IMG_TURTLE_WATER01;
                         }
                           else
                                if(p_spr.i_mainY < MobileGameletImpl.TURTLE_Y +0x500)
                                {
                                    i_idx = IMG_TURTLE_WATER02;
                                }
                                  else
                                       {
                                         i_idx = IMG_TURTLE_WATER;
                                       }
                      }
                      break;
                 case MobileGameletImpl.SPRITE_FISH:           i_idx = IMG_FISH; break;
                    default:
                            continue;
             }
             img = getImage(i_idx);
     ...
     p_spr = p_Gamelet.pHelper;
     if(p_spr.lg_SpriteActive)
     {
         i_x = p_spr.i_mainX>>8;
         i_y = ((p_spr.i_ScreenY+p_spr.i_height)>>8) + (GAMESCREEN_HEIGHT-128) -5;
         if( p_spr.i_ObjectType == MobileGameletImpl.SPRITE_LOADER)
         {
              i_idx = IMG_MAN_LEFT;
         }
           else
                {
                     i_idx = IMG_MAN_RIGHT_LOAD;
                }
     ...
     sun.drawDigits(_graphics,(GAMESCREEN_WIDTH - sun.fontWidth) >> 1,0,i_Timer>>2);


_graphics.translate(0,(GAMESCREEN_HEIGHT-128));
     ...
     if(p_spr.lg_SpriteActive)
     {
         ...
         if(p_spr.i_ObjectState == 1)
         {
            ...
            {
                default:
                case MobileGameletImpl.SPRITE_CARRIER:           i_idx = IMG_PORTER_LOAD; break;
                case MobileGameletImpl.SPRITE_CARRIER_JUMPLEFT:  i_idx = IMG_PORTER_JUMP_LEFT_LOAD; break;
                case MobileGameletImpl.SPRITE_CARRIER_JUMPRIGHT: i_idx = IMG_PORTER_JUMP_RIGHT_LOAD; break;
                case MobileGameletImpl.SPRITE_CARRIER_LOAD:      i_idx = IMG_PORTER_LEFT_LOAD; break;
                case MobileGameletImpl.SPRITE_CARRIER_UNLOAD:    i_idx = IMG_PORTER_RIGHT; break;
                case MobileGameletImpl.SPRITE_CARRIER_DEAD:
                      {
                          drawnman.drawDigits(_graphics,i_x - (drawnman.fontWidth>>1), sky_height - (GAMESCREEN_HEIGHT-128 +3) - (drawnman.fontHeight>>1), p_spr.i_Frame);

                          i_idx = IMG_LOAD_WATER;
                          i_y += 10;
                          i_x += (i_x <= (GAMESCREEN_WIDTH>>1)) ? 10 : -10;
                      }
                      break;
            }
         }
           else
                {
                    ...
                    {
                        default:
                        case MobileGameletImpl.SPRITE_CARRIER:           i_idx = IMG_PORTER; break;
                        case MobileGameletImpl.SPRITE_CARRIER_JUMPLEFT:  i_idx = IMG_PORTER_JUMP_LEFT; break;
                        case MobileGameletImpl.SPRITE_CARRIER_JUMPRIGHT: i_idx = IMG_PORTER_JUMP_RIGHT; break;
                        case MobileGameletImpl.SPRITE_CARRIER_LOAD:      i_idx = IMG_PORTER_LEFT_LOAD; break;
                        case MobileGameletImpl.SPRITE_CARRIER_UNLOAD:    i_idx = IMG_PORTER_RIGHT; break;
                        case MobileGameletImpl.SPRITE_CARRIER_DEAD:
                               {
...
if (lg_FinishImagePictured)
    ...
    public int processGameAction(int _arg)
    {
          ...
          switch(_arg)
          {
               case MobileGameletImpl.GAMEACTION_JUMP:
                          {
                          }
                          break;
               case MobileGameletImpl.GAMEACTION_JUMPED:
                          {
                             i_action_sound = SND_PLAYER_NA_CHEREPAHU_MMF;
                          }
                          break;
               case MobileGameletImpl.GAMEACTION_LOAD:
                          {
                            i_action_sound = SND_PLAYER_BERET_BOX_MMF;
                          }
                          break;
               case MobileGameletImpl.GAMEACTION_UNLOAD:
                          {
                             i_action_sound = SND_PLAYER_OTDAET_BOX_MMF;
                          }
                          break;
               case MobileGameletImpl.GAMEACTION_DEAD:
                          {
                             i_action_sound = SND_PLAYER_PADAET_V_VODU_MMF;
                          }
                          break;
               case MobileGameletImpl.GAMEACTION_TURTLE_TOUCH:
                          {
                          }
                          break;
               case MobileGameletImpl.GAMEACTION_TURTLE_ON_THE_WATER:
                          {
                          }
                          break;
               case MobileGameletImpl.GAMEACTION_EAT_FISH:
                          {
                          }
                          break;
               case MobileGameletImpl.GAMEACTION_FISH_GENERATED:
                          {
                          }
                          break;
          }
    ...
    public Object processGameAction(int _arg1, int _arg2, Object _obj1)
    {
     ...
     private void prepareGraphics()
     {
        ...
        sun      = new drawDigits(getImage(IMG_SUN),2,false);
        drawnman = new drawDigits(getImage(IMG_PORTER_SINK),2,false);

        sky_height = GAMESCREEN_HEIGHT - getImage(IMG_WATER).getHeight();
        sky = Image.createImage(GAMESCREEN_WIDTH, sky_height);

        Graphics gSky = sky.getGraphics();

        fillGradientImage(gSky, 0,0, GAMESCREEN_WIDTH,sky_height, 0x00EEFF, 0xF9FAA5,2);

        Image img = getImage(IMG_PALM_LEFT);
        gSky.drawImage(img,0,sky_height-img.getHeight(),0);

        img = getImage(IMG_PALM_RIGHT);
        gSky.drawImage(img,GAMESCREEN_WIDTH-img.getWidth(),sky_height-img.getHeight(),0);


        imgScore = Image.createImage(font.fontWidth*4,font.fontHeight);
        gScore = imgScore.getGraphics();
        score_x = GAMESCREEN_WIDTH - 2 - imgScore.getWidth();
        score_y = GAMESCREEN_HEIGHT - 2 - imgScore.getHeight();
     }




    private void fillGradientImage(Graphics g,int x, int y, int width,int height,int startc,int endc, int step)
    {
       ...
       int i;
        ...
        for (i = 0; i+step<height; i+=step){
   g.setColor(rs+rd*i/height,
              gs+gd*i/height,
       bs+bd*i/height);
   g.fillRect(0,i,width,step);
   ...
   g.fillRect(0,i,width,step);
    ...
    {
                             int [] sort_idx = {
SND_PLAYER_NA_CHEREPAHU_MMF  ,201,
SND_PLAYER_BERET_BOX_MMF     ,371,
SND_PLAYER_OTDAET_BOX_MMF    ,624,
SND_PLAYER_PADAET_V_VODU_MMF ,1067,
SND_FINAL_MMF                ,0,
SND_START_MMF                ,0
                             };
...
private static final int IMG_FISH = 4;
private static final int IMG_LIFE = 5;
private static final int IMG_LOAD_WATER = 6;
private static final int IMG_MAN_LEFT = 7;
private static final int IMG_MAN_RIGHT_LOAD = 8;
private static final int IMG_PALM_LEFT = 9;
private static final int IMG_PALM_RIGHT = 10;
private static final int IMG_PORTER = 11;
private static final int IMG_PORTER_JUMP_LEFT = 12;
private static final int IMG_PORTER_JUMP_LEFT_LOAD = 13;
private static final int IMG_PORTER_JUMP_RIGHT = 14;
private static final int IMG_PORTER_JUMP_RIGHT_LOAD = 15;
private static final int IMG_PORTER_LEFT_LOAD = 16;
private static final int IMG_PORTER_LOAD = 17;
private static final int IMG_PORTER_RIGHT = 18;
private static final int IMG_PORTER_SINK = 19;
private static final int IMG_SCORE = 20;
private static final int IMG_SUN = 21;
private static final int IMG_TURTLE = 22;
private static final int IMG_TURTLE_LIFT_HEAD = 23;
private static final int IMG_TURTLE_WATER = 24;
private static final int IMG_TURTLE_WATER01 = 25;
private static final int IMG_TURTLE_WATER02 = 26;
private static final int IMG_WATER = 27;
private static final int TOTAL_IMAGES_NUMBER = 28;

private static final int SND_FINAL_MMF = 0;
private static final int SND_PLAYER_BERET_BOX_MMF = 1;
private static final int SND_PLAYER_NA_CHEREPAHU_MMF = 2;
private static final int SND_PLAYER_OTDAET_BOX_MMF = 3;
private static final int SND_PLAYER_PADAET_V_VODU_MMF = 4;

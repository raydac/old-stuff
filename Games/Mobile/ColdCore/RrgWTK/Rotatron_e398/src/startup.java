...
public class startup extends MIDlet implements Runnable, CommandListener
{
    ...
    //#if MODEL=="C380"
    //$protected static final int SCALE_WIDTH = 0xBA;
    //$protected static final int SCALE_HEIGHT = 0x9E;
    //#endif
    //#if MODEL=="E398"
    ...
    public static final int processGameAction(int _arg)
    {
        ...
        if (lg_Option_Sound)
        {
            switch (_arg)
            {
                case Gamelet.GAMEACTION_PLACEBRICKONFIELD :
                {
                    SoundManager.playSound(SoundManager.SOUND_IN,1);
                }
                ;
                break;
                case Gamelet.GAMEACTION_REMOVEBRICKFROMFIELD :
                {
                    SoundManager.playSound(SoundManager.SOUND_OUT,1);
                }
                ;
                break;
        }
    ...
    private static final void drawScores(Graphics _g, int _x, int _y, int _zeroNumbers, int _value)
    {
        ...
        Image p_image = ap_images[MCFFormRender.MCF_IMG_digit0];
        ...
        for (int li = 0; li < i_len; li++)
        {
            ...
            _g.drawImage(ap_images[MCFFormRender.MCF_IMG_digit0 + i_char], _x, _y, 0);
            _x += NUMBER_CHAR_WIDTH;
        }
    }
    ...
    {
        // Восстанавливаем вид поля
        int i_len = Gamelet.ab_GameField.length;
    ...
    protected static final void onLoadingGameResources(Class _this) throws Exception //ONEVENT onLoadingGameResources
    {
        ...
        int i_ofst = MCFFormRender.MCF_CMP_GAMEAREA_GAME_AREA_OFFSET + 1;
        i_offsetGameFieldX = p_GameForm.ab_formsData[i_ofst++] & 0xFF;
        i_offsetGameFieldY = p_GameForm.ab_formsData[i_ofst] & 0xFF;

        Image p_background = p_GameForm.ap_Images[MCFFormRender.MCF_IMG_background];
        p_GameFieldBuffer = Image.createImage(p_background.getWidth(), p_background.getHeight());
        p_GameFieldBufferGraphics = p_GameFieldBuffer.getGraphics();
        //#if SOUND
    ...
    private static final boolean onInitNewGame(Class _this, int _selectedGameLevel) //ONEVENT onInitNewGame
    {
        ...
        p_GameFieldBuffer.getGraphics().drawImage(p_GameForm.ap_Images[MCFFormRender.MCF_IMG_background], 0, 0, 0);
    ...
    private static final void onGameStep() //ONEVENT onGameStep
    {
        ...
        // увеличиваем кадр курсора
        if (i_CursorDelay <= 0)
        {
            i_CursorDelay = CURSOR_DELAY;
            i_CursorFrame++;
            if (i_CursorFrame >= CURSOR_MAXFRAMES) i_CursorFrame = 0;
        }
        else
            i_CursorDelay--;
    }
            ...
            case MCFFormRender.MCF_FORM_GAMEAREA :
            {
                switch (_channel)
                {
                    case 0:
                    {
                        ...
                        _g.translate(_x, _y);
                        // Курсор игрока
                        switch (Gamelet.i_CurrentSubMode)
                        {
                            case Gamelet.SUBMODE_GENERATEBRICK:
                            {
                                drawBrickSprites(_g);
                            }
                            ;
                            break;
                            case Gamelet.SUBMODE_BRICKMOVING:
                            {
                                drawBrickSprites(_g);
                            }
                            ;
                            break;
                            case Gamelet.SUBMODE_BRICKROTATING:
                            {
                                drawBrickSprites(_g);
                            }
                            ;
                            break;
                            case Gamelet.SUBMODE_DESTROYINGBRICK:
                            {
                                drawBrickSprites(_g);
                                drawPartsSprites(_g);
                            }
                            ;
                            break;
                            case Gamelet.SUBMODE_BRICKSELECTED:
                            {
                                drawBrickSprites(_g);
                                drawPlayerCursor(_g);
                            }
                            ;
                            break;
                            case Gamelet.SUBMODE_WAITPLAYERMOVE:
                            {
                                drawPlayerCursor(_g);
                            }
                            ;
                    ...
                    {
                        // очки
                        int i_scores = Gamelet.getPlayerScore();
                        if (i_scores > 9999) i_scores = 9999;
                        drawScores(_g, _x, _y, 4, i_scores);
                    }
                    ;
                    break;
                    case 2:
                    {
                        // иконка доступности вращения
                        if (Gamelet.i_CurrentSubMode == Gamelet.SUBMODE_BRICKSELECTED)
                        {
                            if (Gamelet.i_SelectedBrickCoordX == Gamelet.i_PlayerCursorX && Gamelet.i_SelectedBrickCoordY == Gamelet.i_PlayerCursorY)
                            {
                                // отрисовываем
                                if ((i_CurrentModeTickCounter & 3) > 1)
                                {
                                    _g.drawImage(p_GameForm.ap_Images[MCFFormRender.MCF_IMG_RotateIco], _x, _y, 0);
                                }
                            }
                        }
                    }
    ...
    {
        _g.drawImage(p_GameFieldBuffer, GAMESCREEN_OFFSETX, GAMESCREEN_OFFSETY, 0);
    ...
    private final static void onKeyPressed(int _keyCode) //ONEVENT onKeyPressed
    {
        switch (_keyCode)
        {
            ...
            case KEY_CODE_SOFT_RIGHT:
            {
                i_KeyFlags = Gamelet.BUTTON_RIGHTROTATE;
            }
            ;
            break;
            case KEY_CODE_UP:
    ...
    private static int i_offsetGameFieldX;
    private static int i_offsetGameFieldY;

    private static int i_CursorFrame;
    private static int i_CursorDelay;
    private static final int CURSOR_DELAY = 2;
    private static final int CURSOR_MAXFRAMES = 4;

    private static Image p_GameFieldBuffer;
    private static Graphics p_GameFieldBufferGraphics;

    public static void updateCell(int _offset)
    {
        Graphics p_gr = p_GameFieldBufferGraphics;
        Image [] ap_Images = p_GameForm.ap_Images;

        int i_x = _offset % Gamelet.GAMEFIELDWIDTH;
        int i_y = _offset / Gamelet.GAMEFIELDWIDTH;

        i_x = ((Gamelet.getCellX8(i_x) + 0x7F) >> 8) + i_offsetGameFieldX;
        i_y = ((Gamelet.getCellY8(i_y) + 0x7F) >> 8) + i_offsetGameFieldY;

        int i_w = (Gamelet.CELLWIDTH + 0x7F) >> 8;
        int i_h = (Gamelet.CELLHEIGHT + 0x7F) >> 8;

        // Стираем в ячейке информацию
        p_gr.setClip(i_x, i_y, i_w, i_h);
        p_gr.drawImage(ap_Images[MCFFormRender.MCF_IMG_background], 0, 0, 0);
        p_gr.setClip(0, 0, p_GameFieldBuffer.getWidth(), p_GameFieldBuffer.getHeight());

        int i_brick = Gamelet.ab_GameField[_offset];
        if ((i_brick & Gamelet.BRICK_PROCESSFLAG) != 0)
        {
            return;
        }
        int i_sprite = 0;
        switch (i_brick)
        {
            case Gamelet.BRICK_BLUE:
            {
                i_sprite = MCFFormRender.MCF_IMG_Cube_bue;
            }
            ;
            break;
            case Gamelet.BRICK_RED:
            {
                i_sprite = MCFFormRender.MCF_IMG_Cube_Red;
            }
            ;
            break;
            case Gamelet.BRICK_GREEN:
            {
                i_sprite = MCFFormRender.MCF_IMG_Cube_green;
            }
            ;
            break;
            case Gamelet.BRICK_YELLOW:
            {
                i_sprite = MCFFormRender.MCF_IMG_Cube_yellow;
            }
            ;
            break;
            default :
        ...
        p_gr.drawImage(ap_Images[i_sprite], i_x, i_y, 0);
    }

    private static void drawBrickSprites(Graphics _g)
    {
        SpriteCollection p_col = Gamelet.p_SpriteBricksCollection;
        p_col.initIterator();
        Image [] ap_images = p_GameForm.ap_Images;
        while (true)
        {
            ...
            long l_xy = p_col.getSpriteScreenXY(i_sprOffset);
            int i_x = (int) (l_xy >>> 32);
            int i_y = (int) l_xy;
            ...
            switch (i_State)
            {
                case Gamelet.SPRITE_BRICKBLUE_INCOMING :
                {
                    i_typeObject = MCFFormRender.MCF_IMG_appear_01;
                }
                ;
                break;
                case Gamelet.SPRITE_BRICKBLUE_EXPLODING :
                {
                    i_typeObject = MCFFormRender.MCF_IMG_Destroy_1;
                }
                ;
                break;
                case Gamelet.SPRITE_BRICKBLUE_ROTATING :
                {
                    switch (i_Type)
                    {
                        case Gamelet.SPRITE_BRICKBLUE:
                        {
                            i_typeObject = MCFFormRender.MCF_IMG_Cube_B2G;
                        }
                        ;
                        break;
                        case Gamelet.SPRITE_BRICKGREEN:
                        {
                            i_typeObject = MCFFormRender.MCF_IMG_Cube_G2R;
                        }
                        ;
                        break;
                        case Gamelet.SPRITE_BRICKYELLOW:
                        {
                            i_typeObject = MCFFormRender.MCF_IMG_Cube_Y2B;
                        }
                        ;
                        break;
                        case Gamelet.SPRITE_BRICKRED:
                        {
                            i_typeObject = MCFFormRender.MCF_IMG_Cube_R2Y;
                        }
                        ;
                        break;
                    }
                ...
                ;
                break;
                case Gamelet.SPRITE_BRICKBLUE_MOVING :
                case Gamelet.SPRITE_BRICKBLUE_SELECTED :
                {
                    switch (i_Type)
                    {
                        case Gamelet.SPRITE_BRICKBLUE:
                        {
                            i_typeObject = MCFFormRender.MCF_IMG_CubeB_pulse1;
                        }
                        ;
                        break;
                        case Gamelet.SPRITE_BRICKGREEN:
                        {
                            i_typeObject = MCFFormRender.MCF_IMG_CubeG_pulse1;
                        }
                        ;
                        break;
                        case Gamelet.SPRITE_BRICKYELLOW:
                        {
                            i_typeObject = MCFFormRender.MCF_IMG_CubeY_pulse1;
                        }
                        ;
                        break;
                        case Gamelet.SPRITE_BRICKRED:
                        {
                            i_typeObject = MCFFormRender.MCF_IMG_CubeR_pulse1;
                        }
                        ;
                        break;
                    }
            ...
            _g.drawImage(ap_images[i_typeObject + i_frame], i_x, i_y, 0);
        }
    }

    public static void drawPlayerCursor(Graphics _g)
    {
        Image [] ap_images = p_GameForm.ap_Images;

        Image p_imgCur = ap_images[MCFFormRender.MCF_IMG_Cross_1];
        int i_w = p_imgCur.getWidth();
        int i_h = p_imgCur.getHeight();

        int i_x = ((((Gamelet.getCellX8(Gamelet.i_PlayerCursorX) + 0x7F) & 0xFFFFFF00) + (Gamelet.CELLWIDTH >> 1) + 0x7F) >> 8) - (i_w >> 1);
        int i_y = ((((Gamelet.getCellY8(Gamelet.i_PlayerCursorY) + 0x7F) & 0xFFFFFF00) + (Gamelet.CELLHEIGHT >> 1) + 0x7F) >> 8) - (i_h >> 1);

        switch (Gamelet.i_CurrentSubMode)
        {
            case Gamelet.SUBMODE_WAITPLAYERMOVE :
            {
                _g.drawImage(ap_images[MCFFormRender.MCF_IMG_Cross_1], i_x, i_y, 0);
                i_CursorFrame = 0;
            }
            ;
            break;
            case Gamelet.SUBMODE_BRICKSELECTED :
            {
                _g.drawImage(ap_images[MCFFormRender.MCF_IMG_Cross_1 + i_CursorFrame], i_x, i_y, 0);
            }
            ;
            break;
        }
    ...
    private static void drawPartsSprites(Graphics _g)
    {
        SpriteCollection p_col = Gamelet.p_SpritePartsCollection;
        p_col.initIterator();
        Image [] ap_images = p_GameForm.ap_Images;
        while (true)
        {
            ...
            switch (i_State)
            {
                case Gamelet.SPRITE_PART_MOVING :
                {
                    _g.drawImage(ap_images[MCFFormRender.MCF_IMG_IceStar1 + i_frame], i_x, i_y, 0);
                }
                ;
                break;
            }

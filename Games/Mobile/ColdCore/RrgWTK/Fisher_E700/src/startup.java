                ...
                case STATE_FRONTPAGE:
                    {
                         ...
                         {
                            Thread.yield();
                            Melody theme = ap_melody[SND_MAIN_FHR_MMF];
                            if(theme != null)
                            {
                ...
                case STATE_FINISHIMAGE:
                    {
                         if(lg_MainThemeSound && ap_melody!=null)
                         {
                           ...
                           {
                              theme = ap_melody[SND_WIN_FHR_MMF];
                           }
                           else
                           {
                              theme = ap_melody[SND_WIN_FHR_MMF];
...
p_imageForOutput = getImage(2);
    ...
    private void gamePlayDrawing(Graphics _graphics)
    {
        ...
        _graphics.drawImage(imgCoverTop,0,0,0);
        gBottomBuffer.drawImage(imgCoverBottom,0,0,0);

        img = getImage(IMG_WAVE);
        li = Math.abs(img.getHeight() - ((i_Timer >>1) % (img.getHeight()<<1)));
        i_y =  water_y - img.getHeight() +li;
        i_sy = water_y - li;
        ...
        while(i_x < GAMESCREEN_WIDTH){_graphics.drawImage(img,i_x,i_y,0); i_x+=128;}
        while(i_sx < GAMESCREEN_WIDTH){_graphics.drawImage(img,i_sx,i_sy,0); i_sx+=128;}
        ...
        for (li = 0; li < p_Gamelet.ap_Sprites.length; li++)
        {
            ...
            i_y = (p_spr.i_ScreenY >> 8) - water_y +intermodel_dy;

            switch (p_spr.i_ObjectType)
            {
                case MobileGameletImpl.SPRITE_ENEMY_FISH_1_ATTACK:
                               predator1.drawDigits(gBottomBuffer,i_x,i_y,1,2,3);
                               break;
                case MobileGameletImpl.SPRITE_ENEMY_FISH_2_ATTACK:
                               predator2.drawDigits(gBottomBuffer,i_x,i_y,1,2,3);
                               break;
                case MobileGameletImpl.SPRITE_ENEMY_FISH_1:
                               predator1.drawDigits(gBottomBuffer,i_x,i_y,1,p_spr.i_Frame,3);
                               break;
                case MobileGameletImpl.SPRITE_ENEMY_FISH_2:
                               predator2.drawDigits(gBottomBuffer,i_x,i_y,1,p_spr.i_Frame,3);
                               break;
                default:
                      fish[p_spr.i_ObjectType - MobileGameletImpl.SPRITE_FISH_1].drawDigits(gBottomBuffer,i_x,i_y,1,p_spr.i_Frame,2);
                     break;
            }
        }


        gBottomBuffer.drawImage(getImage(IMG_ICON_FISH),fish_icon_x,fish_icon_y,0);
        fish_font.drawDigits(gBottomBuffer,fish_font_x,fish_font_y,1,10,11);
        fish_font.drawDigits(gBottomBuffer,fish_font_x + fish_font.fontWidth,fish_font_y,3,p_Gamelet.m_nFishCounter,10);


        p_spr = p_Gamelet.pPlayerHook;
        i_x = p_spr.i_mainX >> 8;
        i_y = (p_spr.i_ScreenY >> 8) +intermodel_dy;
        switch(p_spr.i_ObjectType)
        {
             case MobileGameletImpl.SPRITE_HOOK_CATCH_1: li = IMG_FISH05_HOOK; i_x -= 2; break;
             case MobileGameletImpl.SPRITE_HOOK_CATCH_2: li = IMG_FISH03_HOOK; i_x -= 2; break;
             case MobileGameletImpl.SPRITE_HOOK_CATCH_3: li = IMG_FISH06_HOOK; i_x -= 2; break;
             case MobileGameletImpl.SPRITE_HOOK_CATCH_4: li = IMG_FISH02_HOOK; i_x -= 2; break;
             case MobileGameletImpl.SPRITE_HOOK_CATCH_5: li = IMG_FISH04_HOOK; i_x -= 2; break;
             case MobileGameletImpl.SPRITE_HOOK_CATCH_6: li = IMG_FISH01_HOOK; i_x -= 2; break;
               default:
                     li = IMG_HOOK;
                     i_x -= 2;
        }
        ...
        if(i_y < water_y)
        {
           _graphics.drawImage(imgBottomBuffer,0,imgCoverTop.getHeight(),0);
           _graphics.drawImage(img,i_x,i_y,0);
        }
          else
               {
                   gBottomBuffer.drawImage(img,i_x,i_y-water_y,0);
                   _graphics.drawImage(imgBottomBuffer,0,imgCoverTop.getHeight(),0);
               }


        i_sx = p_Gamelet.m_FishrodX0 >> 8;
        i_sy = (p_Gamelet.m_FishrodY0 >> 8) +intermodel_dy;
        i_x = p_Gamelet.m_FishrodX1 >> 8;
        i_y = (p_Gamelet.m_FishrodY1 >> 8)  +intermodel_dy;
        li = (p_Gamelet.pPlayerHook.i_ScreenY >> 8) +intermodel_dy;

        _graphics.setColor(0x0);
        _graphics.drawLine(i_sx, i_sy, i_x, i_y);
        _graphics.drawLine(i_sx, i_sy-1, i_x-1, i_y);

        if(p_Gamelet.pPlayerHook.i_ObjectType != MobileGameletImpl.SPRITE_HOOK_FALL)
        {
            _graphics.setColor(0xffffff);
            _graphics.drawLine(i_x, i_y, i_x, li);
        }
        ...
        i_x = SCREENWIDTH - (score_font.fontWidth<<2) - 4;
...
if (lg_FinishImagePictured)
    ...
    public int processGameAction(int _arg)
    {
       int i_action_sound = -1;
          ...
          switch(_arg)
          {
               ...
               case MobileGameletImpl.GAMEACTION_TRY_CATCH_FISH   :
                          {
                          }
                          break;
               case MobileGameletImpl.GAMEACTION_CATCH_FISH       :
                          {
                          }
                          break;
               case MobileGameletImpl.GAMEACTION_CAUGHT_FISH      :
                          {
                            i_action_sound = SND_POIMANA_RYBA_MMF;
                          }
                          break;
               case MobileGameletImpl.GAMEACTION_ENEMY_FISH       :
                          {
                          }
                          break;
               case MobileGameletImpl.GAMEACTION_ENEMY_FISH_ATTACK:
                          {
                            i_action_sound = SND_OTKUSAN_KRYUCHOK_MMF;
                          }
                          break;
               case MobileGameletImpl.GAMEACTION_HOOK_ON_THE_FLOOR:
                          {
                            i_action_sound = SND_POTERYA_KRYUCHKA_MMF;
                          }
                          break;
          }
    ...
    public Object processGameAction(int _arg1, int _arg2, Object _obj1)
    {
     ...
     Image imgCoverTop, imgCoverBottom, imgBottomBuffer;
     Graphics gBottomBuffer;
     drawDigits score_font, fish_font;
     drawDigits []fish = new drawDigits[6];
     drawDigits predator1,predator2;
     int fish_font_x, fish_font_y, fish_icon_x, fish_icon_y;
     int water_y;
     final static int intermodel_dy = 160-128;
     ...
     private void prepareGraphics()
     {
          ...
          fish_font              = new drawDigits(getImage(IMG_SCORE01),11);
          int [] fishes = new int [] {
                       IMG_FISH05, IMG_FISH03, IMG_FISH06,
                       IMG_FISH02, IMG_FISH04, IMG_FISH01
          };
          for(int i = 0; i < 6; i++)
          {
              fish[i] = new drawDigits(getImage(fishes[i]),2,false);
          }
          predator1 = new drawDigits(getImage(IMG_PREDATOR02),3,false);
          predator2 = new drawDigits(getImage(IMG_PREDATOR01),3,false);

          Image img = getImage(IMG_FISHER_CLOUDS);
          ib_ImageArray[IMG_FISHER_CLOUDS] = null;
          water_y = img.getHeight()+intermodel_dy -6;

           imgCoverTop = CreateGradientImage(128,water_y,0x12b4ff,0xa5e5e5);
           Graphics _graphics = imgCoverTop.getGraphics();
           _graphics.drawImage(img,0,intermodel_dy,0);
           _graphics.setColor(0x0b59a6); _graphics.fillRect(0,water_y-1,128,1);


           imgCoverBottom = CreateGradientImage(128,GAMESCREEN_HEIGHT - water_y,0x1091e0,0x606bcc);

          imgBottomBuffer = Image.createImage(imgCoverBottom.getWidth(),imgCoverBottom.getHeight());
          gBottomBuffer = imgBottomBuffer.getGraphics();

          img = getImage(IMG_ICON_FISH);
          int height = Math.max(img.getHeight(),fish_font.fontHeight);
          fish_icon_x = 2;
          fish_icon_y = imgBottomBuffer.getHeight() - height -2;

          fish_font_x = fish_icon_x+img.getWidth()+3;
          fish_font_y = fish_icon_y + ((height - fish_font.fontHeight)>>1);

          fish_icon_y += (height - img.getHeight())>>1;
     }
   ...
   g.fillRect(0,i,x,1);
    ...
    {
                             int [] sort_idx = {
SND_POTERYA_KRYUCHKA_MMF ,219,
SND_OTKUSAN_KRYUCHOK_MMF ,894,
SND_POIMANA_RYBA_MMF     ,171
                             };
...
private static final int IMG_WIN_LOST128X160 = 2;
private static final int IMG_DEMO = 3;
private static final int IMG_FISHER_CLOUDS = 4;
private static final int IMG_HOOK = 5;
private static final int IMG_WAVE = 6;
private static final int IMG_ICON_FISH = 7;
private static final int IMG_ICON_LIFE = 8;
private static final int IMG_SCORE = 9;
private static final int IMG_SCORE01 = 10;
private static final int IMG_FISH01 = 11;
private static final int IMG_FISH02 = 12;
private static final int IMG_FISH03 = 13;
private static final int IMG_FISH04 = 14;
private static final int IMG_FISH05 = 15;
private static final int IMG_FISH06 = 16;
private static final int IMG_FISH01_HOOK = 17;
private static final int IMG_FISH02_HOOK = 18;
private static final int IMG_FISH03_HOOK = 19;
private static final int IMG_FISH04_HOOK = 20;
private static final int IMG_FISH05_HOOK = 21;
private static final int IMG_FISH06_HOOK = 22;
private static final int IMG_PREDATOR01 = 23;
private static final int IMG_PREDATOR02 = 24;
private static final int TOTAL_IMAGES_NUMBER = 25;

private static final int SND_MAIN_FHR_MMF = 0;
private static final int SND_OTKUSAN_KRYUCHOK_MMF = 1;
private static final int SND_POIMANA_RYBA_MMF = 2;
private static final int SND_POTERYA_KRYUCHKA_MMF = 3;
private static final int SND_WIN_FHR_MMF = 4;

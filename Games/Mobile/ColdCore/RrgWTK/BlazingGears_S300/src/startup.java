...
p_imageForOutput = getImage(2);
    ...
    private void gamePlayDrawing(Graphics _graphics)
    {
        ...
        int li,i_sx,i_sy,i_w,i_h,i_x, i_y, i_ndx, exMark;
        Sprite p_spr;
        Image img;
        ...
        updateRoadCache();
        li =  - (p_Gamelet.i_CameraL>>8);
        _graphics.drawImage(imgRoadCache,li,i_local_px_offset,0);
        _graphics.drawImage(imgRoadCache,li,i_local_px_offset + RC_HEIGHT,0);
...

{
        ...
        for(li=0;li<ap_sprites.length;li++)
        {
            ...
            {
                i_x = (p_spr.i_ScreenX - p_Gamelet.i_CameraL >> 8) +1;
                i_y = p_spr.i_ScreenY>>8;

                switch(p_spr.i_ObjectType)
                {
                    default:
                    case MobileGameletImpl.SPRITE_CAR_1 : i_ndx = IMG_CAR01; break;
                    case MobileGameletImpl.SPRITE_CAR_2 : i_ndx = IMG_CAR02; break;
                    case MobileGameletImpl.SPRITE_CAR_3 : i_ndx = IMG_CAR03; break;
                    case MobileGameletImpl.SPRITE_EXPLOSION : sort_buffer[exMark++] = p_spr; continue;
                    case MobileGameletImpl.SPRITE_STONE_SMALL :  i_ndx = IMG_STONE01; break;
                    case MobileGameletImpl.SPRITE_STONE_MEDIUM : i_ndx = IMG_STONE03; break;
                    case MobileGameletImpl.SPRITE_STONE_LARGE :  i_ndx = IMG_STONE02; break;
                }

                _graphics.drawImage(getImage(i_ndx),i_x,i_y,0);
            }
        }

        p_spr = p_Gamelet.pPlayerSprite;

        if(p_Gamelet.getGameWorldState() != MobileGamelet.GAMEWORLDSTATE_PLAYERLOST || (i_Timer&2) == 0)
        {
            i_x = (p_spr.i_mainX - p_Gamelet.i_CameraL >> 8) +1;
            i_y = p_spr.i_mainY>>8;
            img = getImage(IMG_CAR_PLAYER);
            _graphics.drawImage(img,i_x - (img.getWidth()>>1),i_y - (img.getHeight()>>1),0);
        }


        for(li=0;li<exMark;li++)
        {
            ...
            if(p_spr.i_Frame < 5)
            {
                i_x = p_spr.i_mainX - p_Gamelet.i_CameraL >> 8;
                i_y = p_spr.i_mainY>>8;
                img = getImage(IMG_EXPLOZ01 + p_spr.i_Frame);
                _graphics.drawImage(img,i_x - (img.getWidth()>>1),i_y - (img.getHeight()>>1),0);
            }
        }
     ...
     if(last_score != p_Player.m_iPlayerGameScores)
     {
        ...
        gScore.drawImage(imgScoreBoard,0,0,0);
        font.drawDigits(gScore,score_x,score_y,4,last_score,10);
     }


     li = p_Gamelet.pPlayerSprite.i_Speed * 10 >> 8;

     if(last_speed != li)
     {
        last_speed = li;
        gSpeed.drawImage(imgScoreBoard,0,0,0);
        img = getImage(IMG_INDICATOR);
        gSpeed.drawImage(img,speed_x,speed_y,0);
        font.drawDigits(gSpeed,img.getWidth()+speed_x+1,speed_y,3,last_speed,10);
     }

     if(last_timer != (p_Gamelet.i_RaceTimer >> 3) || last_timer == 0)
     {
        last_timer = p_Gamelet.i_RaceTimer >> 3;
        if(last_timer == 0 && (i_Timer & 4) != 0)
        {
           last_timer = -1;
        }
        gTimer.drawImage(imgScoreBoard,0,0,0);
        img = getImage(IMG_CLOCK);
        gTimer.drawImage(img,timer_x,timer_y,0);
        font.drawDigits(gTimer,img.getWidth()+timer_x+1,timer_y+1,2,last_timer,10);
     }
     ...
     if(last_lives != li)
     {
        ...
        gLives.drawImage(imgScoreBoard,0,0,0);
        i_x = lives_x;

        for(; li > 0 ;li--)
        {
            gLives.drawImage(img, i_x, lives_y, 0);
            i_x += img.getWidth()+1;
        }
     }

     _graphics.drawImage(imgLives,0,0,0);
     _graphics.drawImage(imgScore,GAMESCREEN_WIDTH-sb_width,0,0);
     _graphics.drawImage(imgTimer,0,GAMESCREEN_HEIGHT-sb_height,0);
     _graphics.drawImage(imgSpeed,GAMESCREEN_WIDTH-sb_width,GAMESCREEN_HEIGHT-sb_height,0);
...
if (lg_FinishImagePictured)
    ...
    public int processGameAction(int _arg)
    {
          ...
          switch(_arg)
          {
               ...
               case MobileGameletImpl.GAMEACTION_PLAYER_CAR_SPEED_UP:
                          {
                            if(p_Gamelet.pPlayerSprite.i_Speed == MobileGameletImpl.INC_VSPEED)
                            i_action_sound = SND_START_DVIJENIA_MMF;
                          }
                          break;
               case MobileGameletImpl.GAMEACTION_PLAYER_CAR_HIT:
                          {
                          }
                          break;
               case MobileGameletImpl.GAMEACTION_PLAYER_CAR_STOP:
                          {
                          }
                          break;
               case MobileGameletImpl.GAMEACTION_PLAYER_CAR_BRAKE:
                          {
                            if(p_Gamelet.pPlayerSprite.i_Speed >= MobileGameletImpl.MAX_VSPEED - MobileGameletImpl.DEC_VSPEED)
                            {
                               i_action_sound = SND_POVOROT_MMF;
                            }
                          }
                          break;
               case MobileGameletImpl.GAMEACTION_PLAYER_CAR_READY:
                          {
                          }
                          break;
               case MobileGameletImpl.GAMEACTION_CAR_BREAK:
                          {
                            i_action_sound = SND_POVOROT_MMF;
                          }
                          break;
               case MobileGameletImpl.GAMEACTION_CAR_COLLISION:
                          {
                          }
                          break;
               case MobileGameletImpl.GAMEACTION_EXPLOSION:
                          {
                            i_action_sound = SND_STOLKNOVENIE_MMF;
                          }
                          break;

               case MobileGameletImpl.GAMEACTION_TIME_OUT:
                          {
                          }
                          break;
               case MobileGameletImpl.GAMEACTION_GOOD_TIME:
                          {
                            i_action_sound = SND_PROHOD_ETAPA_MMF;
                          }
                          break;
               case MobileGameletImpl.GAMEACTION_BAD_TIME:
                          {
                            i_action_sound = SND_POTERYA_POPYTKI_MMF;
                          }
                          break;
          }
    ...
    public Object processGameAction(int _arg1, int _arg2, Object _obj1)
    {
     ...
     Image imgRoadLine, imgRoadLineDashed;
     Image imgScoreBoard;
     ...
     {
         font              = new drawDigits(getImage(IMG_FONT_SCORE),10);

         Image img = getImage(IMG_SCORE_BOARD);
         imgScoreBoard = img;
         sb_width  = img.getWidth();
         sb_height = img.getHeight();


         imgScore = Image.createImage(sb_width,sb_height);
         imgLives = Image.createImage(sb_width,sb_height);
         imgTimer = Image.createImage(sb_width,sb_height);
         imgSpeed = Image.createImage(sb_width,sb_height);

         gScore = imgScore.getGraphics();
         gLives = imgLives.getGraphics();
         gTimer = imgTimer.getGraphics();
         gSpeed = imgSpeed.getGraphics();
         ...
         score_x = (sb_width - (font.fontWidth<<2))/2;
         score_y = (sb_height - font.fontHeight)/2;

         img = getImage(IMG_ICON_LIFE);
         lives_x = (sb_width - (img.getWidth()+1)*3)/2;
         lives_y = (sb_height - img.getHeight())/2;

         img = getImage(IMG_CLOCK);
         timer_x = (sb_width - (img.getWidth()+font.fontWidth*2+1))/2;
         timer_y = (sb_height - Math.max(img.getHeight(),font.fontHeight))/2;

         speed_x = (sb_width - (getImage(IMG_INDICATOR).getWidth()+font.fontWidth*3))/2;
         speed_y = (sb_height - font.fontHeight)/2;


         imgRoadCache = Image.createImage(RC_WIDTH, RC_SEGMENTS * RC_SEG_SIZE);
         gRoadCache = imgRoadCache.getGraphics();


         imgRoadLine   = Image.createImage(RC_BORDER_WIDTH*2 + ROAD_WIDTH, 2);
         imgRoadLineDashed = Image.createImage(RC_BORDER_WIDTH*2 + ROAD_WIDTH, 2);

         Graphics gRoad = imgRoadLine.getGraphics();

           gRoad.setColor(SAND_COLOR);
           gRoad.fillRect(0,0,RC_BORDER_WIDTH*2 + ROAD_WIDTH,2);
           gRoad.setColor(SAND_DARK_COLOR);
           gRoad.fillRect(RC_BORDER_WIDTH - RC_BORDER_WIDTH,0,RC_BORDER_WIDTH,1);
           gRoad.fillRect(RC_BORDER_WIDTH - RC_BORDER_WIDTH/2,1,RC_BORDER_WIDTH,1);
           gRoad.fillRect(RC_BORDER_WIDTH + ROAD_WIDTH,0,RC_BORDER_WIDTH/2,1);
           gRoad.fillRect(RC_BORDER_WIDTH + ROAD_WIDTH,1,RC_BORDER_WIDTH,1);

           gRoad.setColor(ROAD_COLOR);
           gRoad.fillRect(RC_BORDER_WIDTH,0,ROAD_WIDTH,2);

           gRoad = imgRoadLineDashed.getGraphics();
           gRoad.drawImage(imgRoadLine,0,0,0);
           gRoad.setColor(ROADDASH_COLOR);
           gRoad.fillRect(RC_BORDER_WIDTH + (ROAD_WIDTH>>1),0,2,2);

         imgYellowRect = Image.createImage(192-32,32);
         gRoad = imgYellowRect.getGraphics();

         Image sand = Image.createImage(16,32);
         gRoad = sand.getGraphics();
         gRoad.setColor(SAND_COLOR);
         gRoad.fillRect(0, 0, sand.getWidth(), sand.getHeight());

         gRoad = imgYellowRect.getGraphics();
         for(int i = 0; i < imgYellowRect.getWidth(); i+=sand.getWidth())
         {
            gRoad.drawImage(sand,i,0,0);
         }
         gRoad = null;
         sand = null;
    ...
    void setSoundItemPriority()
    {
                             int [] sort_idx = {
SND_POVOROT_MMF            ,403,
SND_STOLKNOVENIE_MMF       ,913,
SND_PROHOD_ETAPA_MMF       ,417,
SND_START_DVIJENIA_MMF     ,1754,
SND_POTERYA_POPYTKI_MMF    ,769,
SND_FINAL_MMF              ,0,
SND_START_MMF              ,0
                             };
      ...
      final static int OFFROAD_DECOR_WIDTH = 16;
      final static int RC_SEG_SIZE = 32;
      final static int RC_SEG_SHIFT = 5;

      final static int RC_SEGMENTS = 128/RC_SEG_SIZE +1;

      final static int RC_SEGS_VISIBLE = RC_SEGMENTS -1;

      final static int RC_WIDTH = (MobileGameletImpl.RACE_WIDTH >> 8);
      final static int RC_HEIGHT = RC_SEGMENTS * RC_SEG_SIZE;
      final static int ROAD_WIDTH = MobileGameletImpl.ROAD_WIDTH >> 8;

      final static int SAND_COLOR = (241 << 16) | (180 << 8) | 66;
      final static int SAND_DARK_COLOR = (168 << 16) | (126 << 8) | 46;
      final static int ROAD_COLOR = 0x898778;
      final static int ROADDASH_COLOR = 0xffffff;

      Image imgRoadCache;
      Graphics gRoadCache;

      int i_local_px_offset;
      int i_local_seg_offset;
      int i_current_offset;
      int i_current_segment;

      final static int RC_BORDER_WIDTH = 6;
      int [] rc_border = new int [] {
           0 + RC_BORDER_WIDTH, 1 + RC_BORDER_WIDTH,  1 + RC_BORDER_WIDTH,  2 + RC_BORDER_WIDTH,
           2 + RC_BORDER_WIDTH, 2 + RC_BORDER_WIDTH,  2 + RC_BORDER_WIDTH,  1 + RC_BORDER_WIDTH,
           1 + RC_BORDER_WIDTH, 0 + RC_BORDER_WIDTH,  0 + RC_BORDER_WIDTH, -1 + RC_BORDER_WIDTH,
          -1 + RC_BORDER_WIDTH,-1 + RC_BORDER_WIDTH, -1 + RC_BORDER_WIDTH,  0 + RC_BORDER_WIDTH,
      };


      void resetRoadCache()
      {
        i_current_offset = ((p_Gamelet.pPlayerSprite.i_ObjectState >> 8)
                   - MobileGameletImpl.PLAYER_OFFSET + GAMESCREEN_HEIGHT) & 0x1FFF;
        i_local_seg_offset = i_current_offset >> RC_SEG_SHIFT;
        i_current_segment =  1;

        i_local_px_offset = (i_current_offset & (RC_SEG_SIZE -1)) - RC_SEG_SIZE - (i_current_segment << RC_SEG_SHIFT);


        int seg = i_current_segment, pos = i_current_offset | 0x1F;        
        for(int i = 0  ; i < RC_SEGMENTS; i++)
        {
            drawSegment(seg, pos);
            if(++seg >= RC_SEGMENTS) seg -= RC_SEGMENTS;
            pos -= RC_SEG_SIZE;
        }
      }

      void updateRoadCache()
      {

        i_current_offset = (((p_Gamelet.pPlayerSprite.i_ObjectState >> 8) - MobileGameletImpl.PLAYER_OFFSET) + GAMESCREEN_HEIGHT) & 0x1FFF;
        int loc_seg = i_current_offset >> RC_SEG_SHIFT;

        if(loc_seg != i_local_seg_offset)
        {                                   
           int pos = i_current_offset;      


           if( (loc_seg < i_local_seg_offset ? loc_seg + (0x2000>>5) : loc_seg) - i_local_seg_offset > RC_SEGMENTS)
           {
              i_local_seg_offset = (loc_seg - RC_SEGMENTS) & 0x1fff;
           }

           while(i_local_seg_offset != loc_seg)
           {

              i_local_seg_offset = (++i_local_seg_offset) & (0x1fff >> RC_SEG_SHIFT);
              if(--i_current_segment < 0) i_current_segment += RC_SEGMENTS; 
              drawSegment(i_current_segment, i_local_seg_offset << RC_SEG_SHIFT);
           }
        }
      ...
      void drawSegment(int segment, int roadposition)
      {

            int road_L_X=0;
            roadposition = (roadposition>>1) & 0xFF0;
            segment = segment << RC_SEG_SHIFT;
            gRoadCache.setClip(0, segment, RC_WIDTH,RC_SEG_SIZE);



            gRoadCache.drawImage(imgYellowRect,16,segment,0);




            for (int i = segment+RC_SEG_SIZE-2; i >= segment; i-=2)
            {
                  road_L_X = p_Gamelet.ab_Road[roadposition++];                 

                  if ((roadposition & 0x8) == 0)
                  {
                      gRoadCache.drawImage(imgRoadLine,road_L_X-RC_BORDER_WIDTH,i,0);
                  }
                   else
                       {
                           gRoadCache.drawImage(imgRoadLineDashed,road_L_X-RC_BORDER_WIDTH,i,0);
                       }
            }

            if(roadposition == 0x0010)
            {
                  gRoadCache.drawImage(getImage(IMG_FINISH_LINE),road_L_X, segment,0);
            }



            roadposition = (roadposition-1) >> 4; 
            road_L_X = segment - (((roadposition&1)^1) << 5);
            gRoadCache.drawImage(getImage(IMG_00_LEFT + p_Gamelet.ab_LeftViews[roadposition>>1]), 0, road_L_X, 0);
            gRoadCache.drawImage(getImage(IMG_00_LEFT + p_Gamelet.ab_RightViews[roadposition>>1]), RC_WIDTH-16, road_L_X, 0);
...
private static final int IMG_00_LEFT = 4;
private static final int IMG_01_LEFT = 5;
private static final int IMG_02_LEFT = 6;
private static final int IMG_03_LEFT = 7;
private static final int IMG_04_LEFT = 8;
private static final int IMG_05_LEFT = 9;
private static final int IMG_06_LEFT = 10;
private static final int IMG_07_RIGHT = 11;
private static final int IMG_08_RIGHT = 12;
private static final int IMG_09_RIGHT = 13;
private static final int IMG_10_RIGHT = 14;
private static final int IMG_11_RIGHT = 15;
private static final int IMG_12_RIGHT = 16;
private static final int IMG_13_RIGHT = 17;
private static final int IMG_14_LEFT_FINISH = 18;
private static final int IMG_15_RIGHT_FINISH = 19;
private static final int IMG_CAR01 = 20;
private static final int IMG_CAR02 = 21;
private static final int IMG_CAR03 = 22;
private static final int IMG_CAR_PLAYER = 23;
private static final int IMG_EXPLOZ01 = 24;
private static final int IMG_EXPLOZ02 = 25;
private static final int IMG_EXPLOZ03 = 26;
private static final int IMG_EXPLOZ04 = 27;
private static final int IMG_EXPLOZ05 = 28;
private static final int IMG_STONE01 = 29;
private static final int IMG_STONE02 = 30;
private static final int IMG_STONE03 = 31;
private static final int IMG_FONT_SCORE = 32;
private static final int IMG_ICON_LIFE = 33;
private static final int IMG_SCORE_BOARD = 34;
private static final int IMG_CLOCK = 35;
private static final int IMG_INDICATOR = 36;
private static final int IMG_FINISH_LINE = 37;
private static final int TOTAL_IMAGES_NUMBER = 38;

private static final int SND_FINAL_MMF = 0;
private static final int SND_POTERYA_POPYTKI_MMF = 1;
private static final int SND_POVOROT_MMF = 2;
private static final int SND_PROHOD_ETAPA_MMF = 3;
private static final int SND_START_MMF = 4;
private static final int SND_START_DVIJENIA_MMF = 5;
private static final int SND_STOLKNOVENIE_MMF = 6;

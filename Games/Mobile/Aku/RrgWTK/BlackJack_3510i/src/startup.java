...
/////////////////////////////////////////////////////
 ...
 private List gameMenu(){
  List ListItem = null;
    if (canvas.client_sb.isLoadSaveSupporting())
...
//	       case sGAMESAVE:     display.setCurrent(mSave); break;
 ...
 private void rebuildMenuItems(){
  ...
  // text constants
    ...
    canvas.PlayAgain  = saMSG[YourScore]+" ";
    canvas.Insurance  = saMSG[CancelCMD];
    canvas.Double  = saMSG[NewGameTitle];
    canvas.Payment11  = saMSG[LevelName1];
    canvas.Bust  = saMSG[LevelName2];
    canvas.NextCard  = saMSG[LevelName3];
    canvas.YouLost  = saMSG[NewStage];
    canvas.YouWon  = saMSG[Reverse];
//    canvas.NewStage  = saMSG[Reverse);
    canvas.Yes  = saMSG[YesCMD];
    canvas.No  = saMSG[NoCMD];
 }
...
//       lastIndex = -1;
       ...
       if ( scr instanceof List) lastIndex = ((List)scr).getSelectedIndex();
		 ...
		 String s = NameField.getString();
	         if(subMenu != subNEWGAME) PlayerName=s;
		 //NameField = null;
		 switch (subMenu) {
		   case subSAVE:
...
//				break;
                   case subNEWGAME : int n =0;
		                     if (s!=null && s.length()>0 && (n=Integer.parseInt(s))>0 && n<=_bet){
					         showSubMenu(-1,-1);
		                                 canvas.init(n,null);
		                     } break;
...
//	      else
		    ...
		    case subLOAD:
		    case subSAVE:     showSubMenu(-1,-1);break;
		    case subDELETE:
	   ...
	   showSubMenu(menuState, subMenu);
          ...
          if (cmd == cYes){
		    ...
		    case subCLEARALL: {
				      ...
				      break;
		    case subENDGAME : canvas.endGame(true); showSubMenu(-1,-1); break;
...
//		   case 4: showHighScore();break;
		   ...
		   case 5: showSubMenu(menuState,subEXIT);break;
                 }
                }  else
                if (menuState == sGAME && subMenu<0){
		 if(!canvas.client_sb.isLoadSaveSupporting() && lastIndex>0)lastIndex++;
                 switch (lastIndex) {
...
Command bk = new Command(">>",Command.EXIT,10);
...
//---------------- BJ SPECIAL ------------------------
    ...
    void AskBet(int initalcache) {
      ...
      Form FormItem = new Form(saMSG[Sound]+initalcache+")");
      NameField = new TextField("",""+(initalcache>>2),10,TextField.NUMERIC);

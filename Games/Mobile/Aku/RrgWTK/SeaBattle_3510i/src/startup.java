...
/// ADDITIONAL SEQUENCE RELATED SEABATTLE'S ENGINE
		...
		if(scr==Ships){
		  ...
		  if(_ships!=null)
		    for(int i=0;i<4;i++)
		      if(_ships[i]>0)
		        if((c++)==lastIndex) {
			   canvas.InstallShip(i);
			   break;
		        }
		  if((c++)==lastIndex)canvas.ClearDesk();
		  else
		  if(c==lastIndex && isStartMenu)canvas.startPlaying();
		  else
		  if((c++==lastIndex && !isStartMenu) || c==lastIndex) {ShowQuitMenu(true);return;}
		  startme();
		}
...
//======================================================================
    private List Ships=null;
    private int []_ships=null;
    private boolean isStartMenu = false;

    public void ShowShipMenu(int []x, Image [] ships){
      _ships=x;
      Image img = Image.createImage(Image.createImage(ships[0].getWidth(),1));
      Ships = new List("",List.IMPLICIT);
      for (int i=0;i<x.length;i++)
        if(x[i]>0)
	  Ships.append((i+1)+"x : "+x[i],Image.createImage(ships[x.length-i-1]));
      Ships.append(saMSG[ClearAllTitle],img);
      isStartMenu = false;
      if(Ships.size()==1){
         Ships.append(saMSG[AcceptCMD],img/*Image.createImage(1,1)*/);
	 isStartMenu = true;
      }
      Ships.append(saMSG[EndGameCMD],img/*Image.createImage(1,1)*/);
      Ships.addCommand(cBack);
      Ships.setCommandListener(this);
      display.setCurrent(Ships);
    }

...
public class PlayerGun
{
    ...
    public static final int GUN_CHARGING_FRAMES = 5;
    ...
    protected static final int CHARGING_FRAME_HEIGHT = 29;

    public static final int SIGHT_FRAME_WIDTH = 9;
    public static final int SIGHT_FRAME_HEIGHT = 9;
    ...
    public int i_bulletsingun;
    protected boolean lg_sightvisibled;
    protected int i_firingx;
    protected int i_firingy;

    public PlayerGun(int _scrwidth, int _scrheight)
    {
        i_bulletsingun = 0;
        i_scrwidth = _scrwidth;
        i_scrheight = _scrheight;

        i8_vertconstant = ((CHARGING_FRAME_HEIGHT << 8)+_scrheight-1) / _scrheight;
        i8_horzconstant = ((GUN_TURN_FRAMES << 8)+_scrwidth-1) / _scrwidth;
    ...
    public void setSightScrCoord(int _i8_scrx, int _i8_scry)
    {
        ...
        i_gun_scry = i_scrheight - CHARGING_FRAME_HEIGHT + ((i8_vertconstant * i_scry) >> 8) + ab_gunframeoffset[(i_frame << 1) + 1];
        i_gun_scrx = (i_scrwidth >> 1) + ab_gunframeoffset[i_frame << 1];
    }

    public boolean checkShot(int _scrx, int _scry, int _width, int _height)
    {
        return !((_scrx + _width <= i_firingx) || (_scry + _height <= i_firingy) || (_scrx >= i_firingx + DEFEATZONE_WIDTH) || (_scry >= i_firingy + DEFEATZONE_HEIGHT));
    }
            ...
            case STATE_FIRE:
                {
                    ...
                    i_firingx = (i8_sight_scrx >> 8) + DEFEATZONE_OFFSETX;
                    i_firingy = (i8_sight_scry >> 8) + DEFEATZONE_OFFSETY;
                    i_firePhase = 0;
                }
                ;
                break;
    ...
    public int getGunY()
    {
        ...
        {
            case STATE_CHARGING:
                return i_scrheight - CHARGING_FRAME_HEIGHT + ab_chargingoffset[(getFrame() << 1) + 1];
            case STATE_FIRE:
    ...
    protected void sightToCenter()
    {
        setSightScrCoord((i_scrwidth >> 1) - (SIGHT_FRAME_WIDTH >> 1) << 8, ((i_scrheight >> 1) - SIGHT_FRAME_HEIGHT) << 8);
    }

...
public class ImageBlock
{
     ...
     */
    public ImageBlock(String resource_name,com.GameArea loadListener) throws IOException
    {
	...
	storage = null;
        ...
        try
        {
            ...
            {
	      int n = ds.readUnsignedByte();//System.out.print(n+" ("+li+")");
	    try{
	      byte[]b=null;
	      Runtime.getRuntime().gc();
                if(
		   (b = getImageFromInputStream(ds,(n>=0 && n<3)))
                       == null )
//		              System.out.println("null ptr");
                              throw new IOException("I can't create image, indx[" + li + "]");
		if(n>=0 && n<3)_byte_array[n] = b;
                  else if(n>=40 && n<50)_byte_array[n-37] = b;
		    else
		      if(b!=null)
		       try{
/*
			   for(int i = 0;i<b.length;i++){
			    if(i%0x10==0)System.out.print(Integer.toHexString(i)+" ");
			    System.out.print(Integer.toHexString((b[i]&0xf0)>>>4)+Integer.toHexString(b[i]&0xf)+" ");
			    if(i%0x10==0xf)System.out.println();
			   }
*/
//			   System.out.println(b.length);
//                           System.out.println("*** "+n+" "+Runtime.getRuntime().freeMemory());
			   Runtime.getRuntime().gc();
//			   if((n>=0 && n<3)||(n>=40&&n<50))Image.createImage(1,1);
//			   else

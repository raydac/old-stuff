...
/*=============================*/
  ...
  private final static int TOP_BORDER = 53;
...
//          else
    ...
    private void UpdateScreen(int x,int y, int bb){
          //if(!flagFlood) return;
          Image i;
	  x = x*cellW+back_x; y = y*cellH+back_y+TOP_BORDER;
                switch(bb)
                {
                    case Assault_GSB.CELL_DAM: i = GetImage(IMG_DAM); break;
                    case Assault_GSB.CELL_LADDER: i = GetImage(IMG_STAIRWAY); break;
		    default:
			  gBackScreen.setClip(x,y,cellW,cellH);
			  gBackScreen.drawImage(Elements[IMG_ALL_BACK],x,y,0);
			  gBackScreen.setClip(0,0,BackScreen.getWidth(),BackScreen.getHeight());
			  return;
//                          i = iBlank;
//			  i.getGraphics().drawImage(GetImage(IMG_ALL_BACK),-x,-y,0);
                }
    ...
    private void drawBackScreen(){
      ...
      gBackScreen.drawImage(Elements[IMG_ALL_BACK],back_x,back_y,0);
      int k;

      for(int i = 0;i<Assault_SB.FIELD_WIDTH;i++)
        for(int j = 0;j<Assault_SB.FIELD_HEIGHT;j++)
	   if((k=loc.getElement(i,j))>0) UpdateScreen(i,j,k);
...
////////////////////////////////////////////////
	...
	// !>>
	gBuffer.translate(back_x,back_x+TOP_BORDER);
//        gBuffer.setClip(0,0,VisWidth,VisHeight);
	...
	img = Elements[IMG_WALLPART];
        gBuffer.drawImage(img,0,cellH-img.getHeight(),0);
...
//        gBuffer.setClip(0,0,VisWidth,VisHeight);
        ...
        img = Elements[IMG_ENEMY_ICO];
        gBuffer.drawImage(img,0,0,0);
	font.drawDigits(gBuffer,img.getWidth(),(img.getHeight()-font.fontHeight)>>1,3,loc.i_curassaulternumber+loc.i_assaulternumber);

        if(!(gameStatus == Crashed && blink)) {
	   img = Elements[IMG_HEART];
	   for (int i=0;i<loc.i_Attemptions-1;i++)
              gBuffer.drawImage(img,VisWidth-(i+1)*(img.getWidth()+2),0,0);
        }
...
private static final int IMG_ALL_BACK = 6;

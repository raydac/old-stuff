...
public class Assault_GSB extends GameStateBlock
{
    ...
    protected void initPlayer()
    {
        ...
        initScrCoordsForMovingObject(p_Player,Assault_SB.FIELD_WIDTH >> 1,0/*i_playeralt*/);
    }

    protected void initGameArray()
    {
        int i_bottom = i_playeralt*Assault_SB.FIELD_WIDTH;
        for (int li = 0;li < ab_GameField.length-i_bottom;li++)
        {
            ab_GameField[li] = CELL_NONE;
        }
	if(i_bottom>0)
         for (int li = ab_GameField.length-i_bottom;li < ab_GameField.length;li++)
        {
            ab_GameField[li] = CELL_DAM;
        }
    }

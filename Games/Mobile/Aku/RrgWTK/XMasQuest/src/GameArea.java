...
//	   Elements[IMG_NFONT] = Elements[IMG_PL_T2];
	   ...
	   ib = null;
           ...
           V_QUEUE = (client_sb.i_ScreenWidth>>1) / Stages.BLOCKLENGTH +1;
    ...
    protected void keyPressed(int keyCode) {
     // System.out.println(keyCode);
	      ...
	      case gameOver: if (ticks<TOTAL_OBSERVING_TIME) if (ticks>ANTI_CLICK_DELAY && ticks<FINAL_PICTURE_OBSERVING_TIME)
			     ...
			     if(soundIsPlaying())stopAllMelodies();
	      case demoPlay:
              case titleStatus : {
		                    stopAllMelodies();
		                    CleanSpace();
			   ...
			   if (loc.i_GameState==Assault_GSB.GAMESTATE_OVER) {
			       switch (loc.i_PlayerState) {
					...
					case Gamelet.PLAYERSTATE_LOST: ticks =0; gameStatus=Crashed; break;
			                case Gamelet.PLAYERSTATE_WON: ticks = 0; gameStatus=Finished;CleanSpace();
					                         PlayMelody(SND_APPEARANCE_OTT);
					                       /*
								       stage++;
...
//String s="";
	...
	int iiy = client_sb.p_Player.i8_ScreenY >>> 8;
...
//        int i_endX = (client_sb.i8_offsetOfViewX >>> 8) + client_sb.i_ScreenWidth;
	...
	int block_width = Stages.BLOCKLENGTH<<1;
	...
	int _house_block_end = _house_block_start + client_sb.i_ScreenWidth / block_width +1;
        ...
        i_startX= -_house_offset+Stages.BLOCKLENGTH;

	int align;
	int prev = -1;
        if(_house_block_start>0 && _house_block_start<Stages.ai_wayarray.length)
	{
	   prev=Stages.ai_wayarray[_house_block_start-1];
	   if(Elements[IMG_HOUSE1+prev]==null) prev = -1;
	}

//	s="#"+_house_block_start+":"+((client_sb.i_ScreenWidth>>1) / Stages.BLOCKLENGTH +1)+"[";

        for (li = _house_block_start; li <= _house_block_end; li++)
	{
	    idx = Stages.ai_wayarray[li];
	    img = GetImage(IMG_HOUSE1+idx);//Elements[IMG_HOUSE1+align];
	    Elements[IMG_HOUSE1+idx]=img;
	    if(idx==prev)prev = -1;
	    switch(Stages.ai_AlignArray[idx])
	    {
	      case Stages.ALIGN_CENTER:   align = -(img.getWidth()>>1);break;
	      case Stages.ALIGN_LEFT: align = -40;break;
	...
	int i_max = (GameletImpl.MAX_SNOWBALL_FORCE/GameletImpl.SNOWBALL_FORCE_STEP)>>1;
		...
		i_x += 4;
	      ...
	      gBuffer.drawImage(GetImage(IMG_SNOWBALL),i_x,baseY+baseline,0);
               for (li = 0; li < i_pos; li++)
               {
                 gBuffer.fillRect(i_x, i_y/*baseY*/-(baseline+li), 3, baseline+li);
                 i_x += 4;
               }
	    }
        }
...
//	   font.drawDigits(gBuffer,iBomb[0].getWidth(),scrheight-((iBomb[0].getHeight()+font.fontHeight)>>1)-2+BarOffset,2,loc.i_bombs);
           ...
           gBuffer.drawImage(img,2,scrheight-baseY,0);
           BAR(gBuffer,img.getWidth()+2, scrheight-baseY+((img.getHeight()-BAR_HEIGHT)>>1), client_sb.MAX_PLAYER_HEALTH, client_sb.i_PlayerHealth,colorness?0x7cfaff:0xffffff);
	   for (int i=0;i<client_sb.i_Attemptions-1;i++)
              gBuffer.drawImage(img,/*baseX+*/scrwidth-(i+1)*(img.getWidth()+2)-10,/*baseY+*/scrheight-baseY,0);
        }

	i_y = scrheight-baseY+i_max+baseline+2;

	gBuffer.drawImage(GetImage(IMG_TREE10X10),15,i_y,0);
	gBuffer.drawImage(GetImage(IMG_PIPE10X10),scrwidth-25,i_y,0);
	gBuffer.drawImage(GetImage((ticks&2)==0?IMG_SAN10X10_01:IMG_SAN10X10_02),25+
                                       (scrwidth-60)*(client_sb.i8_offsetOfViewX>>>8)/12000,
				       i_y,0);
...
/*
			  ...
			  if(ticks<20)
			  {
			     g.setColor(0x0);g.fillRect(0, 0, scrwidth,scrheight);

			     int i = (scrheight>>1)-24;
			     int w =(scrwidth>>1)-ticks*4;
			     int w1=3;
			     int w2=6;

			     g.drawImage(High,0,i,0);
//			     g.drawImage(Low,0,(scrheight>>1),0);

			     g.setColor(0);
			     g.fillRect(0,i,w,48);  g.fillRect(scrwidth - w,i,w,48);

			     dg.setARGBColor(0xB0404040);
			     g.fillRect(w,i,w1,48);  g.fillRect(scrwidth - w-w1,i,w1,48);

			     dg.setARGBColor(0x80909090);
			     g.fillRect(w+w1,i,w2,48);  g.fillRect(scrwidth - w-w1-w2,i,w2,48);
			  } else
			    if(ticks<30)
			    {
			       g.setColor(0x0);g.fillRect(0, 0, scrwidth,scrheight);
			       int i = (scrheight>>1)-24;
			       int h = ((scrheight-48)>>1)*(ticks-20)/10;
			       g.drawImage(High,0,i-h,0);
//			       g.drawImage(Low,0,i+h+24,0);

			       g.setColor((ticks-20)*0x191919);
			       g.fillRect(0,i-h+24,scrwidth,h<<1);

			  } else
			  if(ticks<40){
			       DoubleBuffer(g);
			       dg.setARGBColor((10-ticks-30)*(0x19191919));
			       g.fillRect(0,24,scrwidth,scrheight-48);
//			       g.drawImage(Low,0,scrheight-24,0);
			  } else gameStatus=playGame;
*/
	                  } break;
...
*/
Runtime.getRuntime().gc();
if(Runtime.getRuntime().freeMemory()<20000)CleanSpace();
			      {
...
accuracy+=m;
g.setFont(Font.getFont(Font.FACE_PROPORTIONAL,Font.STYLE_BOLD,Font.SIZE_SMALL));
g.drawString(m+":"+accuracy,scrwidth>>1,scrheight-13,Graphics.TOP|Graphics.HCENTER);
*/
            }
/*
		          g.setColor(0xffffff);    // drawin' flyin' text
...
//       drawBackScreen();
	...
	if(Runtime.getRuntime().freeMemory()<10000){
	  Runtime.getRuntime().gc();
	  if(Runtime.getRuntime().freeMemory()<10000)CleanSpace();
	}
...
public void CleanSpace(){
  for(int i=IMG_HOUSE1;i<=IMG_HOUSE13;i++) Elements[i]=null;
  for(int i=IMG_MOON01;i<=IMG_MOON03;i++) Elements[i]=null;
  Runtime.getRuntime().gc();
}
...
private static final byte IMG_HOUSE1 = 3;
private static final byte IMG_HOUSE2 = 4;
private static final byte IMG_HOUSE3 = 5;
private static final byte IMG_HOUSE4 = 6;
private static final byte IMG_HOUSE5 = 7;
private static final byte IMG_HOUSE6 = 8;
private static final byte IMG_HOUSE7 = 9;
private static final byte IMG_HOUSE8 = 10;
private static final byte IMG_HOUSE9 = 11;
private static final byte IMG_HOUSE10 = 12;
private static final byte IMG_HOUSE11 = 13;
private static final byte IMG_HOUSE12 = 14;
private static final byte IMG_HOUSE13 = 15;
private static final byte IMG_BACK = 16;
private static final byte IMG_DEMO = 17;
private static final byte IMG_SWALK_LEFT01 = 18;
private static final byte IMG_SWALK_LEFT02 = 19;
private static final byte IMG_SWALK_LEFT03 = 20;
private static final byte IMG_SWALK_LEFT04 = 21;
private static final byte IMG_SWALK_LEFT05 = 22;
private static final byte IMG_SWALK_LEFT06 = 23;
private static final byte IMG_SJUMP_LEFT01 = 26;
private static final byte IMG_SJUMP_LEFT02 = 27;
private static final byte IMG_SJUMP_LEFT03 = 28;
private static final byte IMG_SDROP_LEFT01 = 29;
private static final byte IMG_SDROP_LEFT02 = 30;
private static final byte IMG_SDROP_LEFT03 = 31;
private static final byte IMG_SWALK_RIGHT01 = 32;
private static final byte IMG_SWALK_RIGHT02 = 33;
private static final byte IMG_SWALK_RIGHT03 = 34;
private static final byte IMG_SWALK_RIGHT04 = 35;
private static final byte IMG_SWALK_RIGHT05 = 36;
private static final byte IMG_SWALK_RIGHT06 = 37;
private static final byte IMG_SJUMP_RIGHT01 = 40;
private static final byte IMG_SJUMP_RIGHT02 = 41;
private static final byte IMG_SJUMP_RIGHT03 = 42;
private static final byte IMG_SDROP_RIGHT01 = 43;
private static final byte IMG_SDROP_RIGHT02 = 44;
private static final byte IMG_SDROP_RIGHT03 = 45;
private static final byte IMG_SDEAD01 = 46;
private static final byte IMG_SDEAD02 = 47;
private static final byte IMG_SDEAD03 = 48;
private static final byte IMG_CAT_WALK_LEFT01 = 49;
private static final byte IMG_CAT_WALK_LEFT02 = 50;
private static final byte IMG_CAT_WALK_LEFT03 = 51;
private static final byte IMG_CAT_ATTACK_LEFT01 = 52;
private static final byte IMG_CAT_ATTACK_LEFT02 = 53;
private static final byte IMG_CAT_WALK_RIGHT01 = 54;
private static final byte IMG_CAT_WALK_RIGHT02 = 55;
private static final byte IMG_CAT_WALK_RIGHT03 = 56;
private static final byte IMG_CAT_ATTACK_RIGHT01 = 57;
private static final byte IMG_CAT_ATTACK_RIGHT02 = 58;
private static final byte IMG_CAT_UP01 = 59;
private static final byte IMG_CAT_UP02 = 60;
private static final byte IMG_CAT_DOWN01 = 61;
private static final byte IMG_CAT_DOWN02 = 62;
private static final byte IMG_CAT_FALL01 = 63;
private static final byte IMG_CAT_FALL02 = 64;
private static final byte IMG_GHOST_R_IN01 = 65;
private static final byte IMG_GHOST_R_IN02 = 66;
private static final byte IMG_GHOST_R_IN03 = 67;
private static final byte IMG_GHOST_R_IN04 = 68;
private static final byte IMG_GHOST_R_IN05 = 69;
private static final byte IMG_GHOST_AT01 = 70;
private static final byte IMG_GHOST_AT02 = 71;
private static final byte IMG_GHOST_BIRST01 = 72;
private static final byte IMG_GHOST_BIRST02 = 73;
private static final byte IMG_GHOST_BIRST03 = 74;
private static final byte IMG_GHOST_BIRST04 = 75;
private static final byte IMG_GHOST_BIRST05 = 76;
private static final byte IMG_SMOKE_AP0 = 77;
private static final byte IMG_SMOKE_AP1 = 78;
private static final byte IMG_SMOKE00 = 79;
private static final byte IMG_SMOKE01 = 80;
private static final byte IMG_SMOKE02 = 81;
private static final byte IMG_SMOKE_D_00 = 82;
private static final byte IMG_SMOKE_D_01 = 83;
private static final byte IMG_SMOKE_D_02 = 84;
private static final byte IMG_WITCH_L_01 = 85;
private static final byte IMG_WITCH_L_02 = 86;
private static final byte IMG_WITCH_R_01 = 87;
private static final byte IMG_WITCH_R_02 = 88;
private static final byte IMG_WITCH_FALL_01 = 89;
private static final byte IMG_UFO01 = 90;
private static final byte IMG_UFO02 = 91;
private static final byte IMG_UFO_FALL = 92;
private static final byte IMG_MOON01 = 93;
private static final byte IMG_MOON02 = 94;
private static final byte IMG_MOON03 = 95;
private static final byte IMG_MOON_ATTACK01 = 96;
private static final byte IMG_MOON_ATTACK02 = 97;
private static final byte IMG_MOON_STRIKE01 = 98;
private static final byte IMG_MOON_STRIKE02 = 99;
private static final byte IMG_MOON_STRIKE_FALL01 = 100;
private static final byte IMG_MOON_STRIKE_FALL02 = 101;
private static final byte IMG_PIPE = 102;
private static final byte IMG_ICECREAM = 103;
private static final byte IMG_SNOWBALL = 104;
private static final byte IMG_SNOWBALL_FALL = 105;
private static final byte IMG_STAR01 = 106;
private static final byte IMG_STAR02 = 107;
private static final byte IMG_STAR_DOWN = 108;
private static final byte IMG_HEART = 109;
private static final byte IMG_PIPE10X10 = 110;
private static final byte IMG_SAN10X10_01 = 111;
private static final byte IMG_SAN10X10_02 = 112;
private static final byte IMG_TREE10X10 = 113;
private static final byte IMG_TITLE = 114;
private static final byte IMG_SDOWN_LEFT01 = 24;
private static final byte IMG_SDOWN_LEFT02 = 25;
private static final byte IMG_SDOWN_RIGHT01 = 38;
private static final byte IMG_SDOWN_RIGHT02 = 39;
private static final byte TOTAL_IMAGES_NUMBER = 115;

private static final byte SND_JBELLS_OTT = (byte)0;
private static final byte SND_APPEARANCE_OTT = (byte)1;
private static final byte SND_WON_OTT = (byte)2;
private static final byte SND_LOST_OTT = (byte)3;
}

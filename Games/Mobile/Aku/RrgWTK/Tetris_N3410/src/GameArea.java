...
//       e.printStackTrace();
    ...
    public void preDrawGame()
    {
      ...
      int poleH = HEIGHT_LIMIT;
      gridGW = poleW/client_sb.gridW;
      gridGH = poleH/client_sb.gridH;
      gBuffer.setColor(0x000000);
      gBuffer.drawImage(GetImage((IMG_SCORE)),WIDTH_LIMIT-30,0,0);
      gBuffer.drawImage(GetImage((IMG_LEVEL)),WIDTH_LIMIT-30,17,0);
      // Сетка
      gBuffer.setColor(0x000000);
...
//	serviceRepaints();
   ...
   public void ClearGame()
   {
     ...
     gBuffer.fillRect(0,0,WIDTH_LIMIT,HEIGHT_LIMIT);
     tmp_score = -1;
     tmp_cursepeed = -1;
     preDrawGame();
   }
   ...
   public void BackupFigure()
   {
    ...
    protected void DoubleBuffer(Graphics gBuffer)
    {
      ...
      if(tmp_score != client_sb.score)
      {
       ...
       font.drawDigits(gBuffer,WIDTH_LIMIT-30,7,5,client_sb.score);
      }
      if(tmp_cursepeed != client_sb.cursepeed)
      {
       tmp_cursepeed = client_sb.cursepeed;
       font.drawDigits(gBuffer,WIDTH_LIMIT-23,24,2,Math.abs((client_sb.cursepeed-client_sb.DEFSPEED)));
      }

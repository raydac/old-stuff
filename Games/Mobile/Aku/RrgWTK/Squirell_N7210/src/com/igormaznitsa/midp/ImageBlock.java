...
//	a = new int[num+1][3];
	...
	int native_pixel = DirectUtils.getDirectGraphics(Image.createImage(1,1).getGraphics()).getNativePixelFormat();
	int bites_per_pixel = 12;
	switch(native_pixel){
	    case DirectGraphics.TYPE_BYTE_1_GRAY:                bites_per_pixel = 1; break;
	    case DirectGraphics.TYPE_BYTE_1_GRAY_VERTICAL:       bites_per_pixel = 1; break;
	    case DirectGraphics.TYPE_BYTE_2_GRAY:                bites_per_pixel = 2; break;
	    case DirectGraphics.TYPE_BYTE_332_RGB:               bites_per_pixel = 8; break;
	    case DirectGraphics.TYPE_BYTE_4_GRAY:                bites_per_pixel = 4; break;
	    case DirectGraphics.TYPE_BYTE_8_GRAY:                bites_per_pixel = 8; break;
	    case DirectGraphics.TYPE_INT_888_RGB:                bites_per_pixel = 24; break;
	    case DirectGraphics.TYPE_INT_8888_ARGB:              bites_per_pixel = 32; break;
	    case DirectGraphics.TYPE_USHORT_1555_ARGB:           bites_per_pixel = 16; break;
	    case DirectGraphics.TYPE_USHORT_444_RGB:             bites_per_pixel = 16; break;
	    case DirectGraphics.TYPE_USHORT_4444_ARGB:           bites_per_pixel = 16; break;
	    case DirectGraphics.TYPE_USHORT_555_RGB:             bites_per_pixel = 16; break;
	    case DirectGraphics.TYPE_USHORT_565_RGB:             bites_per_pixel = 16; break;
	}
...
/*
		  a[n][0] = b.length;
		  a[n][1] = ((i_width*i_height*bites_per_pixel)>>3);
		  a[n][2] = 0;
*/
		 if(n<255
		     && (n<9 || n>29)
		     && (b.length < ((i_width*i_height*bites_per_pixel)>>3))
		   )_byte_array[n] = b;
		    else
		      if(b!=null)
		       try{
//			   long f = Runtime.getRuntime().freeMemory();
			   ...
			   Runtime.getRuntime().gc();
//			   a[n][2] = (int)Math.abs(f-Runtime.getRuntime().freeMemory());
...
/*
	...
	for(int i = 0; i<a.length; i++){
	   System.out.println(""+i+"= "+a[i][0]+":"+a[i][1]+(a[i][2]!=0?" ["+a[i][2]+",d="+(a[i][2]-a[i][1])+"]":""));
	    a1 += a[i][0];
	    a2 += a[i][1];

	}
       System.out.println("Summary:" +a1+", "+a2);
*/
    }
	      ...
	      return b;
            ...
            while (p_baos.size() < (len - 12 + PNG_COMMON_PALETTE.length))
            {
                ...
                switch (i_chunk)
                {
                    case HEADER_IHDR:
                        {
                            i_width = _inputstream.readUnsignedByte();
                            i_height = _inputstream.readUnsignedByte();
                            i_flag = _inputstream.readUnsignedByte();

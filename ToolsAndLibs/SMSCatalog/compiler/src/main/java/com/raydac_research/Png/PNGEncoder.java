package com.raydac_research.Png;
...
{
    protected static MediaTracker p_MediaTracker = new MediaTracker(new Button());
    ...
    public static final BufferedImage convertFileToImage(File _imageFile) throws IOException
    {
    ...
    public static final BufferedImage convertFileToImage(byte[] _imageFile) throws IOException
    {
        // May be  it is GIF or JPEG?
        BufferedImage pResultImage = null;
        Image pImage = Toolkit.getDefaultToolkit().createImage(_imageFile);

        p_MediaTracker.addImage(pImage, 1);
        ...
        if (p_MediaTracker.isErrorAny())
        {
            if ((p_MediaTracker.statusID(1, true) & MediaTracker.ERRORED) != 0)
            {
                p_MediaTracker.removeImage(pImage);
                pImage = null;
            }
        }
        else

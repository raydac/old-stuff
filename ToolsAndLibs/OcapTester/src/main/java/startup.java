...
public class startup extends Component implements Xlet, Runnable, IStartupConst
{
    ...
    /**
     * Переменная содержит громкость звукового сопровождения игры
     */
    private static int i_Options_SoundLevel = 4;
                ...
                if (lg_Repaint)
                {
                    ...
                    switch (i_AppState)
                    {
                    	case APPSTATE_GAMEPLAY:
                    	{
                            ...
                            if (i_gameState != Gamelet.GAMESTATE_PLAYED)
                            {
                            	setAppState(APPSTATE_MAINMENU);
                            }
    ...
    private static final void onMenuButtonClicked(String _menuButton) throws Throwable
    {
        ...
        if (_menuButton.equals(MENUBUTTON_OPTIONS))
        {
            ...
            if (_menuButton.equals(MENUBUTTON_EXIT))
            {
                lg_workflag = false;
            }
            else
    ...
    private static final void onAppStateChanged(int _newState, int _oldState) throws Throwable
    {
        ...
        switch (_newState)
        {
        	case APPSTATE_GAMEPLAY:
        	{
        		...
        		Gamelet.initStage(p_This.getClass(),0);
        		GameView.afterInitGameStage();
        	};break;
            case APPSTATE_GAMEMENU:
            case APPSTATE_MAINMENU:
            {
                if (_newState == APPSTATE_MAINMENU && _oldState ==APPSTATE_GAMEPLAY)
                {
    ...
    private static final void onPaintGamestage(Graphics _g)
    {
    ...
    private static final void onPaintMenu(Graphics _g) throws Throwable
    {
        if (i_menuUpdateComponentsNumber > 0)
        {

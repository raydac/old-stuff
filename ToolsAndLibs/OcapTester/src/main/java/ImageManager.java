...
public class ImageManager
{
    ...
    /**
     * Размер хранения в памяти одного пикселя 0 - 1 байт, 1 - 2 байта, 2 - 4
     * байта
     */
     ...
     */
    private static final String DYNIMAGES = "gfx/dyn.bin";

    /**
     * Константа, показывающая имя файла, содержащего пакет внешне-загружаемых
     * изображений
     */
    private static final String EXTERNAL_BIN_FILE = "gfx/ext.bin";
     ...
     * 
     * @param _imageOffset
     *            смещение данных изображения в массиве
     * @param _x
     *            координата X верхнего левого края
     * @param _y
     *            координата Y верхнего левого края
     * @throws Throwable
     *             порождается в случае ошибки работы процедуры
     */
    public static final void drawImage(int _imageOffset, int _x, int _y) throws Throwable
    {
                ...
                try
                {
                    if (CHECK_MEMORY)
                    {
                        ...
                        if (Runtime.getRuntime().freeMemory() <= i_neededMemory)
                        {
                            ...
                            if (Runtime.getRuntime().freeMemory() <= i_neededMemory)
                            {
                                // Пытаемся освободить место в памяти,
                                // освобождая кэш
                                clearTTLImageCache();
                                if (Runtime.getRuntime().freeMemory() <= i_neededMemory)
                                {
            ...
            if (_g == null || p_img == null)
                System.out.println("I have founf null");
            _g.drawImage(p_img, _x, _y, null);
    }

    /*
     * private static void traceImageForOffset(int _offset) {
     * System.out.println("Trace image map array for
     * "+_offset+"\n\r-------------");
     * 
     ...
     * System.out.println("OFFSETX on image "+(ash_imageInfo[_offset++] &
     * 0xFFFF)); System.out.println("OFFSETY on image
     * "+(ash_imageInfo[_offset++] & 0xFFFF)); System.out.println("WIDTH on
     * image "+(ash_imageInfo[_offset++] & 0xFFFF)); System.out.println("HEIGHT
     * on image "+(ash_imageInfo[_offset++] & 0xFFFF));
     * 
     * if (IMAGEINFO_LENGTH == 9) { System.out.println("ORIGWIDTH on image
     * "+(ash_imageInfo[_offset++] & 0xFFFF)); System.out.println("ORIGFHEIGHT
     * on image "+(ash_imageInfo[_offset++] & 0xFFFF)); } }
     */
    ...
    public static final void init(boolean _loadExternal) throws Throwable
    {
        ...
        {
            System.out.println("Can't find dynamic images array");
            p_in = null;
        }
        if (p_in != null)
        {
            ...
            System.out.println("Dynamic array length=" + i_len);

            ab_dynamicarray = new byte[i_len];
            Utils.loadArrayFromStream(p_in, i_len, ab_dynamicarray);
        }
        Utils.closeStream(p_in);
        p_in = null;

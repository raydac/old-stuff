...
public class SpriteCollection
{
    ...
    /**
     * Заполняет оптимизатор областями из коллекции
     * @param _optimizer объект оптимизатора
     * @param _withOptimization добавление области в оптимизатор с оптимизацией
     */
    public final void fillOptimizer(DrawOptimizer _optimizer,boolean _withOptimization)
    {
    	int i_offset = i_lastActiveSpriteOffset;
    	
        final int[] ai_spriteData = ai_spriteDataArray;
        final short[] ash_spriteAnimationData = ash_spriteAnimationDataArray;

        if (_optimizer.lg_PaintFullArea) return;
        
    	while(i_offset>=0)
    	{
    		final int i_listData = ai_spriteData[i_offset + OFFSET_LISTDATA];
            int i_prev = (i_listData & MASK_LISTPREVINDEX) >>> SHR_LISTPREVINDEX;
            
            int i_dataOffset = ai_spriteData[i_offset + OFFSET_STATICDATAOFFSET] + ANIMATIONTABLEEOFFSET_WIDTH;
            int i_w = ash_spriteAnimationData[i_dataOffset++];
            int i_h = ash_spriteAnimationData[i_dataOffset];

            i_dataOffset = i_offset + OFFSET_SCREENX;
            int i_x = (ai_spriteData[i_dataOffset++] + 0x7F) >> 8;
            int i_y = (ai_spriteData[i_dataOffset] + 0x7F) >> 8;

            if (_optimizer.lg_PaintFullArea) return;
            
            //System.out.println("add area to optimizer "+i_x+","+i_y+","+i_w+","+i_h);

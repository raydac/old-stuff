...
public class PathCollection
{
     ...
     */
    private int[] ai_PathDynamicDataArray;
     ...
     */
    public PathCollection(int _collectionID, int _pathsNumber,
            SpriteCollection[] _spriteCollections, short[] _pathArray)
    {
    ...
    public final void processActive()
    {
        ...
        if (i_pathOffset < 0) return;

        final int[] ai_dynPathArray = ai_PathDynamicDataArray;

        while (true)
        {
            final int i_nextPath = ((ai_dynPathArray[i_pathOffset
                    + OFFSET_LISTINDEXES]) & MASK_PREVITEM) >>> SHR_PREVITEM;
            ...
            if ((i_pathSpecial & MASK_PAUSED) == 0)
            {
                ...
                p_SpriteCollection.setMainPointXY(i_spriteOffset, i8_mainX
                        + i8_curX, i8_mainY + i8_curY);
                    ...
                    switch (i_pathType)
                    {
                        case PATHTYPE_NORMAL:
                        {
                            ...
                            else
                            {
                                ...
                                ai_dynPathArray[i_pathOffset
                                        + OFFSET_PATHSPECDATA] = (i_pathSpecial & ~MASK_PATHCURPOINT)
                                        | (i_curPoint << SHR_PATHCURPOINT);
                        ...
                        case PATHTYPE_PENDULUM:
                        {
                            ...
                            if ((i_pathSpecial & MASK_BACKFLAG) != 0)
                            {
                                ...
                                i_pathSpecial = (i_pathSpecial & ~MASK_PATHCURPOINT)
                                        | (i_curPoint << SHR_PATHCURPOINT);
                                ai_dynPathArray[i_pathOffset
                                        + OFFSET_PATHSPECDATA] = i_pathSpecial;
                            }
                            else
                            {
                                i_curPoint++;
    ...
    private void _calculateDelta(int _pathOffset, boolean _forward)
    {
        ...
        else
        {
            ...
            if (i_curPoint == 0)
            {
                ...
                i_curOffset = i_initOffset
                        + ((i_pointsNumber << 1) + i_pointsNumber) - 1;
                i_steps = ash_statPathArrayarray[i_curOffset--];
                i_curOffset--;
            }
            else
        ...
        i8_destX = (int) (((long) i8_destX
                * (long) ai_dynPathArrayarray[i_offset++] + 0x7F) >> 8);
        i8_destY = (int) (((long) i8_destY
                * (long) ai_dynPathArrayarray[i_offset] + 0x7F) >> 8);
     ...
     */
    public final void saveToStream(DataOutputStream _outStream)
            throws Throwable
    {
     ...
     */
    public final void loadFromStream(DataInputStream _inStream)
            throws Throwable
    {
    ...
    public final int nextActiveOffset()
    {
        ...
        if (i_result >= 0)
        {
            int i_indexList = ai_PathDynamicDataArray[i_result
                    + OFFSET_LISTINDEXES];
        ...
        i_lastIteratorPathOffset = i_result;
        return i_result;
    }
    ...
    public final void resetOne(final int _pathOffset)
    {
        ...
        i_offset = _pathOffset + OFFSET_LOCALSCALEWIDTH_I8;
        i8_pointX = (int) (((long) i8_pointX
                * (long) ai_dynPathArray[i_offset++] + 0x7F) >> 8);
        ...
        p_spriteCollection.setMainPointXY(i_sprite, i8_mainX + i8_pointX,
                i8_mainY + i8_pointY);
     ...
     */
    public final void activateOne(final int _pathOffset, int _optionalData,
            int _centerX, int _centerY, int _localScaleW, int _localScaleH,
            int _spriteCollectionIndex, int _spriteOffset, int _offset,
            int _initPathPoint, int _pathLength, int _modify)
    {
        ...
        int i_pathData = ai_PathDynamicDataArray[_pathOffset
                + OFFSET_PATHSPECDATA];
        ...
        i_pathData = MASK_ACTIVE
                | ((_modify << SHR_PATHMODIFY) & MASK_PATHMODIFY)
                | ((i_pathType << SHR_PATHTYPE) & MASK_PATHTYPE)
                | ((i_pathLen << SHR_PATHLEN) & MASK_PATHLEN);

        int i_curPathOffset = _pathOffset + 1;

        // данные смещения и опциональные данные
        ai_dynArray[i_curPathOffset] = ((_optionalData << SHR_OPTIONALDATA) & MASK_OPTIONALDATA)
                | ((i_staticOffset << SHR_INITPATHOFFSET) & MASK_INITPATHOFFSET);
        i_curPathOffset++;
        ...
        // данные спрайта
        ai_dynArray[i_curPathOffset] = ((_spriteCollectionIndex << SHR_SPRITECOLLECTION) & MASK_SPRITECOLLECTION)
                | ((_spriteOffset << SHR_SPRITEOFFSET) & MASK_SPRITEOFFSET);
        i_curPathOffset++;
     ...
     * @return упакованное значени SPRITE_COLLECTION<<<24 | SPRITE_OFFSET
     */
    public final void setSpriteInfo(int _pathOffset, int _collectionIndex,
            int _spriteOffset)
    {
        final int[] ai_arr = ai_PathDynamicDataArray;
        ai_arr[_pathOffset + OFFSET_SPRITEDATA] = ((_collectionIndex << SHR_SPRITECOLLECTION) & MASK_SPRITECOLLECTION)
                | ((_spriteOffset << SHR_SPRITEOFFSET) & MASK_SPRITEOFFSET);
        ...
        _pathOffset++;
        ap_SpriteCollections[_collectionIndex].setMainPointXY(_spriteOffset,
                i8_mainX + i8_curX, i8_mainY + i8_curY);
    }
    ...
    private final void _moveInactivePathToActiveList(int _pathOffset)
    {
        ...
        else
        {
            ...
            int i_listData = ai_dynPathArray[i_lastinPathOffset
                    + OFFSET_LISTINDEXES];
            ...
            i_listData = (i_lastinPathOffset << SHR_PREVITEM)
                    | (0xFFFF << SHR_NEXTITEM);
    ...
    public final void cloneActive(int _srcPath, int _dstPath)
    {
        ...
        System.arraycopy(ai_PathDynamicDataArray, _srcPath,
                ai_PathDynamicDataArray, _dstPath, PATHDATACELLSNUMBER);
    }
    ...
    {
        final int i_spec = ai_PathDynamicDataArray[_pathOffset
                + OFFSET_PATHSPECDATA];
    ...
    {
        int i_sprieData = ai_PathDynamicDataArray[_pathOffset
                + OFFSET_SPRITEDATA];
    ...
    {
        int i_pathSpec = ai_PathDynamicDataArray[_pathOffset
                + OFFSET_PATHSPECDATA];

...
//===========================================================================================
    ...
    public static final String getOperationToken(String _string, int _position)
    {
        ...
        //Checking for one char operators
    ...
    private static final FormulaStack convertStringToFormulaStack(String _string, HashMap _globalvariables, HashMap _localvariables) throws IOException
    {
        ...
        while (i_pos < _string.length())
        {
            ...
            if (s_ar.length() != 0)
            {
                ...
                if (Function.isFunction(s_ar))
                {
                    p_stack.add(new Function(s_ar));
                }
                else
                {
                    Value p_val = null;
    ...
    private static final void sortFormulaStack(FormulaStack _stack) throws IOException
    {
        // Sorting for operation priorities
        for (int li = 0; li < _stack.size() - 1; li++)
        {
            ...
            for (int lx = li + 1; lx < _stack.size(); lx++)
            {
                ...
                else if (i_priorityOfNewObj == PRIORITY_CLOSE_BRACKET)
                {
    ...
    private static final Value calculateFormulaStack(FormulaStack _stack) throws IOException
    {
        int i_indx = 0;
        while (_stack.size() != 1)
        {
        ...
        if (_stack.size() > 1) throw new IOException("There is an operand without an operation");
        return (Value) _stack.elementAt(0);
    }

    public static final Value evaluateFormula(String _string, HashMap _globalvariables, HashMap _localvariables) throws IOException
    {
        FormulaStack p_stack = convertStringToFormulaStack(_string, _globalvariables, _localvariables);

//        p_stack.printFormulaStack();
//        System.out.println("-------------");
        sortFormulaStack(p_stack);

//        p_stack.printFormulaStack();

        return calculateFormulaStack(p_stack);
    }

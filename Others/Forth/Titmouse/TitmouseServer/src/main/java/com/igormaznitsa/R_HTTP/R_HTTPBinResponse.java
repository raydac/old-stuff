...
public class R_HTTPBinResponse {
  ...
  public R_HTTPBinResponse(Thread mainthr, BusServerParams bsp, Socket clsock, String h_dir, String r_name, server_par sp, HTTPQueryRecord qrec) throws IOException {
    ...
    if (li < 0) {
      rsrc_name = rsrc_name + sp.default_page;
      li = rsrc_name.lastIndexOf('.');
    }
      ...
      switch (cur_char) {
        case '&': {
          if (name_par.length() > 0) {
            ...
            name_par = new StringBuffer("");
            body_par = new StringBuffer("");
            is_name = true;
          }
        }
        ;
        ...
        default: {
          if (is_name)
            name_par.append(cur_char);
          else
            body_par.append(cur_char);
        }
        ;
      }
    }
  ...
  public Vector GetResponseHeader(String client_ip_address) throws Exception {
    ...
    if (typ_file.equalsIgnoreCase("fsp")) {
      ...
      try {
        ...
        if (ltmpos instanceof ScriptForthBuffer) forth_body = (ScriptForthBuffer) ltmpos;
        else forth_body = null;
      } catch (Exception e) {
        Exception ee = e;
        try {
          scr_frt.close();

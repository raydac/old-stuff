...
//------------------------------------------
  ...
  void SQL_AUTOCOMMIT() throws Exception // DB_AUTOCOMMIT Режим автотранзакции (AS: connect_indx int_mode -> )
  {
    try {
      if (State) Acm.addElement(new Integer(INDX_SQL_AUTOCOMMIT));
        ...
        if (isolmode.intValue() == 0) cur_con.setAutoCommit(false);
        else cur_con.setAutoCommit(true);
      }
    } catch (Exception e) {
    ...
    try {
      if (State) Acm.addElement(new Integer(INDX_SQL_TRANSISOLATION));
        ...
        switch (isolmode.intValue()) {
          case 0:
            cur_con.setTransactionIsolation(cur_con.TRANSACTION_READ_UNCOMMITTED);
            break;
          case 1:
            cur_con.setTransactionIsolation(cur_con.TRANSACTION_READ_COMMITTED);
            break;
          case 2:
            cur_con.setTransactionIsolation(cur_con.TRANSACTION_REPEATABLE_READ);
            break;
          case 3:
            cur_con.setTransactionIsolation(cur_con.TRANSACTION_SERIALIZABLE);
            break;
          case 4:
            cur_con.setTransactionIsolation(cur_con.TRANSACTION_NONE);
            break;
          default:
  ...
  void FIND() throws Exception // Ищет в словаре слово с именем из следующего слова, если есть то кладет на АС true иначе false
  {
    ...
    switch (ltt) {
      case TOKEN_EOF:
        throw new Exception("Error: After \"'\" must be word");
      case TOKEN_NUMBER:
        throw new Exception("Error: After \"'\" must be word");
      case TOKEN_STRING:
        throw new Exception("Error: After \"'\" must be word");
  ...
  {
    try {
      if (State) Acm.addElement(new Integer(INDX_CYCLE_I));
      else {
  ...
  {
    try {
      if (State) Acm.addElement(new Integer(INDX_CYCLE_J));
      else {
  ...
  {
    try {
      if (State) Acm.addElement(new Integer(INDX_DO));
      else {
  ...
  {
    try {
      if (State) Acm.addElement(new Integer(INDX_BEGIN));
      else {
        Cycle_Index.addElement(new Integer(PC_Counter));
  ...
  void S_EQU() throws Exception // S= Сравнивает две строки, с адресами на АС, с учетом регистра
  {
    try {
      if (State) Acm.addElement(new Integer(INDX_S_EQU));
        ...
        if (s1.compareTo(s2) == 0) li1 = new Long(-1);
        else li1 = new Long(0);
  ...
  void S_EQUNC() throws Exception // S== Сравнивает две строки, с адресами на АС, без учета регистра
  {
    try {
      if (State) Acm.addElement(new Integer(INDX_S_EQUNC));
        ...
        String s1 = (String) StringVariable.elementAt(li1.intValue());
        String s2 = (String) StringVariable.elementAt(li2.intValue());
        if (s1.equalsIgnoreCase(s2)) li1 = new Long(-1);
        else li1 = new Long(0);
        ArithmeticStack.addElement(li1);
      }
    } catch (Exception e) {
  ...
  void EQU_ZERO() throws Exception // 0= Сравнивает вершину фрифметического стека с нулем
  {
    try {
      if (State) Acm.addElement(new Integer(INDX_EQU_ZERO));
        ...
        Long obj1 = (Long) ArithmeticStack.lastElement();
        ArithmeticStack.removeElementAt(ArithmeticStack.size() - 1);
        if (obj1.longValue() == 0) obj1 = new Long(-1);
        else obj1 = new Long(0);
        ArithmeticStack.addElement(obj1);
      }
    } catch (Exception e) {
  ...
  void MORE_ZERO() throws Exception // 0> Проверяет вершину АС на превосходство над нулем
  {
    try {
      if (State) Acm.addElement(new Integer(INDX_MORE_ZERO));
        ...
        Long obj1 = (Long) ArithmeticStack.lastElement();
        ArithmeticStack.removeElementAt(ArithmeticStack.size() - 1);
        if (obj1.intValue() > 0) obj1 = new Long(-1);
        else obj1 = new Long(0);
        ArithmeticStack.addElement(obj1);
      }
    } catch (Exception e) {
  ...
  void SMALL() throws Exception // > Проверяет второй элемент АС на превосходство над вершиной
  {
    try {
      if (State) Acm.addElement(new Integer(INDX_SMALL));
        ...
        Long obj2 = (Long) ArithmeticStack.lastElement();
        ArithmeticStack.removeElementAt(ArithmeticStack.size() - 1);
        if (obj2.longValue() < obj1.longValue()) obj1 = new Long(-1);
        else obj1 = new Long(0);
        ArithmeticStack.addElement(obj1);
      }
    } catch (Exception e) {
  ...
  void VARIABLE() throws Exception // Variable создает переменную, выделяет ей память
  {
    ...
    switch (ltt) {
      case TOKEN_EOF:
        throw new Exception("Not var name");
      case TOKEN_NUMBER:
        throw new Exception("Bad var name");
      case TOKEN_STRING:
        throw new Exception("Bad var name");
      case TOKEN_WORD: {
        svar = svar.toUpperCase();
...
//------------------------------------------
  ...
  void ExecuteCompileCommand(int cmnd) throws Exception {
    switch (cmnd) {
      ...
      case INDX_MULL_TWO:
        MUL_TWO();
        break;
      case INDX_NEW_WORD_END:
        NEW_WORD_END();
        break;
      case INDX_NEW_WORD_START:
        NEW_WORD_START();
        break;
      case INDX_NEXT:
        NEXT();
      ...
      case INDX_CHR:
        CHR();
        break;
      case INDX_STR_URLDECODE:
        URL_DEC();
        break;
      case INDX_STR_URLENCODE:
        URL_ENC();
        break;
      case INDX_TO_STRING:
        TO_STRING();
        break;
      case INDX_SYS_QCONNECT:
        QCONNECT();
        break;
      ...
      case INDX_SQL_CLOSE:
        SQL_CLOSE();
        break;
      case INDX_SQL_CONNECT:
        SQL_CONNECT();
        break;
      case INDX_INCMEM:
        INCMEM();
        break;
      case INDX_SQL_EXECUTEQ:
        SQL_EXECUTEQ();
        break;
      case INDX_SQL_EXECUTEU:
        SQL_EXECUTEU();
        break;
      case INDX_SQL_GETCOLUMNINT:
        SQL_GETCOLUMNINT();
        break;
      case INDX_SQL_GETCOLUMNSTR:
        SQL_GETCOLUMNSTR();
        break;
      case INDX_SQL_LOADDRIVER:
        SQL_LOADDRIVER();
        break;
      case INDX_SQL_NEXT:
        SQL_NEXT();
        break;
      case INDX_SQL_AUTOCOMMIT:
        SQL_AUTOCOMMIT();
        break;
      case INDX_SQL_GETNAMECOLUMNINT:
        SQL_GETCOLUMNINTFORNAME();
        break;
      case INDX_SQL_GETNAMECOLUMNSTR:
        SQL_GETCOLUMNSTRFORNAME();
        break;
      case INDX_SQL_COMMIT:
        SQL_COMMIT();
        break;
      case INDX_SQL_ROLLBACK:
        SQL_ROLLBACK();
        break;
      case INDX_SQL_TRANSISOLATION:
        SQL_SETTRANSISOLATION();
        break;

      case INDX_ADM_CLIENTADD:
        ADMIN_CLIENTADD();
        break;
      case INDX_ADM_CLIENTCONNECT:
        ADMIN_CLIENTCONNECT();
        break;
      case INDX_ADM_CLIENTEXISTS:
        ADMIN_CLIENTEXIST();
        break;
      case INDX_ADDCHAR2STR:
        ADD_CHAR2STR();
        break;
      case INDX_ADM_CLIENTNAME:
        ADMIN_CLIENTNAME();
        break;
      case INDX_ADM_CLIENTNEXT:
        ADMIN_CLIENTNEXT();
        break;
      case INDX_ADM_CLIENTPASSWORD:
        ADMIN_CLIENTPASSWORD();
        break;
      case INDX_ADM_CLIENTREMOVE:
        ADMIN_CLIENTREMOVE();
        break;

      case INDX_SOCKET_READBYTE:
        SOCKET_READBYTE();
        break;
      case INDX_SOCKET_WRITEBYTE:
        SOCKET_WRITEBYTE();
        break;
      case INDX_SOCKET_READSTRING:
        SOCKET_READSTRING();
        break;
      case INDX_SOCKET_WRITESTRING:
        SOCKET_WRITESTRING();
        break;

      case INDX_SYS_MESSAGE:
        SYS_MESSAGE();
        break;

      case INDX_SQL_NGETBLOB:
        SQL_GETBLOBSTREAMFORNAME();
        break;
      case INDX_SQL_GETBLOB:
        SQL_GETBLOBSTREAMFORNUMBER();
        break;
      case INDX_SQL_READBLOBBYTE:
        SQL_READBLOBBYTE();
        break;
      case INDX_SQL_CLOSEINPBLOB:
        SQL_CLOSEINPBLOB();
        break;

      case INDX_DT_DATE_BREAK:
        DATE_BREAK();
        break;
      case INDX_DT_DATE_PACK:
        DATE_PACK();
        break;
      case INDX_DT_DATETIME:
        DATETIME();
        break;
      case INDX_DT_TIME_BREAK:
        TIME_BREAK();
        break;
      case INDX_DT_SET_HOURS:
        TIME_SETHH();
        break;
      case INDX_DT_SET_MINUTES:
        TIME_SETMM();
        break;
      case INDX_DT_SET_SECONDS:
        TIME_SETSS();
        break;
      ...
      case INDX_S2I:
        S2I();
        break;
      case INDX_SOCKET_WRITENUM:
        SOCKET_WRITENUM();
        break;
      default:
        throw new Exception("Unknown command code " + String.valueOf(cmnd));
    }
  }
  ...
  void ExecuteTranslateCommand(String cmnd) throws Exception {
    ...
    Vector cur_cmnd;
    if (cmnd.compareTo("SN.") == 0) SOCKET_WRITENUM();
    else if (cmnd.compareTo("DATE_BREAK") == 0) DATE_BREAK();
    else if (cmnd.compareTo("DATE_PACK") == 0) DATE_PACK();
    else if (cmnd.compareTo("DATETIME") == 0) DATETIME();
    else if (cmnd.compareTo("TIME_BREAK") == 0) TIME_BREAK();
    else if (cmnd.compareTo("SETHH") == 0) TIME_SETHH();
    else if (cmnd.compareTo("SETMM") == 0) TIME_SETMM();
    else if (cmnd.compareTo("SETSS") == 0) TIME_SETSS();
    else if (cmnd.compareTo("MSG.") == 0) SYS_MESSAGE();
    else if (cmnd.compareTo("SCKT_RDB") == 0) SOCKET_READBYTE();
    else if (cmnd.compareTo("SCKT_RDS") == 0) SOCKET_READSTRING();
    else if (cmnd.compareTo("SCKT_WRB") == 0) SOCKET_WRITEBYTE();
    else if (cmnd.compareTo("SS.") == 0) SOCKET_WRITESTRING();
    else
...
//                if (cmnd.compareTo("ADM_CLIENTEXT")==0) ADMIN_CLIENTEXIST(); else
      if (cmnd.compareTo("CHR+") == 0) ADD_CHAR2STR();
      else
...
//                if (cmnd.compareTo("ADM_CLIENTREM")==0) ADMIN_CLIENTREMOVE(); else
        if (cmnd.compareTo("DB_CLOSE") == 0) SQL_CLOSE();
        else if (cmnd.compareTo("DB_CONNECT") == 0) SQL_CONNECT();
        else if (cmnd.compareTo("1+!") == 0) INCMEM();
        else if (cmnd.compareTo("DB_EXQ") == 0) SQL_EXECUTEQ();
        else if (cmnd.compareTo("DB_EXU") == 0) SQL_EXECUTEU();
        else if (cmnd.compareTo("DB_GETINT") == 0) SQL_GETCOLUMNINT();
        else if (cmnd.compareTo("DB_GETSTR") == 0) SQL_GETCOLUMNSTR();
        else if (cmnd.compareTo("DB_LOAD") == 0) SQL_LOADDRIVER();
        else if (cmnd.compareTo("DB_NEXT") == 0) SQL_NEXT();
        else if (cmnd.compareTo("S!") == 0) TO_STRING();
        else if (cmnd.compareTo("S_PS") == 0) SYMBOL_POS();
        else if (cmnd.compareTo("CHR") == 0) CHR();
        else if (cmnd.compareTo("S_UC") == 0) UPPERCASE();
        else if (cmnd.compareTo("S_LC") == 0) LOWERCASE();
        else if (cmnd.compareTo("S_AT") == 0) SYMBOLAT();
        else if (cmnd.compareTo("S_LN") == 0) SYMBOLLENGTH();
        else if (cmnd.compareTo("LEAVE") == 0) LEAVE();
        else if (cmnd.compareTo("IF") == 0) IF();
        else if (cmnd.compareTo("THEN") == 0) THEN();
        else if (cmnd.compareTo("ELSE") == 0) ELSE();
        else if (cmnd.compareTo(".") == 0) PRINT_N();
        else if (cmnd.compareTo("S.") == 0) PRINT_S();
        else if (cmnd.compareTo("S+") == 0) S_ADD();
        else if (cmnd.compareTo("DROPALL") == 0) DROPALL();
        else if (cmnd.compareTo("DROPALLR") == 0) DROPALLR();
        else if (cmnd.compareTo("DROP") == 0) DROP();
        else if (cmnd.compareTo("DROPR") == 0) DROPR();
        else if (cmnd.compareTo("DUP") == 0) DUP();
        else if (cmnd.compareTo("DUPR") == 0) DUPR();
        else if (cmnd.compareTo("SWAP") == 0) SWAP();
        else if (cmnd.compareTo("OVER") == 0) OVER();
        else if (cmnd.compareTo("ROT") == 0) ROT();
        else if (cmnd.compareTo("ROTR") == 0) ROTR();
        else if (cmnd.compareTo(">R") == 0) TO_R();
        else if (cmnd.compareTo("R>") == 0) FROM_R();
        else if (cmnd.compareTo("R@") == 0) COPY_R();
        else if (cmnd.compareTo("+") == 0) ADD();
        else if (cmnd.compareTo("-") == 0) SUB();
        else if (cmnd.compareTo("*") == 0) MUL();
        else if (cmnd.compareTo("AND") == 0) AND();
        else if (cmnd.compareTo("OR") == 0) OR();
        else if (cmnd.compareTo("XOR") == 0) XOR();
        else if (cmnd.compareTo("NOT") == 0) NOT();
        else if (cmnd.compareTo("/") == 0) DIV();
        else if (cmnd.compareTo("0=") == 0) EQU_ZERO();
        else if (cmnd.compareTo("0>") == 0) MORE_ZERO();
        else if (cmnd.compareTo("0<") == 0) SMALL_ZERO();
        else if (cmnd.compareTo(">") == 0) SMALL();
        else if (cmnd.compareTo("<") == 0) MORE();
        else if (cmnd.compareTo("=") == 0) EQU();
        else if (cmnd.compareTo("2*") == 0) MUL_TWO();
        else if (cmnd.compareTo("2/") == 0) DIV_TWO();
        else if (cmnd.compareTo("!") == 0) TO_MEM();
        else if (cmnd.compareTo("@") == 0) FROM_MEM();
        else if (cmnd.compareTo("VARIABLE") == 0) VARIABLE();
        else if (cmnd.compareTo(":") == 0) NEW_WORD_START();
        else if (cmnd.compareTo(";") == 0) NEW_WORD_END();
        else if (cmnd.compareTo("NEXT") == 0) NEW_WORD_END();
        else if (cmnd.compareTo("DEPTH") == 0) DEPTH();
        else if (cmnd.compareTo("DEPTHR") == 0) DEPTHR();
        else if (cmnd.compareTo("S=") == 0) S_EQU();
        else if (cmnd.compareTo("S==") == 0) S_EQUNC();
        else if (cmnd.compareTo("DO") == 0) DO();
        else if (cmnd.compareTo("LOOP") == 0) LOOP();
        else if (cmnd.compareTo("+LOOP") == 0) ADD_LOOP();
        else if (cmnd.compareTo("I") == 0) CYCLE_I();
        else if (cmnd.compareTo("J") == 0) CYCLE_J();
        else if (cmnd.compareTo("BEGIN") == 0) BEGIN();
        else if (cmnd.compareTo("?CONNECT") == 0) QCONNECT();
        else if (cmnd.compareTo("UNTIL") == 0) UNTIL();
        else if (cmnd.compareTo("ALLOT") == 0) ALLOT();
        else if (cmnd.compareTo("1+") == 0) INC();
        else if (cmnd.compareTo("1-") == 0) DEC();
        else if (cmnd.compareTo("<>") == 0) NO_EQU();
        else if (cmnd.compareTo("'") == 0) FIND();
        else if (cmnd.compareTo("?") == 0) FORCE();
        else if (cmnd.compareTo("QUIT") == 0) QUIT();
        else if (cmnd.compareTo("S_ENC") == 0) URL_ENC();
        else if (cmnd.compareTo("S_DEC") == 0) URL_DEC();
        else if (cmnd.compareTo("DB_AUTOCOMMIT") == 0) SQL_AUTOCOMMIT();
        else if (cmnd.compareTo("DB_NGETINT") == 0) SQL_GETCOLUMNINTFORNAME();
        else if (cmnd.compareTo("DB_NGETSTR") == 0) SQL_GETCOLUMNSTRFORNAME();
        else if (cmnd.compareTo("DB_COMMIT") == 0) SQL_COMMIT();
        else if (cmnd.compareTo("DB_ROLLBACK") == 0) SQL_ROLLBACK();
        else if (cmnd.compareTo("DB_TRANS") == 0) SQL_SETTRANSISOLATION();
        else if (cmnd.compareTo("DB_NGETBLOB") == 0) SQL_GETBLOBSTREAMFORNAME();
        else if (cmnd.compareTo("DB_GETBLOB") == 0) SQL_GETBLOBSTREAMFORNUMBER();
        else if (cmnd.compareTo("DB_RDBLOB") == 0) SQL_READBLOBBYTE();
        else if (cmnd.compareTo("DB_CLOSEINBLOB") == 0) SQL_CLOSEINPBLOB();
        else if (cmnd.compareTo("PLAYFILE") == 0) PLAYFILE();
        else if (cmnd.compareTo("I2S") == 0) I2S();
        else if (cmnd.compareTo("S2I") == 0) S2I();
        else {
          if (State) {

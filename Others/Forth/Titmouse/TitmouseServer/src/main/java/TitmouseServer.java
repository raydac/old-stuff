...
public class TitmouseServer
{
		...
		static String server_version = "1.5"; // Версия сервера, подставляется в сведения , выводимые на консоль
		static String fsp_version = "2.0a"; // Версия FSP
		
		static String scripts_patch; // Содержит путь к файлам скриптов

		/** Выводит на экран список алиасов для БД	 */
		...
		// Запускаем на выполнение скрипт с заданным именем
		static void StartScriptWithName(String scriptname, BusServerParams bus)
		{
			...
			catch (Exception e)
			{
				...
				System.out.println("Error in script \'"+scriptname+"\' ["+e.getMessage()+"]"); 
			}
		}
		
		// Приостанавливает работу указанного сервера
		...
		static void LoadInitServerFile(String filename,Hashtable mimetable,Hashtable services,Hashtable dbaliases,Vector ascripts,Hashtable aaliases,BusServerParams bus,Vector StartBat,Vector StopBat) throws Exception 
		{
			...
			try
			{
				...
				while (	true)
				{
					...
					if (ltmpstr.compareTo("[SERVERS]")==0)
					{
						curtype = 1;
						continue;
					}
					else
					if (ltmpstr.compareTo("[DBALIASES]")==0)
					{
						curtype = 2;
						continue;
					}
					...
					if (ltmpstr.compareTo("[MAIL]")==0)
					{
						curtype = 4;
						continue;
					}
					else
					if (ltmpstr.compareTo("[ALIASES]")==0)
					{
						curtype = 5;
						continue;
					}
					else
					if (ltmpstr.compareTo("[SERVER_START]")==0)
					{
						curtype = 6;
						continue;
					}
					else
					if (ltmpstr.compareTo("[SERVER_STOP]")==0)
					{
						curtype = 7;
						continue;
					}
			...
			catch (Exception e)
			{
				...
				switch (curtype)
				{
					case 0 : throw new Exception("Error in MIME section ["+e.getMessage()+"]");
					case 1 : throw new Exception("Error in SERVERS section ["+e.getMessage()+"]");
					case 2 : throw new Exception("Error in DBALIASES section ["+e.getMessage()+"]");
					case 3 : throw new Exception("Error in SCRIPTS section ["+e.getMessage()+"]");
					case 4 : throw new Exception("Error in MAIL section ["+e.getMessage()+"]");
					case 5 : throw new Exception("Error in ALIASES section ["+e.getMessage()+"]");
				}
				throw new Exception ("Error in loadingtime of server.ini");
			}
		}
		
		/* Добавляет алиас в список алиасов */
		...
		/* добавляет тип в таблицу соответствия MIME */
		...
		/* Добавляет алиас в таблицу содержащую алиасы баз данных */
		...
		/* Добавляет сервер в список серверов */
        static void AddService(String str,Hashtable svector) throws Exception 
        {
				...
				{
					throw new Exception("Duplicate server name "+a_ls);
				}
        }      

		// Загружает файл содержащий список скриптов с временной привязкой
        static void AddScript(String str,Vector svector,String pth) throws Exception 
        {
				...
				catch (NumberFormatException el)
				{
					 ...
					 {
						 if (lsweekday.compareTo("SUN")==0) liweekday = 0;
						 else
						 if (lsweekday.compareTo("MON")==0) liweekday = 1;	 
						 else
						 if (lsweekday.compareTo("TUE")==0) liweekday = 2;	 
						 else
						 if (lsweekday.compareTo("WED")==0) liweekday = 3;	 
						 else
						 if (lsweekday.compareTo("THU")==0) liweekday = 4;	 
						 else
						 if (lsweekday.compareTo("FRI")==0) liweekday = 5;	 
						 else
						 if (lsweekday.compareTo("SAT")==0) liweekday = 6;	 
						 else
						 if (lsweekday.compareTo("ALL")==0) liweekday = -1;	 
						 else
		...
		static void ViewScriptList(Vector scv)
		{
			...
			for(int li=0;li<scv.size();li++)  
            {
				 ...
				 {
					 if (tim_sc.HourOfStart<0)System.out.print("*:"); else System.out.print(tim_sc.HourOfStart+":");
				    if (tim_sc.MinuteOfStart<0)System.out.print("* "); else System.out.print(tim_sc.MinuteOfStart+" ");
				    if (tim_sc.WeekDayOfStart<0)System.out.print("All "); else System.out.print(weekdays[tim_sc.WeekDayOfStart]+" ");
				    if (tim_sc.DayOfStart<0)System.out.print("*/"); else System.out.print(tim_sc.DayOfStart+"/");
				    if (tim_sc.MonathOfStart<0)System.out.print("*/"); else System.out.print(tim_sc.MonathOfStart+"/");
				    if (tim_sc.YearOfStart<0)System.out.print("*/\t"); else System.out.print(tim_sc.YearOfStart+"\t");
				 }
                 if (tim_sc.script_thread!=null)
                 {
                      if ( tim_sc.script_thread.isAlive()) System.out.println(" Active"); else
                      System.out.println(" Passive");
                 }
                 else System.out.println("Passive"); 
            }
            System.out.println("-----------------------------------------------------------------------\n");
		}
        ...
        static void about()
        {
                ...
                System.out.println("       E-Mail: rrg@forth.org.ru");  
                System.out.println("    Home page: http://www.forth.org.ru/~rrg/");
                System.out.println("==============================================================================");
                System.out.println(""); 
        }
        ...
        static void start_image()
        {
              Calendar clndr = Calendar.getInstance() ;   
				...
				System.out.println("||");
				System.out.println("||  (C) 2000 All Copyright by Raydac Research Group URG");
				System.out.println("||");
				System.out.println("||  Author    : Igor A. Maznitsa");
				System.out.println("||  Home Page : http://www.forth.org.ru/~rrg/");
				System.out.println("||  E-Mail    : rrg@forth.org.ru\r\n");
	...
	static boolean TranslateCommandString(String kbd_string, BusServerParams rhttpbusobject,R_HTTPScriptThread script_thread,Vector ScriptVector)
	{
			...
			try
			{
				if (kbd_string.compareTo("servers")==0) 
                {
					...
					System.out.println("--------------------------------------------------------------------------");
					{
						...
						while(lenm.hasMoreElements())
						{
							...
							if (ser_rec.server==null) System.out.print("P\t"); else 
							{
								if (ser_rec.server.IsSuspend())System.out.print("S\t"); else
								System.out.print("A\t");
							}
                ...
                if (kbd_string.compareTo("close")==0) 
                {
					...
					if (kbd_string.startsWith("resume "))
					{
						StringTokenizer ltmpstrt = new StringTokenizer(kbd_string," ");
						ltmpstrt.nextToken();
					...
					if (kbd_string.startsWith("suspend "))
					{
						StringTokenizer ltmpstrt = new StringTokenizer(kbd_string," ");
						ltmpstrt.nextToken();
						String ltserv = ltmpstrt.nextToken().trim().toUpperCase();
					...
					if (kbd_string.compareTo("scripts")==0)
                    {
						 ViewScriptList(ScriptVector);
                    }
                    else
                    if (kbd_string.compareTo("about")==0)
                    {
                         about(); 
                    }
					else
					if (kbd_string.compareTo("dbaliases")==0)
                    {
                         dbaliases_list(rhttpbusobject.dbaliases); 
                    }
					else
                    if (kbd_string.compareTo("traceon")==0)
                    {
						Enumeration lenm = rhttpbusobject.RHTTPServers.elements(); 
						while(lenm.hasMoreElements()) 
						{
					...
					if (kbd_string.compareTo("traceoff")==0)
                    {
						Enumeration lenm = rhttpbusobject.RHTTPServers.elements(); 
						while(lenm.hasMoreElements()) 
						{
                    ...
                    if (kbd_string.compareTo("help")==0)
                    {
                             help(); 
                    }
                    else
                    if (kbd_string.compareTo("mem")==0)
                    {
				...
				while (lenm.hasMoreElements())
				{
					...
					if (ser_rec.StartScript!=null)
					{
						...
						catch (Exception e)
						{
							...
							System.out.println("Error in start script for \'"+ser_rec.Alias+"\' server ["+e.getMessage()+"]"); 
						}
					}
					tsf = null;

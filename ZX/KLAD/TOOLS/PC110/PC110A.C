/*
* Translator of MicroC code into ASM80
* based on the Micro-C by Leon Obuhov 1989
*
* Initial version by Igor Maznitsa in 1997
* Restored alpha version by Igor Maznitsa in 2018
*/
#define intwidth        2
#define charwidth	1
  ...
  int *argv[];
{
  ...
  openout(argv);
  openin (argv);
  outstr(";************** PC-110 alpha ************");nl;
  outstr(";*   Micro-C->ASM80 by Igor Maznitsa    *");nl;
  outstr(";*  (based on Micro-C L.Obuhov          *");nl;
  outstr(";*  (based on Small-C R.Cain)           *");nl;
  outstr(";****************************************");nl;
  parse();
  dumplits();
  dumpglbs();
...
dowhile()	/* while ...   */
{
  ...
  wq[wqlab]=getlabel();
  addwhile (wq);
  outlabel (wq[wqloop]); nl;
  test (wq[wqlab]);
  statement();
  jump (wq[wqloop]);
  outlabel (wq[wqlab]); nl;
  locptr=wq[wqsym];
  delwhile();
}
doreturn()	/* return (...)*/
  ...
  char *sym;
{
  if ((sym[ident]!=pointer) & (sym[type]==cchar))
  {
    outtab ("LD A,C");nl;
    outtab ("LD (");
    outstr (sym+name);outstr("),A");
    nl;
  }
  else
  {
    outtab("LD (");
    outstr(sym+name);outstr("),BC");
    nl;
  }
}
putindr (typeobj)
  ...
  char typeobj;
{
  if (typeobj==cchar)
  {
/*    outtab ("al=*ax, ah=sign(*ax)");nl; */ /* bcw */
      outtab ("PUSH HL");nl;
      outtab ("PUSH BC");nl;
      outtab ("POP HL");nl;
      outtab ("LD C,(HL)");nl;
  ...
  int ptr[];
{
 if (streq(ptr,"!=")!=0)
  {
    outtab ("DEFB #E5,#B7,#C5,#E1,#ED,#52,#E1,#28,05,1,1,0,#18,3,1,0,0");
    nl;
  } else if (streq(ptr,"==")!=0)
  {
    outtab ("DEFB #E5,#B7,#C5,#E1,#ED,#52");nl;
		outtab ("DEFB #E1,#20,05,1,1,0,#18,3,1,0,0");nl;
  } else if (streq(ptr,">=")!=0)
  {
		outtab("DEFB #E5,#D5,#E1,#B7,#ED,#42,#30,5,1,0,0");nl;
		outtab("DEFB #18,3,1,1,0,#E1");nl;
  } else if (streq(ptr,"<=")!=0)
	{
		outtab("DEFB #E5,#C5,#E1,#B7,#ED,#52");nl;
		outtab("DEFB #30,5,1,0,0,#18,3,1,1,0,#E1");nl;
	} else if (streq(ptr,">")!=0)
  {
		outtab("DEFB #E5,#C5,#E1,#B7,#ED,#52");nl;
		outtab("DEFB #30,5,1,1,0,#18,3,1,0,0,#E1");nl;
  } else if (streq(ptr,"<")!=0)
  {
		outtab("DEFB #E5,#D5,#E1,#B7,#ED,#42,#30,5,1,1,0");nl;
		outtab("DEFB #18,3,1,0,0,#E1");nl;
  } else {
		outtab("NOT IMPLEMENTED CONDITION: ");
		outstr(ptr);nl;
	}
}
dumplits()
